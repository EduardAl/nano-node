...
TEST (ipc, asynchronous)
{
	...
	vban::ipc::ipc_server ipc (*system.nodes[0], node_rpc_config);
	vban::ipc::ipc_client client (system.nodes[0]->io_ctx);

	auto req (vban::ipc::prepare_request (vban::ipc::payload_encoding::json_v1, std::string (R"({"action": "block_count"})")));
	auto res (std::make_shared<std::vector<uint8_t>> ());
	std::atomic<bool> call_completed{ false };
	client.async_connect ("::1", 24077, [&client, &req, &res, &call_completed] (vban::error err) {
		client.async_write (req, [&client, &req, &res, &call_completed] (vban::error err_a, size_t size_a) {
			ASSERT_NO_ERROR (static_cast<std::error_code> (err_a));
			ASSERT_EQ (size_a, req.size ());
			// Read length
			client.async_read (res, sizeof (uint32_t), [&client, &res, &call_completed] (vban::error err_read_a, size_t size_read_a) {
				...
				// Read json payload
				client.async_read (res, payload_size_l, [&res, &call_completed] (vban::error err_read_a, size_t size_read_a) {
...
TEST (ipc, synchronous)
{
	...
	std::thread client_thread ([&client, &call_completed] () {
		...
		std::string response (vban::ipc::request (vban::ipc::payload_encoding::json_v1, client, std::string (R"({"action": "block_count"})")));
		std::stringstream ss;
		ss << response;
		// Make sure the response is valid json
		boost::property_tree::ptree blocks;
...
TEST (ipc, permissions_default_user)
{
	...
	ASSERT_TRUE (access.has_access ("", vban::ipc::access_permission::api_account_weight));
}
...
TEST (ipc, permissions_deny_default)
{
	...
	vban::ipc::access access;
	access.deserialize_toml (toml);
	ASSERT_FALSE (access.has_access ("user1", vban::ipc::access_permission::api_account_weight));
}
...
TEST (ipc, permissions_groups)
{
	...
	access.deserialize_toml (toml);
	ASSERT_FALSE (access.has_access ("user1", vban::ipc::access_permission::api_account_weight));
	ASSERT_TRUE (access.has_access_to_all ("user1", { vban::ipc::access_permission::wallet_read, vban::ipc::access_permission::wallet_write }));
}
...
TEST (ipc, permissions_oneof)
{
	...
	vban::ipc::access access;
	access.deserialize_toml (toml);
	ASSERT_TRUE (access.has_access ("user1", vban::ipc::access_permission::api_account_weight));
	ASSERT_TRUE (access.has_access ("user2", vban::ipc::access_permission::api_account_weight));
	ASSERT_FALSE (access.has_access ("user3", vban::ipc::access_permission::api_account_weight));
	ASSERT_TRUE (access.has_access_to_oneof ("user1", { vban::ipc::access_permission::account_query, vban::ipc::access_permission::api_account_weight }));
	ASSERT_TRUE (access.has_access_to_oneof ("user2", { vban::ipc::access_permission::account_query, vban::ipc::access_permission::api_account_weight }));
	ASSERT_FALSE (access.has_access_to_oneof ("user3", { vban::ipc::access_permission::account_query, vban::ipc::access_permission::api_account_weight }));
}
