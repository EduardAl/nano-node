...
vban::rpc_handler::rpc_handler (vban::rpc_config const & rpc_config, std::string const & body_a, std::string const & request_id_a, std::function<void (std::string const &)> const & response_a, vban::rpc_handler_interface & rpc_handler_interface_a, vban::logger_mt & logger) :
	body (body_a),
...
void vban::rpc_handler::process_request (vban::rpc_handler_request_params const & request_params)
{
	try
	{
		...
		else
		{
			if (request_params.rpc_version == 1)
			{
				...
				std::error_code rpc_control_disabled_ec = vban::error_rpc::rpc_control_disabled;
				...
				else
				{
					...
					{
						vban::uint128_union random_id;
						vban::random_pool::generate_block (random_id.bytes.data (), random_id.bytes.size ());
