...
class socket : public std::enable_shared_from_this<vban::socket>
{
...
public:
	 ...
	 */
	explicit socket (vban::node & node, boost::optional<std::chrono::seconds> io_timeout = boost::none);
	virtual ~socket ();
	...
	void async_write (vban::shared_const_buffer const &, std::function<void (boost::system::error_code const &, size_t)> const & = nullptr);
...
public:
	 ...
	 */
	explicit server_socket (vban::node & node_a, boost::asio::ip::tcp::endpoint local_a, size_t max_connections_a);
	...
	void on_connection (std::function<bool (std::shared_ptr<vban::socket> const & new_connection, boost::system::error_code const &)>);
...
private:
	std::vector<std::weak_ptr<vban::socket>> connections;
	...
	void on_connection_requeue_delayed (std::function<bool (std::shared_ptr<vban::socket> const & new_connection, boost::system::error_code const &)>);
};
}
