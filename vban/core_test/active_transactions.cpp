...
TEST (active_transactions, confirm_active)
{
	...
	vban::node_config node_config2 (vban::get_available_port (), system.logging);
	node_config2.frontiers_confirmation = vban::frontiers_confirmation_mode::disabled;
	vban::node_flags node_flags2;
	...
	{
		node1.network.flood_block (send, vban::buffer_drop_policy::no_limiter_drop);
	...
	{
		vban::lock_guard<vban::mutex> guard (node2.rep_crawler.probable_reps_mutex);
		node2.rep_crawler.probable_reps.emplace (vban::dev_genesis_key.pub, vban::genesis_amount, *peers.begin ());
	}
...
TEST (active_transactions, confirm_frontier)
{
	...
	ASSERT_TIMELY (5s, node1.ledger.block_confirmed (node1.store.tx_begin_read (), send->hash ()));
	ASSERT_EQ (vban::process_result::progress, node2.process (*send_copy).code);
...
TEST (active_transactions, keep_local)
{
	...
	vban::keypair key1, key2, key3, key4, key5, key6;
	wallet.insert_adhoc (vban::dev_genesis_key.prv);
	auto send1 (wallet.send_action (vban::dev_genesis_key.pub, key1.pub, node.config.receive_minimum.number ()));
	auto send2 (wallet.send_action (vban::dev_genesis_key.pub, key2.pub, node.config.receive_minimum.number ()));
	auto send3 (wallet.send_action (vban::dev_genesis_key.pub, key3.pub, node.config.receive_minimum.number ()));
	auto send4 (wallet.send_action (vban::dev_genesis_key.pub, key4.pub, node.config.receive_minimum.number ()));
	auto send5 (wallet.send_action (vban::dev_genesis_key.pub, key5.pub, node.config.receive_minimum.number ()));
	auto send6 (wallet.send_action (vban::dev_genesis_key.pub, key6.pub, node.config.receive_minimum.number ()));
...
TEST (active_transactions, inactive_votes_cache)
{
	...
	auto send = vban::send_block_builder ()
				...
				.build_shared ();
	auto vote (std::make_shared<vban::vote> (vban::dev_genesis_key.pub, vban::dev_genesis_key.prv, std::numeric_limits<uint64_t>::max (), std::vector<vban::block_hash> (1, send->hash ())));
	...
	ASSERT_EQ (1, node.stats.count (vban::stat::type::election, vban::stat::detail::vote_cached));
}
...
TEST (active_transactions, inactive_votes_cache_non_final)
{
	...
	auto send = vban::send_block_builder ()
				...
				.work (*system.work.generate (latest))
				.build_shared ();
	auto vote (std::make_shared<vban::vote> (vban::dev_genesis_key.pub, vban::dev_genesis_key.prv, 0, std::vector<vban::block_hash> (1, send->hash ()))); // Non-final vote
	node.vote_processor.vote (vote, std::make_shared<vban::transport::channel_loopback> (node));
	ASSERT_TIMELY (5s, node.active.inactive_votes_cache_size () == 1);
	node.process_active (send);
	node.block_processor.flush ();
	ASSERT_TIMELY (5s, node.stats.count (vban::stat::type::election, vban::stat::detail::vote_cached) == 1);
	auto election = node.active.election (send->qualified_root ());
	ASSERT_NE (nullptr, election);
	ASSERT_FALSE (election->confirmed ());
	ASSERT_EQ (vban::genesis_amount - 100, election->tally ().begin ()->first);
}
...
TEST (active_transactions, inactive_votes_cache_fork)
{
	...
	auto send2 = builder.make_block ()
				 ...
				 .work (*system.work.generate (latest))
				 .build_shared ();
	auto vote (std::make_shared<vban::vote> (vban::dev_genesis_key.pub, vban::dev_genesis_key.prv, std::numeric_limits<uint64_t>::max (), std::vector<vban::block_hash> (1, send1->hash ())));
	node.vote_processor.vote (vote, std::make_shared<vban::transport::channel_loopback> (node));
	auto channel1 (node.network.udp_channels.create (node.network.endpoint ()));
	ASSERT_TIMELY (5s, node.active.inactive_votes_cache_size () == 1);
	node.network.process_message (vban::publish (send2), channel1);
	node.block_processor.flush ();
	...
	node.network.process_message (vban::publish (send1), channel1);
	node.block_processor.flush ();
...
TEST (active_transactions, inactive_votes_cache_existing_vote)
{
	...
	auto send = builder.send ()
				.previous (latest)
				.destination (key.pub)
				.balance (vban::genesis_amount - 100 * vban::Gxrb_ratio)
				.sign (vban::dev_genesis_key.prv, vban::dev_genesis_key.pub)
				.work (*system.work.generate (latest))
				.build_shared ();
	...
	// Insert vote
	auto vote1 (std::make_shared<vban::vote> (key.pub, key.prv, 1, std::vector<vban::block_hash> (1, send->hash ())));
	...
	ASSERT_EQ (1, node.stats.count (vban::stat::type::election, vban::stat::detail::vote_new));
	auto last_vote1 (election->votes ()[key.pub]);
	ASSERT_EQ (send->hash (), last_vote1.hash);
	ASSERT_EQ (1, last_vote1.timestamp);
	// Attempt to change vote with inactive_votes_cache
	vban::unique_lock<vban::mutex> active_lock (node.active.mutex);
	...
	ASSERT_EQ (0, node.stats.count (vban::stat::type::election, vban::stat::detail::vote_cached));
}
...
TEST (active_transactions, inactive_votes_cache_multiple_votes)
{
	...
	node.block_processor.flush ();
	// Process votes
	auto vote1 (std::make_shared<vban::vote> (key1.pub, key1.prv, 0, std::vector<vban::block_hash> (1, send1->hash ())));
	node.vote_processor.vote (vote1, std::make_shared<vban::transport::channel_loopback> (node));
	auto vote2 (std::make_shared<vban::vote> (vban::dev_genesis_key.pub, vban::dev_genesis_key.prv, 0, std::vector<vban::block_hash> (1, send1->hash ())));
	node.vote_processor.vote (vote2, std::make_shared<vban::transport::channel_loopback> (node));
	...
	ASSERT_EQ (2, node.stats.count (vban::stat::type::election, vban::stat::detail::vote_cached));
}
...
TEST (active_transactions, inactive_votes_cache_election_start)
{
	...
	auto send1 = send_block_builder.make_block ()
				 .previous (latest)
				 .destination (key1.pub)
				 .balance (vban::genesis_amount - 5000 * vban::Gxrb_ratio)
				 .sign (vban::dev_genesis_key.prv, vban::dev_genesis_key.pub)
				 .work (*system.work.generate (latest))
				 .build_shared ();
	auto send2 = send_block_builder.make_block ()
				 ...
				 .balance (vban::genesis_amount - 10000 * vban::Gxrb_ratio)
				 .sign (vban::dev_genesis_key.prv, vban::dev_genesis_key.pub)
				 .work (*system.work.generate (send1->hash ()))
				 .build_shared ();
	...
	auto send4 = send_block_builder.make_block ()
				 ...
				 .build_shared ();
	// Inactive votes
	std::vector<vban::block_hash> hashes{ open1->hash (), open2->hash (), send4->hash () };
	auto vote1 (std::make_shared<vban::vote> (key1.pub, key1.prv, 0, hashes));
	node.vote_processor.vote (vote1, std::make_shared<vban::transport::channel_loopback> (node));
	ASSERT_TIMELY (5s, node.active.inactive_votes_cache_size () == 3);
	ASSERT_TRUE (node.active.empty ());
	ASSERT_EQ (1, node.ledger.cache.cemented_count);
	// 2 votes are required to start election (dev network)
	auto vote2 (std::make_shared<vban::vote> (key2.pub, key2.prv, 0, hashes));
	node.vote_processor.vote (vote2, std::make_shared<vban::transport::channel_loopback> (node));
	...
	auto vote0 (std::make_shared<vban::vote> (vban::dev_genesis_key.pub, vban::dev_genesis_key.prv, std::numeric_limits<uint64_t>::max (), hashes)); // Final vote for confirmation
	node.vote_processor.vote (vote0, std::make_shared<vban::transport::channel_loopback> (node));
	ASSERT_TIMELY (5s, node.active.empty ());
...
TEST (active_transactions, vote_replays)
{
	...
	node.process_active (open1);
	vban::blocks_confirm (node, { send1, open1 });
	ASSERT_EQ (2, node.active.size ());
	// First vote is not a replay and confirms the election, second vote should be a replay since the election has confirmed but not yet removed
	auto vote_send1 (std::make_shared<vban::vote> (vban::dev_genesis_key.pub, vban::dev_genesis_key.prv, std::numeric_limits<uint64_t>::max (), send1));
	ASSERT_EQ (vban::vote_code::vote, node.active.vote (vote_send1));
	ASSERT_EQ (2, node.active.size ());
	ASSERT_EQ (vban::vote_code::replay, node.active.vote (vote_send1));
	...
	ASSERT_EQ (vban::vote_code::replay, node.active.vote (vote_send1));
	// Open new account
	auto vote_open1 (std::make_shared<vban::vote> (vban::dev_genesis_key.pub, vban::dev_genesis_key.prv, std::numeric_limits<uint64_t>::max (), open1));
	ASSERT_EQ (vban::vote_code::vote, node.active.vote (vote_open1));
	ASSERT_EQ (1, node.active.size ());
	ASSERT_EQ (vban::vote_code::replay, node.active.vote (vote_open1));
	ASSERT_TIMELY (3s, node.active.empty ());
	ASSERT_EQ (vban::vote_code::replay, node.active.vote (vote_open1));
	ASSERT_EQ (vban::Gxrb_ratio, node.ledger.weight (key.pub));

	auto send2 = builder.make_block ()
				 .account (key.pub)
				 .previous (open1->hash ())
				 .representative (key.pub)
				 .balance (vban::Gxrb_ratio - 1)
				 .link (key.pub)
				 .sign (key.prv, key.pub)
	...
	vban::blocks_confirm (node, { send2 });
	ASSERT_EQ (1, node.active.size ());
	auto vote1_send2 (std::make_shared<vban::vote> (vban::dev_genesis_key.pub, vban::dev_genesis_key.prv, std::numeric_limits<uint64_t>::max (), send2));
	auto vote2_send2 (std::make_shared<vban::vote> (key.pub, key.prv, 0, send2));
	ASSERT_EQ (vban::vote_code::vote, node.active.vote (vote2_send2));
	ASSERT_EQ (1, node.active.size ());
	ASSERT_EQ (vban::vote_code::replay, node.active.vote (vote2_send2));
	ASSERT_EQ (1, node.active.size ());
	ASSERT_EQ (vban::vote_code::vote, node.active.vote (vote1_send2));
	ASSERT_EQ (1, node.active.size ());
	ASSERT_EQ (vban::vote_code::replay, node.active.vote (vote1_send2));
	ASSERT_TIMELY (3s, node.active.empty ());
	...
	ASSERT_EQ (vban::vote_code::indeterminate, node.active.vote (vote_send1));
	ASSERT_EQ (vban::vote_code::indeterminate, node.active.vote (vote_open1));
	ASSERT_EQ (vban::vote_code::indeterminate, node.active.vote (vote1_send2));
	ASSERT_EQ (vban::vote_code::indeterminate, node.active.vote (vote2_send2));
}
}
...
TEST (active_transactions, dropped_cleanup)
{
	...
	block->sideband_set (vban::block_sideband (vban::genesis_account, 0, vban::genesis_amount, 1, vban::seconds_since_epoch (), vban::epoch::epoch_0, false, false, false, vban::epoch::epoch_0));

	// Add to network filter to ensure proper cleanup after the election is dropped
	std::vector<uint8_t> block_bytes;
	{
		vban::vectorstream stream (block_bytes);
	...
	ASSERT_EQ (1, node.stats.count (vban::stat::type::election, vban::stat::detail::election_drop_all));
...
TEST (active_transactions, republish_winner)
{
	...
	ASSERT_TIMELY (3s, node2.stats.count (vban::stat::type::message, vban::stat::detail::publish, vban::stat::dir::in) == 1);
	...
	for (auto i (0); i < 5; i++)
	{
		auto fork = builder.make_block ()
					...
					.previous (genesis.hash ())
					.representative (vban::dev_genesis_key.pub)
					.balance (vban::genesis_amount - 1 - i)
					.link (key.pub)
					.sign (vban::dev_genesis_key.prv, vban::dev_genesis_key.pub)
					.work (*system.work.generate (genesis.hash ()))
					.build_shared ();
	...
	ASSERT_NE (nullptr, election);
	auto vote = std::make_shared<vban::vote> (vban::dev_genesis_key.pub, vban::dev_genesis_key.prv, std::numeric_limits<uint64_t>::max (), std::vector<vban::block_hash>{ fork->hash () });
...
TEST (active_transactions, fork_filter_cleanup)
{
	...
	ASSERT_TIMELY (3s, node1.stats.count (vban::stat::type::message, vban::stat::detail::publish, vban::stat::dir::in) > 0);
	node1.block_processor.flush ();
...
TEST (active_transactions, fork_replacement_tally)
{
	...
	std::vector<vban::keypair> keys (reps_count);
	...
	for (auto i (0); i < reps_count; i++)
	{
		...
		// Confirmation
		auto vote (std::make_shared<vban::vote> (vban::dev_genesis_key.pub, vban::dev_genesis_key.prv, std::numeric_limits<uint64_t>::max (), std::vector<vban::block_hash>{ send->hash (), open->hash () }));
		node1.vote_processor.vote (vote, std::make_shared<vban::transport::channel_loopback> (node1));
	}
	node1.block_processor.flush ();
	...
	auto send_last = builder.make_block ()
					 ...
					 .previous (latest)
					 .representative (vban::dev_genesis_key.pub)
					 .balance (balance - 2 * vban::Gxrb_ratio)
					 .link (key.pub)
					 .sign (vban::dev_genesis_key.prv, vban::dev_genesis_key.pub)
					 .work (*system.work.generate (latest))
					 .build_shared ();
	...
	for (auto i (0); i < reps_count; i++)
	{
		auto fork = builder.make_block ()
					...
					.previous (latest)
					.representative (vban::dev_genesis_key.pub)
					.balance (balance - vban::Gxrb_ratio - i)
					.link (key.pub)
					.sign (vban::dev_genesis_key.prv, vban::dev_genesis_key.pub)
					.work (*system.work.generate (latest))
					.build_shared ();
	...
	for (auto i (0); i < reps_count; i++)
	{
		auto fork = builder.make_block ()
					...
					.work (*system.work.generate (latest))
					.build_shared ();
		auto vote (std::make_shared<vban::vote> (keys[i].pub, keys[i].prv, 0, std::vector<vban::block_hash>{ fork->hash () }));
		node1.vote_processor.vote (vote, std::make_shared<vban::transport::channel_loopback> (node1));
		node1.vote_processor.flush ();
		node1.process_active (fork);
	}
	...
	auto vote (std::make_shared<vban::vote> (vban::dev_genesis_key.pub, vban::dev_genesis_key.prv, 0, std::vector<vban::block_hash>{ send_last->hash () }));
	node1.vote_processor.vote (vote, std::make_shared<vban::transport::channel_loopback> (node1));
	node1.vote_processor.flush ();
	node2.network.flood_block (send_last);
	ASSERT_TIMELY (3s, node1.stats.count (vban::stat::type::message, vban::stat::detail::publish, vban::stat::dir::in) > 1);
	node1.block_processor.flush ();
	std::this_thread::sleep_for (50ms);
	...
	ASSERT_TRUE (votes2.find (vban::dev_genesis_key.pub) != votes2.end ());
}
...
TEST (active_transactions, confirmation_consistency)
{
	...
	{
		auto block (system.wallet (0)->send_action (vban::dev_genesis_key.pub, vban::public_key (), node.config.receive_minimum.number ()));
...
TEST (active_transactions, restart_dropped)
{
	...
	auto send = vban::state_block_builder ()
				...
				.previous (genesis.hash ())
				.representative (vban::dev_genesis_key.pub)
				.balance (vban::genesis_amount - vban::xrb_ratio)
	...
	node.scheduler.flush ();
	ASSERT_EQ (1, node.active.size ());
	ASSERT_EQ (1, node.stats.count (vban::stat::type::election, vban::stat::detail::election_restart));
	...
	ASSERT_EQ (1, node.active.size ());
	ASSERT_EQ (1, node.ledger.cache.cemented_count);
	ASSERT_EQ (2, node.stats.count (vban::stat::type::election, vban::stat::detail::election_restart));
...
TEST (active_transactions, conflicting_block_vote_existing_election)
{
	...
	auto fork = builder.make_block ()
				...
				.work (*system.work.generate (genesis.hash ()))
				.build_shared ();
	auto vote_fork (std::make_shared<vban::vote> (vban::dev_genesis_key.pub, vban::dev_genesis_key.prv, std::numeric_limits<uint64_t>::max (), fork));

	ASSERT_EQ (vban::process_result::progress, node.process_local (send).code);
	node.scheduler.flush ();
	ASSERT_EQ (1, node.active.size ());
	...
	ASSERT_EQ (vban::process_result::fork, node.process_local (fork).code);
...
TEST (active_transactions, pessimistic_elections)
{
	...
	{
		vban::election election1 (
		node, send, [] (auto const &) {}, [] (auto const &) {}, vban::election_behavior::normal);
		vban::election election2 (
		node, open, [] (auto const &) {}, [] (auto const &) {}, vban::election_behavior::normal);
	...
	ASSERT_EQ (2, node.active.expired_optimistic_election_infos.size ());
	auto election_started_it = node.active.expired_optimistic_election_infos.get<vban::active_transactions::tag_election_started> ().begin ();
	ASSERT_EQ (election_started_it->account, vban::genesis_account);
	...
	vban::confirmation_height_info genesis_confirmation_height_info;
	vban::confirmation_height_info key1_confirmation_height_info;
	{
		auto transaction = node.store.tx_begin_read ();
		node.store.confirmation_height_get (transaction, vban::genesis_account, genesis_confirmation_height_info);
...
TEST (active_transactions, list_active)
{
	...
	vban::blocks_confirm (node, { send, send2, open });
...
TEST (active_transactions, fifo)
{
	...
	auto send0 = builder.make_block ()
				 ...
				 .work (*system.work.generate (vban::genesis_hash))
				 .build_shared ();
	ASSERT_EQ (vban::process_result::progress, node.process (*send0).code);
	vban::blocks_confirm (node, { send0 }, true);
	...
	vban::blocks_confirm (node, { send1 }, true);
	...
	auto receive0 = builder.make_block ()
					...
					.build_shared ();
	ASSERT_EQ (vban::process_result::progress, node.process (*receive0).code);
	auto receive1 = builder.make_block ()
					...
					.work (*system.work.generate (key1.pub))
					.build_shared ();
	ASSERT_EQ (vban::process_result::progress, node.process (*receive1).code);
	...
	ASSERT_EQ (1, node.stats.count (vban::stat::type::election, vban::stat::detail::election_drop_overflow));
