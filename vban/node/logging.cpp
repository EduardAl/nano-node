...
BOOST_LOG_ATTRIBUTE_KEYWORD (severity, "Severity", vban::severity_level)

boost::shared_ptr<boost::log::sinks::synchronous_sink<boost::log::sinks::text_file_backend>> vban::logging::file_sink;
std::atomic_flag vban::logging::logging_already_added ATOMIC_FLAG_INIT;

void vban::logging::init (boost::filesystem::path const & application_path_a)
{
	if (!logging_already_added.test_and_set ())
	{
		...
		if (!network_constants.is_dev_network ())
		{
			...
			{
				static auto event_sink = boost::make_shared<boost::log::sinks::synchronous_sink<boost::log::sinks::simple_event_log_backend>> (boost::log::keywords::log_name = "Vban", boost::log::keywords::log_source = "Vban");
				event_sink->set_formatter (format);

				// Currently only mapping sys log errors
				boost::log::sinks::event_log::custom_event_type_mapping<vban::severity_level> mapping ("Severity");
				mapping[vban::severity_level::error] = boost::log::sinks::event_log::error;
				event_sink->locked_backend ()->set_event_type_mapper (mapping);

				// Only allow messages or error or greater severity to the event log
				event_sink->set_filter (severity >= vban::severity_level::error);
			...
			// Currently only mapping sys log errors
			boost::log::sinks::syslog::custom_severity_mapping<vban::severity_level> mapping ("Severity");
			mapping[vban::severity_level::error] = boost::log::sinks::syslog::error;
			sys_sink->locked_backend ()->set_severity_mapper (mapping);

			// Only allow messages or error or greater severity to the sys log
			sys_sink->set_filter (severity >= vban::severity_level::error);
...
//clang-format off
		...
		if (stable_log_filename)
		{
			...
			vban::set_secure_perm_file (file_name);
...
void vban::logging::release_file_sink ()
{
	if (logging_already_added.test_and_set ())
	{
		boost::log::core::get ()->remove_sink (vban::logging::file_sink);
		vban::logging::file_sink.reset ();
		logging_already_added.clear ();
	}
}

vban::error vban::logging::serialize_toml (vban::tomlconfig & toml) const
{
...
vban::error vban::logging::deserialize_toml (vban::tomlconfig & toml)
{
...
vban::error vban::logging::serialize_json (vban::jsonconfig & json) const
{
...
bool vban::logging::upgrade_json (unsigned version_a, vban::jsonconfig & json)
{
	json.put ("version", json_version ());
	switch (version_a)
	{
...
vban::error vban::logging::deserialize_json (bool & upgraded_a, vban::jsonconfig & json)
{
	int version_l{ 1 };
...
bool vban::logging::ledger_logging () const
{
	return ledger_logging_value;
}

bool vban::logging::ledger_duplicate_logging () const
{
	return ledger_logging () && ledger_duplicate_logging_value;
}

bool vban::logging::ledger_rollback_logging () const
{
	return ledger_rollback_logging_value;
}

bool vban::logging::vote_logging () const
{
	return vote_logging_value;
}

bool vban::logging::election_expiration_tally_logging () const
{
	return election_expiration_tally_logging_value;
}

bool vban::logging::election_fork_tally_logging () const
{
	return election_fork_tally_logging_value;
}

bool vban::logging::network_logging () const
{
	return network_logging_value;
}

bool vban::logging::network_timeout_logging () const
{
	return network_logging () && network_timeout_logging_value;
}

bool vban::logging::network_message_logging () const
{
	return network_logging () && network_message_logging_value;
}

bool vban::logging::network_publish_logging () const
{
	return network_logging () && network_publish_logging_value;
}

bool vban::logging::network_packet_logging () const
{
	return network_logging () && network_packet_logging_value;
}

bool vban::logging::network_keepalive_logging () const
{
	return network_logging () && network_keepalive_logging_value;
}

bool vban::logging::network_node_id_handshake_logging () const
{
	return network_logging () && network_node_id_handshake_logging_value;
}

bool vban::logging::network_telemetry_logging () const
{
	return network_logging () && network_telemetry_logging_value;
}

bool vban::logging::network_rejected_logging () const
{
	return network_logging () && network_rejected_logging_value;
}

bool vban::logging::node_lifetime_tracing () const
{
	return node_lifetime_tracing_value;
}

bool vban::logging::insufficient_work_logging () const
{
	return network_logging () && insufficient_work_logging_value;
}

bool vban::logging::log_ipc () const
{
	return network_logging () && log_ipc_value;
}

bool vban::logging::bulk_pull_logging () const
{
	return network_logging () && bulk_pull_logging_value;
}

bool vban::logging::callback_logging () const
{
	return network_logging ();
}

bool vban::logging::work_generation_time () const
{
	return work_generation_time_value;
}

bool vban::logging::upnp_details_logging () const
{
	return upnp_details_logging_value;
}

bool vban::logging::timing_logging () const
{
	return timing_logging_value;
}

bool vban::logging::active_update_logging () const
{
	return active_update_value;
}

bool vban::logging::log_to_cerr () const
{
	return log_to_cerr_value;
}

bool vban::logging::single_line_record () const
{
