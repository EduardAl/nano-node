...
TEST (toml, diff)
{
	vban::tomlconfig defaults, other;
...
TEST (toml, daemon_config_update_array)
{
	...
	vban::daemon_config c (data_path);
...
TEST (toml, rpc_config_deserialize_defaults)
{
	...
	vban::tomlconfig t;
	t.read (ss);
	vban::rpc_config conf;
	vban::rpc_config defaults;
...
TEST (toml, daemon_config_deserialize_defaults)
{
	...
	vban::tomlconfig t;
	t.read (ss);
	vban::daemon_config conf;
	vban::daemon_config defaults;
	conf.deserialize_toml (t);
...
TEST (toml, base_override)
{
	...
	ASSERT_TRUE (t.get_error () == vban::error_config::missing_value);
	t.get_error ().clear ();

	// Query uint16 that's too big, make sure we have an error
	t.get_required<uint16_t> ("node.too_big", port);
	ASSERT_TRUE (t.get_error ());
	ASSERT_TRUE (t.get_error () == vban::error_config::invalid_value);
}

TEST (toml, put)
{
	vban::tomlconfig config;
	vban::tomlconfig config_node;
...
TEST (toml, daemon_config_deserialize_no_defaults)
{
	...
	preconfigured_representatives = ["vban_3arg3asgtigae3xckabaaewkx3bzsh7nwz7jkmjos79ihyaxwphhm6qgjps4"]
	receive_minimum = "999"
...
TEST (toml, daemon_config_deserialize_errors)
{
...

	{
		...
		ASSERT_EQ (conf.node.frontiers_confirmation, vban::frontiers_confirmation_mode::invalid);
	}
...
TEST (toml, daemon_read_config)
{
	...
	ASSERT_FALSE (boost::filesystem::exists (vban::get_node_toml_config_path (path)));
	ASSERT_FALSE (vban::read_node_config_toml (path, config));
	{
		auto error = vban::read_node_config_toml (path, config, invalid_overrides1);
		ASSERT_TRUE (error);
		ASSERT_EQ (error.get_message (), expected_message1);
	}
	{
		auto error = vban::read_node_config_toml (path, config, invalid_overrides2);
		ASSERT_TRUE (error);
	...
	vban::tomlconfig toml;
	toml.write (vban::get_node_toml_config_path (path));

	// Reading when there is a config file
	ASSERT_TRUE (boost::filesystem::exists (vban::get_node_toml_config_path (path)));
	ASSERT_FALSE (vban::read_node_config_toml (path, config));
	{
		auto error = vban::read_node_config_toml (path, config, invalid_overrides1);
		ASSERT_TRUE (error);
