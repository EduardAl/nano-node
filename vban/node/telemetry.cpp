...
vban::telemetry::telemetry (vban::network & network_a, vban::thread_pool & workers_a, vban::observer_set<vban::telemetry_data const &, vban::endpoint const &> & observers_a, vban::stat & stats_a, vban::network_params & network_params_a, bool disable_ongoing_requests_a) :
	network (network_a),
	workers (workers_a),
...
void vban::telemetry::stop ()
{
	stopped = true;
}

void vban::telemetry::set (vban::telemetry_ack const & message_a, vban::transport::channel const & channel_a)
{
	if (!stopped)
	{
		vban::unique_lock<vban::mutex> lk (mutex);
		vban::endpoint endpoint = channel_a.get_endpoint ();
		...
		if (it == recent_or_initial_request_telemetry_data.cend () || !it->undergoing_request)
		{
			...
			stats.inc (vban::stat::type::telemetry, vban::stat::detail::unsolicited_telemetry_ack);
			return;
		}

		recent_or_initial_request_telemetry_data.modify (it, [&message_a] (vban::telemetry_info & telemetry_info_a) {
...
bool vban::telemetry::verify_message (vban::telemetry_ack const & message_a, vban::transport::channel const & channel_a)
{
	...
	if (!node_id_mismatch)
	{
		...
		if (!remove_channel)
		{
			...
			if (remove_channel)
			{
				stats.inc (vban::stat::type::telemetry, vban::stat::detail::different_genesis_hash);
			}
		}
		else
		{
			stats.inc (vban::stat::type::telemetry, vban::stat::detail::invalid_signature);
		}
	}
	else
	{
		stats.inc (vban::stat::type::telemetry, vban::stat::detail::node_id_mismatch);
	}
...
std::chrono::milliseconds vban::telemetry::cache_plus_buffer_cutoff_time () const
{
...
bool vban::telemetry::within_cache_plus_buffer_cutoff (telemetry_info const & telemetry_info) const
{
...
bool vban::telemetry::within_cache_cutoff (telemetry_info const & telemetry_info) const
{
	auto is_within = (telemetry_info.last_response + cache_cutoff) >= std::chrono::steady_clock::now ();
	return !telemetry_info.awaiting_first_response () && is_within;
}

void vban::telemetry::ongoing_req_all_peers (std::chrono::milliseconds next_request_interval)
{
		...
		if (auto this_l = this_w.lock ())
		{
			...
			if (!this_l->stopped)
			{
				...
				struct channel_wrapper
				{
					...
					channel_wrapper (std::shared_ptr<vban::transport::channel> const & channel_a) :
						channel (channel_a)
					{
					}
				...
				mi::indexed_by<
					mi::hashed_unique<mi::tag<tag_endpoint>,
						mi::const_mem_fun<channel_wrapper, vban::endpoint, &channel_wrapper::endpoint>>,
					mi::hashed_unique<mi::tag<tag_channel>,
						mi::member<channel_wrapper, std::shared_ptr<vban::transport::channel>, &channel_wrapper::channel>>>> peers;
				// clang-format on
...

				{
					...
					vban::lock_guard<vban::mutex> guard (this_l->mutex);
...
std::unordered_map<vban::endpoint, vban::telemetry_data> vban::telemetry::get_metrics ()
{
	std::unordered_map<vban::endpoint, vban::telemetry_data> telemetry_data;

	vban::lock_guard<vban::mutex> guard (mutex);
	auto range = boost::make_iterator_range (recent_or_initial_request_telemetry_data);
	// clang-format off
	vban::transform_if (range.begin (), range.end (), std::inserter (telemetry_data, telemetry_data.end ()),
		[this](auto const & telemetry_info) { return this->within_cache_plus_buffer_cutoff (telemetry_info); },
		[](auto const & telemetry_info) { return std::pair<const vban::endpoint, vban::telemetry_data>{ telemetry_info.endpoint, telemetry_info.data }; });
	// clang-format on

	return telemetry_data;
}

void vban::telemetry::get_metrics_single_peer_async (std::shared_ptr<vban::transport::channel> const & channel_a, std::function<void (telemetry_data_response const &)> const & callback_a)
{
	auto invoke_callback_with_error = [&callback_a, &workers = this->workers, channel_a] () {
		...
		workers.push_task ([callback_a, endpoint] () {
			...
			callback_a ({ vban::telemetry_data{}, endpoint, error });
		});
	};
	...
	if (!stopped)
	{
		if (channel_a)
		{
			auto add_callback_async = [&workers = this->workers, &callback_a] (telemetry_data const & telemetry_data_a, vban::endpoint const & endpoint_a) {
			...
			else
			{
				...
				else
				{
					if (it == recent_or_initial_request_telemetry_data.cend ())
					{
						...
						recent_or_initial_request_telemetry_data.emplace (channel_a->get_endpoint (), vban::telemetry_data (), std::chrono::steady_clock::now (), true);
						it = recent_or_initial_request_telemetry_data.find (channel_a->get_endpoint ());
					}
					else
					{
						recent_or_initial_request_telemetry_data.modify (it, [] (vban::telemetry_info & telemetry_info_a) {
...
vban::telemetry_data_response vban::telemetry::get_metrics_single_peer (std::shared_ptr<vban::transport::channel> const & channel_a)
{
...
void vban::telemetry::fire_request_message (std::shared_ptr<vban::transport::channel> const & channel_a)
{
	uint64_t round_l;
	{
		auto it = recent_or_initial_request_telemetry_data.find (channel_a->get_endpoint ());
	...
	std::weak_ptr<vban::telemetry> this_w (shared_from_this ());
	...
	channel_a->send (message, [this_w, endpoint = channel_a->get_endpoint (), round_l](boost::system::error_code const & ec, size_t size_a) {
		if (auto this_l = this_w.lock ())
		{
			if (ec)
			{
				...
				this_l->stats.inc (vban::stat::type::telemetry, vban::stat::detail::failed_send_telemetry_req);
				vban::lock_guard<vban::mutex> guard (this_l->mutex);
			...
			else
			{
				...
				this_l->workers.add_timed_task (std::chrono::steady_clock::now () + this_l->response_time_cutoff, [round_l, this_w, endpoint]() {
					if (auto this_l = this_w.lock ())
					{
						...
						{
							this_l->stats.inc (vban::stat::type::telemetry, vban::stat::detail::no_response_received);
							this_l->channel_processed (endpoint, true);
						}
					}
				});			
	...
	vban::buffer_drop_policy::no_socket_drop);
	// clang-format on
}

void vban::telemetry::channel_processed (vban::endpoint const & endpoint_a, bool error_a)
{
...
void vban::telemetry::flush_callbacks_async (vban::endpoint const & endpoint_a, bool error_a)
{
	// Post to thread_pool so that it's truly async and not on the calling thread (same problem as std::async otherwise)
	workers.push_task ([endpoint_a, error_a, this_w = std::weak_ptr<vban::telemetry> (shared_from_this ())] () {
		if (auto this_l = this_w.lock ())
		{
			vban::unique_lock<vban::mutex> lk (this_l->mutex);
...
void vban::telemetry::invoke_callbacks (vban::endpoint const & endpoint_a, bool error_a)
{
	std::vector<std::function<void (telemetry_data_response const &)>> callbacks_l;
	telemetry_data_response response_data{ vban::telemetry_data (), endpoint_a, error_a };
	{
...
size_t vban::telemetry::telemetry_data_size ()
{
	vban::lock_guard<vban::mutex> guard (mutex);
	return recent_or_initial_request_telemetry_data.size ();
}

vban::telemetry_info::telemetry_info (vban::endpoint const & endpoint_a, vban::telemetry_data const & data_a, std::chrono::steady_clock::time_point last_response_a, bool undergoing_request_a) :
	endpoint (endpoint_a),
	data (data_a),
...
bool vban::telemetry_info::awaiting_first_response () const
{
	return data == vban::telemetry_data ();
}

std::unique_ptr<vban::container_info_component> vban::collect_container_info (telemetry & telemetry, std::string const & name)
{
	auto composite = std::make_unique<container_info_composite> (name);
	size_t callbacks_count;
	{
		vban::lock_guard<vban::mutex> guard (telemetry.mutex);
...
vban::telemetry_data vban::consolidate_telemetry_data (std::vector<vban::telemetry_data> const & telemetry_datas)
{
	...
	std::unordered_map<vban::block_hash, int> genesis_blocks;
	...
	auto strip_outliers_and_sum = [num_either_side_to_remove] (auto & counts) {
		...
		return std::accumulate (counts.begin (), counts.end (), vban::uint256_t (0), [] (vban::uint256_t total, auto count) {
			return total += count;
		});
	};
	...
	vban::telemetry_data consolidated_data;
...
vban::telemetry_data vban::local_telemetry_data (vban::ledger const & ledger_a, vban::network & network_a, uint64_t bandwidth_limit_a, vban::network_params const & network_params_a, std::chrono::steady_clock::time_point statup_time_a, uint64_t active_difficulty_a, vban::keypair const & node_id_a)
{
	...
	telemetry_data.peer_count = vban::narrow_cast<decltype (telemetry_data.peer_count)> (network_a.size ());
	telemetry_data.account_count = ledger_a.cache.account_count;
	telemetry_data.major_version = vban::get_major_node_version ();
	telemetry_data.minor_version = vban::get_minor_node_version ();
	telemetry_data.patch_version = vban::get_patch_node_version ();
	telemetry_data.pre_release_version = vban::get_pre_release_node_version ();
