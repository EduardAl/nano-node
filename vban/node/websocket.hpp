...
#include <vban/boost/beast/core.hpp>
#include <vban/boost/beast/websocket.hpp>
...
namespace websocket
{
	...
	{
	public:
		message (vban::websocket::topic topic_a) :
			topic (topic_a)
		{
		}
		message (vban::websocket::topic topic_a, boost::property_tree::ptree & tree_a) :
			topic (topic_a), contents (tree_a)
		{
		}

		std::string to_string () const;
		vban::websocket::topic topic;
	...
	{
	public:
		message block_confirmed (std::shared_ptr<vban::block> const & block_a, vban::account const & account_a, vban::amount const & amount_a, std::string subtype, bool include_block, vban::election_status const & election_status_a, std::vector<vban::vote_with_weight_info> const & election_votes_a, vban::websocket::confirmation_options const & options_a);
		message stopped_election (vban::block_hash const & hash_a);
		message vote_received (std::shared_ptr<vban::vote> const & vote_a, vban::vote_code code_a);
		message work_generation (vban::work_version const version_a, vban::block_hash const & root_a, uint64_t const work_a, uint64_t const difficulty_a, uint64_t const publish_threshold_a, std::chrono::milliseconds const & duration_a, std::string const & peer_a, std::vector<std::string> const & bad_peers_a, bool const completed_a = true, bool const cancelled_a = false);
		message work_cancelled (vban::work_version const version_a, vban::block_hash const & root_a, uint64_t const difficulty_a, uint64_t const publish_threshold_a, std::chrono::milliseconds const & duration_a, std::vector<std::string> const & bad_peers_a);
		message work_failed (vban::work_version const version_a, vban::block_hash const & root_a, uint64_t const difficulty_a, uint64_t const publish_threshold_a, std::chrono::milliseconds const & duration_a, std::vector<std::string> const & bad_peers_a);
		...
		message telemetry_received (vban::telemetry_data const &, vban::endpoint const &);
		message new_block_arrived (vban::block const & block_a);
	...
	{
	public:
		confirmation_options (vban::wallets & wallets_a);
		confirmation_options (boost::property_tree::ptree const & options_a, vban::wallets & wallets_a, vban::logger_mt & logger_a);
	...
	private:
		...
		boost::optional<vban::logger_mt &> logger;
	...
	{
	public:
		vote_options (boost::property_tree::ptree const & options_a, vban::logger_mt & logger_a);
	...
	public:
		...
		explicit session (vban::websocket::listener & listener_a, socket_type socket_a);
		~session ();
		...
		void write (vban::websocket::message message_a);

	private:
		/** The owning listener */
		vban::websocket::listener & ws_listener;
		/** Websocket */
		...
		vban::mutex subscriptions_mutex;
	...
	{
	public:
		listener (vban::logger_mt & logger_a, vban::wallets & wallets_a, boost::asio::io_context & io_ctx_a, boost::asio::ip::tcp::endpoint endpoint_a);
		...
		void broadcast_confirmation (std::shared_ptr<vban::block> const & block_a, vban::account const & account_a, vban::amount const & amount_a, std::string const & subtype, vban::election_status const & election_status_a, std::vector<vban::vote_with_weight_info> const & election_votes_a);

		/** Broadcast \p message to all session subscribing to the message topic. */
		void broadcast (vban::websocket::message message_a);

		vban::logger_mt & get_logger () const
		{
			return logger;
		}

		vban::wallets & get_wallets () const
		{
			return wallets;
		}
		 ...
		 */
		bool any_subscriber (vban::websocket::topic const & topic_a) const
		{
			return subscriber_count (topic_a) > 0;
		}
		/** Getter for subscriber count of a specific topic*/
		size_t subscriber_count (vban::websocket::topic const & topic_a) const
		{
	...
	private:
		...
		friend vban::websocket::session;

		/** Adds to subscription count of a specific topic*/
		void increase_subscriber_count (vban::websocket::topic const & topic_a);
		/** Removes from subscription count of a specific topic*/
		void decrease_subscriber_count (vban::websocket::topic const & topic_a);

		vban::logger_mt & logger;
		vban::wallets & wallets;
		boost::asio::ip::tcp::acceptor acceptor;
		socket_type socket;
		vban::mutex sessions_mutex;
