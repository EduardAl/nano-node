...
TEST (bulk_pull, no_address)
{
	...
	auto connection (std::make_shared<vban::bootstrap_server> (nullptr, system.nodes[0]));
	auto req = std::make_unique<vban::bulk_pull> ();
	req->start = 1;
	req->end = 2;
	connection->requests.push (std::unique_ptr<vban::message>{});
	auto request (std::make_shared<vban::bulk_pull_server> (connection, std::move (req)));
...
TEST (bulk_pull, genesis_to_end)
{
	...
	auto connection (std::make_shared<vban::bootstrap_server> (nullptr, system.nodes[0]));
	auto req = std::make_unique<vban::bulk_pull> ();
	req->start = vban::dev_genesis_key.pub;
	req->end.clear ();
	connection->requests.push (std::unique_ptr<vban::message>{});
	auto request (std::make_shared<vban::bulk_pull_server> (connection, std::move (req)));
	ASSERT_EQ (system.nodes[0]->latest (vban::dev_genesis_key.pub), request->current);
...
TEST (bulk_pull, end_not_owned)
{
	...
	ASSERT_NE (nullptr, system.wallet (0)->send_action (vban::dev_genesis_key.pub, key2.pub, 100));
	vban::block_hash latest (system.nodes[0]->latest (vban::dev_genesis_key.pub));
	vban::open_block open (0, 1, 2, vban::keypair ().prv, 4, 5);
	open.hashables.account = key2.pub;
	open.hashables.representative = key2.pub;
	open.hashables.source = latest;
	open.refresh ();
	open.signature = vban::sign_message (key2.prv, key2.pub, open.hash ());
...
TEST (bulk_pull, count_limit)
{
	...
	auto send1 (std::make_shared<vban::send_block> (node0->latest (vban::dev_genesis_key.pub), vban::dev_genesis_key.pub, 1, vban::dev_genesis_key.prv, vban::dev_genesis_key.pub, *system.work.generate (node0->latest (vban::dev_genesis_key.pub))));
	ASSERT_EQ (vban::process_result::progress, node0->process (*send1).code);
	auto receive1 (std::make_shared<vban::receive_block> (send1->hash (), send1->hash (), vban::dev_genesis_key.prv, vban::dev_genesis_key.pub, *system.work.generate (send1->hash ())));
	ASSERT_EQ (vban::process_result::progress, node0->process (*receive1).code);

	auto connection (std::make_shared<vban::bootstrap_server> (nullptr, node0));
	auto req = std::make_unique<vban::bulk_pull> ();
...
TEST (bootstrap_processor, process_one)
{
	...
	system.wallet (0)->insert_adhoc (vban::dev_genesis_key.prv);
	auto send (system.wallet (0)->send_action (vban::dev_genesis_key.pub, vban::dev_genesis_key.pub, 100));
	ASSERT_NE (nullptr, send);

	node_config.peering_port = vban::get_available_port ();
	node_flags.disable_rep_crawler = true;
	auto node1 (std::make_shared<vban::node> (system.io_ctx, vban::unique_path (), node_config, system.work, node_flags));
	vban::block_hash hash1 (node0->latest (vban::dev_genesis_key.pub));
	vban::block_hash hash2 (node1->latest (vban::dev_genesis_key.pub));
	ASSERT_NE (hash1, hash2);
	node1->bootstrap_initiator.bootstrap (node0->network.endpoint (), false);
	ASSERT_NE (node1->latest (vban::dev_genesis_key.pub), node0->latest (vban::dev_genesis_key.pub));
	ASSERT_TIMELY (10s, node1->latest (vban::dev_genesis_key.pub) == node0->latest (vban::dev_genesis_key.pub));
...
TEST (bootstrap_processor, process_two)
{
	...
	system.wallet (0)->insert_adhoc (vban::dev_genesis_key.prv);
	vban::block_hash hash1 (node0->latest (vban::dev_genesis_key.pub));
	ASSERT_NE (nullptr, system.wallet (0)->send_action (vban::dev_genesis_key.pub, vban::dev_genesis_key.pub, 50));
	vban::block_hash hash2 (node0->latest (vban::dev_genesis_key.pub));
	ASSERT_NE (nullptr, system.wallet (0)->send_action (vban::dev_genesis_key.pub, vban::dev_genesis_key.pub, 50));
	vban::block_hash hash3 (node0->latest (vban::dev_genesis_key.pub));
	ASSERT_NE (hash1, hash2);
...
TEST (bootstrap_processor, process_state)
{
	...
	ASSERT_EQ (node0->latest (vban::dev_genesis_key.pub), block2->hash ());
	ASSERT_NE (node1->latest (vban::dev_genesis_key.pub), block2->hash ());
	node1->bootstrap_initiator.bootstrap (node0->network.endpoint (), false);
	ASSERT_NE (node1->latest (vban::dev_genesis_key.pub), node0->latest (vban::dev_genesis_key.pub));
	ASSERT_TIMELY (10s, node1->latest (vban::dev_genesis_key.pub) == node0->latest (vban::dev_genesis_key.pub));
...
TEST (bootstrap_processor, process_new)
{
	...
	auto send (system.wallet (0)->send_action (vban::dev_genesis_key.pub, key2.pub, node1->config.receive_minimum.number ()));
	ASSERT_NE (nullptr, send);
	ASSERT_TIMELY (10s, !node1->balance (key2.pub).is_zero ());
	auto receive (node2->block (node2->latest (key2.pub)));
	ASSERT_NE (nullptr, receive);
	vban::uint256_t balance1 (node1->balance (vban::dev_genesis_key.pub));
	vban::uint256_t balance2 (node1->balance (key2.pub));
	...
	auto node3 (std::make_shared<vban::node> (system.io_ctx, vban::get_available_port (), vban::unique_path (), system.logging, system.work));
	...
	ASSERT_EQ (balance1, node3->balance (vban::dev_genesis_key.pub));
	node3->stop ();
}
...
TEST (bootstrap_processor, pull_diamond)
{
	...
	auto node0 (system.add_node (config, node_flags));
	vban::keypair key;
	auto send1 (std::make_shared<vban::send_block> (node0->latest (vban::dev_genesis_key.pub), key.pub, 0, vban::dev_genesis_key.prv, vban::dev_genesis_key.pub, *system.work.generate (node0->latest (vban::dev_genesis_key.pub))));
	ASSERT_EQ (vban::process_result::progress, node0->process (*send1).code);
	auto open (std::make_shared<vban::open_block> (send1->hash (), 1, key.pub, key.prv, key.pub, *system.work.generate (key.pub)));
	ASSERT_EQ (vban::process_result::progress, node0->process (*open).code);
	auto send2 (std::make_shared<vban::send_block> (open->hash (), vban::dev_genesis_key.pub, vban::uint256_t ("50000000000000000000000000000000000000") - 100, key.prv, key.pub, *system.work.generate (open->hash ())));
	ASSERT_EQ (vban::process_result::progress, node0->process (*send2).code);
	auto receive (std::make_shared<vban::receive_block> (send1->hash (), send2->hash (), vban::dev_genesis_key.prv, vban::dev_genesis_key.pub, *system.work.generate (send1->hash ())));
	ASSERT_EQ (vban::process_result::progress, node0->process (*receive).code);
	auto node1 (std::make_shared<vban::node> (system.io_ctx, vban::get_available_port (), vban::unique_path (), system.logging, system.work));
	ASSERT_FALSE (node1->init_error ());
	node1->bootstrap_initiator.bootstrap (node0->network.endpoint (), false);
	ASSERT_TIMELY (10s, node1->balance (vban::dev_genesis_key.pub) == 100);
	ASSERT_EQ (100, node1->balance (vban::dev_genesis_key.pub));
	node1->stop ();
}
...
TEST (bootstrap_processor, DISABLED_pull_requeue_network_error)
{
	...
	{
		vban::unique_lock<vban::mutex> lock (node1->bootstrap_initiator.connections->mutex);
		ASSERT_FALSE (attempt->stopped);
		++attempt->pulling;
		node1->bootstrap_initiator.connections->pulls.emplace_back (vban::dev_genesis_key.pub, send1->hash (), genesis.hash (), attempt->incremental_id);
		node1->bootstrap_initiator.connections->request_pull (lock);
		node2->stop ();
	}
	ASSERT_TIMELY (5s, attempt == nullptr || attempt->requeued_pulls == 1);
	ASSERT_EQ (0, node1->stats.count (vban::stat::type::bootstrap, vban::stat::detail::bulk_pull_failed_account, vban::stat::dir::in)); // Requeue is not increasing failed attempts
}
...
TEST (bootstrap_processor, push_diamond)
{
	...
	auto receive (std::make_shared<vban::receive_block> (send1->hash (), send2->hash (), vban::dev_genesis_key.prv, vban::dev_genesis_key.pub, *system.work.generate (send1->hash ())));
	ASSERT_EQ (vban::process_result::progress, node1->process (*receive).code);
	node1->bootstrap_initiator.bootstrap (node0->network.endpoint (), false);
	ASSERT_TIMELY (10s, node0->balance (vban::dev_genesis_key.pub) == 100);
	ASSERT_EQ (100, node0->balance (vban::dev_genesis_key.pub));
	node1->stop ();
}
...
TEST (bootstrap_processor, push_diamond_pruning)
{
	...
	auto node1 (std::make_shared<vban::node> (system.io_ctx, vban::unique_path (), config, system.work, node_flags, 1));
	ASSERT_FALSE (node1->init_error ());
	auto latest (node0->latest (vban::dev_genesis_key.pub));
	auto send1 (std::make_shared<vban::send_block> (latest, key.pub, 0, vban::dev_genesis_key.prv, vban::dev_genesis_key.pub, *system.work.generate (latest)));
	ASSERT_EQ (vban::process_result::progress, node1->process (*send1).code);
	auto open (std::make_shared<vban::open_block> (send1->hash (), 1, key.pub, key.prv, key.pub, *system.work.generate (key.pub)));
	ASSERT_EQ (vban::process_result::progress, node1->process (*open).code);
	// 1st bootstrap
	node1->bootstrap_initiator.bootstrap (node0->network.endpoint (), false);
	ASSERT_TIMELY (10s, node0->balance (key.pub) == vban::genesis_amount);
...
TEST (bootstrap_processor, push_one)
{
	...
	vban::uint256_t balance1 (node1->balance (vban::dev_genesis_key.pub));
	auto send (wallet->send_action (vban::dev_genesis_key.pub, key1.pub, 100));
	ASSERT_NE (nullptr, send);
	ASSERT_NE (balance1, node1->balance (vban::dev_genesis_key.pub));
	node1->bootstrap_initiator.bootstrap (node0->network.endpoint (), false);
	ASSERT_TIMELY (10s, node0->balance (vban::dev_genesis_key.pub) != balance1);
	node1->stop ();
}
...
TEST (bootstrap_processor, lazy_hash_pruning)
{
	...
	vban::blocks_confirm (*node1, { send1, receive1, change1, change2 }, true);
...
TEST (bootstrap_processor, lazy_unclear_state_link)
{
	...
	ASSERT_EQ (vban::process_result::progress, node1->process (*receive).code);
	// Start lazy bootstrap with last block in chain known
	auto node2 = system.add_node (vban::node_config (vban::get_available_port (), system.logging), node_flags);
	...
	ASSERT_EQ (0, node2->stats.count (vban::stat::type::bootstrap, vban::stat::detail::bulk_pull_failed_account, vban::stat::dir::in));
}
...
TEST (bootstrap_processor, lazy_unclear_state_link_not_existing)
{
	...
	ASSERT_TRUE (node2->ledger.block_or_pruned_exists (open->hash ()));
	ASSERT_TRUE (node2->ledger.block_or_pruned_exists (send2->hash ()));
	ASSERT_EQ (1, node2->stats.count (vban::stat::type::bootstrap, vban::stat::detail::bulk_pull_failed_account, vban::stat::dir::in));
}
...
TEST (bootstrap_processor, DISABLED_lazy_destinations)
{
	...
	auto send2 = builder
				 ...
				 .build_shared ();
	ASSERT_EQ (vban::process_result::progress, node1->process (*send2).code);
	auto open (std::make_shared<vban::open_block> (send1->hash (), key1.pub, key1.pub, key1.prv, key1.pub, *system.work.generate (key1.pub)));
	ASSERT_EQ (vban::process_result::progress, node1->process (*open).code);
	auto state_open = builder
					  ...
					  .build_shared ();
	ASSERT_EQ (vban::process_result::progress, node1->process (*state_open).code);
...
TEST (bootstrap_processor, lazy_pruning_missing_block)
{
	...
	// Confirm last block to prune previous
	vban::blocks_confirm (*node1, { send1, send2, open, state_open }, true);
	...
	// Start lazy bootstrap with last block in sender chain
	config.peering_port = vban::get_available_port ();
	auto node2 (std::make_shared<vban::node> (system.io_ctx, vban::unique_path (), config, system.work, node_flags, 1));
	node2->network.udp_channels.insert (node1->network.endpoint (), node1->network_params.protocol.protocol_version);
	node2->bootstrap_initiator.bootstrap_lazy (send2->hash ());
	// Check processed blocks
	...
	ASSERT_FALSE (node2->ledger.block_or_pruned_exists (state_open->hash ()));
	{
		...
		ASSERT_TRUE (node2->store.unchecked_exists (transaction, vban::unchecked_key (send2->root ().as_block_hash (), send2->hash ())));
	}
...
TEST (bootstrap_processor, multiple_attempts)
{
	...
	auto node2 (std::make_shared<vban::node> (system.io_ctx, vban::unique_path (), node_config, system.work));
...
{
	{
		std::shared_ptr<vban::frontier_req_server> hold; // Destructing tcp acceptor on non-existent io_context
		{
			vban::system system (1);
			auto connection (std::make_shared<vban::bootstrap_server> (nullptr, system.nodes[0]));
			auto req = std::make_unique<vban::frontier_req> ();
			req->start.clear ();
			req->age = std::numeric_limits<decltype (req->age)>::max ();
			req->count = std::numeric_limits<decltype (req->count)>::max ();
			connection->requests.push (std::unique_ptr<vban::message>{});
			hold = std::make_shared<vban::frontier_req_server> (connection, std::move (req));
		}
	}
	ASSERT_TRUE (true);
}
...
TEST (frontier_req, begin)
{
	...
	req->count = std::numeric_limits<decltype (req->count)>::max ();
	connection->requests.push (std::unique_ptr<vban::message>{});
	auto request (std::make_shared<vban::frontier_req_server> (connection, std::move (req)));
	ASSERT_EQ (vban::dev_genesis_key.pub, request->current);
	vban::genesis genesis;
...
TEST (frontier_req, end)
{
	...
	auto connection (std::make_shared<vban::bootstrap_server> (nullptr, system.nodes[0]));
	auto req = std::make_unique<vban::frontier_req> ();
	req->start = vban::dev_genesis_key.pub.number () + 1;
	req->age = std::numeric_limits<decltype (req->age)>::max ();
	req->count = std::numeric_limits<decltype (req->count)>::max ();
	connection->requests.push (std::unique_ptr<vban::message>{});
	auto request (std::make_shared<vban::frontier_req_server> (connection, std::move (req)));
...
TEST (frontier_req, count)
{
	...
	vban::keypair key1 ("ED5AE0A6505B14B67435C29FD9FEEBC26F597D147BC92F6D795FFAD7AFD3D967");
	vban::state_block_builder builder;
	...
	auto connection (std::make_shared<vban::bootstrap_server> (nullptr, node1));
	auto req = std::make_unique<vban::frontier_req> ();
	req->start.clear ();
	req->age = std::numeric_limits<decltype (req->age)>::max ();
	req->count = 1;
...
TEST (frontier_req, time_bound)
{
	...
	auto req2 (std::make_unique<vban::frontier_req> ());
	req2->start.clear ();
	req2->age = 1;
	req2->count = std::numeric_limits<decltype (req2->count)>::max ();
	auto connection2 (std::make_shared<vban::bootstrap_server> (nullptr, system.nodes[0]));
	connection2->requests.push (std::unique_ptr<vban::message>{});
	auto request2 (std::make_shared<vban::frontier_req_server> (connection, std::move (req2)));
...
TEST (frontier_req, confirmed_frontier)
{
	...
	auto node1 = system.nodes[0];
	vban::genesis genesis;
	vban::raw_key priv_key;
	// Public key before genesis in accounts table
	while (vban::pub_key (priv_key).number () >= vban::dev_genesis_key.pub.number ())
	{
		priv_key = vban::keypair ().prv;
	}
	vban::keypair key_before_genesis (priv_key.to_string ());
	// Public key after genesis in accounts table
	while (vban::pub_key (priv_key).number () <= vban::dev_genesis_key.pub.number ())
	{
		priv_key = vban::keypair ().prv;
	}
	vban::keypair key_after_genesis (priv_key.to_string ());
	vban::state_block_builder builder;
	...
	ASSERT_EQ (vban::process_result::progress, node1->process (*receive2).code);
	...
	req->header.flag_set (vban::message_header::frontier_req_only_confirmed);
	...
	auto connection2 (std::make_shared<vban::bootstrap_server> (nullptr, node1));
	auto req2 = std::make_unique<vban::frontier_req> ();
	...
	req2->header.flag_set (vban::message_header::frontier_req_only_confirmed);
	ASSERT_TRUE (req2->header.frontier_req_is_only_confirmed_present ());
	connection2->requests.push (std::unique_ptr<vban::message>{});
	auto request2 (std::make_shared<vban::frontier_req_server> (connection2, std::move (req2)));
	ASSERT_EQ (vban::dev_genesis_key.pub, request2->current);
	ASSERT_EQ (genesis.hash (), request2->frontier);

	// Request starting with account after genesis (confirmed only)
	auto connection3 (std::make_shared<vban::bootstrap_server> (nullptr, node1));
	auto req3 = std::make_unique<vban::frontier_req> ();
	...
	req3->header.flag_set (vban::message_header::frontier_req_only_confirmed);
	ASSERT_TRUE (req3->header.frontier_req_is_only_confirmed_present ());
	connection3->requests.push (std::unique_ptr<vban::message>{});
	auto request3 (std::make_shared<vban::frontier_req_server> (connection3, std::move (req3)));
	ASSERT_TRUE (request3->current.is_zero ());
	ASSERT_TRUE (request3->frontier.is_zero ());

	// Request for all accounts (unconfirmed blocks)
	auto connection4 (std::make_shared<vban::bootstrap_server> (nullptr, node1));
	auto req4 = std::make_unique<vban::frontier_req> ();
	req4->start.clear ();
	...
	connection4->requests.push (std::unique_ptr<vban::message>{});
	auto request4 (std::make_shared<vban::frontier_req_server> (connection4, std::move (req4)));
	...
	auto connection5 (std::make_shared<vban::bootstrap_server> (nullptr, node1));
	auto req5 = std::make_unique<vban::frontier_req> ();
	...
	connection5->requests.push (std::unique_ptr<vban::message>{});
	auto request5 (std::make_shared<vban::frontier_req_server> (connection5, std::move (req5)));
	...
	vban::blocks_confirm (*node1, { send1, receive1 }, true);
	ASSERT_TIMELY (5s, node1->block_confirmed (send1->hash ()) && node1->block_confirmed (receive1->hash ()));
	auto connection6 (std::make_shared<vban::bootstrap_server> (nullptr, node1));
	auto req6 = std::make_unique<vban::frontier_req> ();
	...
	req6->header.flag_set (vban::message_header::frontier_req_only_confirmed);
	ASSERT_TRUE (req6->header.frontier_req_is_only_confirmed_present ());
	connection6->requests.push (std::unique_ptr<vban::message>{});
	auto request6 (std::make_shared<vban::frontier_req_server> (connection6, std::move (req6)));
	...
	vban::blocks_confirm (*node1, { send2, receive2 }, true);
	ASSERT_TIMELY (5s, node1->block_confirmed (send2->hash ()) && node1->block_confirmed (receive2->hash ()));
	auto connection7 (std::make_shared<vban::bootstrap_server> (nullptr, node1));
	auto req7 = std::make_unique<vban::frontier_req> ();
	...
	req7->header.flag_set (vban::message_header::frontier_req_only_confirmed);
	ASSERT_TRUE (req7->header.frontier_req_is_only_confirmed_present ());
	connection7->requests.push (std::unique_ptr<vban::message>{});
	auto request7 (std::make_shared<vban::frontier_req_server> (connection7, std::move (req7)));
...
TEST (bulk, genesis)
{
	...
	vban::keypair key2;
	auto send (system.wallet (0)->send_action (vban::dev_genesis_key.pub, key2.pub, 100));
	ASSERT_NE (nullptr, send);
	vban::block_hash latest3 (node1->latest (vban::dev_genesis_key.pub));
	ASSERT_NE (latest1, latest3);

	node2->bootstrap_initiator.bootstrap (node1->network.endpoint (), false);
	ASSERT_TIMELY (10s, node2->latest (vban::dev_genesis_key.pub) == node1->latest (vban::dev_genesis_key.pub));
	ASSERT_EQ (node2->latest (vban::dev_genesis_key.pub), node1->latest (vban::dev_genesis_key.pub));
	node2->stop ();
}
...
TEST (bulk, offline_send)
{
	...
	vban::keypair key2;
	auto wallet (node2->wallets.create (vban::random_wallet_id ()));
	wallet->insert_adhoc (key2.prv);
	auto send1 (system.wallet (0)->send_action (vban::dev_genesis_key.pub, key2.pub, node1->config.receive_minimum.number ()));
	ASSERT_NE (nullptr, send1);
	ASSERT_NE (std::numeric_limits<vban::uint256_t>::max (), node1->balance (vban::dev_genesis_key.pub));
	node1->block_processor.flush ();
	...
	ASSERT_TIMELY (10s, node2->balance (vban::dev_genesis_key.pub) != std::numeric_limits<vban::uint256_t>::max ());
...
TEST (bulk, genesis_pruning)
{
	...
	ASSERT_EQ (latest1, latest2);
	vban::keypair key2;
	auto send1 (system.wallet (0)->send_action (vban::dev_genesis_key.pub, key2.pub, 100));
	ASSERT_NE (nullptr, send1);
	auto send2 (system.wallet (0)->send_action (vban::dev_genesis_key.pub, key2.pub, 100));
	ASSERT_NE (nullptr, send2);
	auto send3 (system.wallet (0)->send_action (vban::dev_genesis_key.pub, key2.pub, 100));
	...
	ASSERT_TIMELY (25s, node2->stats.count (vban::stat::type::bootstrap, vban::stat::detail::initiate, vban::stat::dir::out) >= 1 && !node2->bootstrap_initiator.in_progress ());
...
TEST (bulk_pull_account, basics)
{
	...
	auto send1 (system.wallet (0)->send_action (vban::genesis_account, key1.pub, 25));
	auto send2 (system.wallet (0)->send_action (vban::genesis_account, key1.pub, 10));
	auto send3 (system.wallet (0)->send_action (vban::genesis_account, key1.pub, 2));
	ASSERT_TIMELY (5s, system.nodes[0]->balance (key1.pub) == 25);
	auto connection (std::make_shared<vban::bootstrap_server> (nullptr, system.nodes[0]));

	{
		auto req = std::make_unique<vban::bulk_pull_account> ();
		req->account = key1.pub;
		req->minimum_amount = 5;
		req->flags = vban::bulk_pull_account_flags ();
		connection->requests.push (std::unique_ptr<vban::message>{});
		auto request (std::make_shared<vban::bulk_pull_account_server> (connection, std::move (req)));
		...
		ASSERT_EQ (vban::uint128_union (10), block_data.second.get ()->amount);
		ASSERT_EQ (vban::genesis_account, block_data.second.get ()->source);
...

	{
		...
		req->flags = vban::bulk_pull_account_flags::pending_address_only;
		auto request (std::make_shared<vban::bulk_pull_account_server> (connection, std::move (req)));
