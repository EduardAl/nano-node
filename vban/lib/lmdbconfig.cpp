...
vban::error vban::lmdb_config::serialize_toml (vban::tomlconfig & toml) const
{
	std::string sync_string;
	switch (sync)
	{
		case vban::lmdb_config::sync_strategy::always:
			sync_string = "always";
			break;
		case vban::lmdb_config::sync_strategy::nosync_safe:
			sync_string = "nosync_safe";
			break;
		case vban::lmdb_config::sync_strategy::nosync_unsafe:
			sync_string = "nosync_unsafe";
			break;
		case vban::lmdb_config::sync_strategy::nosync_unsafe_large_memory:
	...
	toml.put ("max_databases", max_databases, "Maximum open lmdb databases. Increase default if more than 100 wallets is required.\nNote: external management is recommended when a large amounts of wallets are required (see https://docs.vban.org/integration-guides/key-management/).\ntype:uin32");
	toml.put ("map_size", map_size, "Maximum ledger database map size in bytes.\ntype:uint64");
	return toml.get_error ();
}

vban::error vban::lmdb_config::deserialize_toml (vban::tomlconfig & toml, bool is_deprecated_lmdb_dbs_used)
{
	...
	if (!toml.get_error ())
	{
		...
		{
			sync = vban::lmdb_config::sync_strategy::always;
		}
		else if (sync_string == "nosync_safe")
		{
			sync = vban::lmdb_config::sync_strategy::nosync_safe;
		}
		else if (sync_string == "nosync_unsafe")
		{
			sync = vban::lmdb_config::sync_strategy::nosync_unsafe;
		}
		else if (sync_string == "nosync_unsafe_large_memory")
		{
			sync = vban::lmdb_config::sync_strategy::nosync_unsafe_large_memory;
		}
		else
		{
