...
{
public:
	mdb_iterator (vban::transaction const & transaction_a, MDB_dbi db_a, MDB_val const & val_a = MDB_val{}, bool const direction_asc = true)
	{
	...
	mdb_iterator (vban::mdb_iterator<T, U> && other_a)
	{
	...
	mdb_iterator (vban::mdb_iterator<T, U> const &) = delete;
	...
	std::pair<vban::db_val<MDB_val>, vban::db_val<MDB_val>> * operator-> ()
	{
		return &current;
	}
	...
	{
		auto const other_a (boost::polymorphic_downcast<vban::mdb_iterator<T, U> const *> (&base_a));
	...
	void clear ()
	{
		current.first = vban::db_val<MDB_val> ();
		current.second = vban::db_val<MDB_val> ();
		debug_assert (is_end_sentinal ());
	}

	vban::mdb_iterator<T, U> & operator= (vban::mdb_iterator<T, U> && other_a)
	{
		if (cursor != nullptr)
		{
			mdb_cursor_close (cursor);
	...
	std::pair<vban::db_val<MDB_val>, vban::db_val<MDB_val>> current;
...
{
public:
	mdb_merge_iterator (vban::transaction const & transaction_a, MDB_dbi db1_a, MDB_dbi db2_a) :
		impl1 (std::make_unique<vban::mdb_iterator<T, U>> (transaction_a, db1_a)),
		impl2 (std::make_unique<vban::mdb_iterator<T, U>> (transaction_a, db2_a))
	{
	}

	mdb_merge_iterator () :
		impl1 (std::make_unique<vban::mdb_iterator<T, U>> ()),
		impl2 (std::make_unique<vban::mdb_iterator<T, U>> ())
	{
	}

	mdb_merge_iterator (vban::transaction const & transaction_a, MDB_dbi db1_a, MDB_dbi db2_a, MDB_val const & val_a) :
		impl1 (std::make_unique<vban::mdb_iterator<T, U>> (transaction_a, db1_a, val_a)),
		impl2 (std::make_unique<vban::mdb_iterator<T, U>> (transaction_a, db2_a, val_a))
	{
	}

	mdb_merge_iterator (vban::mdb_merge_iterator<T, U> && other_a)
	{
		impl1 = std::move (other_a.impl1);
		impl2 = std::move (other_a.impl2);
	}

	mdb_merge_iterator (vban::mdb_merge_iterator<T, U> const &) = delete;
	...
	bool operator== (vban::store_iterator_impl<T, U> const & base_a) const override
	{
		debug_assert ((dynamic_cast<vban::mdb_merge_iterator<T, U> const *> (&base_a) != nullptr) && "Incompatible iterator comparison");
		auto & other (static_cast<vban::mdb_merge_iterator<T, U> const &> (base_a));
	...
	vban::mdb_merge_iterator<T, U> & operator= (vban::mdb_merge_iterator<T, U> &&) = default;
	vban::mdb_merge_iterator<T, U> & operator= (vban::mdb_merge_iterator<T, U> const &) = delete;

	mutable bool from_first_database{ false };

private:
	vban::mdb_iterator<T, U> & least_iterator () const
	{
		vban::mdb_iterator<T, U> * result;
	...
	std::unique_ptr<vban::mdb_iterator<T, U>> impl1;
	std::unique_ptr<vban::mdb_iterator<T, U>> impl2;
};
}
