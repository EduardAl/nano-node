...
vban::confirmation_height_bounded::confirmation_height_bounded (vban::ledger & ledger_a, vban::write_database_queue & write_database_queue_a, std::chrono::milliseconds batch_separate_pending_min_time_a, vban::logging const & logging_a, vban::logger_mt & logger_a, std::atomic<bool> & stopped_a, uint64_t & batch_write_size_a, std::function<void (std::vector<std::shared_ptr<vban::block>> const &)> const & notify_observers_callback_a, std::function<void (vban::block_hash const &)> const & notify_block_already_cemented_observers_callback_a, std::function<uint64_t ()> const & awaiting_processing_size_callback_a) :
	ledger (ledger_a),
...
vban::confirmation_height_bounded::top_and_next_hash vban::confirmation_height_bounded::get_next_block (boost::optional<top_and_next_hash> const & next_in_receive_chain_a, boost::circular_buffer_space_optimized<vban::block_hash> const & checkpoints_a, boost::circular_buffer_space_optimized<receive_source_pair> const & receive_source_pairs, boost::optional<receive_chain_details> & receive_details_a, vban::block const & original_block)
{
...
void vban::confirmation_height_bounded::process (std::shared_ptr<vban::block> original_block)
{
	if (pending_empty ())
	{
		clear_process_vars ();
	...
	boost::circular_buffer_space_optimized<vban::block_hash> checkpoints{ max_items };
	...
	do
	{
		...
		vban::account account (block->account ());
		if (account.is_zero ())
		{
		...
		if (!hit_receive || (receive_source_pairs.size () == 1 && top_most_non_receive_block_hash != current))
		{
			...
			if ((max_batch_write_size_reached || should_output || force_write) && !pending_writes.empty ())
			{
				...
				if (write_database_queue.process (vban::writer::confirmation_height))
				{
...
vban::block_hash vban::confirmation_height_bounded::get_least_unconfirmed_hash_from_top_level (vban::transaction const & transaction_a, vban::block_hash const & hash_a, vban::account const & account_a, vban::confirmation_height_info const & confirmation_height_info_a, uint64_t & block_height_a)
{
	vban::block_hash least_unconfirmed_hash = hash_a;
...
bool vban::confirmation_height_bounded::iterate (vban::read_transaction const & transaction_a, uint64_t bottom_height_a, vban::block_hash const & bottom_hash_a, boost::circular_buffer_space_optimized<vban::block_hash> & checkpoints_a, vban::block_hash & top_most_non_receive_block_hash_a, vban::block_hash const & top_level_hash_a, boost::circular_buffer_space_optimized<receive_source_pair> & receive_source_pairs_a, vban::account const & account_a)
{
	...
	while (!hash.is_zero () && !reached_target && !stopped)
	{
		...
		if (!source.is_zero () && !ledger.is_epoch_link (source) && ledger.store.block_exists (transaction_a, source))
		{
			...
			auto next = !sideband.successor.is_zero () && sideband.successor != top_level_hash_a ? boost::optional<vban::block_hash> (sideband.successor) : boost::none;
...
void vban::confirmation_height_bounded::prepare_iterated_blocks_for_cementing (preparation_data & preparation_data_a)
{
...
void vban::confirmation_height_bounded::cement_blocks (vban::write_guard & scoped_write_guard_a)
{
	...
	std::vector<std::shared_ptr<vban::block>> cemented_blocks;
	...
	vban::timer<> cemented_batch_timer;
	auto error = false;
	{
		// This only writes to the confirmation_height table and is the only place to do so in a single process
		auto transaction (ledger.store.tx_begin_write ({}, { vban::tables::confirmation_height }));
		...
		while (!error && !pending_writes.empty ())
		{
			...
			auto write_confirmation_height = [&account, &ledger = ledger, &transaction] (uint64_t num_blocks_cemented, uint64_t confirmation_height, vban::block_hash const & confirmed_frontier) {
#ifndef NDEBUG
				// Extra debug checks
				vban::confirmation_height_info confirmation_height_info;
				ledger.store.confirmation_height_get (transaction, account, confirmation_height_info);
...
#endif
				ledger.store.confirmation_height_put (transaction, account, vban::confirmation_height_info{ confirmation_height, confirmed_frontier });
				ledger.cache.cemented_count += num_blocks_cemented;
				ledger.stats.add (vban::stat::type::confirmation_height, vban::stat::detail::blocks_confirmed, vban::stat::dir::in, num_blocks_cemented);
				ledger.stats.add (vban::stat::type::confirmation_height, vban::stat::detail::blocks_confirmed_bounded, vban::stat::dir::in, num_blocks_cemented);
			};
			...
			if (pending.top_height > confirmation_height_info.height)
			{
				...
				vban::block_hash new_cemented_frontier;
				...
				for (auto num_blocks_iterated = 0; num_blocks_confirmed - num_blocks_iterated != 0; ++num_blocks_iterated)
				{
					...
					if (cemented_blocks.size () > batch_write_size + (batch_write_size / 10))
					{
						...
						{
							scoped_write_guard_a = write_database_queue.wait (vban::writer::confirmation_height);
							transaction.renew ();
						}
...
bool vban::confirmation_height_bounded::pending_empty () const
{
	return pending_writes.empty ();
}

void vban::confirmation_height_bounded::clear_process_vars ()
{
	accounts_confirmed_info.clear ();
	accounts_confirmed_info_size = 0;
}

vban::confirmation_height_bounded::receive_chain_details::receive_chain_details (vban::account const & account_a, uint64_t height_a, vban::block_hash const & hash_a, vban::block_hash const & top_level_a, boost::optional<vban::block_hash> next_a, uint64_t bottom_height_a, vban::block_hash const & bottom_most_a) :
	account (account_a),
	height (height_a),
	hash (hash_a),
...
vban::confirmation_height_bounded::write_details::write_details (vban::account const & account_a, uint64_t bottom_height_a, vban::block_hash const & bottom_hash_a, uint64_t top_height_a, vban::block_hash const & top_hash_a) :
	account (account_a),
	bottom_height (bottom_height_a),
...
vban::confirmation_height_bounded::receive_source_pair::receive_source_pair (confirmation_height_bounded::receive_chain_details const & receive_details_a, const block_hash & source_a) :
	receive_details (receive_details_a),
	source_hash (source_a)
{
}

vban::confirmation_height_bounded::confirmed_info::confirmed_info (uint64_t confirmed_height_a, vban::block_hash const & iterated_frontier_a) :
	confirmed_height (confirmed_height_a),
	iterated_frontier (iterated_frontier_a)
{
}

std::unique_ptr<vban::container_info_component> vban::collect_container_info (confirmation_height_bounded & confirmation_height_bounded, std::string const & name_a)
{
