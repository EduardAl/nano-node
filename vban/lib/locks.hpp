...
#define USING_VBAN_TIMED_LOCKS (VBAN_TIMED_LOCKS > 0)
...
{
class mutex;
extern vban::mutex * mutex_to_filter;
extern vban::mutex mutex_to_filter_mutex;
...
template <>
class lock_guard<vban::mutex> final
{
public:
	explicit lock_guard (vban::mutex & mutex_a);
	~lock_guard () noexcept;
...
template <typename Mutex, typename = std::enable_if_t<std::is_same<Mutex, vban::mutex>::value>>
...
/** Assumes std implementations of std::condition_variable never actually call vban::unique_lock::lock/unlock,
...
public:
	...
	void wait (vban::unique_lock<vban::mutex> & lt);

	template <typename Pred>
	void wait (vban::unique_lock<vban::mutex> & lk, Pred pred)
	{
		while (!pred ())
		{
			wait (lk);
	...
	std::cv_status wait_until (vban::unique_lock<vban::mutex> & lk, std::chrono::time_point<Clock, Duration> const & timeout_time)
	{
		if (!lk.mut || !lk.owns)
		{
	...
	bool wait_until (vban::unique_lock<vban::mutex> & lk, std::chrono::time_point<Clock, Duration> const & timeout_time, Pred pred)
	{
		while (!pred ())
		{
	...
	void wait_for (vban::unique_lock<vban::mutex> & lk, std::chrono::duration<Rep, Period> const & rel_time)
	{
		wait_until (lk, std::chrono::steady_clock::now () + rel_time);
	}

	template <typename Rep, typename Period, typename Pred>
	bool wait_for (vban::unique_lock<vban::mutex> & lk, std::chrono::duration<Rep, Period> const & rel_time, Pred pred)
	{
