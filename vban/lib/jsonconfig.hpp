...
class jsonconfig : public vban::configbase
{
public:
	jsonconfig ();
	jsonconfig (boost::property_tree::ptree & tree_a, std::shared_ptr<vban::error> const & error_a = nullptr);
	 ...
	 */
	template <typename T>
	vban::error & read_and_update (T & object, boost::filesystem::path const & path_a)
	{
	...
	jsonconfig & put_child (std::string const & key_a, vban::jsonconfig & conf_a);
	jsonconfig & replace_child (std::string const & key_a, vban::jsonconfig & conf_a);
