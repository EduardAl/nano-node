...
#include <vban/boost/asio/connect.hpp>
#include <vban/boost/asio/ip/tcp.hpp>
#include <vban/boost/asio/spawn.hpp>
...
void write_config_files (boost::filesystem::path const & data_path, int index)
{
	...
	toml.write (vban::get_node_toml_config_path (data_path));
	...
	// Write rpc config
	vban::tomlconfig toml_rpc;
	rpc_config.serialize_toml (toml_rpc);
	toml_rpc.write (vban::get_rpc_toml_config_path (data_path));
}
...
int main (int argc, char * const * argv)
{
	...
	description.add_options ()
		...
		("node_path", boost::program_options::value<std::string> (), "The path to the vban_node to test")
		("rpc_path", boost::program_options::value<std::string> (), "The path to the vban_rpc to test");
	// clang-format on
	...
	{
		auto node_filepath = running_executable_filepath.parent_path () / "vban_node";
	...
	{
		std::cerr << "vban_node executable could not be found in " << node_path << std::endl;
		return 1;
	}
	...
	{
		std::cerr << "vban_rpc executable could not be found in " << rpc_path << std::endl;
		return 1;
	}
	...
	{
		auto data_path = vban::unique_path ();
	...
	vban::signal_handler_impl = [&ioc] () {
		ioc.stop ();
	};

	std::signal (SIGINT, &vban::signal_handler);
	std::signal (SIGTERM, &vban::signal_handler);
	...
	std::thread t ([send_count, &ioc, &primary_node_results, &resolver, &node_count, &destination_count] () {
		...
		wallet_add_rpc (ioc, primary_node_results, wallet, vban::dev_genesis_key.prv.to_string ());
		...
		for (auto i = 0; i < send_count; ++i)
		{
			...
			boost::asio::spawn (ioc, [&ioc, &primary_node_results, &wallet, destination_account, &send_calls_remaining] (boost::asio::yield_context yield) {
				send_receive (ioc, wallet, vban::genesis_account.to_account (), destination_account->as_string, send_calls_remaining, primary_node_results, yield);
			});
		}
	...
	vban::thread_runner runner (ioc, simultaneous_process_calls);
	t.join ();
	runner.join ();
