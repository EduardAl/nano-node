...
TEST (system, receive_while_synchronizing)
{
	std::vector<boost::thread> threads;
	{
		...
		wallet->insert_adhoc (vban::dev_genesis_key.prv); // For voting
		ASSERT_EQ (key.pub, wallet->insert_adhoc (key.prv));
		node1->start ();
		system.nodes.push_back (node1);
		ASSERT_NE (nullptr, vban::establish_tcp (system, *node1, node->network.endpoint ()));
		node1->workers.add_timed_task (std::chrono::steady_clock::now () + std::chrono::milliseconds (200), ([&system, &key] () {
			auto hash (system.wallet (0)->send_sync (vban::dev_genesis_key.pub, key.pub, system.nodes[0]->config.receive_minimum.number ()));
			auto transaction (system.nodes[0]->store.tx_begin_read ());
...
TEST (ledger, deep_account_compute)
{
	...
	vban::keypair key;
	auto balance (vban::genesis_amount - 1);
	vban::send_block send (genesis.hash (), key.pub, balance, vban::dev_genesis_key.prv, vban::dev_genesis_key.pub, *pool.generate (genesis.hash ()));
	ASSERT_EQ (vban::process_result::progress, ledger.process (transaction, send).code);
	vban::open_block open (send.hash (), vban::dev_genesis_key.pub, key.pub, key.prv, key.pub, *pool.generate (key.pub));
	...
	{
		balance -= 1;
		vban::send_block send (sprevious, key.pub, balance, vban::dev_genesis_key.prv, vban::dev_genesis_key.pub, *pool.generate (sprevious));
		ASSERT_EQ (vban::process_result::progress, ledger.process (transaction, send).code);
		sprevious = send.hash ();
		vban::receive_block receive (rprevious, send.hash (), key.prv, key.pub, *pool.generate (rprevious));
...
TEST (wallet, multithreaded_send_async)
{
	std::vector<boost::thread> threads;
	{
		...
		wallet_l->insert_adhoc (vban::dev_genesis_key.prv);
		...
		for (auto i (0); i < 20; ++i)
		{
			threads.push_back (boost::thread ([wallet_l, &key] () {
				...
				{
					wallet_l->send_async (vban::dev_genesis_key.pub, key.pub, 1000, [] (std::shared_ptr<vban::block> const & block_a) {
		...
		ASSERT_TIMELY (1000s, system.nodes[0]->balance (vban::dev_genesis_key.pub) == (vban::genesis_amount - 20 * 1000 * 1000));
	}
	for (auto i (threads.begin ()), n (threads.end ()); i != n; ++i)
	{
		i->join ();
		...
		threads.push_back (boost::thread ([&system] () {
			for (auto i (0); i != 1000; ++i)
			{
				...
				for (auto j (0); j != 10; ++j)
				{
					...
					system.nodes[0]->store.confirmation_height_put (transaction, account, { 0, vban::block_hash (0) });
					system.nodes[0]->store.account_put (transaction, account, vban::account_info ());
				}
			}
		}));
	}
...
TEST (node, fork_storm)
{
	...
	vban::system system (64, vban::transport::transport_type::tcp, flags);
	system.wallet (0)->insert_adhoc (vban::dev_genesis_key.prv);
	auto previous (system.nodes[0]->latest (vban::dev_genesis_key.pub));
	auto balance (system.nodes[0]->balance (vban::dev_genesis_key.pub));
	ASSERT_FALSE (previous.is_zero ());
	for (auto j (0); j != system.nodes.size (); ++j)
	{
		balance -= 1;
		vban::keypair key;
		vban::send_block send (previous, key.pub, balance, vban::dev_genesis_key.prv, vban::dev_genesis_key.pub, 0);
		system.nodes[j]->work_generate_blocking (send);
		previous = send.hash ();
		for (auto i (0); i != system.nodes.size (); ++i)
		{
			auto send_result (system.nodes[i]->process (send));
			ASSERT_EQ (vban::process_result::progress, send_result.code);
			vban::keypair rep;
			auto open (std::make_shared<vban::open_block> (previous, rep.pub, key.pub, key.prv, key.pub, 0));
			system.nodes[i]->work_generate_blocking (*open);
			auto open_result (system.nodes[i]->process (*open));
			ASSERT_EQ (vban::process_result::progress, open_result.code);
	...
	while (again)
	{
		auto empty = 0;
		auto single = 0;
		std::for_each (system.nodes.begin (), system.nodes.end (), [&] (std::shared_ptr<vban::node> const & node_a) {
			...
			{
				vban::unique_lock<vban::mutex> lock (node_a->active.mutex);
		...
		for (auto i (nodes.begin ()), n (nodes.end ()); i != n; ++i)
		{
			switch (*i)
			{
				...
				case 1:
					...
					{
						++message_count;
						auto entry (vban::random_pool::generate_word32 (0, node_count - 1));
						switch (nodes[entry])
						{
							case 0:
								nodes[entry] = 1;
...
TEST (store, unchecked_load)
{
	...
	auto block (std::make_shared<vban::send_block> (0, 0, 0, vban::dev_genesis_key.prv, vban::dev_genesis_key.pub, 0));
...
TEST (store, vote_load)
{
	...
	for (auto i (0); i < 1000000; ++i)
	{
		auto vote (std::make_shared<vban::vote> (vban::dev_genesis_key.pub, vban::dev_genesis_key.prv, i, block));
		node.vote_processor.vote (vote, std::make_shared<vban::transport::channel_loopback> (node));
	}
}
...
TEST (store, pruned_load)
{
	...
	auto const expected_result = vban::using_rocksdb_in_tests () ? num_pruned : num_pruned / 2;
	constexpr auto batch_size = 20;
	boost::unordered_set<vban::block_hash> hashes;
	{
...
TEST (wallets, rep_scan)
{
	...
	node.wallets.foreach_representative ([] (vban::public_key const & pub_a, vban::raw_key const & prv_a) {
	});
...
TEST (node, mass_vote_by_hash)
{
	vban::system system (1);
	system.wallet (0)->insert_adhoc (vban::dev_genesis_key.prv);
	vban::block_hash previous (vban::genesis_hash);
	vban::keypair key;
	std::vector<std::shared_ptr<vban::state_block>> blocks;
	for (auto i (0); i < 10000; ++i)
	{
		auto block (std::make_shared<vban::state_block> (vban::dev_genesis_key.pub, previous, vban::dev_genesis_key.pub, vban::genesis_amount - (i + 1) * vban::Gxrb_ratio, key.pub, vban::dev_genesis_key.prv, vban::dev_genesis_key.pub, *system.work.generate (previous)));
		previous = block->hash ();
		blocks.push_back (block);
	}
	for (auto i (blocks.begin ()), n (blocks.end ()); i != n; ++i)
	{
		system.nodes[0]->block_processor.add (*i, vban::seconds_since_epoch ());
	}
}
...
TEST (confirmation_height, many_accounts_single_confirmation)
{
	...
	// The number of frontiers should be more than the vban::confirmation_height::unbounded_cutoff to test the amount of blocks confirmed is correct.
	node->confirmation_height_processor.batch_write_size = 500;
	auto const num_accounts = vban::confirmation_height::unbounded_cutoff * 2 + 50;
	vban::keypair last_keypair = vban::dev_genesis_key;
	auto last_open_hash = node->latest (vban::dev_genesis_key.pub);
	{
		auto transaction = node->store.tx_begin_write ();
		for (auto i = num_accounts - 1; i > 0; --i)
		{
			...
			vban::send_block send (last_open_hash, key.pub, node->online_reps.delta (), last_keypair.prv, last_keypair.pub, *system.work.generate (last_open_hash));
			ASSERT_EQ (vban::process_result::progress, node->ledger.process (transaction, send).code);
			vban::open_block open (send.hash (), last_keypair.pub, key.pub, key.prv, key.pub, *system.work.generate (key.pub));
	...
	ASSERT_EQ (node->ledger.stats.count (vban::stat::type::confirmation_height, vban::stat::detail::blocks_confirmed, vban::stat::dir::in), num_accounts * 2 - 2);
	ASSERT_EQ (node->ledger.stats.count (vban::stat::type::confirmation_height, vban::stat::detail::blocks_confirmed_bounded, vban::stat::dir::in), num_accounts * 2 - 2);
	ASSERT_EQ (node->ledger.stats.count (vban::stat::type::confirmation_height, vban::stat::detail::blocks_confirmed_unbounded, vban::stat::dir::in), 0);

	ASSERT_TIMELY (40s, (node->ledger.cache.cemented_count - 1) == node->stats.count (vban::stat::type::confirmation_observer, vban::stat::detail::all, vban::stat::dir::out));
...
TEST (confirmation_height, many_accounts_many_confirmations)
{
	...
	node->confirmation_height_processor.batch_write_size = 500;
	auto const num_accounts = vban::confirmation_height::unbounded_cutoff * 2 + 50;
	auto latest_genesis = node->latest (vban::dev_genesis_key.pub);
	std::vector<std::shared_ptr<vban::open_block>> open_blocks;
	{
		auto transaction = node->store.tx_begin_write ();
		for (auto i = num_accounts - 1; i > 0; --i)
		{
			...
			vban::send_block send (latest_genesis, key.pub, node->online_reps.delta (), vban::dev_genesis_key.prv, vban::dev_genesis_key.pub, *system.work.generate (latest_genesis));
			ASSERT_EQ (vban::process_result::progress, node->ledger.process (transaction, send).code);
			auto open = std::make_shared<vban::open_block> (send.hash (), vban::dev_genesis_key.pub, key.pub, key.prv, key.pub, *system.work.generate (key.pub));
			ASSERT_EQ (vban::process_result::progress, node->ledger.process (transaction, *open).code);
	...
	ASSERT_TIMELY (1500s, node->stats.count (vban::stat::type::confirmation_height, vban::stat::detail::blocks_confirmed, vban::stat::dir::in) == num_blocks_to_confirm);

	auto num_confirmed_bounded = node->ledger.stats.count (vban::stat::type::confirmation_height, vban::stat::detail::blocks_confirmed_bounded, vban::stat::dir::in);
	ASSERT_GE (num_confirmed_bounded, vban::confirmation_height::unbounded_cutoff);
	ASSERT_EQ (node->ledger.stats.count (vban::stat::type::confirmation_height, vban::stat::detail::blocks_confirmed_unbounded, vban::stat::dir::in), num_blocks_to_confirm - num_confirmed_bounded);

	ASSERT_TIMELY (60s, (node->ledger.cache.cemented_count - 1) == node->stats.count (vban::stat::type::confirmation_observer, vban::stat::detail::all, vban::stat::dir::out));
	...
	ASSERT_TIMELY (20s, (node->ledger.cache.cemented_count - 1) == node->stats.count (vban::stat::type::confirmation_observer, vban::stat::detail::all, vban::stat::dir::out));
...
TEST (confirmation_height, long_chains)
{
	...
	node->confirmation_height_processor.batch_write_size = 500;
	auto const num_blocks = vban::confirmation_height::unbounded_cutoff * 2 + 50;

	// First open the other account
	vban::send_block send (latest, key1.pub, vban::genesis_amount - vban::Gxrb_ratio + num_blocks + 1, vban::dev_genesis_key.prv, vban::dev_genesis_key.pub, *system.work.generate (latest));
	...
	auto previous_destination_chain_hash = open.hash ();
	{
		...
		{
			vban::send_block send (previous_genesis_chain_hash, key1.pub, vban::genesis_amount - vban::Gxrb_ratio + i + 1, vban::dev_genesis_key.prv, vban::dev_genesis_key.pub, *system.work.generate (previous_genesis_chain_hash));
			ASSERT_EQ (vban::process_result::progress, node->ledger.process (transaction, send).code);
			vban::receive_block receive (previous_destination_chain_hash, send.hash (), key1.prv, key1.pub, *system.work.generate (previous_destination_chain_hash));
	...
	vban::send_block send1 (previous_destination_chain_hash, vban::dev_genesis_key.pub, vban::Gxrb_ratio - 2, key1.prv, key1.pub, *system.work.generate (previous_destination_chain_hash));
	auto receive1 (std::make_shared<vban::state_block> (vban::dev_genesis_key.pub, previous_genesis_chain_hash, vban::genesis_account, vban::genesis_amount - vban::Gxrb_ratio + 1, send1.hash (), vban::dev_genesis_key.prv, vban::dev_genesis_key.pub, *system.work.generate (previous_genesis_chain_hash)));
	...
	vban::keypair key2;
	vban::state_block send2 (vban::genesis_account, receive1->hash (), vban::genesis_account, vban::genesis_amount - vban::Gxrb_ratio, key2.pub, vban::dev_genesis_key.prv, vban::dev_genesis_key.pub, *system.work.generate (receive1->hash ()));
	...
	ASSERT_EQ (cemented_count, node->ledger.cache.cemented_count);
	ASSERT_EQ (node->ledger.stats.count (vban::stat::type::confirmation_height, vban::stat::detail::blocks_confirmed, vban::stat::dir::in), num_blocks * 2 + 2);
	ASSERT_EQ (node->ledger.stats.count (vban::stat::type::confirmation_height, vban::stat::detail::blocks_confirmed_bounded, vban::stat::dir::in), num_blocks * 2 + 2);
	ASSERT_EQ (node->ledger.stats.count (vban::stat::type::confirmation_height, vban::stat::detail::blocks_confirmed_unbounded, vban::stat::dir::in), 0);
...
TEST (confirmation_height, dynamic_algorithm)
{
	...
	vban::keypair key;
	system.wallet (0)->insert_adhoc (vban::dev_genesis_key.prv);
	auto const num_blocks = vban::confirmation_height::unbounded_cutoff;
	auto latest_genesis = vban::genesis ().open;
	std::vector<std::shared_ptr<vban::state_block>> state_blocks;
	for (auto i = 0; i < num_blocks; ++i)
	{
		auto send (std::make_shared<vban::state_block> (vban::dev_genesis_key.pub, latest_genesis->hash (), vban::dev_genesis_key.pub, vban::genesis_amount - i - 1, key.pub, vban::dev_genesis_key.prv, vban::dev_genesis_key.pub, *system.work.generate (latest_genesis->hash ())));
		latest_genesis = send;
	...
	ASSERT_EQ (node->ledger.stats.count (vban::stat::type::confirmation_height, vban::stat::detail::blocks_confirmed, vban::stat::dir::in), num_blocks);
	ASSERT_EQ (node->ledger.stats.count (vban::stat::type::confirmation_height, vban::stat::detail::blocks_confirmed_bounded, vban::stat::dir::in), 1);
	ASSERT_EQ (node->ledger.stats.count (vban::stat::type::confirmation_height, vban::stat::detail::blocks_confirmed_unbounded, vban::stat::dir::in), num_blocks - 1);
	ASSERT_TIMELY (10s, node->active.election_winner_details_size () == 0);
}
...
TEST (confirmation_height, dynamic_algorithm_no_transition_while_pending)
{
	...
	for (auto _ = 0; _ < 3; ++_)
	{
		...
		auto latest_genesis = node->latest (vban::dev_genesis_key.pub);
		std::vector<std::shared_ptr<vban::state_block>> state_blocks;
		auto const num_blocks = vban::confirmation_height::unbounded_cutoff - 2;

		auto add_block_to_genesis_chain = [&] (vban::write_transaction & transaction) {
			static int num = 0;
			auto send (std::make_shared<vban::state_block> (vban::dev_genesis_key.pub, latest_genesis, vban::dev_genesis_key.pub, vban::genesis_amount - num - 1, key.pub, vban::dev_genesis_key.prv, vban::dev_genesis_key.pub, *system.work.generate (latest_genesis)));
			latest_genesis = send->hash ();
			state_blocks.push_back (send);
			ASSERT_EQ (vban::process_result::progress, node->ledger.process (transaction, *send).code);
			++num;
		...
		ASSERT_EQ (node->ledger.stats.count (vban::stat::type::confirmation_height, vban::stat::detail::blocks_confirmed, vban::stat::dir::in), num_blocks);
		ASSERT_EQ (node->ledger.stats.count (vban::stat::type::confirmation_height, vban::stat::detail::blocks_confirmed_bounded, vban::stat::dir::in), 0);
		ASSERT_EQ (node->ledger.stats.count (vban::stat::type::confirmation_height, vban::stat::detail::blocks_confirmed_unbounded, vban::stat::dir::in), num_blocks);
		ASSERT_TIMELY (10s, node->active.election_winner_details_size () == 0);
	}
}
...
TEST (confirmation_height, many_accounts_send_receive_self)
{
	...
	vban::node_flags node_flags;
	node_flags.confirmation_height_processor_mode = vban::confirmation_height_mode::unbounded;
	auto node = system.add_node (node_config);
	system.wallet (0)->insert_adhoc (vban::dev_genesis_key.prv);
	...
	std::vector<std::shared_ptr<vban::open_block>> open_blocks;
	{
		...
		for (auto i = 0; i < num_accounts; ++i)
		{
			...
			vban::send_block send (latest_genesis, key.pub, vban::genesis_amount - 1 - i, vban::dev_genesis_key.prv, vban::dev_genesis_key.pub, *system.work.generate (latest_genesis));
			ASSERT_EQ (vban::process_result::progress, node->ledger.process (transaction, send).code);
			auto open = std::make_shared<vban::open_block> (send.hash (), vban::dev_genesis_key.pub, key.pub, key.prv, key.pub, *system.work.generate (key.pub));
			ASSERT_EQ (vban::process_result::progress, node->ledger.process (transaction, *open).code);
			open_blocks.push_back (std::move (open));
	...
	while (node->stats.count (vban::stat::type::confirmation_height, vban::stat::detail::blocks_confirmed, vban::stat::dir::in) != num_blocks_to_confirm)
	{
		ASSERT_NO_ERROR (system.poll ());
	}

	std::vector<std::shared_ptr<vban::send_block>> send_blocks;
	std::vector<std::shared_ptr<vban::receive_block>> receive_blocks;

	for (int i = 0; i < open_blocks.size (); ++i)
	{
		auto open_block = open_blocks[i];
		auto & keypair = keys[i];
		send_blocks.emplace_back (std::make_shared<vban::send_block> (open_block->hash (), keypair.pub, 1, keypair.prv, keypair.pub, *system.work.generate (open_block->hash ())));
		receive_blocks.emplace_back (std::make_shared<vban::receive_block> (send_blocks.back ()->hash (), send_blocks.back ()->hash (), keypair.prv, keypair.pub, *system.work.generate (send_blocks.back ()->hash ())));
	}
	...
	while ((node->ledger.cache.cemented_count - 1) != node->stats.count (vban::stat::type::confirmation_observer, vban::stat::detail::all, vban::stat::dir::out))
	{
		ASSERT_NO_ERROR (system.poll ());
	}
...
TEST (confirmation_height, many_accounts_send_receive_self_no_elections)
{
	...
	vban::block_hash block_hash_being_processed{ 0 };
	vban::confirmation_height_processor confirmation_height_processor{ ledger, write_database_queue, 10ms, logging, logger, initialized_latch, confirmation_height_mode::automatic };

	auto const num_accounts = 100000;

	auto latest_genesis = vban::genesis_hash;
	std::vector<vban::keypair> keys;
	std::vector<std::shared_ptr<vban::open_block>> open_blocks;
...

	{
		...
		for (auto i = 0; i < num_accounts; ++i)
		{
			vban::keypair key;
			keys.emplace_back (key);
			vban::send_block send (latest_genesis, key.pub, vban::genesis_amount - 1 - i, vban::dev_genesis_key.prv, vban::dev_genesis_key.pub, *pool.generate (latest_genesis));
			ASSERT_EQ (vban::process_result::progress, ledger.process (transaction, send).code);
			auto open = std::make_shared<vban::open_block> (send.hash (), vban::dev_genesis_key.pub, key.pub, key.prv, key.pub, *pool.generate (key.pub));
			ASSERT_EQ (vban::process_result::progress, ledger.process (transaction, *open).code);
			open_blocks.push_back (std::move (open));
			latest_genesis = send.hash ();
		}
	...
	auto num_blocks_to_confirm = num_accounts * 2;
	while (stats.count (vban::stat::type::confirmation_height, vban::stat::detail::blocks_confirmed, vban::stat::dir::in) != num_blocks_to_confirm)
	{
		ASSERT_NO_ERROR (system.poll ());
	}
	...
	// Now add all send/receive blocks
	{
		...
		for (int i = 0; i < open_blocks.size (); ++i)
		{
			...
			ASSERT_EQ (vban::process_result::progress, ledger.process (transaction, *send_blocks.back ()).code);
			ASSERT_EQ (vban::process_result::progress, ledger.process (transaction, *receive_blocks.back ()).code);
		}
	}
...
TEST (confirmation_height, prioritize_frontiers_overwrite)
{
...

	{
		...
		for (auto i = num_accounts - 1; i > 0; --i)
		{
			...
			vban::send_block send (last_open_hash, key.pub, vban::Gxrb_ratio - 1, last_keypair.prv, last_keypair.pub, *system.work.generate (last_open_hash));
			ASSERT_EQ (vban::process_result::progress, node->ledger.process (transaction, send).code);
			vban::open_block open (send.hash (), last_keypair.pub, key.pub, key.prv, key.pub, *system.work.generate (key.pub));
			ASSERT_EQ (vban::process_result::progress, node->ledger.process (transaction, open).code);
			last_open_hash = open.hash ();
	...
	vban::keypair key;
	auto latest_genesis = node->latest (vban::dev_genesis_key.pub);
	vban::send_block send (latest_genesis, key.pub, vban::Gxrb_ratio - 1, vban::dev_genesis_key.prv, vban::dev_genesis_key.pub, *system.work.generate (latest_genesis));
	vban::open_block open (send.hash (), vban::dev_genesis_key.pub, key.pub, key.prv, key.pub, *system.work.generate (key.pub));
	{
		auto transaction = node->store.tx_begin_write ();
		ASSERT_EQ (vban::process_result::progress, node->ledger.process (transaction, send).code);
		ASSERT_EQ (vban::process_result::progress, node->ledger.process (transaction, open).code);
	...
	vban::send_block send1 (send.hash (), key.pub, vban::Gxrb_ratio - 2, vban::dev_genesis_key.prv, vban::dev_genesis_key.pub, *system.work.generate (send.hash ()));
	vban::receive_block receive (open.hash (), send1.hash (), key.prv, key.pub, *system.work.generate (open.hash ()));
	{
		auto transaction = node->store.tx_begin_write ();
		ASSERT_EQ (vban::process_result::progress, node->ledger.process (transaction, send1).code);
		ASSERT_EQ (vban::process_result::progress, node->ledger.process (transaction, receive).code);
...
{
public:
	vban::util::counted_completion write_completion{ 0 };
...
TEST (telemetry, ongoing_requests)
{
	...
	ASSERT_EQ (0, node_client->stats.count (vban::stat::type::bootstrap, vban::stat::detail::telemetry_ack, vban::stat::dir::in));
	ASSERT_EQ (0, node_client->stats.count (vban::stat::type::bootstrap, vban::stat::detail::telemetry_req, vban::stat::dir::out));

	ASSERT_TIMELY (20s, node_client->stats.count (vban::stat::type::message, vban::stat::detail::telemetry_ack, vban::stat::dir::in) == 1 && node_server->stats.count (vban::stat::type::message, vban::stat::detail::telemetry_ack, vban::stat::dir::in) == 1);
	...
	ASSERT_EQ (2, node_client->stats.count (vban::stat::type::message, vban::stat::detail::telemetry_ack, vban::stat::dir::in));
	ASSERT_EQ (2, node_client->stats.count (vban::stat::type::message, vban::stat::detail::telemetry_req, vban::stat::dir::in));
	ASSERT_EQ (2, node_client->stats.count (vban::stat::type::message, vban::stat::detail::telemetry_req, vban::stat::dir::out));
	ASSERT_EQ (2, node_server->stats.count (vban::stat::type::message, vban::stat::detail::telemetry_ack, vban::stat::dir::in));
	ASSERT_EQ (2, node_server->stats.count (vban::stat::type::message, vban::stat::detail::telemetry_req, vban::stat::dir::in));
	ASSERT_EQ (2, node_server->stats.count (vban::stat::type::message, vban::stat::detail::telemetry_req, vban::stat::dir::out));
}
				...
				while (std::any_of (node_data.cbegin (), node_data.cend (), [] (auto const & data) { return data.keep_requesting_metrics.load (); }))
				{
					for (auto & data : node_data)
					{
						...
						if (data.keep_requesting_metrics)
						{
							...
							data.node->telemetry->get_metrics_single_peer_async (peer, [&shared_data, &data, &node_data] (vban::telemetry_data_response const & telemetry_data_response_a) {
...
TEST (telemetry, under_load)
{
	...
	auto send (std::make_shared<vban::state_block> (vban::dev_genesis_key.pub, latest_genesis, vban::dev_genesis_key.pub, vban::genesis_amount - num_blocks, key.pub, vban::dev_genesis_key.prv, vban::dev_genesis_key.pub, *system.work.generate (latest_genesis)));
	node->process_active (send);
	latest_genesis = send->hash ();
	auto open (std::make_shared<vban::state_block> (key.pub, 0, key.pub, num_blocks, send->hash (), key.prv, key.pub, *system.work.generate (key.pub)));
	node->process_active (open);
	auto latest_key = open->hash ();

	auto thread_func = [key1, &system, node, num_blocks] (vban::keypair const & keypair, vban::block_hash const & latest, vban::uint256_t const initial_amount) {
		auto latest_l = latest;
		for (int i = 0; i < num_blocks; ++i)
		{
			auto send (std::make_shared<vban::state_block> (keypair.pub, latest_l, keypair.pub, initial_amount - i - 1, key1.pub, keypair.prv, keypair.pub, *system.work.generate (latest_l)));
			latest_l = send->hash ();
			node->process_active (send);
		}
	};

	std::thread thread1 (thread_func, vban::dev_genesis_key, latest_genesis, vban::genesis_amount - num_blocks);
	...
	{
		ASSERT_EQ (0, node->stats.count (vban::stat::type::telemetry, vban::stat::detail::failed_send_telemetry_req));
		ASSERT_EQ (0, node->stats.count (vban::stat::type::telemetry, vban::stat::detail::request_within_protection_cache_zone));
		ASSERT_EQ (0, node->stats.count (vban::stat::type::telemetry, vban::stat::detail::unsolicited_telemetry_ack));
		ASSERT_EQ (0, node->stats.count (vban::stat::type::telemetry, vban::stat::detail::no_response_received));
	}
}
...
TEST (telemetry, all_peers_use_single_request_cache)
{
	...
	vban::telemetry_data telemetry_data;
	{
		...
		node_client->telemetry->get_metrics_single_peer_async (channel, [&done, &telemetry_data] (vban::telemetry_data_response const & response_a) {
	...
	ASSERT_EQ (1, node_client->stats.count (vban::stat::type::message, vban::stat::detail::telemetry_ack, vban::stat::dir::in));
	ASSERT_EQ (0, node_client->stats.count (vban::stat::type::message, vban::stat::detail::telemetry_req, vban::stat::dir::in));
	ASSERT_EQ (1, node_client->stats.count (vban::stat::type::message, vban::stat::detail::telemetry_req, vban::stat::dir::out));
	ASSERT_EQ (0, node_server->stats.count (vban::stat::type::message, vban::stat::detail::telemetry_ack, vban::stat::dir::in));
	ASSERT_EQ (1, node_server->stats.count (vban::stat::type::message, vban::stat::detail::telemetry_req, vban::stat::dir::in));
	ASSERT_EQ (0, node_server->stats.count (vban::stat::type::message, vban::stat::detail::telemetry_req, vban::stat::dir::out));
...
TEST (telemetry, many_nodes)
{
	...
	const auto num_nodes = (is_sanitizer_build || vban::running_within_valgrind ()) ? 4 : 10;
	for (auto i = 0; i < num_nodes; ++i)
	{
		vban::node_config node_config (vban::get_available_port (), system.logging);
		...
		auto node = std::make_shared<vban::node> (system.io_ctx, vban::unique_path (), node_config, system.work, node_flags);
		node->start ();
	...
	vban::keypair key;
	vban::genesis genesis;
	vban::state_block send (vban::dev_genesis_key.pub, genesis.hash (), vban::dev_genesis_key.pub, vban::genesis_amount - vban::Mxrb_ratio, key.pub, vban::dev_genesis_key.prv, vban::dev_genesis_key.pub, *system.work.generate (genesis.hash ()));
	...
	{
		node_client->telemetry->get_metrics_single_peer_async (peer, [&telemetry_datas, &mutex] (vban::telemetry_data_response const & response_a) {
	...
	for (auto & data : telemetry_datas)
	{
		...
		ASSERT_EQ (data.major_version, vban::get_major_node_version ());
		ASSERT_EQ (data.minor_version, vban::get_minor_node_version ());
		ASSERT_EQ (data.patch_version, vban::get_patch_node_version ());
		ASSERT_EQ (data.pre_release_version, vban::get_pre_release_node_version ());
		...
		ASSERT_EQ (data.active_difficulty, system.nodes.front ()->default_difficulty (vban::work_version::work_1));
	}
...
TEST (signature_checker, mass_boundary_checks)
{
	...
	for (auto i = 1; i <= 10; ++i)
	{
		add_boundary (vban::signature_checker::batch_size * i);
	}

	for (auto num_threads = 0; num_threads < 5; ++num_threads)
	{
		vban::signature_checker checker (num_threads);
		auto max_size = *(sizes.end () - 1);
		std::vector<vban::uint256_union> hashes;
		...
		vban::keypair key;
		vban::state_block block (key.pub, 0, key.pub, 0, 0, key.prv, key.pub, 0);
...
TEST (node, mass_epoch_upgrader)
{
	auto perform_test = [] (size_t const batch_size) {
		...
		auto balance = node.balance (vban::dev_genesis_key.pub);
		auto latest = node.latest (vban::dev_genesis_key.pub);
		vban::uint256_t amount = 1;
		...
		for (auto & accounts : all)
		{
			for (auto & info : *accounts)
			{
				...
				auto block = builder
							 ...
							 .work (*node.work_generate_blocking (latest, vban::work_threshold (vban::work_version::work_1, vban::block_details (vban::epoch::epoch_0, false, false, false))))
							 .build (ec);
				ASSERT_FALSE (ec);
				ASSERT_NE (nullptr, block);
				ASSERT_EQ (vban::process_result::progress, node.process (*block).code);
				latest = block->hash ();
		...
		for (auto const & info : opened)
		{
			...
			auto block = builder
						 ...
						 .work (*node.work_generate_blocking (info.key.pub, vban::work_threshold (vban::work_version::work_1, vban::block_details (vban::epoch::epoch_0, false, false, false))))
						 .build (ec);
			ASSERT_FALSE (ec);
			ASSERT_NE (nullptr, block);
			ASSERT_EQ (vban::process_result::progress, node.process (*block).code);
		...
		while (node.ledger.cache.block_count != block_count_before + total_to_upgrade)
		{
			...
			ASSERT_FALSE (node.epoch_upgrader (epoch_signer.prv, vban::epoch::epoch_1, upgrade_count, threads));
			// Already ongoing - should fail
			ASSERT_TRUE (node.epoch_upgrader (epoch_signer.prv, vban::epoch::epoch_1, upgrade_count, threads));
			system.deadline_set (60s);
...
TEST (node, mass_block_new)
{
	...
	// Upgrade to epoch_2
	system.upgrade_genesis_epoch (node, vban::epoch::epoch_1);
	system.upgrade_genesis_epoch (node, vban::epoch::epoch_2);

	auto next_block_count = num_blocks + 3;
	auto process_all = [&] (std::vector<std::shared_ptr<vban::state_block>> const & blocks_a) {
	...
	vban::genesis genesis;
	vban::keypair key;
	std::vector<vban::keypair> keys (num_blocks);
	vban::state_block_builder builder;
	std::vector<std::shared_ptr<vban::state_block>> send_blocks;
	auto send_threshold (vban::work_threshold (vban::work_version::work_1, vban::block_details (vban::epoch::epoch_2, true, false, false)));
	auto latest_genesis = node.latest (vban::dev_genesis_key.pub);
	for (auto i = 0; i < num_blocks; ++i)
	{
		auto send = builder.make_block ()
					...
					.link (keys[i].pub)
					.sign (vban::dev_genesis_key.prv, vban::dev_genesis_key.pub)
					.work (*system.work.generate (vban::work_version::work_1, latest_genesis, send_threshold))
					.build ();
		latest_genesis = send->hash ();
	...
	std::vector<std::shared_ptr<vban::state_block>> open_blocks;
	auto receive_threshold (vban::work_threshold (vban::work_version::work_1, vban::block_details (vban::epoch::epoch_2, false, true, false)));
	for (auto i = 0; i < num_blocks; ++i)
	{
		...
		auto open = builder.make_block ()
					...
					.work (*system.work.generate (vban::work_version::work_1, key.pub, receive_threshold))
					.build ();
		open_blocks.push_back (std::move (open));
	}
	...
	std::vector<std::shared_ptr<vban::state_block>> send_blocks2;
	for (auto i = 0; i < num_blocks; ++i)
	{
		auto const & key = keys[i];
		...
		auto send2 = builder.make_block ()
					 ...
					 .link (key.pub)
					 .sign (key.prv, key.pub)
					 .work (*system.work.generate (vban::work_version::work_1, latest->hash (), send_threshold))
					 .build ();
	...
	std::vector<std::shared_ptr<vban::state_block>> receive_blocks;
	for (auto i = 0; i < num_blocks; ++i)
	{
		auto const & key = keys[i];
		...
		auto send2 = builder.make_block ()
					 ...
					 .link (latest->hash ())
					 .sign (key.prv, key.pub)
					 .work (*system.work.generate (vban::work_version::work_1, latest->hash (), receive_threshold))
					 .build ();
...
TEST (node, wallet_create_block_confirm_conflicts)
{
	for (int i = 0; i < 5; ++i)
	{
		...
		// First open the other account
		auto latest = vban::genesis_hash;
		vban::keypair key1;
		{
			auto transaction = node->store.tx_begin_write ();
			for (auto i = num_blocks - 1; i > 0; --i)
			{
				vban::send_block send (latest, key1.pub, vban::genesis_amount - vban::Gxrb_ratio + i + 1, vban::dev_genesis_key.prv, vban::dev_genesis_key.pub, *system.work.generate (latest));
				ASSERT_EQ (vban::process_result::progress, node->ledger.process (transaction, send).code);
				latest = send.hash ();
			}
		}
