...
double constexpr vban::node::price_max;
double constexpr vban::node::free_cutoff;
size_t constexpr vban::block_arrival::arrival_size_min;
std::chrono::seconds constexpr vban::block_arrival::arrival_time_min;

namespace vban
{
extern unsigned char vban_bootstrap_weights_live[];
extern size_t vban_bootstrap_weights_live_size;
extern unsigned char vban_bootstrap_weights_beta[];
extern size_t vban_bootstrap_weights_beta_size;
}

void vban::node::keepalive (std::string const & address_a, uint16_t port_a)
{
	...
	network.resolver.async_resolve (boost::asio::ip::udp::resolver::query (address_a, std::to_string (port_a)), [node_l, address_a, port_a] (boost::system::error_code const & ec, boost::asio::ip::udp::resolver::iterator i_a) {
		if (!ec)
		{
			...
			{
				auto endpoint (vban::transport::map_endpoint_to_v6 (i->endpoint ()));
				std::weak_ptr<vban::node> node_w (node_l);
				auto channel (node_l->network.find_channel (endpoint));
				if (!channel)
				{
					node_l->network.tcp_channels.start_tcp (endpoint, [node_w] (std::shared_ptr<vban::transport::channel> const & channel_a) {
...
std::unique_ptr<vban::container_info_component> vban::collect_container_info (rep_crawler & rep_crawler, std::string const & name)
{
	size_t count;
	{
		vban::lock_guard<vban::mutex> guard (rep_crawler.active_mutex);
...
vban::node::node (boost::asio::io_context & io_ctx_a, uint16_t peering_port_a, boost::filesystem::path const & application_path_a, vban::logging const & logging_a, vban::work_pool & work_a, vban::node_flags flags_a, unsigned seq) :
	node (io_ctx_a, application_path_a, vban::node_config (peering_port_a, logging_a), work_a, flags_a, seq)
{
}

vban::node::node (boost::asio::io_context & io_ctx_a, boost::filesystem::path const & application_path_a, vban::node_config const & config_a, vban::work_pool & work_a, vban::node_flags flags_a, unsigned seq) :
	write_database_queue (!flags_a.force_use_write_database_queue && (config_a.rocksdb_config.enable || vban::using_rocksdb_in_tests ())),
	io_ctx (io_ctx_a),
	node_initialized_latch (1),
	config (config_a),
	stats (config.stat_config),
	workers (std::max (3u, config.io_threads / 4), vban::thread_role::name::worker),
	flags (flags_a),
	work (work_a),
	distributed_work (*this),
	logger (config_a.logging.min_time_between_log_output),
	store_impl (vban::make_store (logger, application_path_a, flags.read_only, true, config_a.rocksdb_config, config_a.diagnostics_config.txn_tracking, config_a.block_processor_batch_max_time, config_a.lmdb_config, config_a.backup_before_upgrade)),
	store (*store_impl),
	wallets_store_impl (std::make_unique<vban::mdb_wallets_store> (application_path_a / "wallets.ldb", config_a.lmdb_config)),
	wallets_store (*wallets_store_impl),
	gap_cache (*this),
	ledger (store, stats, flags_a.generate_cache),
	checker (config.signature_checker_threads),
	network (*this, config.peering_port),
	telemetry (std::make_shared<vban::telemetry> (network, workers, observers.telemetry, stats, network_params, flags.disable_ongoing_telemetry_requests)),
	...
	node_seq (seq)
{
	if (!init_error ())
	{
		...
		{
			auto endpoint_l (vban::tcp_endpoint (boost::asio::ip::make_address_v6 (config.websocket_config.address), config.websocket_config.port));
			websocket_server = std::make_shared<vban::websocket::listener> (logger, wallets, io_ctx, endpoint_l);
		...
		network.channel_observer = [this] (std::shared_ptr<vban::transport::channel> const & channel_a) {
		...
		{
			observers.blocks.add ([this] (vban::election_status const & status_a, std::vector<vban::vote_with_weight_info> const & votes_a, vban::account const & account_a, vban::amount const & amount_a, bool is_state_send_a) {
				auto block_a (status_a.winner);
				if ((status_a.type == vban::election_status_type::active_confirmed_quorum || status_a.type == vban::election_status_type::active_confirmation_height) && this->block_arrival.recent (block_a->hash ()))
				{
					auto node_l (shared_from_this ());
					background ([node_l, block_a, account_a, amount_a, is_state_send_a] () {
						...
						resolver->async_resolve (boost::asio::ip::tcp::resolver::query (address, std::to_string (port)), [node_l, address, port, target, body, resolver] (boost::system::error_code const & ec, boost::asio::ip::tcp::resolver::iterator i_a) {
							...
							else
							{
								...
								node_l->stats.inc (vban::stat::type::error, vban::stat::detail::http_callback, vban::stat::dir::out);
							}
						});
					});
				}
		...
		{
			observers.blocks.add ([this] (vban::election_status const & status_a, std::vector<vban::vote_with_weight_info> const & votes_a, vban::account const & account_a, vban::amount const & amount_a, bool is_state_send_a) {
				debug_assert (status_a.type != vban::election_status_type::ongoing);

				if (this->websocket_server->any_subscriber (vban::websocket::topic::confirmation))
				{
					auto block_a (status_a.winner);
					std::string subtype;
					if (is_state_send_a)
			...
			observers.active_stopped.add ([this] (vban::block_hash const & hash_a) {
				if (this->websocket_server->any_subscriber (vban::websocket::topic::stopped_election))
				{
			...
			observers.telemetry.add ([this] (vban::telemetry_data const & telemetry_data, vban::endpoint const & endpoint) {
				if (this->websocket_server->any_subscriber (vban::websocket::topic::telemetry))
				{
					vban::websocket::message_builder builder;
		...
		observers.blocks.add ([this] (vban::election_status const & status_a, std::vector<vban::vote_with_weight_info> const & votes_a, vban::account const & account_a, vban::amount const & amount_a, bool is_state_send_a) {
			...
			{
				case vban::election_status_type::active_confirmed_quorum:
					this->stats.inc (vban::stat::type::confirmation_observer, vban::stat::detail::active_quorum, vban::stat::dir::out);
					break;
				case vban::election_status_type::active_confirmation_height:
					this->stats.inc (vban::stat::type::confirmation_observer, vban::stat::detail::active_conf_height, vban::stat::dir::out);
					break;
				case vban::election_status_type::inactive_confirmation_height:
					this->stats.inc (vban::stat::type::confirmation_observer, vban::stat::detail::inactive_conf_height, vban::stat::dir::out);
					break;
				default:
					break;
			}
		});
		observers.endpoint.add ([this] (std::shared_ptr<vban::transport::channel> const & channel_a) {
			if (channel_a->get_type () == vban::transport::transport_type::udp)
			{
		...
		observers.vote.add ([this] (std::shared_ptr<vban::vote> vote_a, std::shared_ptr<vban::transport::channel> const & channel_a, vban::vote_code code_a) {
			debug_assert (code_a != vban::vote_code::invalid);
			// The vote_code::vote is handled inside the election
			if (code_a == vban::vote_code::indeterminate)
			{
		...
		{
			observers.vote.add ([this] (std::shared_ptr<vban::vote> vote_a, std::shared_ptr<vban::transport::channel> const & channel_a, vban::vote_code code_a) {
				if (this->websocket_server->any_subscriber (vban::websocket::topic::vote))
				{
					vban::websocket::message_builder builder;
		...
		observers.work_cancel.add ([this] (vban::root const & root_a) {
			this->work.cancel (root_a);
			this->distributed_work.cancel (root_a);
		});

		logger.always_log ("Node starting, version: ", VBAN_VERSION_STRING);
		...
		if ((network_params.network.is_live_network () || network_params.network.is_beta_network ()) && !flags.inactive_node)
		{
			...
			if (use_bootstrap_weight)
			{
				...
				{
					logger.always_log ("Using bootstrap rep weight: ", rep.first.to_account (), " -> ", vban::uint128_union (rep.second).format_balance (Mxrb_ratio, 0, true), " XRB");
				}
			}
...
void vban::node::do_rpc_callback (boost::asio::ip::tcp::resolver::iterator i_a, std::string const & address, uint16_t port, std::shared_ptr<std::string> const & target, std::shared_ptr<std::string> const & body, std::shared_ptr<boost::asio::ip::tcp::resolver> const & resolver)
{
					...
					if (!ec)
					{
						...
						boost::beast::http::async_read (*sock, *sb, *resp, [node_l, sb, resp, sock, address, port, i_a, target, body, resolver] (boost::system::error_code const & ec, size_t bytes_transferred) mutable {
							if (!ec)
							{
								...
								{
									node_l->stats.inc (vban::stat::type::http_callback, vban::stat::detail::initiate, vban::stat::dir::out);
								}
								else
								{
									if (node_l->config.logging.callback_logging ())
...
bool vban::node::copy_with_compaction (boost::filesystem::path const & destination)
{
	return store.copy_db (destination);
}

std::unique_ptr<vban::container_info_component> vban::collect_container_info (node & node, std::string const & name)
{
	auto composite = std::make_unique<container_info_composite> (name);
...
void vban::node::process_active (std::shared_ptr<vban::block> const & incoming)
{
	block_arrival.add (incoming->hash ());
	block_processor.add (incoming, vban::seconds_since_epoch ());
}

vban::process_return vban::node::process (vban::block & block_a)
{
...
vban::process_return vban::node::process_local (std::shared_ptr<vban::block> const & block_a)
{
	...
	auto transaction (store.tx_begin_write ({ tables::accounts, tables::blocks, tables::frontiers, tables::pending }));
	return block_processor.process_one (transaction, post_events, info, false, vban::block_origin::local);
}

void vban::node::process_local_async (std::shared_ptr<vban::block> const & block_a)
{
	// Add block hash as recently arrived to trigger automatic rebroadcast and election
	block_arrival.add (block_a->hash ());
	// Set current time to trigger automatic rebroadcast and election
...
void vban::node::start ()
{
	...
	if (config.frontiers_confirmation != vban::frontiers_confirmation_mode::disabled)
	{
...
void vban::node::keepalive_preconfigured (std::vector<std::string> const & peers_a)
{
...
vban::block_hash vban::node::latest (vban::account const & account_a)
{
	auto transaction (store.tx_begin_read ());
	return ledger.latest (transaction, account_a);
}

vban::uint256_t vban::node::balance (vban::account const & account_a)
{
	auto transaction (store.tx_begin_read ());
	return ledger.account_balance (transaction, account_a);
}

std::shared_ptr<vban::block> vban::node::block (vban::block_hash const & hash_a)
{
	auto transaction (store.tx_begin_read ());
	return store.block_get (transaction, hash_a);
}

std::pair<vban::uint256_t, vban::uint256_t> vban::node::balance_pending (vban::account const & account_a, bool only_confirmed_a)
{
	std::pair<vban::uint256_t, vban::uint256_t> result;
	auto transaction (store.tx_begin_read ());
...
vban::uint256_t vban::node::weight (vban::account const & account_a)
{
	return ledger.weight (account_a);
}

vban::block_hash vban::node::rep_block (vban::account const & account_a)
{
	auto transaction (store.tx_begin_read ());
...
vban::uint256_t vban::node::minimum_principal_weight ()
{
	return minimum_principal_weight (online_reps.trended ());
}

vban::uint256_t vban::node::minimum_principal_weight (vban::uint256_t const & online_stake)
{
	return online_stake / network_params.network.principal_weight_factor;
}

void vban::node::long_inactivity_cleanup ()
{
...
void vban::node::ongoing_rep_calculation ()
{
	auto now (std::chrono::steady_clock::now ());
	vote_processor.calculate_weights ();
	std::weak_ptr<vban::node> node_w (shared_from_this ());
...
void vban::node::ongoing_bootstrap ()
{
	...
	auto previous_bootstrap_count (stats.count (vban::stat::type::bootstrap, vban::stat::detail::initiate, vban::stat::dir::out) + stats.count (vban::stat::type::bootstrap, vban::stat::detail::initiate_legacy_age, vban::stat::dir::out));
	/* 
...
void vban::node::ongoing_peer_store ()
{
...
void vban::node::backup_wallet ()
{
	...
	for (auto i (wallets.items.begin ()), n (wallets.items.end ()); i != n; ++i)
	{
		...
		vban::set_secure_perm_directory (backup_path, error_chmod);
...
void vban::node::bootstrap_wallet ()
{
	std::deque<vban::account> accounts;
	{
		vban::lock_guard<vban::mutex> lock (wallets.mutex);
		auto transaction (wallets.tx_begin_read ());
		for (auto i (wallets.items.begin ()), n (wallets.items.end ()); i != n && accounts.size () < 128; ++i)
		{
			auto & wallet (*i->second);
			vban::lock_guard<std::recursive_mutex> wallet_lock (wallet.store.mutex);
			for (auto j (wallet.store.begin (transaction)), m (wallet.store.end ()); j != m && accounts.size () < 128; ++j)
			{
				vban::account account (j->first);
...
void vban::node::unchecked_cleanup ()
{
	std::vector<vban::uint256_t> digests;
	std::deque<vban::unchecked_key> cleaning_list;
	...
	if (ledger.cache.block_count >= ledger.bootstrap_weight_max_blocks && !long_attempt)
	{
		...
		for (auto i (store.unchecked_begin (transaction)), n (store.unchecked_end ()); i != n && cleaning_list.size () < 1024 * 1024 && vban::seconds_since_epoch () - now < 120; ++i)
		{
...
void vban::node::ongoing_unchecked_cleanup ()
{
	unchecked_cleanup ();
...
void vban::node::ongoing_backlog_population ()
{
	populate_backlog ();
...
bool vban::node::collect_ledger_pruning_targets (std::deque<vban::block_hash> & pruning_targets_a, vban::account & last_account_a, uint64_t const batch_read_size_a, uint64_t const max_depth_a, uint64_t const cutoff_time_a)
{
	...
	for (auto i (store.confirmation_height_begin (transaction, last_account_a)), n (store.confirmation_height_end ()); i != n && !finish_transaction;)
	{
		...
		vban::block_hash hash (i->second.frontier);
		uint64_t depth (0);
...
void vban::node::ledger_pruning (uint64_t const batch_size_a, bool bootstrap_weight_reached_a, bool log_to_cout_a)
{
	uint64_t const max_depth (config.max_pruning_depth != 0 ? config.max_pruning_depth : std::numeric_limits<uint64_t>::max ());
	uint64_t const cutoff_time (bootstrap_weight_reached_a ? vban::seconds_since_epoch () - config.max_pruning_age.count () : std::numeric_limits<uint64_t>::max ());
	uint64_t pruned_count (0);
	uint64_t transaction_write_count (0);
	vban::account last_account (1); // 0 Burn account is never opened. So it can be used to break loop
	std::deque<vban::block_hash> pruning_targets;
	...
	while ((transaction_write_count != 0 || !target_finished) && !stopped)
	{
		...
		{
			auto scoped_write_guard = write_database_queue.wait (vban::writer::pruning);
...
void vban::node::ongoing_ledger_pruning ()
{
	auto bootstrap_weight_reached (ledger.cache.block_count >= ledger.bootstrap_weight_max_blocks);
...
int vban::node::price (vban::uint256_t const & balance_a, int amount_a)
{
	debug_assert (balance_a >= amount_a * vban::Gxrb_ratio);
	auto balance_l (balance_a);
	double result (0.0);
	for (auto i (0); i < amount_a; ++i)
	{
		balance_l -= vban::Gxrb_ratio;
		auto balance_scaled ((balance_l / vban::Mxrb_ratio).convert_to<double> ());
...
uint64_t vban::node::default_difficulty (vban::work_version const version_a) const
{
	uint64_t result{ std::numeric_limits<uint64_t>::max () };
	switch (version_a)
	{
		case vban::work_version::work_1:
			result = vban::work_threshold_base (version_a);
			break;
		default:
...
uint64_t vban::node::default_receive_difficulty (vban::work_version const version_a) const
{
	uint64_t result{ std::numeric_limits<uint64_t>::max () };
	switch (version_a)
	{
...
uint64_t vban::node::max_work_generate_difficulty (vban::work_version const version_a) const
{
	return vban::difficulty::from_multiplier (config.max_work_generate_multiplier, default_difficulty (version_a));
}

bool vban::node::local_work_generation_enabled () const
{
	return config.work_threads > 0 || work.opencl;
}

bool vban::node::work_generation_enabled () const
{
	return work_generation_enabled (config.work_peers);
}

bool vban::node::work_generation_enabled (std::vector<std::pair<std::string, uint16_t>> const & peers_a) const
{
	return !peers_a.empty () || local_work_generation_enabled ();
}

boost::optional<uint64_t> vban::node::work_generate_blocking (vban::block & block_a, uint64_t difficulty_a)
{
...
void vban::node::work_generate (vban::work_version const version_a, vban::root const & root_a, uint64_t difficulty_a, std::function<void (boost::optional<uint64_t>)> callback_a, boost::optional<vban::account> const & account_a, bool secondary_work_peers_a)
{
...
boost::optional<uint64_t> vban::node::work_generate_blocking (vban::work_version const version_a, vban::root const & root_a, uint64_t difficulty_a, boost::optional<vban::account> const & account_a)
{
...
boost::optional<uint64_t> vban::node::work_generate_blocking (vban::block & block_a)
{
	debug_assert (network_params.network.is_dev_network ());
	return work_generate_blocking (block_a, default_difficulty (vban::work_version::work_1));
}

boost::optional<uint64_t> vban::node::work_generate_blocking (vban::root const & root_a)
{
	debug_assert (network_params.network.is_dev_network ());
	return work_generate_blocking (root_a, default_difficulty (vban::work_version::work_1));
}

boost::optional<uint64_t> vban::node::work_generate_blocking (vban::root const & root_a, uint64_t difficulty_a)
{
	debug_assert (network_params.network.is_dev_network ());
	return work_generate_blocking (vban::work_version::work_1, root_a, difficulty_a);
}

void vban::node::add_initial_peers ()
{
	auto transaction (store.tx_begin_read ());
	for (auto i (store.peers_begin (transaction)), n (store.peers_end ()); i != n; ++i)
	{
		vban::endpoint endpoint (boost::asio::ip::address_v6 (i->first.address_bytes ()), i->first.port ());
		if (!network.reachout (endpoint, config.allow_local_peers))
		{
			std::weak_ptr<vban::node> node_w (shared_from_this ());
			network.tcp_channels.start_tcp (endpoint, [node_w] (std::shared_ptr<vban::transport::channel> const & channel_a) {
				if (auto node_l = node_w.lock ())
				{
					node_l->network.send_keepalive (channel_a);
...
void vban::node::block_confirm (std::shared_ptr<vban::block> const & block_a)
{
...
bool vban::node::block_confirmed (vban::block_hash const & hash_a)
{
	auto transaction (store.tx_begin_read ());
	return ledger.block_confirmed (transaction, hash_a);
}

bool vban::node::block_confirmed_or_being_confirmed (vban::transaction const & transaction_a, vban::block_hash const & hash_a)
{
	return confirmation_height_processor.is_processing_block (hash_a) || ledger.block_confirmed (transaction_a, hash_a);
}

void vban::node::ongoing_online_weight_calculation_queue ()
{
	std::weak_ptr<vban::node> node_w (shared_from_this ());
...
bool vban::node::online () const
{
	return rep_crawler.total_weight () > online_reps.delta ();
}

void vban::node::ongoing_online_weight_calculation ()
{
	online_reps.sample ();
	ongoing_online_weight_calculation_queue ();
}

void vban::node::receive_confirmed (vban::transaction const & block_transaction_a, vban::block_hash const & hash_a, vban::account const & destination_a)
{
	vban::unique_lock<vban::mutex> lk (wallets.mutex);
	...
	for ([[maybe_unused]] auto const & [id, wallet] : wallets_l)
	{
		if (wallet->store.exists (wallet_transaction, destination_a))
		{
			...
			auto error (store.pending_get (block_transaction_a, vban::pending_key (destination_a, hash_a), pending));
			if (!error)
			{
				auto amount (pending.amount.number ());
				wallet->receive_async (hash_a, representative, amount, destination_a, [] (std::shared_ptr<vban::block> const &) {});
			}
			else
			{
...
void vban::node::process_confirmed_data (vban::transaction const & transaction_a, std::shared_ptr<vban::block> const & block_a, vban::block_hash const & hash_a, vban::account & account_a, vban::uint256_t & amount_a, bool & is_state_send_a, vban::account & pending_account_a)
{
	...
	if (auto state = dynamic_cast<vban::state_block *> (block_a.get ()))
	{
	...
	if (auto send = dynamic_cast<vban::send_block *> (block_a.get ()))
	{
		pending_account_a = send->hashables.destination;
	}
}

void vban::node::process_confirmed (vban::election_status const & status_a, uint64_t iteration_a)
{
	...
	{
		iteration_a++;
		std::weak_ptr<vban::node> node_w (shared ());
...
bool vban::block_arrival::add (vban::block_hash const & hash_a)
{
	vban::lock_guard<vban::mutex> lock (mutex);
	auto now (std::chrono::steady_clock::now ());
	auto inserted (arrival.get<tag_sequence> ().emplace_back (vban::block_arrival_info{ now, hash_a }));
	auto result (!inserted.second);
	return result;
}

bool vban::block_arrival::recent (vban::block_hash const & hash_a)
{
	vban::lock_guard<vban::mutex> lock (mutex);
	auto now (std::chrono::steady_clock::now ());
...
std::unique_ptr<vban::container_info_component> vban::collect_container_info (block_arrival & block_arrival, std::string const & name)
{
	size_t count = 0;
	{
		vban::lock_guard<vban::mutex> guard (block_arrival.mutex);
...
std::shared_ptr<vban::node> vban::node::shared ()
{
	return shared_from_this ();
}

int vban::node::store_version ()
{
	auto transaction (store.tx_begin_read ());
	return store.version_get (transaction);
}

bool vban::node::init_error () const
{
	return store.init_error () || wallets_store.init_error ();
}

bool vban::node::epoch_upgrader (vban::raw_key const & prv_a, vban::epoch epoch_a, uint64_t count_limit, uint64_t threads)
{
	...
	if (!error)
	{
		...
		if (!error)
		{
			*epoch_upgrade = std::async (std::launch::async, &vban::node::epoch_upgrader_impl, this, prv_a, epoch_a, count_limit, threads);
		}
	}
	return error;
}

void vban::node::set_bandwidth_params (size_t limit, double ratio)
{
...
void vban::node::epoch_upgrader_impl (vban::raw_key const & prv_a, vban::epoch epoch_a, uint64_t count_limit, uint64_t threads)
{
	vban::thread_role::set (vban::thread_role::name::epoch_upgrader);
	auto upgrader_process = [] (vban::node & node_a, std::atomic<uint64_t> & counter, std::shared_ptr<vban::block> const & epoch, uint64_t difficulty, vban::public_key const & signer_a, vban::root const & root_a, vban::account const & account_a) {
		epoch->block_work_set (node_a.work_generate_blocking (vban::work_version::work_1, root_a, difficulty).value_or (0));
		bool valid_signature (!vban::validate_message (signer_a, epoch->hash (), epoch->block_signature ()));
		bool valid_work (epoch->difficulty () >= difficulty);
		vban::process_result result (vban::process_result::old);
		if (valid_signature && valid_work)
		{
			result = node_a.process_local (epoch).code;
		}
		if (result == vban::process_result::progress)
		{
			++counter;
		}
		else
		{
			bool fork (result == vban::process_result::fork);
	...
	vban::raw_key raw_key;
	raw_key = prv_a;
	auto signer (vban::pub_key (prv_a));
	debug_assert (signer == ledger.epoch_signer (link));

	vban::mutex upgrader_mutex;
	vban::condition_variable upgrader_condition;
	...
	boost::multi_index::indexed_by<
		...
		boost::multi_index::hashed_unique<boost::multi_index::tag<account_tag>,
			boost::multi_index::member<account_upgrade_item, vban::account, &account_upgrade_item::account>>>>
	accounts_list;
	// clang-format on
	...
	while (!finished_upgrade && !stopped)
	{
		...
		{
			{
				...
				for (auto i (store.accounts_begin (transaction)), n (store.accounts_end ()); i != n && accounts_list.size () < count_limit; ++i)
				{
					...
					{
						release_assert (vban::epochs::is_sequential (info.epoch (), epoch_a));
			...
			for (auto i (accounts_list.get<modified_tag> ().begin ()), n (accounts_list.get<modified_tag> ().end ()); i != n && attempts < upgrade_batch_size && attempts < count_limit && !stopped; ++i)
			{
				auto transaction (store.tx_begin_read ());
				vban::account_info info;
				vban::account const & account (i->account);
				if (!store.account_get (transaction, account, info) && info.epoch () < epoch_a)
				{
					++attempts;
					auto difficulty (vban::work_threshold (vban::work_version::work_1, vban::block_details (epoch_a, false, false, true)));
					vban::root const & root (info.head);
					std::shared_ptr<vban::block> epoch = builder.state ()
														 .account (account)
														 .previous (info.head)
					...
					{
						{
							vban::unique_lock<vban::mutex> lock (upgrader_mutex);
							++workers;
						...
						this->workers.push_task ([node_l = shared_from_this (), &upgrader_process, &upgrader_mutex, &upgrader_condition, &upgraded_accounts, &workers, epoch, difficulty, signer, root, account] () {
							...
							{
								vban::lock_guard<vban::mutex> lock (upgrader_mutex);
								--workers;
							}
		...
		while (!finished_pending && count_limit != 0 && !stopped)
		{
			...
			uint64_t attempts (0);
			auto transaction (store.tx_begin_read ());
			for (auto i (store.pending_begin (transaction, vban::pending_key (1, 0))), n (store.pending_end ()); i != n && attempts < upgrade_batch_size && attempts < count_limit && !stopped;)
			{
				...
				if (!store.account_exists (transaction, key.account))
				{
					...
					if (info.epoch < epoch_a)
					{
						...
						release_assert (vban::epochs::is_sequential (info.epoch, epoch_a));
						auto difficulty (vban::work_threshold (vban::work_version::work_1, vban::block_details (epoch_a, false, false, true)));
						vban::root const & root (key.account);
						vban::account const & account (key.account);
						std::shared_ptr<vban::block> epoch = builder.state ()
															 .account (key.account)
															 .previous (0)
															 .representative (0)
				...
				if (to_next_account)
				{
					...
					if (key.account.number () == std::numeric_limits<vban::uint256_t>::max ())
					{
						break;
					}
					else
					{
						i = store.pending_begin (transaction, vban::pending_key (key.account.number () + 1, 0));
					}
				}
				else
				{
...
std::pair<uint64_t, decltype (vban::ledger::bootstrap_weights)> vban::node::get_bootstrap_weights () const
{
	std::unordered_map<vban::account, vban::uint256_t> weights;
	const uint8_t * weight_buffer = network_params.network.is_live_network () ? vban_bootstrap_weights_live : vban_bootstrap_weights_beta;
	size_t weight_size = network_params.network.is_live_network () ? vban_bootstrap_weights_live_size : vban_bootstrap_weights_beta_size;
	vban::bufferstream weight_stream ((const uint8_t *)weight_buffer, weight_size);
	vban::uint128_union block_height;
	uint64_t max_blocks = 0;
	if (!vban::try_read (weight_stream, block_height))
	{
		max_blocks = vban::narrow_cast<uint64_t> (block_height.number ());
		while (true)
		{
			vban::account account;
			if (vban::try_read (weight_stream, account.bytes))
			{
				break;
			}
			vban::amount weight;
			if (vban::try_read (weight_stream, weight.bytes))
			{
				break;
			}
...
void vban::node::populate_backlog ()
{
	auto done = false;
...
vban::node_wrapper::node_wrapper (boost::filesystem::path const & path_a, boost::filesystem::path const & config_path_a, vban::node_flags const & node_flags_a) :
	...
	work (1)
{
	...
	vban::set_secure_perm_directory (path_a, error_chmod);
	vban::daemon_config daemon_config (path_a);
	auto error = vban::read_node_config_toml (config_path_a, daemon_config, node_flags_a.config_overrides);
	if (error)
	{
	...
	node = std::make_shared<vban::node> (*io_context, path_a, node_config, work, node_flags_a);
}
...
vban::inactive_node::inactive_node (boost::filesystem::path const & path_a, boost::filesystem::path const & config_path_a, vban::node_flags const & node_flags_a) :
...
vban::inactive_node::inactive_node (boost::filesystem::path const & path_a, vban::node_flags const & node_flags_a) :
	inactive_node (path_a, path_a, node_flags_a)
{
}

vban::node_flags const & vban::inactive_node_flag_defaults ()
{
	static vban::node_flags node_flags;
...
std::unique_ptr<vban::block_store> vban::make_store (vban::logger_mt & logger, boost::filesystem::path const & path, bool read_only, bool add_db_postfix, vban::rocksdb_config const & rocksdb_config, vban::txn_tracking_config const & txn_tracking_config_a, std::chrono::milliseconds block_processor_batch_max_time_a, vban::lmdb_config const & lmdb_config_a, bool backup_before_upgrade)
{
	if (rocksdb_config.enable || using_rocksdb_in_tests ())
	{
		return std::make_unique<vban::rocksdb_store> (logger, add_db_postfix ? path / "rocksdb" : path, rocksdb_config, read_only);
	}

	return std::make_unique<vban::mdb_store> (logger, add_db_postfix ? path / "data.ldb" : path, txn_tracking_config_a, block_processor_batch_max_time_a, lmdb_config_a, backup_before_upgrade);
}
