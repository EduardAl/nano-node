...
inline boost::log::formatting_ostream & operator<< (boost::log::formatting_ostream & strm, boost::log::to_log_manip<vban::severity_level, severity_tag> const & manip)
{
	...
	vban::severity_level level = manip.get ();
...
private:
	...
	void output (vban::severity_level severity_level, LogItems &&... log_items)
	{
...
public:
	 ...
	 */
	template <typename... LogItems>
	void always_log (vban::severity_level severity_level, LogItems &&... log_items)
	{
	...
	{
		always_log (vban::severity_level::normal, std::forward<LogItems> (log_items)...);
	}
	 ...
	 */
	template <typename... LogItems>
	bool try_log (vban::severity_level severity_level, LogItems &&... log_items)
	{
		auto error (true);
		auto time_now = std::chrono::steady_clock::now ();
		vban::unique_lock<vban::mutex> lk (last_log_time_mutex);
	...
	{
		return try_log (vban::severity_level::normal, std::forward<LogItems> (log_items)...);
	}

	std::chrono::milliseconds min_log_delta{ 0 };

private:
	vban::mutex last_log_time_mutex;
