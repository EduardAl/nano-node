...
#include <vban/boost/beast/http.hpp>
...
public:
	...
	{
		sock.async_connect (vban::tcp_endpoint (boost::asio::ip::address_v6::loopback (), port_a), [this] (boost::system::error_code const & ec) {
			if (!ec)
			{
...
std::shared_ptr<vban::node> add_ipc_enabled_node (vban::system & system, vban::node_config & node_config, vban::node_flags const & node_flags)
{
	node_config.ipc_config.transport_tcp.enabled = true;
	node_config.ipc_config.transport_tcp.port = vban::get_available_port ();
	return system.add_node (node_config, node_flags);
}

std::shared_ptr<vban::node> add_ipc_enabled_node (vban::system & system, vban::node_config & node_config)
{
	return add_ipc_enabled_node (system, node_config, vban::node_flags ());
}

std::shared_ptr<vban::node> add_ipc_enabled_node (vban::system & system)
{
	vban::node_config node_config (vban::get_available_port (), system.logging);
	return add_ipc_enabled_node (system, node_config);
}

void reset_confirmation_height (vban::block_store & store, vban::account const & account)
{
...
void check_block_response_count (vban::system & system, vban::rpc & rpc, boost::property_tree::ptree & request, uint64_t size_count)
{
...
public:
	...
	void reset ()
	{
		vban::thread_role::set (vban::thread_role::name::unknown);
	}

	void renew ()
	{
		vban::thread_role::set (vban::thread_role::name::io);
	}
};
}
...
TEST (rpc, wrapped_task)
{
	...
	vban::node_rpc_config node_rpc_config;
	...
	auto handler_l (std::make_shared<vban::json_handler> (node, node_rpc_config, "", response_handler_l));
	auto task (handler_l->create_worker_task ([] (std::shared_ptr<vban::json_handler> const &) {
...
TEST (rpc, account_balance)
{
	...
	vban::node_rpc_config node_rpc_config;
	vban::ipc::ipc_server ipc_server (*node, node_rpc_config);
	vban::rpc_config rpc_config (vban::get_available_port (), true);
	rpc_config.rpc_process.ipc_port = node->config.ipc_config.transport_tcp.port;
	vban::ipc_rpc_processor ipc_rpc_processor (system.io_ctx, rpc_config);
	vban::rpc rpc (system.io_ctx, rpc_config, ipc_rpc_processor);
	rpc.start ();

	boost::property_tree::ptree request;
	request.put ("action", "account_balance");
	request.put ("account", vban::dev_genesis_key.pub.to_account ());
...
TEST (rpc, account_create)
{
	...
	ASSERT_EQ (std::error_code (vban::error_common::invalid_index).message (), response2.json.get<std::string> ("error"));
}
...
TEST (rpc, account_weight)
{
	...
	vban::change_block block (latest, key.pub, vban::dev_genesis_key.prv, vban::dev_genesis_key.pub, *node1.work_generate_blocking (latest));
	ASSERT_EQ (vban::process_result::progress, node1.process (block).code);
	scoped_io_thread_name_change scoped_thread_name_io;
	vban::node_rpc_config node_rpc_config;
	vban::ipc::ipc_server ipc_server (node1, node_rpc_config);
	vban::rpc_config rpc_config (vban::get_available_port (), true);
...
TEST (rpc, validate_account_invalid)
{
	...
	rpc.start ();
	std::string account;
	vban::dev_genesis_key.pub.encode_account (account);
	account[0] ^= 0x1;
	boost::property_tree::ptree request;
	request.put ("action", "validate_account_number");
...
TEST (rpc, send)
{
	...
	request.put ("source", vban::dev_genesis_key.pub.to_account ());
	request.put ("destination", vban::dev_genesis_key.pub.to_account ());
	request.put ("amount", "100");
	ASSERT_EQ (node->balance (vban::dev_genesis_key.pub), vban::genesis_amount);
	test_response response (request, rpc.config.port, system.io_ctx);
	ASSERT_TIMELY (10s, response.status != 0);
	ASSERT_EQ (200, response.status);
	std::string block_text (response.json.get<std::string> ("block"));
	vban::block_hash block;
	ASSERT_FALSE (block.decode_hex (block_text));
	ASSERT_TRUE (node->ledger.block_or_pruned_exists (block));
	ASSERT_EQ (node->latest (vban::dev_genesis_key.pub), block);
	ASSERT_NE (node->balance (vban::dev_genesis_key.pub), vban::genesis_amount);
}
...
TEST (rpc, send_fail)
{
	...
	test_response response (request, rpc.config.port, system.io_ctx);
	ASSERT_TIMELY (10s, response.status != 0);
	ASSERT_EQ (std::error_code (vban::error_common::account_not_found_wallet).message (), response.json.get<std::string> ("error"));
}
...
TEST (rpc, send_work)
{
	...
	test_response response (request, rpc.config.port, system.io_ctx);
	ASSERT_TIMELY (10s, response.status != 0);
	ASSERT_EQ (std::error_code (vban::error_common::invalid_work).message (), response.json.get<std::string> ("error"));
	request.erase ("work");
	request.put ("work", vban::to_string_hex (*node->work_generate_blocking (node->latest (vban::dev_genesis_key.pub))));
	test_response response2 (request, rpc.config.port, system.io_ctx);
...
TEST (rpc, send_work_disabled)
{
	...
	request.put ("amount", "100");
	{
		...
		ASSERT_TIMELY (10s, response.status != 0);
		ASSERT_EQ (200, response.status);
		ASSERT_EQ (std::error_code (vban::error_common::disabled_work_generation).message (), response.json.get<std::string> ("error"));
	}
}
...
TEST (rpc, send_idempotent)
{
	...
	request.put ("action", "send");
	request.put ("source", vban::dev_genesis_key.pub.to_account ());
	request.put ("destination", vban::account (0).to_account ());
	request.put ("amount", (vban::genesis_amount - (vban::genesis_amount / 4)).convert_to<std::string> ());
	...
	ASSERT_FALSE (block.decode_hex (block_text));
	ASSERT_TRUE (node->ledger.block_or_pruned_exists (block));
	ASSERT_EQ (node->balance (vban::dev_genesis_key.pub), vban::genesis_amount / 4);
	test_response response2 (request, rpc.config.port, system.io_ctx);
	ASSERT_TIMELY (5s, response2.status != 0);
	ASSERT_EQ (200, response2.status);
	...
	ASSERT_EQ (std::error_code (vban::error_common::insufficient_balance).message (), response3.json.get<std::string> ("error"));
}
...
TEST (rpc, send_epoch_2)
{
	...
	auto target_difficulty = vban::work_threshold (vban::work_version::work_1, vban::block_details (vban::epoch::epoch_2, true, false, false));
	...
	request.put ("action", "send");
	request.put ("source", vban::dev_genesis_key.pub.to_account ());
	request.put ("destination", vban::keypair ().pub.to_account ());
	request.put ("amount", "1");

	// Test that the correct error is given if there is insufficient work
	auto insufficient = system.work_generate_limited (vban::genesis_hash, min_difficulty, target_difficulty);
	request.put ("work", vban::to_string_hex (insufficient));
	{
		test_response response (request, rpc.config.port, system.io_ctx);
		ASSERT_TIMELY (5s, response.status != 0);
		ASSERT_EQ (200, response.status);
		std::error_code ec (vban::error_common::invalid_work);
...
TEST (rpc, wallet_representative)
{
	...
	ASSERT_EQ (account_text1, vban::genesis_account.to_account ());
}
...
TEST (rpc, wallet_representative_set_force)
{
	...
	while (representative != key.pub)
	{
		...
		if (!node->store.account_get (transaction, vban::dev_genesis_key.pub, info))
		{
...
TEST (rpc, wallet_create_seed)
{
	...
	scoped_io_thread_name_change scoped_thread_name_io;
	vban::raw_key seed;
	vban::random_pool::generate_block (seed.bytes.data (), seed.bytes.size ());
...
TEST (rpc, wallet_export)
{
	...
	vban::kdf kdf;
	vban::wallet_store store (error, kdf, transaction, vban::genesis_account, 1, "0", wallet_json);
	ASSERT_FALSE (error);
	ASSERT_TRUE (store.exists (transaction, vban::dev_genesis_key.pub));
}
...
TEST (rpc, account_move)
{
	...
	destination->insert_adhoc (vban::dev_genesis_key.prv);
	vban::keypair key;
	auto source_id = vban::random_wallet_id ();
	...
	ASSERT_TRUE (destination->exists (vban::dev_genesis_key.pub));
	auto transaction (node->wallets.tx_begin_read ());
...
TEST (rpc, block)
{
	...
	request.put ("hash", node->latest (vban::genesis_account).to_string ());
	test_response response (request, rpc.config.port, system.io_ctx);
	ASSERT_TIMELY (5s, response.status != 0);
	ASSERT_EQ (200, response.status);
...
TEST (rpc, chain)
{
	...
	system.wallet (0)->insert_adhoc (vban::dev_genesis_key.prv);
	vban::keypair key;
	auto genesis (node->latest (vban::dev_genesis_key.pub));
	ASSERT_FALSE (genesis.is_zero ());
	auto block (system.wallet (0)->send_action (vban::dev_genesis_key.pub, key.pub, 1));
	...
	std::vector<vban::block_hash> blocks;
	for (auto i (blocks_node.begin ()), n (blocks_node.end ()); i != n; ++i)
	{
		blocks.push_back (vban::block_hash (i->second.get<std::string> ("")));
	}
...
TEST (rpc, frontier)
{
	vban::system system;
	auto node = add_ipc_enabled_node (system);
	std::unordered_map<vban::account, vban::block_hash> source;
	{
		...
		for (auto i (0); i < 1000; ++i)
		{
			...
			source[key.pub] = hash;
			node->store.confirmation_height_put (transaction, key.pub, { 0, vban::block_hash (0) });
			node->store.account_put (transaction, key.pub, vban::account_info (hash, 0, 0, 0, 0, 0, vban::epoch::epoch_0));
		}
	}
	scoped_io_thread_name_change scoped_thread_name_io;
	vban::keypair key;
	...
	request.put ("account", vban::account (0).to_account ());
	request.put ("count", std::to_string (std::numeric_limits<uint64_t>::max ()));
	test_response response (request, rpc.config.port, system.io_ctx);
	ASSERT_TIMELY (5s, response.status != 0);
	ASSERT_EQ (200, response.status);
	auto & frontiers_node (response.json.get_child ("frontiers"));
	std::unordered_map<vban::account, vban::block_hash> frontiers;
	...
	ASSERT_EQ (1, frontiers.erase (vban::dev_genesis_key.pub));
...
TEST (rpc, history)
{
	...
	system.wallet (0)->insert_adhoc (vban::dev_genesis_key.prv);
	auto change (system.wallet (0)->change_action (vban::dev_genesis_key.pub, vban::dev_genesis_key.pub));
	ASSERT_NE (nullptr, change);
	auto send (system.wallet (0)->send_action (vban::dev_genesis_key.pub, vban::dev_genesis_key.pub, node0->config.receive_minimum.number ()));
	ASSERT_NE (nullptr, send);
	auto receive (system.wallet (0)->receive_action (send->hash (), vban::dev_genesis_key.pub, node0->config.receive_minimum.number (), send->link ().as_account ()));
	ASSERT_NE (nullptr, receive);
	vban::genesis genesis;
	vban::state_block usend (vban::genesis_account, node0->latest (vban::genesis_account), vban::genesis_account, vban::genesis_amount - vban::Gxrb_ratio, vban::genesis_account, vban::dev_genesis_key.prv, vban::dev_genesis_key.pub, *node0->work_generate_blocking (node0->latest (vban::genesis_account)));
	vban::state_block ureceive (vban::genesis_account, usend.hash (), vban::genesis_account, vban::genesis_amount, usend.hash (), vban::dev_genesis_key.prv, vban::dev_genesis_key.pub, *node0->work_generate_blocking (usend.hash ()));
	vban::state_block uchange (vban::genesis_account, ureceive.hash (), vban::keypair ().pub, vban::genesis_amount, 0, vban::dev_genesis_key.prv, vban::dev_genesis_key.pub, *node0->work_generate_blocking (ureceive.hash ()));
	{
		auto transaction (node0->store.tx_begin_write ());
		ASSERT_EQ (vban::process_result::progress, node0->ledger.process (transaction, usend).code);
		ASSERT_EQ (vban::process_result::progress, node0->ledger.process (transaction, ureceive).code);
		ASSERT_EQ (vban::process_result::progress, node0->ledger.process (transaction, uchange).code);
	}
	scoped_io_thread_name_change scoped_thread_name_io;
	vban::node_rpc_config node_rpc_config;
	vban::ipc::ipc_server ipc_server (*node0, node_rpc_config);
	vban::rpc_config rpc_config (vban::get_available_port (), true);
	...
	ASSERT_EQ (vban::dev_genesis_key.pub.to_account (), std::get<1> (history_l[0]));
	ASSERT_EQ (vban::Gxrb_ratio.convert_to<std::string> (), std::get<2> (history_l[0]));
	ASSERT_EQ (5, history_l.size ());
	ASSERT_EQ ("send", std::get<0> (history_l[1]));
	ASSERT_EQ (usend.hash ().to_string (), std::get<3> (history_l[1]));
	ASSERT_EQ (vban::dev_genesis_key.pub.to_account (), std::get<1> (history_l[1]));
	ASSERT_EQ (vban::Gxrb_ratio.convert_to<std::string> (), std::get<2> (history_l[1]));
	ASSERT_EQ ("receive", std::get<0> (history_l[2]));
	ASSERT_EQ (vban::dev_genesis_key.pub.to_account (), std::get<1> (history_l[2]));
	...
	ASSERT_EQ (vban::dev_genesis_key.pub.to_account (), std::get<1> (history_l[3]));
	...
	ASSERT_EQ (vban::dev_genesis_key.pub.to_account (), std::get<1> (history_l[4]));
	ASSERT_EQ (vban::genesis_amount.convert_to<std::string> (), std::get<2> (history_l[4]));
...
TEST (rpc, account_history)
{
	...
	rpc.start ();
	{
		...
		request.put ("account", vban::genesis_account.to_account ());
		request.put ("count", 100);
		test_response response (request, rpc.config.port, system.io_ctx);
		ASSERT_TIMELY (10s, response.status != 0);
		ASSERT_EQ (200, response.status);
	...
	auto send2 (system.wallet (0)->send_action (vban::dev_genesis_key.pub, account2, node0->config.receive_minimum.number ()));
	...
	// Test filter for receive state blocks
	{
		...
		boost::property_tree::ptree other_account;
		other_account.put ("", vban::dev_genesis_key.pub.to_account ());
		boost::property_tree::ptree filtered_accounts;
		filtered_accounts.push_back (std::make_pair ("", other_account));
		request.add_child ("account_filter", filtered_accounts);
		request.put ("count", 100);
...
TEST (rpc, history_count)
{
	...
	auto change (system.wallet (0)->change_action (vban::dev_genesis_key.pub, vban::dev_genesis_key.pub));
	ASSERT_NE (nullptr, change);
	auto send (system.wallet (0)->send_action (vban::dev_genesis_key.pub, vban::dev_genesis_key.pub, node->config.receive_minimum.number ()));
	ASSERT_NE (nullptr, send);
	auto receive (system.wallet (0)->receive_action (send->hash (), vban::dev_genesis_key.pub, node->config.receive_minimum.number (), send->link ().as_account ()));
	ASSERT_NE (nullptr, receive);
	scoped_io_thread_name_change scoped_thread_name_io;
	vban::node_rpc_config node_rpc_config;
	vban::ipc::ipc_server ipc_server (*node, node_rpc_config);
...
TEST (rpc, history_pruning)
{
	...
	vban::genesis genesis;
	auto change (std::make_shared<vban::change_block> (genesis.hash (), vban::dev_genesis_key.pub, vban::dev_genesis_key.prv, vban::dev_genesis_key.pub, *node0->work.generate (genesis.hash ())));
	node0->process_active (change);
	auto send (std::make_shared<vban::send_block> (change->hash (), vban::dev_genesis_key.pub, vban::genesis_amount - node0->config.receive_minimum.number (), vban::dev_genesis_key.prv, vban::dev_genesis_key.pub, *node0->work.generate (change->hash ())));
	node0->process_active (send);
	auto receive (std::make_shared<vban::receive_block> (send->hash (), send->hash (), vban::dev_genesis_key.prv, vban::dev_genesis_key.pub, *node0->work.generate (send->hash ())));
	node0->process_active (receive);
	auto usend (std::make_shared<vban::state_block> (vban::genesis_account, receive->hash (), vban::genesis_account, vban::genesis_amount - vban::Gxrb_ratio, vban::genesis_account, vban::dev_genesis_key.prv, vban::dev_genesis_key.pub, *node0->work_generate_blocking (receive->hash ())));
	auto ureceive (std::make_shared<vban::state_block> (vban::genesis_account, usend->hash (), vban::genesis_account, vban::genesis_amount, usend->hash (), vban::dev_genesis_key.prv, vban::dev_genesis_key.pub, *node0->work_generate_blocking (usend->hash ())));
	auto uchange (std::make_shared<vban::state_block> (vban::genesis_account, ureceive->hash (), vban::keypair ().pub, vban::genesis_amount, 0, vban::dev_genesis_key.prv, vban::dev_genesis_key.pub, *node0->work_generate_blocking (ureceive->hash ())));
...
TEST (rpc, process_block)
{
	...
	scoped_io_thread_name_change scoped_thread_name_io;
	vban::keypair key;
	auto latest (node1.latest (vban::dev_genesis_key.pub));
	vban::send_block send (latest, key.pub, 100, vban::dev_genesis_key.prv, vban::dev_genesis_key.pub, *node1.work_generate_blocking (latest));
	vban::node_rpc_config node_rpc_config;
	vban::ipc::ipc_server ipc_server (node1, node_rpc_config);
	vban::rpc_config rpc_config (vban::get_available_port (), true);
	rpc_config.rpc_process.ipc_port = node1.config.ipc_config.transport_tcp.port;
	...
	request.put ("block", json);
	{
		...
		ASSERT_TIMELY (5s, response.status != 0);
		ASSERT_EQ (200, response.status);
		ASSERT_TIMELY (10s, node1.latest (vban::dev_genesis_key.pub) == send.hash ());
	...
	request.put ("json_block", true);
	{
		...
		ASSERT_TIMELY (5s, response.status != 0);
		ASSERT_EQ (200, response.status);
		std::error_code ec (vban::error_blocks::invalid_block);
...
TEST (rpc, process_block_async)
{
	...
	request.put ("json_block", false);
	{
		...
		ASSERT_TIMELY (5s, response.status != 0);
		ASSERT_EQ (200, response.status);
		std::error_code ec (vban::error_common::is_not_state_block);
		ASSERT_EQ (ec.message (), response.json.get<std::string> ("error"));
	}

	auto state_send (std::make_shared<vban::state_block> (vban::dev_genesis_key.pub, latest, vban::dev_genesis_key.pub, vban::genesis_amount - 100, vban::dev_genesis_key.pub, vban::dev_genesis_key.prv, vban::dev_genesis_key.pub, *system.work.generate (latest)));
	std::string json1;
	...
	request.put ("block", json1);
	{
		...
		ASSERT_TIMELY (10s, node1.latest (vban::dev_genesis_key.pub) == state_send->hash ());
	}
}
...
TEST (rpc, process_republish)
{
	...
	vban::keypair key;
	auto latest (node1.latest (vban::dev_genesis_key.pub));
	vban::send_block send (latest, key.pub, 100, vban::dev_genesis_key.prv, vban::dev_genesis_key.pub, *node3.work_generate_blocking (latest));
	vban::node_rpc_config node_rpc_config;
	vban::ipc::ipc_server ipc_server (node3, node_rpc_config);
	vban::rpc_config rpc_config (vban::get_available_port (), true);
	...
	ASSERT_TIMELY (5s, response.status != 0);
	ASSERT_EQ (200, response.status);
	ASSERT_TIMELY (10s, node2.latest (vban::dev_genesis_key.pub) == send.hash ());
}
...
TEST (rpc, process_subtype_send)
{
	...
	vban::keypair key;
	auto latest (node1.latest (vban::dev_genesis_key.pub));
	vban::state_block send (vban::genesis_account, latest, vban::genesis_account, vban::genesis_amount - vban::Gxrb_ratio, key.pub, vban::dev_genesis_key.prv, vban::dev_genesis_key.pub, *node1.work_generate_blocking (latest));
	vban::node_rpc_config node_rpc_config;
	vban::ipc::ipc_server ipc_server (node1, node_rpc_config);
	vban::rpc_config rpc_config (vban::get_available_port (), true);
	rpc_config.rpc_process.ipc_port = node1.config.ipc_config.transport_tcp.port;
	...
	ASSERT_TIMELY (5s, response.status != 0);
	ASSERT_EQ (200, response.status);
	std::error_code ec (vban::error_rpc::invalid_subtype_balance);
	ASSERT_EQ (response.json.get<std::string> ("error"), ec.message ());
	...
	ASSERT_TIMELY (10s, system.nodes[1]->latest (vban::dev_genesis_key.pub) == send.hash ());
}
...
TEST (rpc, process_subtype_open)
{
	...
	vban::keypair key;
	auto latest (node1.latest (vban::dev_genesis_key.pub));
	auto send = std::make_shared<vban::state_block> (vban::genesis_account, latest, vban::genesis_account, vban::genesis_amount - vban::Gxrb_ratio, key.pub, vban::dev_genesis_key.prv, vban::dev_genesis_key.pub, *node1.work_generate_blocking (latest));
	ASSERT_EQ (vban::process_result::progress, node1.process (*send).code);
	ASSERT_EQ (vban::process_result::progress, node2.process (*send).code);
	scoped_io_thread_name_change scoped_thread_name_io;
	node1.scheduler.manual (send);
	vban::state_block open (key.pub, 0, key.pub, vban::Gxrb_ratio, send->hash (), key.prv, key.pub, *node1.work_generate_blocking (key.pub));
	vban::node_rpc_config node_rpc_config;
	vban::ipc::ipc_server ipc_server (node1, node_rpc_config);
	vban::rpc_config rpc_config (vban::get_available_port (), true);
	rpc_config.rpc_process.ipc_port = node1.config.ipc_config.transport_tcp.port;
...
TEST (rpc, process_subtype_receive)
{
	...
	auto & node2 = *system.add_node ();
	auto latest (node1.latest (vban::dev_genesis_key.pub));
	auto send = std::make_shared<vban::state_block> (vban::genesis_account, latest, vban::genesis_account, vban::genesis_amount - vban::Gxrb_ratio, vban::dev_genesis_key.pub, vban::dev_genesis_key.prv, vban::dev_genesis_key.pub, *node1.work_generate_blocking (latest));
	ASSERT_EQ (vban::process_result::progress, node1.process (*send).code);
	ASSERT_EQ (vban::process_result::progress, node2.process (*send).code);
	scoped_io_thread_name_change scoped_thread_name_io;
	node1.scheduler.manual (send);
	vban::state_block receive (vban::dev_genesis_key.pub, send->hash (), vban::dev_genesis_key.pub, vban::genesis_amount, send->hash (), vban::dev_genesis_key.prv, vban::dev_genesis_key.pub, *node1.work_generate_blocking (send->hash ()));
	vban::node_rpc_config node_rpc_config;
	vban::ipc::ipc_server ipc_server (node1, node_rpc_config);
	vban::rpc_config rpc_config (vban::get_available_port (), true);
	rpc_config.rpc_process.ipc_port = node1.config.ipc_config.transport_tcp.port;
	...
	ASSERT_TIMELY (10s, response2.status != 0);
	ASSERT_EQ (200, response2.status);
	ec = vban::error_rpc::invalid_subtype_previous;
	ASSERT_EQ (response2.json.get<std::string> ("error"), ec.message ());
	request.put ("subtype", "receive");
	test_response response3 (request, rpc.config.port, system.io_ctx);
	ASSERT_TIMELY (10s, response3.status != 0);
	...
	ASSERT_TIMELY (10s, node2.latest (vban::dev_genesis_key.pub) == receive.hash ());
}
...
TEST (rpc, process_ledger_insufficient_work)
{
	...
	auto latest (node.latest (vban::dev_genesis_key.pub));
	auto min_difficulty = node.network_params.network.publish_thresholds.entry;
	auto max_difficulty = node.network_params.network.publish_thresholds.epoch_1;
	vban::state_block send (vban::genesis_account, latest, vban::genesis_account, vban::genesis_amount - vban::Gxrb_ratio, vban::dev_genesis_key.pub, vban::dev_genesis_key.prv, vban::dev_genesis_key.pub, system.work_generate_limited (latest, min_difficulty, max_difficulty));
	...
	ASSERT_TIMELY (5s, response.status != 0);
	ASSERT_EQ (200, response.status);
	std::error_code ec (vban::error_process::insufficient_work);
	ASSERT_EQ (1, response.json.count ("error"));
	ASSERT_EQ (response.json.get<std::string> ("error"), ec.message ());
}
...
TEST (rpc, keepalive)
{
	vban::system system;
	auto node0 = add_ipc_enabled_node (system);
	auto node1 (std::make_shared<vban::node> (system.io_ctx, vban::get_available_port (), vban::unique_path (), system.logging, system.work));
	node1->start ();
...
TEST (rpc, peers)
{
	vban::system system;
	auto node = add_ipc_enabled_node (system);
	auto port = vban::get_available_port ();
	system.add_node (vban::node_config (port, system.logging));
	scoped_io_thread_name_change scoped_thread_name_io;
	vban::endpoint endpoint (boost::asio::ip::make_address_v6 ("fc00::1"), 4000);
...
TEST (rpc, pending)
{
	...
	vban::keypair key1;
	system.wallet (0)->insert_adhoc (vban::dev_genesis_key.prv);
	auto block1 (system.wallet (0)->send_action (vban::dev_genesis_key.pub, key1.pub, 100));
	scoped_io_thread_name_change scoped_thread_name_io;
	...
	request.put ("count", "100");
	{
		...
		vban::block_hash hash (blocks_node.begin ()->second.get<std::string> (""));
	...
	request.put ("sorting", "true"); // Sorting test
	{
		...
		auto & blocks_node (response.json.get_child ("blocks"));
		ASSERT_EQ (1, blocks_node.size ());
		vban::block_hash hash (blocks_node.begin ()->first);
		ASSERT_EQ (block1->hash (), hash);
	...
	request.put ("threshold", "100"); // Threshold test
	{
		...
		auto & blocks_node (response.json.get_child ("blocks"));
		ASSERT_EQ (1, blocks_node.size ());
		std::unordered_map<vban::block_hash, vban::uint128_union> blocks;
		for (auto i (blocks_node.begin ()), j (blocks_node.end ()); i != j; ++i)
		{
			vban::block_hash hash;
			hash.decode_hex (i->first);
			vban::uint128_union amount;
	...
	request.put ("min_version", "true");
	{
		...
		auto & blocks_node (response.json.get_child ("blocks"));
		ASSERT_EQ (1, blocks_node.size ());
		std::unordered_map<vban::block_hash, vban::uint128_union> amounts;
		std::unordered_map<vban::block_hash, vban::account> sources;
		for (auto i (blocks_node.begin ()), j (blocks_node.end ()); i != j; ++i)
		{
			vban::block_hash hash;
			hash.decode_hex (i->first);
		...
		ASSERT_EQ (sources[block1->hash ()], vban::dev_genesis_key.pub);
	}
	...
	scoped_thread_name_io.reset ();
	node->store.confirmation_height_put (node->store.tx_begin_write (), vban::dev_genesis_key.pub, { 2, block1->hash () });
	auto block2 (system.wallet (0)->send_action (vban::dev_genesis_key.pub, key1.pub, 200));
	auto block3 (system.wallet (0)->send_action (vban::dev_genesis_key.pub, key1.pub, 300));
	auto block4 (system.wallet (0)->send_action (vban::dev_genesis_key.pub, key1.pub, 400));
	scoped_thread_name_io.renew ();
...

	{
		...
		vban::block_hash hash (blocks_node.begin ()->first);
		vban::block_hash hash1 ((++blocks_node.begin ())->first);
...
TEST (rpc, pending_burn)
{
	...
	vban::account burn (0);
	system.wallet (0)->insert_adhoc (vban::dev_genesis_key.prv);
	auto block1 (system.wallet (0)->send_action (vban::dev_genesis_key.pub, burn, 100));
	scoped_io_thread_name_change scoped_thread_name_io;
	node->scheduler.flush ();
	ASSERT_TIMELY (5s, !node->active.active (*block1));
	ASSERT_TIMELY (5s, node->ledger.cache.cemented_count == 2 && node->confirmation_height_processor.current ().is_zero () && node->confirmation_height_processor.awaiting_processing_size () == 0);
...
TEST (rpc, search_pending)
{
	...
	auto latest (node->latest (vban::dev_genesis_key.pub));
	vban::send_block block (latest, vban::dev_genesis_key.pub, vban::genesis_amount - node->config.receive_minimum.number (), vban::dev_genesis_key.prv, vban::dev_genesis_key.pub, *node->work_generate_blocking (latest));
	{
		auto transaction (node->store.tx_begin_write ());
		ASSERT_EQ (vban::process_result::progress, node->ledger.process (transaction, block).code);
	}
	scoped_io_thread_name_change scoped_thread_name_io;
	vban::node_rpc_config node_rpc_config;
	vban::ipc::ipc_server ipc_server (*node, node_rpc_config);
	...
	ASSERT_TIMELY (5s, response.status != 0);
	ASSERT_EQ (200, response.status);
	ASSERT_TIMELY (10s, node->balance (vban::dev_genesis_key.pub) == vban::genesis_amount);
}
...
TEST (rpc, version)
{
	...
	ASSERT_EQ (boost::str (boost::format ("Vban %1%") % VBAN_VERSION_STRING), response1.json.get<std::string> ("node_vendor"));
	...
	auto genesis_open (node1->latest (vban::dev_genesis_key.pub));
...
TEST (rpc, work_generate)
{
	...
	auto verify_response = [node, &rpc, &system] (auto & request, auto & hash) {
		...
		uint64_t work;
		ASSERT_FALSE (vban::from_string_hex (work_text, work));
		auto result_difficulty (vban::work_difficulty (vban::work_version::work_1, hash, work));
		auto response_difficulty_text (response.json.get<std::string> ("difficulty"));
		uint64_t response_difficulty;
		ASSERT_FALSE (vban::from_string_hex (response_difficulty_text, response_difficulty));
		ASSERT_EQ (result_difficulty, response_difficulty);
		auto multiplier = response.json.get<double> ("multiplier");
		ASSERT_NEAR (vban::difficulty::to_multiplier (result_difficulty, node->default_difficulty (vban::work_version::work_1)), multiplier, 1e-6);
	};
...
TEST (rpc, work_generate_difficulty)
{
	...
	request.put ("hash", hash.to_string ());
	{
		...
		request.put ("difficulty", vban::to_string_hex (difficulty));
		test_response response (request, rpc.config.port, system.io_ctx);
		ASSERT_TIMELY (10s, response.status != 0);
		ASSERT_EQ (200, response.status);
		auto work_text (response.json.get<std::string> ("work"));
		...
		ASSERT_NEAR (vban::difficulty::to_multiplier (result_difficulty, node->default_difficulty (vban::work_version::work_1)), multiplier, 1e-10);
	...
	{
		uint64_t difficulty (node->max_work_generate_difficulty (vban::work_version::work_1) + 1);
		request.put ("difficulty", vban::to_string_hex (difficulty));
		test_response response (request, rpc.config.port, system.io_ctx);
		ASSERT_TIMELY (5s, response.status != 0);
		ASSERT_EQ (200, response.status);
		std::error_code ec (vban::error_rpc::difficulty_limit);
		ASSERT_EQ (response.json.get<std::string> ("error"), ec.message ());
	}
}
...
TEST (rpc, work_generate_multiplier)
{
	...
	request.put ("hash", hash.to_string ());
	{
		...
		request.put ("difficulty", vban::to_string_hex (0xff00000000000000));
	...
	}
	{
		...
		ASSERT_TIMELY (5s, response.status != 0);
		ASSERT_EQ (200, response.status);
		std::error_code ec (vban::error_rpc::bad_multiplier_format);
		ASSERT_EQ (response.json.get<std::string> ("error"), ec.message ());
	}
	{
		double max_multiplier (vban::difficulty::to_multiplier (node->max_work_generate_difficulty (vban::work_version::work_1), node->default_difficulty (vban::work_version::work_1)));
...
TEST (rpc, work_generate_block_high)
{
	...
	rpc.start ();
	vban::keypair key;
	vban::state_block block (key.pub, 0, vban::dev_genesis_key.pub, vban::Gxrb_ratio, 123, key.prv, key.pub, *node->work_generate_blocking (key.pub));
	vban::block_hash hash (block.root ().as_block_hash ());
	auto block_difficulty (vban::work_difficulty (vban::work_version::work_1, hash, block.block_work ()));
	boost::property_tree::ptree request;
	request.put ("action", "work_generate");
	request.put ("hash", hash.to_string ());
	request.put ("json_block", "true");
	...
	request.add_child ("block", json);
	{
		...
		ASSERT_EQ (200, response.status);
		ASSERT_EQ (1, response.json.count ("error"));
		ASSERT_EQ (std::error_code (vban::error_rpc::block_work_enough).message (), response.json.get<std::string> ("error"));
	}
}
...
TEST (rpc, work_generate_block_low)
{
	...
	rpc.start ();
	vban::keypair key;
	vban::state_block block (key.pub, 0, vban::dev_genesis_key.pub, vban::Gxrb_ratio, 123, key.prv, key.pub, 0);
	auto threshold (node->default_difficulty (block.work_version ()));
	block.block_work_set (system.work_generate_limited (block.root ().as_block_hash (), threshold, vban::difficulty::from_multiplier (node->config.max_work_generate_multiplier / 10, threshold)));
	vban::block_hash hash (block.root ().as_block_hash ());
	auto block_difficulty (block.difficulty ());
	boost::property_tree::ptree request;
	request.put ("action", "work_generate");
	request.put ("hash", hash.to_string ());
	request.put ("difficulty", vban::to_string_hex (block_difficulty + 1));
	request.put ("json_block", "false");
	std::string json;
	block.serialize_json (json);
	request.put ("block", json);
...
TEST (rpc, work_generate_block_root_mismatch)
{
	...
	request.put ("block", json);
	{
		...
		ASSERT_EQ (200, response.status);
		ASSERT_EQ (1, response.json.count ("error"));
		ASSERT_EQ (std::error_code (vban::error_rpc::block_root_mismatch).message (), response.json.get<std::string> ("error"));
	}
}
...
TEST (rpc, work_generate_block_ledger_epoch_2)
{
	...
	vban::keypair key;
	system.wallet (0)->insert_adhoc (vban::dev_genesis_key.prv);
	auto send_block (system.wallet (0)->send_action (vban::dev_genesis_key.pub, key.pub, vban::Gxrb_ratio));
	...
	vban::rpc rpc (system.io_ctx, rpc_config, ipc_rpc_processor);
	rpc.start ();
	vban::state_block block (key.pub, 0, vban::dev_genesis_key.pub, vban::Gxrb_ratio, send_block->hash (), key.prv, key.pub, 0);
	auto threshold (vban::work_threshold (block.work_version (), vban::block_details (vban::epoch::epoch_2, false, true, false)));
...
TEST (rpc, work_cancel)
{
	...
	while (!done)
	{
		system.work.generate (vban::work_version::work_1, hash1, node1.network_params.network.publish_thresholds.base, [&done] (boost::optional<uint64_t> work_a) {
			done = !work_a;
		});
		test_response response1 (request1, rpc.config.port, system.io_ctx);
		std::error_code ec;
...
TEST (rpc, work_peer_bad)
{
	...
	node2.work_generate (vban::work_version::work_1, hash1, node2.network_params.network.publish_thresholds.base, [&work] (boost::optional<uint64_t> work_a) {
		ASSERT_TRUE (work_a.is_initialized ());
		work = *work_a;
	});
	ASSERT_TIMELY (5s, vban::work_difficulty (vban::work_version::work_1, hash1, work) >= vban::work_threshold_base (vban::work_version::work_1));
}
...
TEST (rpc, work_peer_one)
{
	...
	vban::keypair key1;
	std::atomic<uint64_t> work (0);
	node2.work_generate (vban::work_version::work_1, key1.pub, node1.network_params.network.publish_thresholds.base, [&work] (boost::optional<uint64_t> work_a) {
		ASSERT_TRUE (work_a.is_initialized ());
		work = *work_a;
	});
	ASSERT_TIMELY (5s, vban::work_difficulty (vban::work_version::work_1, key1.pub, work) >= vban::work_threshold_base (vban::work_version::work_1));
}
...
TEST (rpc, work_peer_many)
{
	...
	vban::system system2;
	vban::system system3 (1);
	vban::system system4 (1);
	...
	vban::keypair key;
	vban::rpc_config config2 (vban::get_available_port (), true);
	scoped_io_thread_name_change scoped_thread_name_io;
	vban::node_rpc_config node_rpc_config;
	vban::ipc::ipc_server ipc_server2 (node2, node_rpc_config);
	vban::ipc_rpc_processor ipc_rpc_processor2 (system2.io_ctx, config2);
	vban::rpc rpc2 (system2.io_ctx, config2, ipc_rpc_processor2);
	rpc2.start ();
	vban::rpc_config config3 (vban::get_available_port (), true);
	vban::ipc::ipc_server ipc_server3 (node3, node_rpc_config);
	vban::ipc_rpc_processor ipc_rpc_processor3 (system3.io_ctx, config3);
	vban::rpc rpc3 (system3.io_ctx, config3, ipc_rpc_processor3);
	rpc3.start ();
	vban::rpc_config config4 (vban::get_available_port (), true);
	vban::ipc::ipc_server ipc_server4 (node4, node_rpc_config);
	vban::ipc_rpc_processor ipc_rpc_processor4 (system4.io_ctx, config4);
	vban::rpc rpc4 (system2.io_ctx, config4, ipc_rpc_processor4);
	rpc4.start ();
	...
	{
		vban::keypair key1;
		node1.work_generate (vban::work_version::work_1, key1.pub, node1.network_params.network.publish_thresholds.base, [&work = works[i]] (boost::optional<uint64_t> work_a) {
			work = *work_a;
		});
		while (vban::work_difficulty (vban::work_version::work_1, key1.pub, works[i]) < vban::work_threshold_base (vban::work_version::work_1))
		{
			system1.poll ();
			system2.poll ();
			system3.poll ();
...
TEST (rpc, work_version_invalid)
{
	...
	request.put ("version", "work_invalid");
	{
		...
		ASSERT_EQ (200, response.status);
		ASSERT_EQ (1, response.json.count ("error"));
		ASSERT_EQ (std::error_code (vban::error_rpc::bad_work_version).message (), response.json.get<std::string> ("error"));
	}
...
TEST (rpc, block_count_pruning)
{
	...
	auto latest (node1.latest (vban::dev_genesis_key.pub));
	auto send1 (std::make_shared<vban::send_block> (latest, vban::dev_genesis_key.pub, vban::genesis_amount - vban::Gxrb_ratio, vban::dev_genesis_key.prv, vban::dev_genesis_key.pub, *node1.work_generate_blocking (latest)));
	node1.process_active (send1);
	auto receive1 (std::make_shared<vban::receive_block> (send1->hash (), send1->hash (), vban::dev_genesis_key.prv, vban::dev_genesis_key.pub, *node1.work_generate_blocking (send1->hash ())));
...
TEST (rpc, available_supply)
{
	...
	scoped_thread_name_io.reset ();
	auto block2 (system.wallet (0)->send_action (vban::dev_genesis_key.pub, 0, 100)); // Sending to burning 0 account
	scoped_thread_name_io.renew ();
...
TEST (rpc, mrai_to_raw)
{
	...
	ASSERT_TIMELY (5s, response1.status != 0);
	ASSERT_EQ (200, response1.status);
	ASSERT_EQ (vban::Mxrb_ratio.convert_to<std::string> (), response1.json.get<std::string> ("amount"));
}
...
TEST (rpc, mrai_from_raw)
{
	...
	request1.put ("amount", vban::Mxrb_ratio.convert_to<std::string> ());
	test_response response1 (request1, rpc.config.port, system.io_ctx);
	ASSERT_TIMELY (5s, response1.status != 0);
	ASSERT_EQ (200, response1.status);
...
TEST (rpc, krai_to_raw)
{
	...
	ASSERT_TIMELY (5s, response1.status != 0);
	ASSERT_EQ (200, response1.status);
	ASSERT_EQ (vban::kxrb_ratio.convert_to<std::string> (), response1.json.get<std::string> ("amount"));
}
...
TEST (rpc, krai_from_raw)
{
	...
	request1.put ("amount", vban::kxrb_ratio.convert_to<std::string> ());
	test_response response1 (request1, rpc.config.port, system.io_ctx);
	ASSERT_TIMELY (5s, response1.status != 0);
	ASSERT_EQ (200, response1.status);
	ASSERT_EQ ("1", response1.json.get<std::string> ("amount"));
...
TEST (rpc, vban_to_raw)
{
	...
	rpc.start ();
	boost::property_tree::ptree request1;
	request1.put ("action", "vban_to_raw");
	request1.put ("amount", "1");
	test_response response1 (request1, rpc.config.port, system.io_ctx);
	ASSERT_TIMELY (5s, response1.status != 0);
	ASSERT_EQ (200, response1.status);
	ASSERT_EQ (vban::xrb_ratio.convert_to<std::string> (), response1.json.get<std::string> ("amount"));
}

TEST (rpc, vban_from_raw)
{
	vban::system system;
	auto & node1 = *add_ipc_enabled_node (system);
	scoped_io_thread_name_change scoped_thread_name_io;
	...
	rpc.start ();
	boost::property_tree::ptree request1;
	request1.put ("action", "vban_from_raw");
	request1.put ("amount", vban::xrb_ratio.convert_to<std::string> ());
	test_response response1 (request1, rpc.config.port, system.io_ctx);
	ASSERT_TIMELY (5s, response1.status != 0);
	ASSERT_EQ (200, response1.status);
	ASSERT_EQ ("1", response1.json.get<std::string> ("amount"));
...
TEST (rpc, account_representative_set_epoch_2)
{
	...
	auto target_difficulty = vban::work_threshold (vban::work_version::work_1, vban::block_details (vban::epoch::epoch_2, false, false, false));
	ASSERT_LT (node.network_params.network.publish_thresholds.entry, target_difficulty);
	auto min_difficulty = node.network_params.network.publish_thresholds.entry;
	...
	request.put ("action", "account_representative_set");
	request.put ("account", vban::dev_genesis_key.pub.to_account ());
	request.put ("representative", vban::keypair ().pub.to_account ());
...
TEST (rpc, bootstrap)
{
	...
	vban::system system1 (1);
	auto latest (system1.nodes[0]->latest (vban::dev_genesis_key.pub));
	vban::send_block send (latest, vban::genesis_account, 100, vban::dev_genesis_key.prv, vban::dev_genesis_key.pub, *system1.nodes[0]->work_generate_blocking (latest));
	{
		auto transaction (system1.nodes[0]->store.tx_begin_write ());
		ASSERT_EQ (vban::process_result::progress, system1.nodes[0]->ledger.process (transaction, send).code);
	}
	scoped_io_thread_name_change scoped_thread_name_io;
	vban::node_rpc_config node_rpc_config;
	vban::ipc::ipc_server ipc_server (*node, node_rpc_config);
	vban::rpc_config rpc_config (vban::get_available_port (), true);
	rpc_config.rpc_process.ipc_port = node->config.ipc_config.transport_tcp.port;
	vban::ipc_rpc_processor ipc_rpc_processor (system0.io_ctx, rpc_config);
	vban::rpc rpc (system0.io_ctx, rpc_config, ipc_rpc_processor);
	rpc.start ();
	boost::property_tree::ptree request;
	...
	while (system0.nodes[0]->latest (vban::genesis_account) != system1.nodes[0]->latest (vban::genesis_account))
	{
...
TEST (rpc, representatives)
{
	...
	std::vector<vban::account> representatives;
	...
	ASSERT_EQ (vban::genesis_account, representatives[0]);
}
...
TEST (rpc, wallet_change_seed)
{
	...
	vban::random_pool::generate_block (seed.bytes.data (), seed.bytes.size ());
	{
		...
		vban::raw_key seed0;
		vban::random_pool::generate_block (seed0.bytes.data (), seed0.bytes.size ());
...
TEST (rpc, wallet_frontiers)
{
	...
	ASSERT_EQ (200, response.status);
	auto & frontiers_node (response.json.get_child ("frontiers"));
	std::vector<vban::account> frontiers;
	for (auto i (frontiers_node.begin ()), n (frontiers_node.end ()); i != n; ++i)
	{
		frontiers.push_back (vban::account (i->second.get<std::string> ("")));
	}
	ASSERT_EQ (1, frontiers.size ());
	ASSERT_EQ (system0.nodes[0]->latest (vban::genesis_account), frontiers[0]);
}
...
TEST (rpc, work_validate)
{
	...
	request.put ("action", "work_validate");
	request.put ("hash", hash.to_string ());
	request.put ("work", vban::to_string_hex (work1));
	{
		test_response response (request, rpc.config.port, system.io_ctx);
		ASSERT_TIMELY (5s, response.status != 0);
		ASSERT_EQ (200, response.status);
		...
		uint64_t difficulty;
		ASSERT_FALSE (vban::from_string_hex (difficulty_text, difficulty));
		ASSERT_GE (difficulty, node1.default_difficulty (vban::work_version::work_1));
		double multiplier (response.json.get<double> ("multiplier"));
		ASSERT_NEAR (multiplier, vban::difficulty::to_multiplier (difficulty, node1.default_difficulty (vban::work_version::work_1)), 1e-6);
	}
	uint64_t work2 (0);
	request.put ("work", vban::to_string_hex (work2));
	{
		test_response response (request, rpc.config.port, system.io_ctx);
		ASSERT_TIMELY (5s, response.status != 0);
		ASSERT_EQ (200, response.status);
		...
		uint64_t difficulty;
		ASSERT_FALSE (vban::from_string_hex (difficulty_text, difficulty));
		ASSERT_GE (node1.default_difficulty (vban::work_version::work_1), difficulty);
		double multiplier (response.json.get<double> ("multiplier"));
		ASSERT_NEAR (multiplier, vban::difficulty::to_multiplier (difficulty, node1.default_difficulty (vban::work_version::work_1)), 1e-6);
	}
	auto result_difficulty (vban::work_difficulty (vban::work_version::work_1, hash, work1));
	ASSERT_GE (result_difficulty, node1.default_difficulty (vban::work_version::work_1));
	request.put ("work", vban::to_string_hex (work1));
	request.put ("difficulty", vban::to_string_hex (result_difficulty));
	{
		test_response response (request, rpc.config.port, system.io_ctx);
		ASSERT_TIMELY (5s, response.status != 0);
		ASSERT_EQ (200, response.status);
	...
	request.put ("work", vban::to_string_hex (work1));
	request.put ("difficulty", vban::to_string_hex (difficulty4));
	{
		test_response response (request, rpc.config.port, system.io_ctx);
		ASSERT_TIMELY (5s, response.status != 0);
		ASSERT_EQ (200, response.status);
		ASSERT_EQ (result_difficulty >= difficulty4, response.json.get<bool> ("valid"));
		ASSERT_EQ (result_difficulty >= node1.default_difficulty (vban::work_version::work_1), response.json.get<bool> ("valid_all"));
	...
	request.put ("work", vban::to_string_hex (work3));
	{
		test_response response (request, rpc.config.port, system.io_ctx);
		ASSERT_TIMELY (5s, response.status != 0);
		ASSERT_EQ (200, response.status);
...
TEST (rpc, work_validate_epoch_2)
{
	...
	request.put ("work", vban::to_string_hex (work));
	{
		test_response response (request, rpc.config.port, system.io_ctx);
		ASSERT_TIMELY (5s, response.status != 0);
		ASSERT_EQ (200, response.status);
		...
		ASSERT_FALSE (vban::from_string_hex (difficulty_text, difficulty));
		double multiplier (response.json.get<double> ("multiplier"));
		ASSERT_NEAR (multiplier, vban::difficulty::to_multiplier (difficulty, node->network_params.network.publish_thresholds.epoch_2), 1e-6);
	};
	...
	scoped_thread_name_io.renew ();
	{
		...
		ASSERT_FALSE (vban::from_string_hex (difficulty_text, difficulty));
		double multiplier (response.json.get<double> ("multiplier"));
		ASSERT_NEAR (multiplier, vban::difficulty::to_multiplier (difficulty, node->default_difficulty (vban::work_version::work_1)), 1e-6);
	};
}
...
TEST (rpc, republish)
{
	...
	vban::send_block send (latest, key.pub, 100, vban::dev_genesis_key.prv, vban::dev_genesis_key.pub, *node1.work_generate_blocking (latest));
	node1.process (send);
	vban::open_block open (send.hash (), key.pub, key.pub, key.prv, key.pub, *node1.work_generate_blocking (key.pub));
	...
	ASSERT_TIMELY (5s, response.status != 0);
	ASSERT_EQ (200, response.status);
	ASSERT_TIMELY (10s, system.nodes[1]->balance (vban::dev_genesis_key.pub) != vban::genesis_amount);
	auto & blocks_node (response.json.get_child ("blocks"));
	std::vector<vban::block_hash> blocks;
	for (auto i (blocks_node.begin ()), n (blocks_node.end ()); i != n; ++i)
	{
...
TEST (rpc, deterministic_key)
{
	...
	vban::account account0 (system0.wallet (0)->deterministic_insert ());
	vban::account account1 (system0.wallet (0)->deterministic_insert ());
	vban::account account2 (system0.wallet (0)->deterministic_insert ());
	scoped_io_thread_name_change scoped_thread_name_io;
	vban::node_rpc_config node_rpc_config;
	vban::ipc::ipc_server ipc_server (*node, node_rpc_config);
	vban::rpc_config rpc_config (vban::get_available_port (), true);
...
TEST (rpc, accounts_balances)
{
	...
	entry.put ("", vban::dev_genesis_key.pub.to_account ());
	...
	for (auto & balances : response.json.get_child ("balances"))
	{
		...
		ASSERT_EQ (vban::dev_genesis_key.pub.to_account (), account_text);
...
TEST (rpc, accounts_frontiers)
{
	...
	for (auto & frontiers : response.json.get_child ("frontiers"))
	{
		...
		ASSERT_EQ (node->latest (vban::genesis_account), frontier_text);
	}
}
...
TEST (rpc, accounts_pending)
{
	...
	request.put ("count", "100");
	{
		...
		for (auto & blocks : response.json.get_child ("blocks"))
		{
			...
			vban::block_hash hash1 (blocks.second.begin ()->second.get<std::string> (""));
	...
	request.put ("sorting", "true"); // Sorting test
	{
		...
		for (auto & blocks : response.json.get_child ("blocks"))
		{
			std::string account_text (blocks.first);
			ASSERT_EQ (key1.pub.to_account (), account_text);
			vban::block_hash hash1 (blocks.second.begin ()->first);
			ASSERT_EQ (block1->hash (), hash1);
...
TEST (rpc, blocks)
{
	...
	boost::property_tree::ptree entry;
	boost::property_tree::ptree peers_l;
	entry.put ("", node->latest (vban::genesis_account).to_string ());
	peers_l.push_back (std::make_pair ("", entry));
	...
	for (auto & blocks : response.json.get_child ("blocks"))
	{
		...
		ASSERT_EQ (node->latest (vban::genesis_account).to_string (), hash_text);
...
TEST (rpc, wallet_info)
{
	...
	vban::keypair key;
	system.wallet (0)->insert_adhoc (key.prv);
	auto send (system.wallet (0)->send_action (vban::dev_genesis_key.pub, key.pub, 1));
	vban::account account (system.wallet (0)->deterministic_insert ());
	{
		auto transaction (node->wallets.tx_begin_write ());
...
TEST (rpc, wallet_balances)
{
	...
	auto send (system0.wallet (0)->send_action (vban::dev_genesis_key.pub, key.pub, 1));
	scoped_thread_name_io.renew ();
...
TEST (rpc, pending_exists)
{
	...
	vban::keypair key1;
	system.wallet (0)->insert_adhoc (vban::dev_genesis_key.prv);
	auto hash0 (node->latest (vban::genesis_account));
	auto block1 (system.wallet (0)->send_action (vban::dev_genesis_key.pub, key1.pub, 100));
	scoped_io_thread_name_change scoped_thread_name_io;
	node->scheduler.flush ();
	ASSERT_TIMELY (5s, !node->active.active (*block1));
	...
	node->store.pending_exists (node->store.tx_begin_read (), vban::pending_key (vban::dev_genesis_key.pub, block1->hash ()));
...
TEST (rpc, wallet_pending)
{
	...
	auto iterations (0);
	auto block1 (system0.wallet (0)->send_action (vban::dev_genesis_key.pub, key1.pub, 100));
	scoped_io_thread_name_change scoped_thread_name_io;
	node->scheduler.flush ();
	...
	for (auto & pending : response.json.get_child ("blocks"))
	{
		std::string account_text (pending.first);
		ASSERT_EQ (key1.pub.to_account (), account_text);
		vban::block_hash hash1 (pending.second.begin ()->second.get<std::string> (""));
		ASSERT_EQ (block1->hash (), hash1);
	}
	request.put ("threshold", "100"); // Threshold test
	test_response response0 (request, rpc.config.port, system0.io_ctx);
...
TEST (rpc, work_get)
{
	...
	auto node = add_ipc_enabled_node (system);
	system.wallet (0)->insert_adhoc (vban::dev_genesis_key.prv);
	system.wallet (0)->work_cache_blocking (vban::dev_genesis_key.pub, node->latest (vban::dev_genesis_key.pub));
	scoped_io_thread_name_change scoped_thread_name_io;
	vban::node_rpc_config node_rpc_config;
	vban::ipc::ipc_server ipc_server (*node, node_rpc_config);
	vban::rpc_config rpc_config (vban::get_available_port (), true);
	...
	uint64_t work (1);
	auto transaction (node->wallets.tx_begin_read ());
	node->wallets.items.begin ()->second->store.work_get (transaction, vban::genesis_account, work);
	ASSERT_EQ (vban::to_string_hex (work), work_text);
}
...
TEST (rpc, work_set)
{
	...
	request.put ("wallet", node->wallets.items.begin ()->first.to_string ());
	request.put ("account", vban::dev_genesis_key.pub.to_account ());
	request.put ("work", vban::to_string_hex (work0));
	test_response response (request, rpc.config.port, system.io_ctx);
	ASSERT_TIMELY (5s, response.status != 0);
	ASSERT_EQ (200, response.status);
	std::string success (response.json.get<std::string> ("success"));
	...
	uint64_t work1 (1);
	auto transaction (node->wallets.tx_begin_read ());
	node->wallets.items.begin ()->second->store.work_get (transaction, vban::genesis_account, work1);
...
TEST (rpc, wallet_republish)
{
	...
	vban::genesis genesis;
	vban::keypair key;
	while (key.pub < vban::dev_genesis_key.pub)
	{
		vban::keypair key1;
		key.pub = key1.pub;
		key.prv = key1.prv;
...
TEST (rpc, delegators)
{
	...
	vban::send_block send (latest, key.pub, 100, vban::dev_genesis_key.prv, vban::dev_genesis_key.pub, *node1.work_generate_blocking (latest));
	node1.process (send);
	vban::open_block open (send.hash (), vban::dev_genesis_key.pub, key.pub, key.prv, key.pub, *node1.work_generate_blocking (key.pub));
	ASSERT_EQ (vban::process_result::progress, node1.process (open).code);
	scoped_io_thread_name_change scoped_thread_name_io;
	vban::node_rpc_config node_rpc_config;
	vban::ipc::ipc_server ipc_server (node1, node_rpc_config);
	...
	ASSERT_EQ ("100", delegators.get<std::string> (vban::dev_genesis_key.pub.to_account ()));
...
TEST (rpc, account_info)
{
	...
	request.put ("account", vban::account ().to_account ());
	...
	// Test for a non existing account
	{
		...
		ASSERT_EQ (error.get (), std::error_code (vban::error_common::account_not_found).message ());
	}
	...
	vban::send_block send (latest, key.pub, 100, vban::dev_genesis_key.prv, vban::dev_genesis_key.pub, *node1.work_generate_blocking (latest));
	node1.process (send);
	auto time (vban::seconds_since_epoch ());
	{
		auto transaction = node1.store.tx_begin_write ();
		node1.store.confirmation_height_put (transaction, vban::dev_genesis_key.pub, { 1, genesis.hash () });
	}
	scoped_thread_name_io.renew ();
	...
	request.put ("representative", "1");
	{
		...
		ASSERT_EQ (vban::dev_genesis_key.pub.to_account (), representative2);
	}
	...
	vban::keypair key1;
	{
		latest = node1.latest (vban::dev_genesis_key.pub);
		vban::send_block send1 (latest, key1.pub, 50, vban::dev_genesis_key.prv, vban::dev_genesis_key.pub, *node1.work_generate_blocking (latest));
		node1.process (send1);
		vban::send_block send2 (send1.hash (), key1.pub, 25, vban::dev_genesis_key.prv, vban::dev_genesis_key.pub, *node1.work_generate_blocking (send1.hash ()));
		node1.process (send2);

		vban::state_block state_change (vban::dev_genesis_key.pub, send2.hash (), key1.pub, 25, 0, vban::dev_genesis_key.prv, vban::dev_genesis_key.pub, *node1.work_generate_blocking (send2.hash ()));
		node1.process (state_change);

		vban::open_block open (send1.hash (), vban::dev_genesis_key.pub, key1.pub, key1.prv, key1.pub, *node1.work_generate_blocking (key1.pub));
		node1.process (open);
	}
	...
	request.put ("include_confirmed", true);
	{
		...
		ASSERT_EQ (confirmed_representative, vban::dev_genesis_key.pub.to_account ());

		auto confirmed_frontier (response.json.get<std::string> ("confirmed_frontier"));
		ASSERT_EQ (vban::genesis_hash.to_string (), confirmed_frontier);
...
TEST (rpc, json_block_input)
{
	...
	vban::keypair key;
	system.wallet (0)->insert_adhoc (key.prv);
	vban::state_block send (vban::genesis_account, node1.latest (vban::dev_genesis_key.pub), vban::genesis_account, vban::genesis_amount - vban::Gxrb_ratio, key.pub, vban::dev_genesis_key.prv, vban::dev_genesis_key.pub, 0);
	scoped_io_thread_name_change scoped_thread_name_io;
	vban::node_rpc_config node_rpc_config;
	vban::ipc::ipc_server ipc_server (node1, node_rpc_config);
	vban::rpc_config rpc_config (vban::get_available_port (), true);
	...
	vban::state_block block (json_error, response.json.get_child ("block"));
	ASSERT_FALSE (json_error);

	ASSERT_FALSE (vban::validate_message (key.pub, send.hash (), block.block_signature ()));
...
TEST (rpc, json_block_output)
{
	...
	bool json_error{ false };
	vban::send_block send_from_json (json_error, response.json.get_child ("contents"));
	ASSERT_FALSE (json_error);
}
...
TEST (rpc, blocks_info)
{
	...
	auto check_blocks = [node] (test_response & response) {
		for (auto & blocks : response.json.get_child ("blocks"))
		{
			...
			ASSERT_EQ (vban::genesis_amount.convert_to<std::string> (), amount_text);
			...
			ASSERT_EQ (vban::genesis_amount.convert_to<std::string> (), balance_text);
	...
	std::string random_hash = vban::block_hash ().to_string ();
	...
	request.add_child ("hashes", hashes);
	{
		...
		ASSERT_TIMELY (5s, response.status != 0);
		ASSERT_EQ (200, response.status);
		ASSERT_EQ (std::error_code (vban::error_blocks::not_found).message (), response.json.get<std::string> ("error"));
	}
...
TEST (rpc, blocks_info_subtype)
{
	...
	system.wallet (0)->insert_adhoc (vban::dev_genesis_key.prv);
	system.wallet (0)->insert_adhoc (key.prv);
	auto send (system.wallet (0)->send_action (vban::dev_genesis_key.pub, vban::dev_genesis_key.pub, vban::Gxrb_ratio));
	ASSERT_NE (nullptr, send);
	auto receive (system.wallet (0)->receive_action (send->hash (), key.pub, vban::Gxrb_ratio, send->link ().as_account ()));
	ASSERT_NE (nullptr, receive);
	auto change (system.wallet (0)->change_action (vban::dev_genesis_key.pub, key.pub));
	ASSERT_NE (nullptr, change);
	scoped_io_thread_name_change scoped_thread_name_io;
	vban::node_rpc_config node_rpc_config;
	vban::ipc::ipc_server ipc_server (node1, node_rpc_config);
...
{
	vban::system system;
	vban::node_config node_config0 (vban::get_available_port (), system.logging);
	node_config0.receive_minimum = vban::genesis_amount; // Prevent auto-receive & receive1 block conflicts
	auto & node0 = *system.add_node (node_config0);
	vban::node_config node_config1 (vban::get_available_port (), system.logging);
	...
	ASSERT_FALSE (amount.is_initialized ()); // Cannot calculate amount
	bool json_error{ false };
	vban::receive_block receive_from_json (json_error, response2.json.get_child ("contents"));
	ASSERT_FALSE (json_error);
...
TEST (rpc, ledger)
{
	...
	auto & node1 (*system.nodes[0]);
	auto latest (node1.latest (vban::dev_genesis_key.pub));
	auto genesis_balance (vban::genesis_amount);
	auto send_amount (genesis_balance - 100);
	genesis_balance -= send_amount;
	vban::send_block send (latest, key.pub, genesis_balance, vban::dev_genesis_key.prv, vban::dev_genesis_key.pub, *node1.work_generate_blocking (latest));
	node1.process (send);
	vban::open_block open (send.hash (), vban::dev_genesis_key.pub, key.pub, key.prv, key.pub, *node1.work_generate_blocking (key.pub));
	ASSERT_EQ (vban::process_result::progress, node1.process (open).code);
	auto time (vban::seconds_since_epoch ());
	...
	request.put ("representative", true);
	{
		...
		for (auto & account : response.json.get_child ("accounts"))
		{
			...
			ASSERT_EQ (vban::dev_genesis_key.pub.to_account (), representative.get ());
		}
	}
	// Test threshold
	request.put ("count", 2);
	...
	vban::send_block send2 (send.hash (), key.pub, genesis_balance, vban::dev_genesis_key.prv, vban::dev_genesis_key.pub, *node1.work_generate_blocking (send.hash ()));
	scoped_thread_name_io.reset ();
	node1.process (send2);
	scoped_thread_name_io.renew ();
...
TEST (rpc, block_create)
{
	...
	vban::send_block send (latest, key.pub, 100, vban::dev_genesis_key.prv, vban::dev_genesis_key.pub, send_work);
	auto open_work = *node1.work_generate_blocking (key.pub);
	vban::open_block open (send.hash (), vban::dev_genesis_key.pub, key.pub, key.prv, key.pub, open_work);
	scoped_io_thread_name_change scoped_thread_name_io;
	vban::node_rpc_config node_rpc_config;
	vban::ipc::ipc_server ipc_server (node1, node_rpc_config);
	vban::rpc_config rpc_config (vban::get_available_port (), true);
	...
	request.put ("work", vban::to_string_hex (send_work));
	test_response response (request, rpc.config.port, system.io_ctx);
	ASSERT_TIMELY (5s, response.status != 0);
	ASSERT_EQ (200, response.status);
	std::string send_hash (response.json.get<std::string> ("hash"));
	...
	ASSERT_EQ (vban::to_string_hex (send.difficulty ()), send_difficulty);
	auto send_text (response.json.get<std::string> ("block"));
	boost::property_tree::ptree block_l;
	std::stringstream block_stream (send_text);
	boost::property_tree::read_json (block_stream, block_l);
	auto send_block (vban::deserialize_block_json (block_l));
	...
	request1.put ("representative", vban::dev_genesis_key.pub.to_account ());
	request1.put ("source", send.hash ().to_string ());
	request1.put ("work", vban::to_string_hex (open_work));
	test_response response1 (request1, rpc.config.port, system.io_ctx);
	ASSERT_TIMELY (5s, response1.status != 0);
	ASSERT_EQ (200, response1.status);
	...
	auto open_block (vban::deserialize_block_json (block_l));
	...
	vban::change_block change (open.hash (), key.pub, key.prv, key.pub, change_work);
	request1.put ("type", "change");
	request1.put ("work", vban::to_string_hex (change_work));
	...
	auto change_block (vban::deserialize_block_json (block_l));
	ASSERT_EQ (change.hash (), change_block->hash ());
	scoped_thread_name_io.reset ();
	ASSERT_EQ (vban::process_result::progress, node1.process (change).code);
	vban::send_block send2 (send.hash (), key.pub, 0, vban::dev_genesis_key.prv, vban::dev_genesis_key.pub, *node1.work_generate_blocking (send.hash ()));
	...
	request2.put ("work", vban::to_string_hex (*node1.work_generate_blocking (change.hash ())));
	...
	auto receive_block (vban::deserialize_block_json (block_l));
...
TEST (rpc, block_create_state)
{
	...
	request.put ("representative", vban::dev_genesis_key.pub.to_account ());
	request.put ("balance", (vban::genesis_amount - vban::Gxrb_ratio).convert_to<std::string> ());
	request.put ("link", key.pub.to_account ());
	request.put ("work", vban::to_string_hex (*node->work_generate_blocking (genesis.hash ())));
	scoped_io_thread_name_change scoped_thread_name_io;
	vban::node_rpc_config node_rpc_config;
	vban::ipc::ipc_server ipc_server (*node, node_rpc_config);
	vban::rpc_config rpc_config (vban::get_available_port (), true);
	...
	boost::property_tree::ptree block_l;
	boost::property_tree::read_json (block_stream, block_l);
	auto state_block (vban::deserialize_block_json (block_l));
	ASSERT_NE (nullptr, state_block);
	ASSERT_EQ (vban::block_type::state, state_block->type ());
	ASSERT_EQ (state_hash, state_block->hash ().to_string ());
	scoped_thread_name_io.reset ();
	auto process_result (node->process (*state_block));
	ASSERT_EQ (vban::process_result::progress, process_result.code);
}
...
TEST (rpc, block_create_state_open)
{
	...
	request.put ("representative", vban::dev_genesis_key.pub.to_account ());
	request.put ("balance", vban::Gxrb_ratio.convert_to<std::string> ());
	request.put ("link", send_block->hash ().to_string ());
	request.put ("work", vban::to_string_hex (*node->work_generate_blocking (key.pub)));
	scoped_io_thread_name_change scoped_thread_name_io;
	vban::node_rpc_config node_rpc_config;
	vban::ipc::ipc_server ipc_server (*node, node_rpc_config);
	vban::rpc_config rpc_config (vban::get_available_port (), true);
	...
	ASSERT_GT (difficulty, vban::work_threshold (state_block->work_version (), vban::block_details (vban::epoch::epoch_0, false, true, false)));
	ASSERT_TRUE (node->latest (key.pub).is_zero ());
	scoped_thread_name_io.reset ();
	auto process_result (node->process (*state_block));
	ASSERT_EQ (vban::process_result::progress, process_result.code);
	ASSERT_EQ (state_block->sideband ().details.epoch, vban::epoch::epoch_0);
...
TEST (rpc, block_create_state_request_work)
{
	...
	std::unique_ptr<vban::state_block> epoch2;
	{
		vban::system system (1);
		system.upgrade_genesis_epoch (*system.nodes.front (), vban::epoch::epoch_1);
		epoch2 = system.upgrade_genesis_epoch (*system.nodes.front (), vban::epoch::epoch_2);
	}
	...
	for (auto previous : previous_test_input)
	{
		...
		boost::property_tree::read_json (block_stream, block_l);
		auto block (vban::deserialize_block_json (block_l));
		ASSERT_NE (nullptr, block);
		ASSERT_GE (block->difficulty (), node->default_difficulty (vban::work_version::work_1));
	}
}
...
TEST (rpc, block_create_open_epoch_v2)
{
	...
	ASSERT_EQ (state_hash, state_block->hash ().to_string ());
	auto difficulty (state_block->difficulty ());
	ASSERT_GT (difficulty, vban::work_threshold (state_block->work_version (), vban::block_details (vban::epoch::epoch_2, false, true, false)));
	ASSERT_TRUE (node->latest (key.pub).is_zero ());
	scoped_thread_name_io.reset ();
	auto process_result (node->process (*state_block));
	ASSERT_EQ (vban::process_result::progress, process_result.code);
	ASSERT_EQ (state_block->sideband ().details.epoch, vban::epoch::epoch_2);
	ASSERT_TRUE (state_block->sideband ().details.is_receive);
	ASSERT_FALSE (node->latest (key.pub).is_zero ());
}
...
TEST (rpc, block_create_receive_epoch_v2)
{
	...
	auto send_block (system.wallet (0)->send_action (vban::dev_genesis_key.pub, key.pub, vban::Gxrb_ratio));
	ASSERT_NE (nullptr, send_block);
	vban::state_block open (key.pub, 0, vban::dev_genesis_key.pub, vban::Gxrb_ratio, send_block->hash (), key.prv, key.pub, *node->work_generate_blocking (key.pub));
	ASSERT_EQ (vban::process_result::progress, node->process (open).code);
	ASSERT_NE (nullptr, system.upgrade_genesis_epoch (*node, vban::epoch::epoch_2));
	auto send_block_2 (system.wallet (0)->send_action (vban::dev_genesis_key.pub, key.pub, vban::Gxrb_ratio));
	boost::property_tree::ptree request;
	request.put ("action", "block_create");
	request.put ("type", "state");
	request.put ("key", key.prv.to_string ());
	...
	request.put ("representative", vban::dev_genesis_key.pub.to_account ());
	request.put ("balance", (2 * vban::Gxrb_ratio).convert_to<std::string> ());
...
TEST (rpc, block_create_send_epoch_v2)
{
	...
	request.put ("link", vban::dev_genesis_key.pub.to_string ());
	scoped_io_thread_name_change scoped_thread_name_io;
	vban::node_rpc_config node_rpc_config;
	vban::ipc::ipc_server ipc_server (*node, node_rpc_config);
	vban::rpc_config rpc_config (vban::get_available_port (), true);
	...
	ASSERT_EQ (state_hash, state_block->hash ().to_string ());
	auto difficulty (state_block->difficulty ());
	ASSERT_GT (difficulty, vban::work_threshold (state_block->work_version (), vban::block_details (vban::epoch::epoch_2, true, false, false)));
	scoped_thread_name_io.reset ();
	auto process_result (node->process (*state_block));
	ASSERT_EQ (vban::process_result::progress, process_result.code);
	ASSERT_EQ (state_block->sideband ().details.epoch, vban::epoch::epoch_2);
...
TEST (rpc, wallet_create_fail)
{
	...
	test_response response (request, rpc.config.port, system.io_ctx);
	ASSERT_TIMELY (5s, response.status != 0);
	ASSERT_EQ (std::error_code (vban::error_common::wallet_lmdb_max_dbs).message (), response.json.get<std::string> ("error"));
}
...
TEST (rpc, wallet_add_watch)
{
	...
	std::string success (response.json.get<std::string> ("success"));
	ASSERT_TRUE (success.empty ());
	ASSERT_TRUE (system.wallet (0)->exists (vban::dev_genesis_key.pub));

	// Make sure using special wallet key as pubkey fails
	vban::public_key bad_key (1);
	...
	std::error_code ec (vban::error_common::bad_public_key);
...
TEST (rpc, online_reps)
{
	...
	ASSERT_EQ (node2->online_reps.online (), vban::genesis_amount - vban::Gxrb_ratio);
	vban::node_rpc_config node_rpc_config;
	vban::ipc::ipc_server ipc_server (*system.nodes[1], node_rpc_config);
	vban::rpc_config rpc_config (vban::get_available_port (), true);
	...
	ASSERT_EQ (vban::dev_genesis_key.pub.to_account (), item->second.get<std::string> (""));
	...
	ASSERT_EQ (vban::dev_genesis_key.pub.to_account (), item2->first);
	auto weight2 (item2->second.get<std::string> ("weight"));
	ASSERT_EQ (node2->weight (vban::dev_genesis_key.pub).convert_to<std::string> (), weight2);
	//Test accounts filter
	scoped_thread_name_io.reset ();
	auto new_rep (system.wallet (1)->deterministic_insert ());
	auto send (system.wallet (0)->send_action (vban::dev_genesis_key.pub, new_rep, node1->config.receive_minimum.number ()));
	scoped_thread_name_io.renew ();
	ASSERT_NE (nullptr, send);
	...
	scoped_thread_name_io.reset ();
	auto change (system.wallet (0)->change_action (vban::dev_genesis_key.pub, new_rep));
	scoped_thread_name_io.renew ();
	ASSERT_NE (nullptr, change);
...
TEST (rpc, confirmation_height_currently_processing)
{
	...
	auto previous_genesis_chain_hash = node->latest (vban::dev_genesis_key.pub);
	{
		auto transaction = node->store.tx_begin_write ();
		vban::keypair key1;
		vban::send_block send (previous_genesis_chain_hash, key1.pub, vban::genesis_amount - vban::Gxrb_ratio - 1, vban::dev_genesis_key.prv, vban::dev_genesis_key.pub, *system.work.generate (previous_genesis_chain_hash));
	...
	std::shared_ptr<vban::block> frontier;
	{
		auto transaction = node->store.tx_begin_read ();
	...
	// Make the same request, it should now return an error
	{
		...
		ASSERT_TIMELY (10s, response.status != 0);
		ASSERT_EQ (200, response.status);
		std::error_code ec (vban::error_rpc::confirmation_height_not_processing);
		ASSERT_EQ (response.json.get<std::string> ("error"), ec.message ());
	}
}
...
TEST (rpc, confirmation_history)
{
	...
	auto block (system.wallet (0)->send_action (vban::dev_genesis_key.pub, key.pub, vban::Gxrb_ratio));
	scoped_io_thread_name_change scoped_thread_name_io;
	...
	vban::amount tally_num;
	tally_num.decode_dec (tally);
	debug_assert (tally_num == vban::genesis_amount || tally_num == (vban::genesis_amount - vban::Gxrb_ratio));
	system.stop ();
}
...
TEST (rpc, confirmation_history_hash)
{
	...
	system.wallet (0)->insert_adhoc (vban::dev_genesis_key.prv);
	ASSERT_TRUE (node->active.list_recently_cemented ().empty ());
	auto send1 (system.wallet (0)->send_action (vban::dev_genesis_key.pub, key.pub, vban::Gxrb_ratio));
	auto send2 (system.wallet (0)->send_action (vban::dev_genesis_key.pub, key.pub, vban::Gxrb_ratio));
	auto send3 (system.wallet (0)->send_action (vban::dev_genesis_key.pub, key.pub, vban::Gxrb_ratio));
	scoped_io_thread_name_change scoped_thread_name_io;
	...
	vban::amount tally_num;
	tally_num.decode_dec (tally);
	debug_assert (tally_num == vban::genesis_amount || tally_num == (vban::genesis_amount - vban::Gxrb_ratio) || tally_num == (vban::genesis_amount - 2 * vban::Gxrb_ratio) || tally_num == (vban::genesis_amount - 3 * vban::Gxrb_ratio));
	system.stop ();
}
...
TEST (rpc, block_confirm_confirmed)
{
	...
	config.callback_port = vban::get_available_port ();
	...
	ASSERT_EQ (0, node->stats.count (vban::stat::type::error, vban::stat::detail::http_callback, vban::stat::dir::out));
	scoped_io_thread_name_change scoped_thread_name_io;
	vban::node_rpc_config node_rpc_config;
	vban::ipc::ipc_server ipc_server (*node, node_rpc_config);
	vban::rpc_config rpc_config (vban::get_available_port (), true);
	...
	ASSERT_EQ (vban::genesis_hash, confirmed.begin ()->winner->hash ());
	// Check callback
	ASSERT_TIMELY (10s, node->stats.count (vban::stat::type::error, vban::stat::detail::http_callback, vban::stat::dir::out) != 0);
	// Callback result is error because callback target port isn't listening
	ASSERT_EQ (1, node->stats.count (vban::stat::type::error, vban::stat::detail::http_callback, vban::stat::dir::out));
	node->stop ();
}
...
TEST (rpc, stats_clear)
{
	...
	vban::rpc rpc (system.io_ctx, rpc_config, ipc_rpc_processor);
	rpc.start ();
	node->stats.inc (vban::stat::type::ledger, vban::stat::dir::in);
	ASSERT_EQ (1, node->stats.count (vban::stat::type::ledger, vban::stat::dir::in));
	boost::property_tree::ptree request;
	...
	std::string success (response.json.get<std::string> ("success"));
	ASSERT_TRUE (success.empty ());
	ASSERT_EQ (0, node->stats.count (vban::stat::type::ledger, vban::stat::dir::in));
...
TEST (rpc, unchecked)
{
	...
	vban::rpc rpc (system.io_ctx, rpc_config, ipc_rpc_processor);
	rpc.start ();
	auto open (std::make_shared<vban::state_block> (key.pub, 0, key.pub, 1, key.pub, key.prv, key.pub, *system.work.generate (key.pub)));
	auto open2 (std::make_shared<vban::state_block> (key.pub, 0, key.pub, 2, key.pub, key.prv, key.pub, *system.work.generate (key.pub)));
...
TEST (rpc, unchecked_get)
{
	...
	request.put ("hash", open->hash ().to_string ());
	{
		...
		ASSERT_LE (timestamp, vban::seconds_since_epoch ());
	}
	request.put ("json_block", true);
	{
		test_response response (request, rpc.config.port, system.io_ctx);
...
TEST (rpc, unopened)
{
	...
	auto node = add_ipc_enabled_node (system);
	system.wallet (0)->insert_adhoc (vban::dev_genesis_key.prv);
	vban::account account1 (1), account2 (account1.number () + 1);
	auto genesis (node->latest (vban::dev_genesis_key.pub));
	ASSERT_FALSE (genesis.is_zero ());
	auto send (system.wallet (0)->send_action (vban::dev_genesis_key.pub, account1, 1));
	ASSERT_NE (nullptr, send);
	auto send2 (system.wallet (0)->send_action (vban::dev_genesis_key.pub, account2, 10));
	ASSERT_NE (nullptr, send2);
	scoped_io_thread_name_change scoped_thread_name_io;
	vban::node_rpc_config node_rpc_config;
	vban::ipc::ipc_server ipc_server (*node, node_rpc_config);
	...
	}
	{
		...
		boost::property_tree::ptree request;
		request.put ("action", "unopened");
		request.put ("account", vban::account (account2.number () + 1).to_account ());
		test_response response (request, rpc.config.port, system.io_ctx);
		ASSERT_TIMELY (5s, response.status != 0);
		ASSERT_EQ (200, response.status);
		auto & accounts (response.json.get_child ("accounts"));
...
TEST (rpc, unopened_burn)
{
	...
	auto genesis (node->latest (vban::dev_genesis_key.pub));
	ASSERT_FALSE (genesis.is_zero ());
	auto send (system.wallet (0)->send_action (vban::dev_genesis_key.pub, vban::burn_account, 1));
	ASSERT_NE (nullptr, send);
	scoped_io_thread_name_change scoped_thread_name_io;
	vban::node_rpc_config node_rpc_config;
	vban::ipc::ipc_server ipc_server (*node, node_rpc_config);
...
TEST (rpc, wallet_history)
{
	...
	auto node = add_ipc_enabled_node (system, node_config);
	system.wallet (0)->insert_adhoc (vban::dev_genesis_key.prv);
	auto timestamp1 (vban::seconds_since_epoch ());
	auto send (system.wallet (0)->send_action (vban::dev_genesis_key.pub, vban::dev_genesis_key.pub, node->config.receive_minimum.number ()));
	ASSERT_NE (nullptr, send);
	auto timestamp2 (vban::seconds_since_epoch ());
	auto receive (system.wallet (0)->receive_action (send->hash (), vban::dev_genesis_key.pub, node->config.receive_minimum.number (), send->link ().as_account ()));
	ASSERT_NE (nullptr, receive);
	vban::keypair key;
	auto timestamp3 (vban::seconds_since_epoch ());
	auto send2 (system.wallet (0)->send_action (vban::dev_genesis_key.pub, key.pub, node->config.receive_minimum.number ()));
	ASSERT_NE (nullptr, send2);
	system.deadline_set (10s);
	scoped_io_thread_name_change scoped_thread_name_io;
	vban::node_rpc_config node_rpc_config;
	...
	ASSERT_EQ (vban::dev_genesis_key.pub.to_account (), std::get<4> (history_l[0]));
	...
	ASSERT_EQ (vban::dev_genesis_key.pub.to_account (), std::get<4> (history_l[1]));
	...
	ASSERT_EQ (vban::dev_genesis_key.pub.to_account (), std::get<4> (history_l[2]));
	ASSERT_EQ (std::to_string (timestamp1), std::get<5> (history_l[2]));
	// Genesis block
	ASSERT_EQ ("receive", std::get<0> (history_l[3]));
	ASSERT_EQ (vban::dev_genesis_key.pub.to_account (), std::get<1> (history_l[3]));
	ASSERT_EQ (vban::genesis_amount.convert_to<std::string> (), std::get<2> (history_l[3]));
	ASSERT_EQ (vban::genesis_hash.to_string (), std::get<3> (history_l[3]));
	ASSERT_EQ (vban::dev_genesis_key.pub.to_account (), std::get<4> (history_l[3]));
}
...
TEST (rpc, sign_hash)
{
	...
	ASSERT_TIMELY (10s, response.status != 0);
	ASSERT_EQ (200, response.status);
	std::error_code ec (vban::error_rpc::sign_hash_disabled);
	ASSERT_EQ (response.json.get<std::string> ("error"), ec.message ());
	...
	ASSERT_FALSE (vban::validate_message (key.pub, send.hash (), signature));
}
...
TEST (rpc, memory_stats)
{
	...
	auto vote (std::make_shared<vban::vote> (key.pub, key.prv, 0, hashes));
...
TEST (rpc, block_confirmed)
{
	...
	ASSERT_TIMELY (5s, response.status != 0);
	ASSERT_EQ (200, response.status);
	ASSERT_EQ (std::error_code (vban::error_blocks::invalid_block_hash).message (), response.json.get<std::string> ("error"));

	request.put ("hash", "0");
	test_response response1 (request, rpc.config.port, system.io_ctx);
	ASSERT_TIMELY (5s, response1.status != 0);
	ASSERT_EQ (200, response1.status);
	ASSERT_EQ (std::error_code (vban::error_blocks::not_found).message (), response1.json.get<std::string> ("error"));
	...
	{
		auto transaction = node->store.tx_begin_write ();
		vban::block_hash latest (node->latest (vban::dev_genesis_key.pub));
		vban::send_block send1 (latest, key.pub, 300, vban::dev_genesis_key.prv, vban::dev_genesis_key.pub, *system.work.generate (latest));
		ASSERT_EQ (vban::process_result::progress, node->ledger.process (transaction, send1).code);

		vban::open_block open1 (send1.hash (), vban::genesis_account, key.pub, key.prv, key.pub, *system.work.generate (key.pub));
	...
	auto send = std::make_shared<vban::send_block> (latest, key.pub, 10, vban::dev_genesis_key.prv, vban::dev_genesis_key.pub, *system.work.generate (latest));
...
TEST (rpc, database_txn_tracker)
{
	...
	// First try when database tracking is disabled
	{
		...
		request.put ("action", "database_txn_tracker");
		{
			...
			ASSERT_TIMELY (5s, response.status != 0);
			ASSERT_EQ (200, response.status);
			std::error_code ec (vban::error_common::tracking_not_enabled);
			ASSERT_EQ (response.json.get<std::string> ("error"), ec.message ());
		}
	}
	...
	auto check_not_correct_amount = [&system, &request, &rpc_port = rpc.config.port] () {
		...
		ASSERT_TIMELY (5s, response.status != 0);
		ASSERT_EQ (200, response.status);
		std::error_code ec (vban::error_common::invalid_amount);
		ASSERT_EQ (response.json.get<std::string> ("error"), ec.message ());
	};
	...
	std::thread thread ([&store = node->store, &keep_txn_alive_promise, &txn_created_promise] () {
		...
		vban::thread_role::set (vban::thread_role::name::rpc_process_container);
	...
	auto thread_name = vban::thread_role::get_string (vban::thread_role::name::rpc_process_container);
...
TEST (rpc, active_difficulty)
{
	vban::system system;
	auto node = add_ipc_enabled_node (system);
	ASSERT_EQ (node->default_difficulty (vban::work_version::work_1), node->network_params.network.publish_thresholds.epoch_2);
	scoped_io_thread_name_change scoped_thread_name_io;
	vban::node_rpc_config node_rpc_config;
	vban::ipc::ipc_server ipc_server (*node, node_rpc_config);
	vban::rpc_config rpc_config (vban::get_available_port (), true);
	...
	auto expected_multiplier{ 1.0 };
	{
		...
		ASSERT_FALSE (vban::from_string_hex (network_minimum_text, network_minimum));
		ASSERT_EQ (node->default_difficulty (vban::work_version::work_1), network_minimum);
		auto network_receive_minimum_text (response.json.get<std::string> ("network_receive_minimum"));
		uint64_t network_receive_minimum;
		ASSERT_FALSE (vban::from_string_hex (network_receive_minimum_text, network_receive_minimum));
		ASSERT_EQ (node->default_receive_difficulty (vban::work_version::work_1), network_receive_minimum);
		auto multiplier (response.json.get<double> ("multiplier"));
		...
		ASSERT_FALSE (vban::from_string_hex (network_current_text, network_current));
		ASSERT_EQ (vban::difficulty::from_multiplier (expected_multiplier, node->default_difficulty (vban::work_version::work_1)), network_current);
		auto network_receive_current_text (response.json.get<std::string> ("network_receive_current"));
		uint64_t network_receive_current;
		ASSERT_FALSE (vban::from_string_hex (network_receive_current_text, network_receive_current));
		auto network_receive_current_multiplier (vban::difficulty::to_multiplier (network_receive_current, network_receive_minimum));
		auto network_receive_current_normalized_multiplier (vban::normalized_multiplier (network_receive_current_multiplier, network_receive_minimum));
...
TEST (rpc, simultaneous_calls)
{
	...
	auto node = add_ipc_enabled_node (system);
	scoped_io_thread_name_change scoped_thread_name_io;
	vban::thread_runner runner (system.io_ctx, node->config.io_threads);
	vban::node_rpc_config node_rpc_config;
	vban::ipc::ipc_server ipc_server (*node, node_rpc_config);
	vban::rpc_config rpc_config (vban::get_available_port (), true);
	rpc_config.rpc_process.ipc_port = node->config.ipc_config.transport_tcp.port;
...
TEST (rpc, in_process)
{
	...
	vban::node_rpc_config node_rpc_config;
	vban::ipc::ipc_server ipc_server (*node, node_rpc_config);
	vban::inprocess_rpc_handler inprocess_rpc_handler (*node, ipc_server, node_rpc_config);
	vban::rpc rpc (system.io_ctx, rpc_config, inprocess_rpc_handler);
	rpc.start ();
	boost::property_tree::ptree request;
	request.put ("action", "account_balance");
	request.put ("account", vban::dev_genesis_key.pub.to_account ());
...
TEST (rpc, deprecated_account_format)
{
	...
	std::string account_text (vban::dev_genesis_key.pub.to_account ());
	account_text[4] = '-';
	...
	ASSERT_EQ (200, response2.status);
	std::string frontier (response.json.get<std::string> ("frontier"));
	ASSERT_EQ (vban::genesis_hash.to_string (), frontier);
...
TEST (rpc, epoch_upgrade)
{
	...
	auto send1 (std::make_shared<vban::state_block> (vban::dev_genesis_key.pub, vban::genesis_hash, vban::dev_genesis_key.pub, vban::genesis_amount - 1, key1.pub, vban::dev_genesis_key.prv, vban::dev_genesis_key.pub, *system.work.generate (vban::genesis_hash))); // to opened account
	ASSERT_EQ (vban::process_result::progress, node->process (*send1).code);
	auto send2 (std::make_shared<vban::state_block> (vban::dev_genesis_key.pub, send1->hash (), vban::dev_genesis_key.pub, vban::genesis_amount - 2, key2.pub, vban::dev_genesis_key.prv, vban::dev_genesis_key.pub, *system.work.generate (send1->hash ()))); // to unopened account (pending)
	ASSERT_EQ (vban::process_result::progress, node->process (*send2).code);
	auto send3 (std::make_shared<vban::state_block> (vban::dev_genesis_key.pub, send2->hash (), vban::dev_genesis_key.pub, vban::genesis_amount - 3, 0, vban::dev_genesis_key.prv, vban::dev_genesis_key.pub, *system.work.generate (send2->hash ()))); // to burn (0)
	ASSERT_EQ (vban::process_result::progress, node->process (*send3).code);
	vban::account max_account (std::numeric_limits<vban::uint256_t>::max ());
	auto send4 (std::make_shared<vban::state_block> (vban::dev_genesis_key.pub, send3->hash (), vban::dev_genesis_key.pub, vban::genesis_amount - 4, max_account, vban::dev_genesis_key.prv, vban::dev_genesis_key.pub, *system.work.generate (send3->hash ()))); // to max account
	ASSERT_EQ (vban::process_result::progress, node->process (*send4).code);
	auto open (std::make_shared<vban::state_block> (key1.pub, 0, key1.pub, 1, send1->hash (), key1.prv, key1.pub, *system.work.generate (key1.pub)));
	ASSERT_EQ (vban::process_result::progress, node->process (*open).code);
	// Check accounts epochs
	{
		...
		{
			vban::account_info info (i->second);
			ASSERT_EQ (info.epoch (), vban::epoch::epoch_0);
		}
	}
	vban::node_rpc_config node_rpc_config;
	vban::ipc::ipc_server ipc_server (*node, node_rpc_config);
	...
	// Check upgrade
	{
		...
		ASSERT_TRUE (node->store.account_exists (transaction, std::numeric_limits<vban::uint256_t>::max ()));
		ASSERT_FALSE (node->store.account_exists (transaction, 0));
	}

	// Epoch 2 upgrade
	auto genesis_latest (node->latest (vban::dev_genesis_key.pub));
	auto send5 (std::make_shared<vban::state_block> (vban::dev_genesis_key.pub, genesis_latest, vban::dev_genesis_key.pub, vban::genesis_amount - 5, 0, vban::dev_genesis_key.prv, vban::dev_genesis_key.pub, *system.work.generate (genesis_latest))); // to burn (0)
	ASSERT_EQ (vban::process_result::progress, node->process (*send5).code);
	auto send6 (std::make_shared<vban::state_block> (vban::dev_genesis_key.pub, send5->hash (), vban::dev_genesis_key.pub, vban::genesis_amount - 6, key1.pub, vban::dev_genesis_key.prv, vban::dev_genesis_key.pub, *system.work.generate (send5->hash ()))); // to key1 (again)
	ASSERT_EQ (vban::process_result::progress, node->process (*send6).code);
	auto key1_latest (node->latest (key1.pub));
	auto send7 (std::make_shared<vban::state_block> (key1.pub, key1_latest, key1.pub, 0, key3.pub, key1.prv, key1.pub, *system.work.generate (key1_latest))); // to key3
	ASSERT_EQ (vban::process_result::progress, node->process (*send7).code);
	{
		// Check pending entry
		auto transaction (node->store.tx_begin_read ());
		vban::pending_info info;
		ASSERT_FALSE (node->store.pending_get (transaction, vban::pending_key (key3.pub, send7->hash ()), info));
	...
	// Check upgrade
	{
		...
		{
			vban::account_info info (i->second);
			ASSERT_EQ (info.epoch (), vban::epoch::epoch_2);
		}
		ASSERT_TRUE (node->store.account_exists (transaction, key1.pub));
		ASSERT_TRUE (node->store.account_exists (transaction, key2.pub));
...
TEST (rpc, account_lazy_start)
{
	...
	auto send1 (std::make_shared<vban::state_block> (vban::dev_genesis_key.pub, vban::genesis_hash, vban::dev_genesis_key.pub, vban::genesis_amount - vban::Gxrb_ratio, key.pub, vban::dev_genesis_key.prv, vban::dev_genesis_key.pub, *system.work.generate (vban::genesis_hash)));
	...
	vban::node_rpc_config node_rpc_config;
	vban::ipc::ipc_server ipc_server (*node2, node_rpc_config);
	vban::rpc_config rpc_config (vban::get_available_port (), true);
...
TEST (rpc, receive)
{
	...
	auto send1 (wallet->send_action (vban::dev_genesis_key.pub, key1.pub, node.config.receive_minimum.number (), *node.work_generate_blocking (vban::genesis_hash)));
	ASSERT_TIMELY (5s, node.balance (vban::dev_genesis_key.pub) != vban::genesis_amount);
	ASSERT_TIMELY (10s, !node.store.account_exists (node.store.tx_begin_read (), key1.pub));
	// Send below minimum receive amount
	auto send2 (wallet->send_action (vban::dev_genesis_key.pub, key1.pub, node.config.receive_minimum.number () - 1, *node.work_generate_blocking (send1->hash ())));
	scoped_io_thread_name_change scoped_thread_name_io;
	vban::node_rpc_config node_rpc_config;
	vban::ipc::ipc_server ipc_server (node, node_rpc_config);
	vban::rpc_config rpc_config (vban::get_available_port (), true);
	...
	// Trying to receive the same block should fail with unreceivable
	{
		...
		ASSERT_TIMELY (5s, response.status != 0);
		ASSERT_EQ (200, response.status);
		ASSERT_EQ (std::error_code (vban::error_process::unreceivable).message (), response.json.get<std::string> ("error"));
	}
	// Trying to receive a non-existing block should fail
	request.put ("block", vban::block_hash (send2->hash ().number () + 1).to_string ());
	{
		test_response response (request, rpc.config.port, system.io_ctx);
		ASSERT_TIMELY (5s, response.status != 0);
		ASSERT_EQ (200, response.status);
...
TEST (rpc, receive_unopened)
{
	...
	vban::keypair key1;
	auto send1 (wallet->send_action (vban::dev_genesis_key.pub, key1.pub, node.config.receive_minimum.number () - 1, *node.work_generate_blocking (vban::genesis_hash)));
	ASSERT_TIMELY (5s, !node.balance (vban::dev_genesis_key.pub) != vban::genesis_amount);
	...
	request.put ("block", send1->hash ().to_string ());
	{
		...
		ASSERT_EQ (info.representative, vban::dev_genesis_key.pub);
	}
	scoped_thread_name_io.reset ();

	// Test receiving for an unopened with a different wallet representative
	vban::keypair key2;
	auto prev_amount (node.balance (vban::dev_genesis_key.pub));
	auto send2 (wallet->send_action (vban::dev_genesis_key.pub, key2.pub, node.config.receive_minimum.number () - 1, *node.work_generate_blocking (send1->hash ())));
	ASSERT_TIMELY (5s, !node.balance (vban::dev_genesis_key.pub) != prev_amount);
...
TEST (rpc, receive_work_disabled)
{
	...
	auto send1 (wallet->send_action (vban::dev_genesis_key.pub, key1.pub, node.config.receive_minimum.number () - 1, *worker_node.work_generate_blocking (genesis.hash ()), false));
...
TEST (rpc, receive_pruned)
{
	...
	auto send1 (wallet1->send_action (vban::dev_genesis_key.pub, key1.pub, node2.config.receive_minimum.number (), *node2.work_generate_blocking (vban::genesis_hash)));
	ASSERT_TIMELY (5s, node2.balance (vban::dev_genesis_key.pub) != vban::genesis_amount);
	ASSERT_TIMELY (10s, !node2.store.account_exists (node2.store.tx_begin_read (), key1.pub));
	// Send below minimum receive amount
	auto send2 (wallet1->send_action (vban::dev_genesis_key.pub, key1.pub, node2.config.receive_minimum.number () - 1, *node2.work_generate_blocking (send1->hash ())));
	// Extra send frontier
	auto send3 (wallet1->send_action (vban::dev_genesis_key.pub, key1.pub, node2.config.receive_minimum.number (), *node2.work_generate_blocking (send1->hash ())));
	// Pruning
...
TEST (rpc, telemetry_single)
{
...

	{
		...
		ASSERT_TIMELY (10s, response.status != 0);
		ASSERT_EQ (200, response.status);
		ASSERT_EQ (std::error_code (vban::error_rpc::requires_port_and_address).message (), response.json.get<std::string> ("error"));
	}
...

	{
		...
		ASSERT_TIMELY (10s, response.status != 0);
		ASSERT_EQ (200, response.status);
		ASSERT_EQ (std::error_code (vban::error_common::invalid_ip_address).message (), response.json.get<std::string> ("error"));
	}
	...
	request.put ("port", "invalid port");
	{
		...
		ASSERT_TIMELY (10s, response.status != 0);
		ASSERT_EQ (200, response.status);
		ASSERT_EQ (std::error_code (vban::error_common::invalid_port).message (), response.json.get<std::string> ("error"));
	}
	...
	request.put ("port", node->network.endpoint ().port ());
	{
		...
		vban::jsonconfig config (response.json);
		...
		vban::compare_default_telemetry_response_data (telemetry_data, node->network_params, node->config.bandwidth_limit, node->default_difficulty (vban::work_version::work_1), node->node_id);
	}
}
...
TEST (rpc, telemetry_all)
{
	...
	std::atomic<bool> done{ false };
	auto node = system.nodes.front ();
	node1.telemetry->get_metrics_single_peer_async (node1.network.find_channel (node->network.endpoint ()), [&done] (vban::telemetry_data_response const & telemetry_data_response_a) {
	...
	request.put ("action", "telemetry");
	{
		...
		ASSERT_FALSE (telemetry_data.deserialize_json (config, should_ignore_identification_metrics));
		vban::compare_default_telemetry_response_data_excluding_signature (telemetry_data, node->network_params, node->config.bandwidth_limit, node->default_difficulty (vban::work_version::work_1));
	...
	vban::jsonconfig config (metrics);
	vban::telemetry_data data;
	auto const should_ignore_identification_metrics = false;
	ASSERT_FALSE (data.deserialize_json (config, should_ignore_identification_metrics));
	vban::compare_default_telemetry_response_data (data, node->network_params, node->config.bandwidth_limit, node->default_difficulty (vban::work_version::work_1), node->node_id);
...
TEST (rpc, telemetry_self)
{
	...
	node1.network.udp_channels.insert (vban::endpoint (boost::asio::ip::make_address_v6 ("::1"), vban::get_available_port ()), 0);
	...
	auto const should_ignore_identification_metrics = false;
	{
		...
		vban::jsonconfig config (response.json);
		ASSERT_FALSE (data.deserialize_json (config, should_ignore_identification_metrics));
		vban::compare_default_telemetry_response_data (data, node1.network_params, node1.config.bandwidth_limit, node1.default_difficulty (vban::work_version::work_1), node1.node_id);
	}
	...
	request.put ("port", "0");
	{
		...
		ASSERT_TIMELY (10s, response.status != 0);
		ASSERT_EQ (200, response.status);
		ASSERT_EQ (std::error_code (vban::error_rpc::peer_not_found).message (), response.json.get<std::string> ("error"));
	}
}
...
TEST (rpc, confirmation_active)
{
	...
	vban::genesis genesis;
	auto send1 (std::make_shared<vban::send_block> (genesis.hash (), vban::public_key (), vban::genesis_amount - 100, vban::dev_genesis_key.prv, vban::dev_genesis_key.pub, *system.work.generate (genesis.hash ())));
	auto send2 (std::make_shared<vban::send_block> (send1->hash (), vban::public_key (), vban::genesis_amount - 200, vban::dev_genesis_key.prv, vban::dev_genesis_key.pub, *system.work.generate (send1->hash ())));
	node1.process_active (send1);
...
TEST (rpc, confirmation_info)
{
	...
	vban::genesis genesis;
	auto send (std::make_shared<vban::send_block> (genesis.hash (), vban::public_key (), vban::genesis_amount - 100, vban::dev_genesis_key.prv, vban::dev_genesis_key.pub, *system.work.generate (genesis.hash ())));
