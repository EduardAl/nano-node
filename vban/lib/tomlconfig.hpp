...
#include <vban/lib/configbase.hpp>
...
class tomlconfig : public vban::configbase
{
public:
	tomlconfig ();
	tomlconfig (std::shared_ptr<cpptoml::table> const & tree_a, std::shared_ptr<vban::error> const & error_a = nullptr);
	void doc (std::string const & key, std::string const & doc);
	vban::error & read (boost::filesystem::path const & path_a);
	vban::error & read (std::istream & stream_overrides, boost::filesystem::path const & path_a);
	vban::error & read (std::istream & stream_a);
	vban::error & read (std::istream & stream_first_a, std::istream & stream_second_a);
	...
	tomlconfig & put_child (std::string const & key_a, vban::tomlconfig & conf_a);
	tomlconfig & replace_child (std::string const & key_a, vban::tomlconfig & conf_a);
	...
	tomlconfig & array_entries_required (std::string const & key, std::function<void (T)> callback)
	{
		...
		{
			conditionally_set_error<T> (vban::error_config::missing_value, false, key);
		}
		return *this;
	}
	 ...
	 * @return May return vban::error_config::invalid_value
	 */
	template <typename T>
	...
	/**
	 * Get required value.
	 * @note May set vban::error_config::missing_value if \p key is missing, vban::error_config::invalid_value if value is invalid.
	 */
	template <typename T>
...
protected:
	template <typename T, typename = std::enable_if_t<vban::is_lexical_castable<T>::value>>
	tomlconfig & get_config (bool optional, std::string const & key, T & target, T default_value = T ())
	{
		try
		{
			if (tree->contains_qualified (key))
			{
				...
				{
					conditionally_set_error<T> (vban::error_config::invalid_value, optional, key);
				}
			}
			else if (!optional)
			{
				conditionally_set_error<T> (vban::error_config::missing_value, optional, key);
			}
			else
			{
