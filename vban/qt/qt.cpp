...
bool vban_qt::eventloop_processor::event (QEvent * event_a)
{
	debug_assert (dynamic_cast<vban_qt::eventloop_event *> (event_a) != nullptr);
	static_cast<vban_qt::eventloop_event *> (event_a)->action ();
	return true;
}

vban_qt::eventloop_event::eventloop_event (std::function<void ()> const & action_a) :
	QEvent (QEvent::Type::User),
	action (action_a)
{
}

vban_qt::self_pane::self_pane (vban_qt::wallet & wallet_a, vban::account const & account_a) :
	window (new QWidget),
	layout (new QVBoxLayout),
	self_layout (new QHBoxLayout),
	self_window (new QWidget),
	your_account_label (new QLabel ("Your Vban account:")),
	...
	wallet (wallet_a)
{
	...
	version = new QLabel (boost::str (boost::format ("%1% %2% network") % VBAN_VERSION_STRING % network).c_str ());
...
void vban_qt::self_pane::set_balance_text (std::pair<vban::uint256_t, vban::uint256_t> balance_a)
{
...
vban_qt::accounts::accounts (vban_qt::wallet & wallet_a) :
...
void vban_qt::accounts::refresh_wallet_balance ()
{
	auto transaction (this->wallet.wallet_m->wallets.tx_begin_read ());
	...
	{
		vban::public_key const & key (i->first);
...
vban_qt::import::import (vban_qt::wallet & wallet_a) :
	window (new QWidget),
	layout (new QVBoxLayout),
...
vban_qt::history::history (vban::ledger & ledger_a, vban::account const & account_a, vban_qt::wallet & wallet_a) :
	window (new QWidget),
	layout (new QVBoxLayout),
	model (new QStandardItemModel),
	view (new QTableView),
...
namespace
{
class short_text_visitor : public vban::block_visitor
{
public:
	short_text_visitor (vban::transaction const & transaction_a, vban::ledger & ledger_a) :
...
vban_qt::block_viewer::block_viewer (vban_qt::wallet & wallet_a) :
	window (new QWidget),
	layout (new QVBoxLayout),
...
void vban_qt::block_viewer::rebroadcast_action (vban::block_hash const & hash_a)
{
	auto done (true);
...
vban_qt::account_viewer::account_viewer (vban_qt::wallet & wallet_a) :
	window (new QWidget),
	layout (new QVBoxLayout),
	...
	wallet (wallet_a)
{
	...
	QObject::connect (refresh, &QPushButton::released, [this] () {
		...
		if (!account.decode_account (account_line->text ().toStdString ()))
		{
			...
			auto final_text (std::string ("Balance (VBAN): ") + wallet.format_balance (balance.first));
...
vban_qt::stats_viewer::stats_viewer (vban_qt::wallet & wallet_a) :
	window (new QWidget),
	layout (new QVBoxLayout),
	refresh (new QPushButton ("Refresh")),
...
void vban_qt::stats_viewer::refresh_stats ()
{
	model->removeRows (0, model->rowCount ());
...
vban_qt::status::status (vban_qt::wallet & wallet_a) :
	wallet (wallet_a)
{
	wallet.status->setToolTip ("Wallet status, block count (blocks downloaded)");
	active.insert (vban_qt::status_types::nominal);
	set_text ();
}

void vban_qt::status::erase (vban_qt::status_types status_a)
{
	debug_assert (status_a != vban_qt::status_types::nominal);
	auto erased (active.erase (status_a));
	(void)erased;
	set_text ();
}

void vban_qt::status::insert (vban_qt::status_types status_a)
{
	debug_assert (status_a != vban_qt::status_types::nominal);
...
std::string vban_qt::status::text ()
{
	...
	switch (*active.begin ())
	{
		case vban_qt::status_types::disconnected:
			result = "Status: Disconnected";
			break;
		case vban_qt::status_types::working:
			result = "Status: Generating proof of work";
			break;
		case vban_qt::status_types::synchronizing:
			result = "Status: Synchronizing";
			break;
		case vban_qt::status_types::locked:
			result = "Status: Wallet locked";
			break;
		case vban_qt::status_types::vulnerable:
			result = "Status: Wallet password empty";
			break;
		case vban_qt::status_types::active:
			result = "Status: Wallet active";
			break;
		case vban_qt::status_types::nominal:
...
std::string vban_qt::status::color ()
{
	debug_assert (!active.empty ());
	std::string result;
	switch (*active.begin ())
...
vban_qt::wallet::wallet (QApplication & application_a, vban_qt::eventloop_processor & processor_a, vban::node & node_a, std::shared_ptr<vban::wallet> const & wallet_a, vban::account & account_a) :
	rendering_ratio (vban::Mxrb_ratio),
	node (node_a),
	wallet_m (wallet_a),
	account (account_a),
...
void vban_qt::wallet::ongoing_refresh ()
{
	std::weak_ptr<vban_qt::wallet> wallet_w (shared_from_this ());
...
void vban_qt::wallet::start ()
{
	ongoing_refresh ();
	std::weak_ptr<vban_qt::wallet> this_w (shared_from_this ());
					...
					if (actual <= balance)
					{
						...
						if (this_l->wallet_m->store.valid_password (transaction))
						{
							...
							this_l->node.background ([this_w, account_l, actual] () {
								if (auto this_l = this_w.lock ())
								{
									this_l->wallet_m->send_async (this_l->account, account_l, actual, [this_w] (std::shared_ptr<vban::block> const & block_a) {
										if (auto this_l = this_w.lock ())
										{
	...
	node.observers.blocks.add ([this_w] (vban::election_status const & status_a, std::vector<vban::vote_with_weight_info> const & votes_a, vban::account const & account_a, vban::uint256_t const & amount_a, bool) {
		if (auto this_l = this_w.lock ())
		{
	...
	node.observers.account_balance.add ([this_w] (vban::account const & account_a, bool is_pending) {
		if (auto this_l = this_w.lock ())
		{
		...
		if (auto this_l = this_w.lock ())
		{
			this_l->application.postEvent (&this_l->processor, new eventloop_event ([this_w, active_a] () {
				if (auto this_l = this_w.lock ())
				{
					if (active_a)
					{
						this_l->active_status.insert (vban_qt::status_types::active);
					}
					else
					{
						this_l->active_status.erase (vban_qt::status_types::active);
					}
				}
			}));
		}
	});
	node.observers.endpoint.add ([this_w] (std::shared_ptr<vban::transport::channel> const &) {
		if (auto this_l = this_w.lock ())
		{
			this_l->application.postEvent (&this_l->processor, new eventloop_event ([this_w] () {
				if (auto this_l = this_w.lock ())
		...
		if (auto this_l = this_w.lock ())
		{
			this_l->application.postEvent (&this_l->processor, new eventloop_event ([this_w, active_a] () {
				if (auto this_l = this_w.lock ())
				{
					if (active_a)
					{
						this_l->active_status.insert (vban_qt::status_types::synchronizing);
					}
					else
					{
						this_l->active_status.erase (vban_qt::status_types::synchronizing);
		...
		if (auto this_l = this_w.lock ())
		{
			this_l->application.postEvent (&this_l->processor, new eventloop_event ([this_w, working] () {
				if (auto this_l = this_w.lock ())
				{
					if (working)
					{
						this_l->active_status.insert (vban_qt::status_types::working);
					}
					else
					{
						this_l->active_status.erase (vban_qt::status_types::working);
					}
				}
			}));
		}
...
void vban_qt::wallet::update_connected ()
{
	if (node.network.empty ())
	{
		active_status.insert (vban_qt::status_types::disconnected);
	}
	else
	{
		active_status.erase (vban_qt::status_types::disconnected);
	}
}

void vban_qt::wallet::empty_password ()
{
...
void vban_qt::wallet::change_rendering_ratio (vban::uint256_t const & rendering_ratio_a)
{
...
std::string vban_qt::wallet::format_balance (vban::uint256_t const & balance) const
{
	auto balance_str = vban::amount (balance).format_balance (rendering_ratio, 3, false);
	auto unit = std::string ("VBAN");
	if (rendering_ratio == vban::kxrb_ratio)
	{
		unit = std::string ("kvban");
	}
	else if (rendering_ratio == vban::xrb_ratio)
	{
		unit = std::string ("vban");
	}
	else if (rendering_ratio == vban::raw_ratio)
	{
		unit = std::string ("raw");
	}
...
void vban_qt::wallet::push_main_stack (QWidget * widget_a)
{
	main_stack->addWidget (widget_a);
	main_stack->setCurrentIndex (main_stack->count () - 1);
}

void vban_qt::wallet::pop_main_stack ()
{
	main_stack->removeWidget (main_stack->currentWidget ());
}

vban_qt::settings::settings (vban_qt::wallet & wallet_a) :
	window (new QWidget),
	layout (new QVBoxLayout),
	password (new QLineEdit),
...
void vban_qt::settings::refresh_representative ()
{
...
void vban_qt::settings::activate ()
{
	this->wallet.push_main_stack (window);
}

void vban_qt::settings::update_locked (bool invalid, bool vulnerable)
{
	if (invalid)
	{
		this->wallet.active_status.insert (vban_qt::status_types::locked);
	}
	else
	{
		this->wallet.active_status.erase (vban_qt::status_types::locked);
	}
	if (vulnerable)
	{
		this->wallet.active_status.insert (vban_qt::status_types::vulnerable);
	}
	else
	{
		this->wallet.active_status.erase (vban_qt::status_types::vulnerable);
	}
}

vban_qt::advanced_actions::advanced_actions (vban_qt::wallet & wallet_a) :
	window (new QWidget),
	layout (new QVBoxLayout),
	...
	mvban_unit (new QRadioButton ("Mvban")),
	kvban_unit (new QRadioButton ("kvban")),
	vban_unit (new QRadioButton ("vban")),
	...
	wallet (wallet_a)
{
	ratio_group->addButton (mvban_unit);
	ratio_group->addButton (kvban_unit);
	ratio_group->addButton (vban_unit);
	ratio_group->addButton (raw_unit);
	ratio_group->setId (mvban_unit, 0);
	ratio_group->setId (kvban_unit, 1);
	ratio_group->setId (vban_unit, 2);
	ratio_group->setId (raw_unit, 3);
	scale_layout->addWidget (scale_label);
	scale_layout->addWidget (mvban_unit);
	scale_layout->addWidget (kvban_unit);
	scale_layout->addWidget (vban_unit);
	...
	QObject::connect (mvban_unit, &QRadioButton::toggled, [this] () {
		if (mvban_unit->isChecked ())
		{
			QSettings ().setValue (saved_ratio_key, ratio_group->id (mvban_unit));
			this->wallet.change_rendering_ratio (vban::Mxrb_ratio);
		}
	});
	QObject::connect (kvban_unit, &QRadioButton::toggled, [this] () {
		if (kvban_unit->isChecked ())
		{
			QSettings ().setValue (saved_ratio_key, ratio_group->id (kvban_unit));
			this->wallet.change_rendering_ratio (vban::kxrb_ratio);
		}
	});
	QObject::connect (vban_unit, &QRadioButton::toggled, [this] () {
		if (vban_unit->isChecked ())
		{
			QSettings ().setValue (saved_ratio_key, ratio_group->id (vban_unit));
			this->wallet.change_rendering_ratio (vban::xrb_ratio);
		}
	});
	QObject::connect (raw_unit, &QRadioButton::toggled, [this] () {
		if (raw_unit->isChecked ())
		{
			...
			this->wallet.change_rendering_ratio (vban::raw_ratio);
		}
	});
	auto selected_ratio_id (QSettings ().value (saved_ratio_key, ratio_group->id (mvban_unit)).toInt ());
	...
	QObject::connect (peers_bootstrap, &QPushButton::released, [this] () {
		...
		auto error (vban::parse_endpoint (bootstrap_line->text ().toStdString (), endpoint));
		if (!error)
		{
...
void vban_qt::advanced_actions::refresh_peers ()
{
...
void vban_qt::advanced_actions::refresh_ledger ()
{
	...
	for (auto i (wallet.node.ledger.store.accounts_begin (transaction)), j (wallet.node.ledger.store.accounts_end ()); i != j; ++i)
	{
		...
		std::string balance;
		vban::amount (info.balance.number () / wallet.rendering_ratio).encode_dec (balance);
...
void vban_qt::advanced_actions::refresh_stats ()
{
	wallet.stats_viewer.refresh_stats ();
}

vban_qt::block_entry::block_entry (vban_qt::wallet & wallet_a) :
	window (new QWidget),
	layout (new QVBoxLayout),
	block (new QPlainTextEdit),
	status (new QLabel),
	...
	wallet (wallet_a)
{
	...
	QObject::connect (process, &QPushButton::released, [this] () {
		...
		try
		{
			...
			auto block_l (vban::deserialize_block_json (tree));
			if (block_l != nullptr)
			{
				show_label_ok (*status);
				this->status->setText ("");
				if (!vban::work_validate_entry (*block_l))
				{
...
vban_qt::block_creation::block_creation (vban_qt::wallet & wallet_a) :
	window (new QWidget),
	layout (new QVBoxLayout),
...
void vban_qt::block_creation::deactivate_all ()
{
	account_label->hide ();
	account->hide ();
	source_label->hide ();
...
void vban_qt::block_creation::activate_send ()
{
	account_label->show ();
	account->show ();
	amount_label->show ();
...
void vban_qt::block_creation::activate_receive ()
{
	source_label->show ();
	source->show ();
}

void vban_qt::block_creation::activate_open ()
{
	source_label->show ();
	source->show ();
...
void vban_qt::block_creation::activate_change ()
{
	account_label->show ();
	account->show ();
	representative_label->show ();
			...
			if (!error)
			{
				...
				if (!wallet.wallet_m->store.fetch (transaction, account_l, key))
				{
					...
					if (amount_l.number () <= balance)
					{
						...
						(void)error;
						debug_assert (!error);
						vban::state_block send (account_l, info.head, info.representative, balance - amount_l.number (), destination_l, key, account_l, 0);
						vban::block_details details;
						details.is_send = true;
						details.epoch = info.epoch ();
						auto const required_difficulty{ vban::work_threshold (send.work_version (), details) };
...
void vban_qt::block_creation::create_receive ()
{
	vban::block_hash source_l;
	...
	if (!error)
	{
		...
		if (block_l != nullptr)
		{
			...
			{
				vban::pending_key pending_key (destination, source_l);
				...
				if (!wallet.node.store.pending_get (block_transaction, pending_key, pending))
				{
					...
					if (!error)
					{
						...
						if (!error)
						{
							vban::state_block receive (pending_key.account, info.head, info.representative, info.balance.number () + pending.amount.number (), source_l, key, pending_key.account, 0);
							vban::block_details details;
							details.is_receive = true;
							details.epoch = std::max (info.epoch (), pending.epoch);
							auto required_difficulty{ vban::work_threshold (receive.work_version (), details) };
...
void vban_qt::block_creation::create_change ()
{
	vban::account account_l;
	auto error (account_l.decode_account (account->text ().toStdString ()));
	if (!error)
	{
		...
		if (!error)
		{
			...
			if (!error)
			{
				...
				if (!error)
				{
					vban::state_block change (account_l, info.head, representative_l, info.balance, 0, key, account_l, 0);
					vban::block_details details;
					details.epoch = info.epoch ();
					auto const required_difficulty{ vban::work_threshold (change.work_version (), details) };
...
void vban_qt::block_creation::create_open ()
{
	vban::block_hash source_l;
	auto error (source_l.decode_hex (source->text ().toStdString ()));
	if (!error)
				...
				if (!destination.is_zero ())
				{
					...
					if (!wallet.node.store.pending_get (block_transaction, pending_key, pending))
					{
						...
						if (error)
						{
							...
							if (!error)
							{
								vban::state_block open (pending_key.account, 0, representative_l, pending.amount, source_l, key, pending_key.account, 0);
								vban::block_details details;
								details.is_receive = true;
								details.epoch = pending.epoch;
								auto const required_difficulty{ vban::work_threshold (open.work_version (), details) };
