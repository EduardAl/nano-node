...
vban::node_config::node_config () :
	node_config (0, vban::logging ())
{
}

vban::node_config::node_config (uint16_t peering_port_a, vban::logging const & logging_a) :
	...
	external_address (boost::asio::ip::address_v6{}.to_string ())
{
	...
	switch (network_params.network.network ())
	{
		...
		case vban::vban_networks::vban_beta_network:
		{
			...
			vban::account offline_representative;
			release_assert (!offline_representative.decode_account ("vban_1defau1t9off1ine9rep99999999999999999999999999999999wgmuzxxy"));
		...
		case vban::vban_networks::vban_live_network:
			...
			preconfigured_representatives.emplace_back ("21D5F05BCD38AC08FF4D9D45A7975ED500725DFC71AFCAADFABF7C6F0AABFC5D");
...
vban::error vban::node_config::serialize_toml (vban::tomlconfig & toml) const
{
	...
	vban::tomlconfig experimental_l;
...
vban::error vban::node_config::deserialize_toml (vban::tomlconfig & toml)
{
	try
	{
		...
		vban::network_constants network;
		...
		if (frontiers_confirmation == vban::frontiers_confirmation_mode::invalid)
		{
...
vban::error vban::node_config::serialize_json (vban::jsonconfig & json) const
{
...
bool vban::node_config::upgrade_json (unsigned version_a, vban::jsonconfig & json)
{
	json.put ("version", json_version ());
	switch (version_a)
	{
...
vban::error vban::node_config::deserialize_json (bool & upgraded_a, vban::jsonconfig & json)
{
	try
	{
...
std::string vban::node_config::serialize_frontiers_confirmation (vban::frontiers_confirmation_mode mode_a) const
{
	switch (mode_a)
	{
		case vban::frontiers_confirmation_mode::always:
			return "always";
		case vban::frontiers_confirmation_mode::automatic:
			return "auto";
		case vban::frontiers_confirmation_mode::disabled:
			return "disabled";
		default:
			return "auto";
	}
...
vban::frontiers_confirmation_mode vban::node_config::deserialize_frontiers_confirmation (std::string const & string_a)
{
	if (string_a == "always")
	{
		return vban::frontiers_confirmation_mode::always;
	}
	else if (string_a == "auto")
	{
		return vban::frontiers_confirmation_mode::automatic;
	}
	else if (string_a == "disabled")
	{
		return vban::frontiers_confirmation_mode::disabled;
	}
	else
	{
		return vban::frontiers_confirmation_mode::invalid;
	}
}

void vban::node_config::deserialize_address (std::string const & entry_a, std::vector<std::pair<std::string, uint16_t>> & container_a) const
{
...
vban::account vban::node_config::random_representative () const
{
	debug_assert (!preconfigured_representatives.empty ());
	size_t index (vban::random_pool::generate_word32 (0, static_cast<CryptoPP::word32> (preconfigured_representatives.size () - 1)));
