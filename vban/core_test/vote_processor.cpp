...
TEST (vote_processor, codes)
{
	...
	vban::keypair key;
	auto vote (std::make_shared<vban::vote> (key.pub, key.prv, 1, std::vector<vban::block_hash>{ genesis.open->hash () }));
	auto vote_invalid = std::make_shared<vban::vote> (*vote);
	vote_invalid->signature.bytes[0] ^= 1;
	auto channel (std::make_shared<vban::transport::channel_loopback> (node));

	// Invalid signature
	ASSERT_EQ (vban::vote_code::invalid, node.vote_processor.vote_blocking (vote_invalid, channel, false));

	// Hint of pre-validation
	ASSERT_NE (vban::vote_code::invalid, node.vote_processor.vote_blocking (vote_invalid, channel, true));

	// No ongoing election
	ASSERT_EQ (vban::vote_code::indeterminate, node.vote_processor.vote_blocking (vote, channel));
	...
	ASSERT_EQ (vban::vote_code::vote, node.vote_processor.vote_blocking (vote, channel));

	// Processing the same vote is a replay
	ASSERT_EQ (vban::vote_code::replay, node.vote_processor.vote_blocking (vote, channel));

	// Invalid takes precedence
	ASSERT_EQ (vban::vote_code::invalid, node.vote_processor.vote_blocking (vote_invalid, channel));

	// A higher timestamp is not a replay
	++vote->timestamp;
	ASSERT_EQ (vban::vote_code::invalid, node.vote_processor.vote_blocking (vote, channel));
	vote->signature = vban::sign_message (key.prv, key.pub, vote->hash ());
	ASSERT_EQ (vban::vote_code::vote, node.vote_processor.vote_blocking (vote, channel));
...
TEST (vote_processor, flush)
{
	...
	auto & node (*system.nodes[0]);
	vban::genesis genesis;
	auto vote (std::make_shared<vban::vote> (vban::dev_genesis_key.pub, vban::dev_genesis_key.prv, 1, std::vector<vban::block_hash>{ genesis.open->hash () }));
	auto channel (std::make_shared<vban::transport::channel_loopback> (node));
	for (unsigned i = 0; i < 2000; ++i)
	{
		auto new_vote (std::make_shared<vban::vote> (*vote));
...
TEST (vote_processor, overflow)
{
	...
	ASSERT_EQ (not_processed, node.stats.count (vban::stat::type::vote, vban::stat::detail::vote_overflow));
}
...
TEST (vote_processor, weights)
{
	...
	system.wallet (0)->send_sync (vban::dev_genesis_key.pub, key0.pub, level0);
	system.wallet (0)->send_sync (vban::dev_genesis_key.pub, key1.pub, level1);
	system.wallet (0)->send_sync (vban::dev_genesis_key.pub, key2.pub, level2);
	...
	ASSERT_NE (node.vote_processor.representatives_1.end (), node.vote_processor.representatives_1.find (vban::dev_genesis_key.pub));
	ASSERT_NE (node.vote_processor.representatives_2.end (), node.vote_processor.representatives_2.find (vban::dev_genesis_key.pub));
	ASSERT_NE (node.vote_processor.representatives_3.end (), node.vote_processor.representatives_3.find (vban::dev_genesis_key.pub));
}
}
...
TEST (vote_processor, no_broadcast_local)
{
	...
	vban::keypair key;
	std::shared_ptr<vban::block> send = builder.state ()
	...
	ASSERT_EQ (2 * node.config.vote_minimum.number (), node.weight (vban::dev_genesis_key.pub));
	// Insert account in wallet
	system.wallet (0)->insert_adhoc (vban::dev_genesis_key.prv);
	node.wallets.compute_reps ();
	ASSERT_TRUE (node.wallets.reps ().exists (vban::dev_genesis_key.pub));
	ASSERT_FALSE (node.wallets.reps ().have_half_rep ());
	// Process a vote
	auto vote = std::make_shared<vban::vote> (vban::dev_genesis_key.pub, vban::dev_genesis_key.prv, vban::milliseconds_since_epoch (), std::vector<vban::block_hash>{ send->hash () });
	ASSERT_EQ (vban::vote_code::vote, node.active.vote (vote));
	// Make sure the vote was processed
	auto election (node.active.election (send->qualified_root ()));
	ASSERT_NE (nullptr, election);
	auto votes (election->votes ());
	auto existing (votes.find (vban::dev_genesis_key.pub));
	...
	ASSERT_EQ (1, node.stats.count (vban::stat::type::message, vban::stat::detail::publish, vban::stat::dir::out));

	// Repeat test with no representative
	// Erase account from the wallet
	system.wallet (0)->store.erase (node.wallets.tx_begin_write (), vban::dev_genesis_key.pub);
	node.wallets.compute_reps ();
	ASSERT_FALSE (node.wallets.reps ().exists (vban::dev_genesis_key.pub));
	...
	std::shared_ptr<vban::block> send2 = builder.state ()
										 ...
										 .build (ec);
	ASSERT_FALSE (ec);
	ASSERT_EQ (vban::process_result::progress, node.process_local (send2).code);
	ASSERT_EQ (node.config.vote_minimum, node.weight (vban::dev_genesis_key.pub));
	node.block_confirm (send2);
	// Process a vote
	auto vote2 = std::make_shared<vban::vote> (vban::dev_genesis_key.pub, vban::dev_genesis_key.prv, vban::milliseconds_since_epoch (), std::vector<vban::block_hash>{ send2->hash () });
	ASSERT_EQ (vban::vote_code::vote, node.active.vote (vote2));
	// Make sure the vote was processed
	auto election2 (node.active.election (send2->qualified_root ()));
	ASSERT_NE (nullptr, election2);
	auto votes2 (election2->votes ());
	auto existing2 (votes2.find (vban::dev_genesis_key.pub));
	...
	ASSERT_EQ (2, node.stats.count (vban::stat::type::message, vban::stat::detail::publish, vban::stat::dir::out));

	// Repeat test with a PR in the wallet
	// Increase the genesis weight again
	std::shared_ptr<vban::block> open = builder.state ()
										.account (key.pub)
										.representative (vban::dev_genesis_key.pub)
										.previous (0)
										.balance (vban::genesis_amount - 2 * node.config.vote_minimum.number ())
										.link (send->hash ())
										.sign (key.prv, key.pub)
										.work (*system.work.generate (key.pub))
										.build (ec);
	ASSERT_FALSE (ec);
	ASSERT_EQ (vban::process_result::progress, node.process_local (open).code);
	ASSERT_EQ (vban::genesis_amount - node.config.vote_minimum.number (), node.weight (vban::dev_genesis_key.pub));
	...
	// Process a vote
	auto vote3 = std::make_shared<vban::vote> (vban::dev_genesis_key.pub, vban::dev_genesis_key.prv, vban::milliseconds_since_epoch (), std::vector<vban::block_hash>{ open->hash () });
	ASSERT_EQ (vban::vote_code::vote, node.active.vote (vote3));
	// Make sure the vote was processed
	auto election3 (node.active.election (open->qualified_root ()));
	ASSERT_NE (nullptr, election3);
	auto votes3 (election3->votes ());
	auto existing3 (votes3.find (vban::dev_genesis_key.pub));
	ASSERT_NE (votes3.end (), existing3);
	ASSERT_EQ (vote3->timestamp, existing3->second.timestamp);
	// Ensure the vote wass not broadcasst
	ASSERT_EQ (1, node.stats.count (vban::stat::type::message, vban::stat::detail::confirm_ack, vban::stat::dir::out));
	ASSERT_EQ (3, node.stats.count (vban::stat::type::message, vban::stat::detail::publish, vban::stat::dir::out));
}
