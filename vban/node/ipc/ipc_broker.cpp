...
vban::ipc::broker::broker (vban::node & node_a) :
	node (node_a)
{
}

std::shared_ptr<flatbuffers::Parser> vban::ipc::subscriber::get_parser (vban::ipc::ipc_config const & ipc_config_a)
{
	if (!parser)
	{
		parser = vban::ipc::flatbuffers_handler::make_flatbuffers_parser (ipc_config_a);
	}
	return parser;
}

void vban::ipc::broker::start ()
{
	node.observers.blocks.add ([this_l = shared_from_this ()] (vban::election_status const & status_a, std::vector<vban::vote_with_weight_info> const & votes_a, vban::account const & account_a, vban::amount const & amount_a, bool is_state_send_a) {
		...
		try
		{
			...
			{
				auto confirmation (std::make_shared<vbanapi::EventConfirmationT> ());
				...
				switch (status_a.type)
				{
					case vban::election_status_type::active_confirmed_quorum:
						confirmation->confirmation_type = vbanapi::TopicConfirmationType::TopicConfirmationType_active_quorum;
						break;
					case vban::election_status_type::active_confirmation_height:
						confirmation->confirmation_type = vbanapi::TopicConfirmationType::TopicConfirmationType_active_confirmation_height;
						break;
					case vban::election_status_type::inactive_confirmation_height:
						confirmation->confirmation_type = vbanapi::TopicConfirmationType::TopicConfirmationType_inactive;
						break;
					default:
						debug_assert (false);
						break;
				...
				confirmation->confirmation_type = vbanapi::TopicConfirmationType::TopicConfirmationType_active_quorum;
				confirmation->block = vban::ipc::flatbuffers_builder::block_to_union (*status_a.winner, amount_a, is_state_send_a);
				confirmation->election_info = std::make_unique<vbanapi::ElectionInfoT> ();
...
void subscribe_or_unsubscribe (vban::logger_mt & logger, COLL & subscriber_collection, std::weak_ptr<vban::ipc::subscriber> const & subscriber_a, TOPIC_TYPE topic_a)
{
...
void vban::ipc::broker::subscribe (std::weak_ptr<vban::ipc::subscriber> const & subscriber_a, std::shared_ptr<vbanapi::TopicConfirmationT> const & confirmation_a)
{
...
void vban::ipc::broker::broadcast (std::shared_ptr<vbanapi::EventConfirmationT> const & confirmation_a)
{
	using Filter = vbanapi::TopicConfirmationTypeFilter;
	decltype (confirmation_a->election_info) election_info;
	vbanapi::BlockUnion block;
	...
	while (itr != confirmation_subscribers->end ())
	{
		if (auto subscriber_l = itr->subscriber.lock ())
		{
			auto should_filter = [this, &itr, confirmation_a] () {
				...
				if ((confirmation_a->confirmation_type == vbanapi::TopicConfirmationType::TopicConfirmationType_active_quorum || confirmation_a->confirmation_type == vbanapi::TopicConfirmationType::TopicConfirmationType_active_confirmation_height) && (all_filter || active_filter))
				{
					should_filter_conf_type_l = false;
				}
				else if (confirmation_a->confirmation_type == vbanapi::TopicConfirmationType::TopicConfirmationType_inactive && (all_filter || inactive_filter))
				{
					should_filter_conf_type_l = false;
				}
			...
			{
				auto fb (vban::ipc::flatbuffer_producer::make_buffer (*confirmation_a));

				if (subscriber_l->get_active_encoding () == vban::ipc::payload_encoding::flatbuffers_json)
				{
					...
					subscriber_l->async_send_message (reinterpret_cast<uint8_t const *> (json->data ()), json->size (), [json] (const vban::error & err) {});
				}
				else
				{
					subscriber_l->async_send_message (fb->GetBufferPointer (), fb->GetSize (), [fb] (const vban::error & err) {});
				}
			}
			...
			if (block.type != vbanapi::Block::Block_NONE)
			{
...
size_t vban::ipc::broker::confirmation_subscriber_count () const
{
	return confirmation_subscribers->size ();
}

void vban::ipc::broker::service_register (std::string const & service_name_a, std::weak_ptr<vban::ipc::subscriber> const & subscriber_a)
{
...
void vban::ipc::broker::service_stop (std::string const & service_name_a)
{
	...
	for (auto & subcription : subscribers.get ())
	{
		if (auto subscriber_l = subcription.subscriber.lock ())
		{
			...
			{
				vbanapi::EventServiceStopT event_stop;
				auto fb (vban::ipc::flatbuffer_producer::make_buffer (event_stop));
				subscriber_l->async_send_message (fb->GetBufferPointer (), fb->GetSize (), [fb] (const vban::error & err) {});
...
void vban::ipc::broker::subscribe (std::weak_ptr<vban::ipc::subscriber> const & subscriber_a, std::shared_ptr<vbanapi::TopicServiceStopT> const & service_stop_a)
{
	auto subscribers = service_stop_subscribers.lock ();
