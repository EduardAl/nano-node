...
if [ -n "$DOCKER_PASSWORD" ]; then
    echo "$DOCKER_PASSWORD" | docker login -u vbanreleaseteam --password-stdin
    ...
    if [[ "$TRAVIS_BRANCH" == "master" || "$TRAVIS_BRANCH" == "docker_cache" ]] && [[ "${TRAVIS_BUILD_STAGE_NAME}" =~ 'Build' ]]; then
        ci_image_name="nanocurrency/vban-env:$TRAVIS_JOB_NAME"
    ...
    if [[ "$TRAVIS_BUILD_STAGE_NAME" == "Master_beta_docker" ]]; then
        ...
        "$scripts"/custom-timeout.sh 30 docker build --build-arg NETWORK=beta --build-arg CI_BUILD=true --build-arg TRAVIS_TAG="$TRAVIS_TAG" -f docker/node/Dockerfile -t nanocurrency/vban-beta:master --cache-from nanocurrency/vban-beta:master .
        "$scripts"/custom-timeout.sh 30 docker push nanocurrency/vban-beta:master
        ...
        if [[ "$TRAVIS_JOB_NAME" =~ "live" ]]; then
            ...
            cached="--cache-from=nanocurrency/vban-beta:master"
            docker pull nanocurrency/vban-beta:master
        fi

        docker_image_name="nanocurrency/vban${network_tag_suffix}"
