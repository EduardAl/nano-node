...
vban::error vban::ipc::ipc_config::serialize_toml (vban::tomlconfig & toml) const
{
	vban::tomlconfig tcp_l;
...
vban::error vban::ipc::ipc_config::deserialize_toml (vban::tomlconfig & toml)
{
...
vban::error vban::ipc::ipc_config::serialize_json (vban::jsonconfig & json) const
{
	vban::jsonconfig tcp_l;
	// Only write out experimental config values if they're previously set explicitly in the config file
	if (transport_tcp.io_threads >= 0)
	{
...
vban::error vban::ipc::ipc_config::deserialize_json (bool & upgraded_a, vban::jsonconfig & json)
{
