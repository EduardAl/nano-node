...
int main (int argc, char * const * argv)
{
	...
	description.add_options ()
		...
		("config", boost::program_options::value<std::vector<vban::config_key_value_pair>>()->multitoken(), "Pass node configuration values. This takes precedence over any values in the configuration file. This option can be repeated multiple times.")
		...
		("debug_output_last_backtrace_dump", "Displays the contents of the latest backtrace in the event of a vban_node crash")
		("debug_generate_crash_report", "Consolidates the vban_node_backtrace.dump file. Requires addr2line installed on Linux")
		...
		("debug_profile_process", "Profile active blocks processing (only for vban_dev_network)")
		("debug_profile_votes", "Profile votes processing (only for vban_dev_network)")
		("debug_profile_frontiers_confirmation", "Profile frontiers confirmation speed (only for vban_dev_network)")
	...
	if (ec == vban::error_cli::unknown_command)
	{
		...
		{
			if (!vban::network_constants ().is_dev_network ())
			{
				auto node_flags = vban::inactive_node_flag_defaults ();
				vban::update_flags (node_flags, vm);
				node_flags.generate_cache.reps = true;
				vban::inactive_node inactive_node (data_path, node_flags);
				...
				auto get_total = [] (decltype (bootstrap_weights.second) const & reps) -> vban::uint128_union {
					return std::accumulate (reps.begin (), reps.end (), vban::uint256_t{ 0 }, [] (auto sum, auto const & rep) { return sum + rep.second; });
				};
				...
				{
					std::vector<std::pair<vban::account, vban::uint256_t>> sorted;
				...
				{
					vban::account rep;
					vban::uint128_union hardcoded;
					vban::uint128_union ledger;
					vban::uint128_union diff;
					std::string get_entry () const
					{
						return boost::str (boost::format ("representative %1% hardcoded %2% ledger %3% mismatch %4%")
						% rep.to_account () % hardcoded.format_balance (vban::Mxrb_ratio, 0, true) % ledger.format_balance (vban::Mxrb_ratio, 0, true) % diff.format_balance (vban::Mxrb_ratio, 0, true));
					}
				};
				...
				std::transform (hardcoded.begin (), hardcoded.end (), std::back_inserter (mismatched), [&ledger] (auto const & rep) {
					...
					vban::uint256_t ledger_weight = (ledger_rep == ledger.end () ? 0 : ledger_rep->second);
				...
				vban::uint128_union const mismatch_total = std::accumulate (mismatched.begin (), mismatched.end (), vban::uint256_t{ 0 }, [] (auto sum, mismatched_t const & sample) { return sum + sample.diff.number (); });
				vban::uint128_union const mismatch_mean = mismatch_total.number () / mismatched.size ();

				vban::uint512_union mismatch_variance = std::accumulate (mismatched.begin (), mismatched.end (), vban::uint512_t (0), [M = mismatch_mean.number (), N = mismatched.size ()] (vban::uint512_t sum, mismatched_t const & sample) {
					auto x = sample.diff.number ();
					vban::uint512_t const mean_diff = x > M ? x - M : M - x;
					vban::uint512_t const sqr = mean_diff * mean_diff;
					return sum + sqr;
				})
				/ mismatched.size ();

				vban::uint128_union const mismatch_stddev = vban::narrow_cast<vban::uint256_t> (boost::multiprecision::sqrt (mismatch_variance.number ()));

				auto const outlier_threshold = std::max (vban::Gxrb_ratio, mismatch_mean.number () + 1 * mismatch_stddev.number ());
				...
				auto const newcomer_threshold = std::max (vban::Gxrb_ratio, mismatch_mean.number ());
				std::vector<std::pair<vban::account, vban::uint256_t>> newcomers;
				...
				auto newcomer_entry = [] (auto const & rep) {
					return boost::str (boost::format ("representative %1% hardcoded --- ledger %2%") % rep.first.to_account () % vban::uint128_union (rep.second).format_balance (vban::Mxrb_ratio, 0, true));
				};

				std::cout << boost::str (boost::format ("hardcoded weight %1% Mvban at %2% blocks\nledger weight %3% Mvban at %4% blocks\nmismatched\n\tsamples %5%\n\ttotal %6% Mvban\n\tmean %7% Mvban\n\tsigma %8% Mvban\n")
				% total_hardcoded.format_balance (vban::Mxrb_ratio, 0, true)
				% hardcoded_height
				% total_ledger.format_balance (vban::Mxrb_ratio, 0, true)
				% ledger_height
				% mismatched.size ()
				% mismatch_total.format_balance (vban::Mxrb_ratio, 0, true)
				% mismatch_mean.format_balance (vban::Mxrb_ratio, 0, true)
				% mismatch_stddev.format_balance (vban::Mxrb_ratio, 0, true));
				...
				// Log more data
				auto const log_threshold = vban::Gxrb_ratio;
		...
		else if (vm.count ("debug_bootstrap_generate"))
		{
			...
			if (key_it != vm.end ())
			{
				vban::uint256_union key;
				if (!key.decode_hex (key_it->second.as<std::string> ()))
				{
					vban::keypair genesis (key.to_string ());
					...
					vban::uint256_t balance (vban::uint256_t ("50000000000000000000000000000000000000"));
					vban::open_block genesis_block (reinterpret_cast<const vban::block_hash &> (genesis.pub), genesis.pub, genesis.pub, genesis.prv, genesis.pub, *work.generate (vban::work_version::work_1, genesis.pub, network_constants.publish_thresholds.epoch_1));
					std::cout << genesis_block.to_json ();
					std::cout.flush ();
					vban::block_hash previous (genesis_block.hash ());
					for (auto i (0); i != 8; ++i)
					{
						vban::uint256_t yearly_distribution (vban::uint256_t (1) << (127 - (i == 7 ? 6 : i)));
						...
						for (auto j (0); j != 52; ++j)
						{
							...
							vban::send_block send (previous, landing.pub, balance, genesis.prv, genesis.pub, *work.generate (vban::work_version::work_1, previous, network_constants.publish_thresholds.epoch_1));
							previous = send.hash ();
		...
		{
			auto inactive_node = vban::default_inactive_node (data_path, vm);
			auto node = inactive_node->node;
		...
		else if (vm.count ("debug_dump_representatives"))
		{
			...
			auto node = inactive_node.node;
			auto transaction (node->store.tx_begin_read ());
			vban::uint256_t total;
			auto rep_amounts = node->ledger.cache.rep_weights.get_rep_amounts ();
			std::map<vban::account, vban::uint256_t> ordered_reps (rep_amounts.begin (), rep_amounts.end ());
		...
		else if (vm.count ("debug_dump_frontier_unchecked_dependents"))
		{
			...
			auto transaction (node->store.tx_begin_read ());
			std::unordered_set<vban::block_hash> frontier_hashes;
		...
		else if (vm.count ("debug_profile_kdf"))
		{
			...
			vban::uint256_union result;
			vban::uint256_union salt (0);
			std::string password ("");
			while (true)
			{
		...
		else if (vm.count ("debug_profile_generate"))
		{
			...
			if (multiplier_it != vm.end ())
			{
				try
				{
					...
					difficulty = vban::difficulty::from_multiplier (multiplier, difficulty);
				}
			...
			else
			{
				...
				{
					if (vban::from_string_hex (difficulty_it->second.as<std::string> (), difficulty))
					{
			...
			vban::work_pool work (std::numeric_limits<unsigned>::max (), pow_rate_limiter);
			vban::change_block block (0, 0, vban::keypair ().prv, 0, 0);
			if (!result)
			{
				std::cerr << boost::str (boost::format ("Starting generation profiling. Difficulty: %1$#x (%2%x from base difficulty %3$#x)\n") % difficulty % vban::to_string (vban::difficulty::to_multiplier (difficulty, network_constants.publish_full.base), 4) % network_constants.publish_full.base);
				while (!result)
				{
					block.hashables.previous.qwords[0] += 1;
					auto begin1 (std::chrono::high_resolution_clock::now ());
					block.block_work_set (*work.generate (vban::work_version::work_1, block.root (), difficulty));
					auto end1 (std::chrono::high_resolution_clock::now ());
		...
		{
			uint64_t difficulty{ vban::network_constants ().publish_full.base };
			...
			{
				valid = vban::work_v1::value (hash, i) > difficulty;
			}
				...
				if (!result)
				{
					...
					if (!error)
					{
						...
						if (!error)
						{
							...
							vban::opencl_config config (platform, device, threads);
							auto opencl (vban::opencl_work::create (true, config, logger));
							vban::work_pool work_pool (0, std::chrono::nanoseconds (0), opencl ? [&opencl] (vban::work_version const version_a, vban::root const & root_a, uint64_t difficulty_a, std::atomic<int> &) {
								return opencl->generate_work (version_a, root_a, difficulty_a);
							}
																							   : std::function<boost::optional<uint64_t> (vban::work_version const, vban::root const &, uint64_t, std::atomic<int> &)> (nullptr));
							vban::change_block block (0, 0, vban::keypair ().prv, 0, 0);
							std::cerr << boost::str (boost::format ("Starting OpenCL generation profiling. Platform: %1%. Device: %2%. Threads: %3%. Difficulty: %4$#x (%5%x from base difficulty %6$#x)\n") % platform % device % threads % difficulty % vban::to_string (vban::difficulty::to_multiplier (difficulty, network_constants.publish_full.base), 4) % network_constants.publish_full.base);
							for (uint64_t i (0); true; ++i)
							{
								block.hashables.previous.qwords[0] += 1;
								auto begin1 (std::chrono::high_resolution_clock::now ());
								block.block_work_set (*work_pool.generate (vban::work_version::work_1, block.root (), difficulty));
								auto end1 (std::chrono::high_resolution_clock::now ());
								std::cerr << boost::str (boost::format ("%|1$ 12d|\n") % std::chrono::duration_cast<std::chrono::microseconds> (end1 - begin1).count ());
							}
						}
		...
		{
			if (boost::filesystem::exists ("vban_node_backtrace.dump"))
			{
				// There is a backtrace, so output the contents
				std::ifstream ifs ("vban_node_backtrace.dump");
		...
		else if (vm.count ("debug_generate_crash_report"))
		{
			if (boost::filesystem::exists ("vban_node_backtrace.dump"))
			{
				...
				std::string crash_report_filename = "vban_node_crash_report.txt";
				...
				ofs << st;
#else
				// Read all the vban node files
				boost::system::error_code err;
				auto running_executable_filepath = boost::dll::program_location (err);
				if (!err)
				{
					auto num = 0;
					auto format = boost::format ("vban_node_crash_load_address_dump_%1%.txt");
					...
					{
						std::ofstream ofs (crash_report_filename);
						vban::set_secure_perm_file (crash_report_filename);
					}
					...
					// Now run using relative addresses. This will give actual results for other dlls, the results from the vban_node executable.
					run_addr2line (true);

					if (std::find (system_codes.begin (), system_codes.end (), 0) == system_codes.end ())
					{
						std::cerr << "Error: Check that addr2line is installed and that vban_node_crash_load_address_dump_*.txt files exist." << std::endl;
						result = -1;
					}
				}
				else
			...
			{
				std::cerr << "Error: vban_node_backtrace.dump could not be found";
				result = -1;
			}
		}
		...
		{
			vban::keypair key;
			vban::uint256_union message;
			auto signature = vban::sign_message (key.prv, key.pub, message);
			auto begin (std::chrono::high_resolution_clock::now ());
			for (auto i (0u); i < 1000; ++i)
			{
				vban::validate_message (key.pub, message, signature);
			}
		...
		else if (vm.count ("debug_verify_profile_batch"))
		{
			...
			vban::uint256_union message;
			vban::uint512_union signature (vban::sign_message (key.prv, key.pub, message));
			...
			auto begin (std::chrono::high_resolution_clock::now ());
			vban::validate_message_batch (messages.data (), lengths.data (), pub_keys.data (), signatures.data (), batch_count, verifications.data ());
			auto end (std::chrono::high_resolution_clock::now ());
		...
		else if (vm.count ("debug_profile_sign"))
		{
			...
			{
				vban::keypair key;
				vban::block_hash latest (0);
				auto begin1 (std::chrono::high_resolution_clock::now ());
				for (uint64_t balance (0); balance < 1000; ++balance)
				{
					vban::send_block send (latest, key.pub, balance, key.prv, key.pub, 0);
					latest = send.hash ();
				}
				auto end1 (std::chrono::high_resolution_clock::now ());
				std::cerr << boost::str (boost::format ("%|1$ 12d|\n") % std::chrono::duration_cast<std::chrono::microseconds> (end1 - begin1).count ());
		...
		{
			vban::network_constants::set_active_network (vban::vban_networks::vban_dev_network);
			vban::network_params dev_params;
			...
			vban::update_flags (node_flags, vm);
			vban::inactive_node inactive_node (vban::unique_path (), data_path, node_flags);
			auto node = inactive_node.node;

			vban::block_hash genesis_latest (node->latest (dev_params.ledger.dev_genesis_key.pub));
			vban::uint256_t genesis_balance (vban::uint256_t ("50000000000000000000000000000000000000"));
			// Generating keys
			std::vector<vban::keypair> keys (num_accounts);
			std::vector<vban::root> frontiers (num_accounts);
			std::vector<vban::uint256_t> balances (num_accounts, 1000000000);
			// Generating blocks
			std::deque<std::shared_ptr<vban::block>> blocks;
			for (auto i (0); i != num_accounts; ++i)
			{
				...
				auto send = builder.state ()
							...
							.work (*node->work.generate (vban::work_version::work_1, genesis_latest, node->network_params.network.publish_thresholds.epoch_1))
							.build ();
				...
				auto open = builder.state ()
							...
							.work (*node->work.generate (vban::work_version::work_1, keys[i].pub, node->network_params.network.publish_thresholds.epoch_1))
							.build ();
			...
			for (auto i (0); i != num_iterations; ++i)
			{
				for (auto j (0); j != num_accounts; ++j)
				{
					...
					auto send = builder.state ()
								...
								.work (*node->work.generate (vban::work_version::work_1, frontiers[j], node->network_params.network.publish_thresholds.epoch_1))
								.build ();
					...
					auto receive = builder.state ()
								   ...
								   .work (*node->work.generate (vban::work_version::work_1, frontiers[other], node->network_params.network.publish_thresholds.epoch_1))
								   .build ();
			...
			vban::timer<std::chrono::seconds> timer_l (vban::timer_state::started);
		...
		else if (vm.count ("debug_profile_votes"))
		{
			...
			vban::node_flags node_flags;
			vban::update_flags (node_flags, vm);
			vban::node_wrapper node_wrapper (vban::unique_path (), data_path, node_flags);
			auto node = node_wrapper.node;

			vban::block_hash genesis_latest (node->latest (dev_params.ledger.dev_genesis_key.pub));
			vban::uint256_t genesis_balance (vban::uint256_t ("50000000000000000000000000000000000000"));
			// Generating keys
			std::vector<vban::keypair> keys (num_representatives);
			vban::uint256_t balance ((node->config.online_weight_minimum.number () / num_representatives) + 1);
			...
			// Generating votes
			std::deque<std::shared_ptr<vban::vote>> votes;
			for (auto j (0); j != num_representatives; ++j)
			{
				uint64_t sequence (1);
				for (auto & i : blocks)
				{
					auto vote (std::make_shared<vban::vote> (keys[j].pub, keys[j].prv, sequence, std::vector<vban::block_hash> (1, i->hash ())));
			...
			while (!votes.empty ())
			{
				...
				auto channel (std::make_shared<vban::transport::channel_loopback> (*node));
		...
		else if (vm.count ("debug_profile_frontiers_confirmation"))
		{
			...
			vban::daemon_config daemon_config (data_path);
			auto error = vban::read_node_config_toml (data_path, daemon_config, config_overrides);

			vban::node_config config1 = daemon_config.node;
			config1.peering_port = 24000;

			vban::node_flags flags;
			vban::update_flags (flags, vm);
			flags.disable_lazy_bootstrap = true;
			flags.disable_legacy_bootstrap = true;
			flags.disable_wallet_bootstrap = true;
			flags.disable_bootstrap_listener = true;
			auto node1 (std::make_shared<vban::node> (io_ctx1, path1, config1, work, flags, 0));
			vban::block_hash genesis_latest (node1->latest (dev_params.ledger.dev_genesis_key.pub));
			vban::uint256_t genesis_balance (vban::uint256_t ("50000000000000000000000000000000000000"));
			// Generating blocks
			std::deque<std::shared_ptr<vban::block>> blocks;
			for (auto i (0); i != count; ++i)
			{
				...
				auto send = builder.state ()
							...
							.link (key.pub)
							.sign (dev_params.ledger.dev_genesis_key.prv, dev_params.ledger.dev_genesis_key.pub)
							.work (*work.generate (vban::work_version::work_1, genesis_latest, dev_params.network.publish_thresholds.epoch_1))
							.build ();
				...
				auto open = builder.state ()
							...
							.work (*work.generate (vban::work_version::work_1, key.pub, dev_params.network.publish_thresholds.epoch_1))
							.build ();
			...
			node1->start ();
			vban::thread_runner runner1 (io_ctx1, node1->config.io_threads);
			...
			// Start new node
			vban::node_config config2 = daemon_config.node;
			...
			auto node2 (std::make_shared<vban::node> (io_ctx2, path2, config2, work, flags, 1));
			node2->start ();
			vban::thread_runner runner2 (io_ctx2, node2->config.io_threads);
			...
			auto wallet (node1->wallets.create (vban::random_wallet_id ()));
		...
		else if (vm.count ("debug_random_feed"))
		{
			 ...
			 *
			 *   ./vban_node --debug_random_feed | dieharder -a -g 200
			 */
			vban::raw_key seed;
			for (;;)
			{
		...
		else if (vm.count ("debug_rpc"))
		{
			...
			vban::inactive_node inactive_node_l (data_path, node_flags);

			vban::node_rpc_config config;
			vban::ipc::ipc_server server (*inactive_node_l.node, config);
			auto handler_l (std::make_shared<vban::json_handler> (*inactive_node_l.node, config, command_l.str (), response_handler_l));
			handler_l->process_request ();
		}
		else if (vm.count ("validate_blocks") || vm.count ("debug_validate_blocks"))
		{
			vban::timer<std::chrono::seconds> timer;
			timer.start ();
			auto node_flags = vban::inactive_node_flag_defaults ();
			vban::update_flags (node_flags, vm);
			node_flags.generate_cache.block_count = true;
			...
			std::deque<std::pair<vban::account, vban::account_info>> accounts;
			...
			auto print_error_message = [&silent, &errors] (std::string const & error_message_a) {
				if (!silent)
				{
					static vban::mutex cerr_mutex;
					vban::lock_guard<vban::mutex> lock (cerr_mutex);
			...
			auto check_account = [&print_error_message, &silent, &count, &block_count] (std::shared_ptr<vban::node> const & node, vban::read_transaction const & transaction, vban::account const & account, vban::account_info const & info) {
				++count;
				...
				vban::block_hash calculated_hash (0);
				...
				vban::account calculated_representative (0);
				while (!hash.is_zero () && block != nullptr)
				{
					...
					if (block->type () == vban::block_type::open || block->type () == vban::block_type::state)
					{
					...
					if (height == 0 && block->type () != vban::block_type::open && block->type () != vban::block_type::state)
					{
					...
					if (validate_message (account, hash, block->block_signature ()))
					{
						...
						{
							auto & state_block (static_cast<vban::state_block &> (*block.get ()));
					...
					if (block->difficulty () < vban::work_threshold (block->work_version (), block->sideband ().details))
					{
						print_error_message (boost::str (boost::format ("Invalid work for block %1% value: %2%\n") % hash.to_string () % vban::to_string_hex (block->block_work ())));
					}
					...
					if (block->type () == vban::block_type::open || block->type () == vban::block_type::change || block->type () == vban::block_type::state)
					{
			...
			count = 0;
			finished = false;
			std::deque<std::pair<vban::pending_key, vban::pending_info>> pending;

			auto check_pending = [&print_error_message, &silent, &count] (std::shared_ptr<vban::node> const & node, vban::read_transaction const & transaction, vban::pending_key const & key, vban::pending_info const & info) {
				++count;
				...
				else
				{
					...
					if (auto state = dynamic_cast<vban::state_block *> (block.get ()))
					{
					...
					else if (auto send = dynamic_cast<vban::send_block *> (block.get ()))
					{
		...
		else if (vm.count ("debug_profile_bootstrap"))
		{
			...
			vban::update_flags (node_flags, vm);
			vban::inactive_node node (vban::unique_path (), node_flags);
			vban::genesis genesis;
			auto begin (std::chrono::high_resolution_clock::now ());
			uint64_t block_count (0);
			size_t count (0);
			std::deque<vban::unchecked_info> epoch_open_blocks;
			{
				auto node_flags = vban::inactive_node_flag_defaults ();
				vban::update_flags (node_flags, vm);
				node_flags.generate_cache.block_count = true;
				...
				for (auto i (source_node->store.accounts_begin (transaction)), n (source_node->store.accounts_end ()); i != n; ++i)
				{
					...
					while (!hash.is_zero ())
					{
						...
						if (block != nullptr)
						{
							...
							vban::unchecked_info unchecked_info (block, account, 0, vban::signature_verification::unknown);
							node.node->block_processor.add (unchecked_info);
							if (block->type () == vban::block_type::state && block->previous ().is_zero () && source_node->ledger.is_epoch_link (block->link ()))
							{
		...
		else if (vm.count ("debug_peers"))
		{
			...
			{
				std::cout << boost::str (boost::format ("%1%\n") % vban::endpoint (boost::asio::ip::address_v6 (i->first.address_bytes ()), i->first.port ()));
			}
		}
		else if (vm.count ("debug_cemented_block_count"))
		{
			...
			vban::update_flags (node_flags, vm);
			vban::inactive_node node (data_path, node_flags);
		...
		{
#ifdef BOOST_WINDOWS
			if (!vban::event_log_reg_entry_exists () && !vban::is_windows_elevated ())
			{
				std::cerr << "The event log requires the HKEY_LOCAL_MACHINE\\SYSTEM\\CurrentControlSet\\Services\\EventLog\\Vban\\Vban registry entry, run again as administator to create it.\n";
				return 1;
			}
#endif
			auto inactive_node = vban::default_inactive_node (data_path, vm);
			inactive_node->node->logger.always_log (vban::severity_level::error, "Testing system logger");
		}
		else if (vm.count ("debug_account_versions"))
		{
			auto inactive_node = vban::default_inactive_node (data_path, vm);
			auto node = inactive_node->node;
			auto const epoch_count = vban::normalized_epoch (vban::epoch::max) + static_cast<std::underlying_type<vban::epoch>::type> (1);
			// Cache the accounts in a collection to make searching quicker against unchecked keys. Group by epoch
			vban::locked<std::vector<boost::unordered_set<vban::account>>> opened_account_versions_shared (epoch_count);
			using opened_account_versions_t = decltype (opened_account_versions_shared)::value_type;
			node->store.accounts_for_each_par (
			[&opened_account_versions_shared, epoch_count] (vban::read_transaction const & /*unused*/, vban::store_iterator<vban::account, vban::account_info> i, vban::store_iterator<vban::account, vban::account_info> n) {
				// First cache locally
				...
				for (; i != n; ++i)
				{
					...
					auto epoch_idx = vban::normalized_epoch (account_info.epoch ());
			...
			boost::unordered_set<vban::account> opened_accounts;
			{
				auto opened_account_versions = opened_account_versions_shared.lock ();
			...
			vban::locked<boost::unordered_map<vban::account, std::underlying_type_t<vban::epoch>>> unopened_highest_pending_shared;
			using unopened_highest_pending_t = decltype (unopened_highest_pending_shared)::value_type;
			node->store.pending_for_each_par (
			[&unopened_highest_pending_shared, &opened_accounts] (vban::read_transaction const & /*unused*/, vban::store_iterator<vban::pending_key, vban::pending_info> i, vban::store_iterator<vban::pending_key, vban::pending_info> n) {
				// First cache locally
				...
				for (; i != n; ++i)
				{
					...
					if (!exists)
					{
						...
						auto epoch = vban::normalized_epoch (info.epoch);
