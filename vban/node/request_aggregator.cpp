...
vban::request_aggregator::request_aggregator (vban::network_constants const & network_constants_a, vban::node_config const & config_a, vban::stat & stats_a, vban::vote_generator & generator_a, vban::vote_generator & final_generator_a, vban::local_vote_history & history_a, vban::ledger & ledger_a, vban::wallets & wallets_a, vban::active_transactions & active_a) :
	...
	thread ([this] () { run (); })
{
	generator.set_reply_action ([this] (std::shared_ptr<vban::vote> const & vote_a, std::shared_ptr<vban::transport::channel> const & channel_a) {
		this->reply_action (vote_a, channel_a);
	});
	final_generator.set_reply_action ([this] (std::shared_ptr<vban::vote> const & vote_a, std::shared_ptr<vban::transport::channel> const & channel_a) {
		this->reply_action (vote_a, channel_a);
	});
...
void vban::request_aggregator::add (std::shared_ptr<vban::transport::channel> const & channel_a, std::vector<std::pair<vban::block_hash, vban::root>> const & hashes_roots_a)
{
	debug_assert (wallets.reps ().voting > 0);
	bool error = true;
	auto const endpoint (vban::transport::map_endpoint_to_v6 (channel_a->get_endpoint ()));
	vban::unique_lock<vban::mutex> lock (mutex);
	...
	stats.inc (vban::stat::type::aggregator, !error ? vban::stat::detail::aggregator_accepted : vban::stat::detail::aggregator_dropped);
}

void vban::request_aggregator::run ()
{
	vban::thread_role::set (vban::thread_role::name::request_aggregator);
	vban::unique_lock<vban::mutex> lock (mutex);
	started = true;
	lock.unlock ();
	condition.notify_all ();
		...
		if (!requests.empty ())
		{
			...
			if (front->deadline < std::chrono::steady_clock::now ())
			{
				...
				if (!remaining.first.empty ())
				{
					...
					stats.add (vban::stat::type::requests, vban::stat::detail::requests_cannot_vote, stat::dir::in, remaining.first.size () - generated);
				}
				if (!remaining.second.empty ())
				{
					...
					stats.add (vban::stat::type::requests, vban::stat::detail::requests_cannot_vote, stat::dir::in, remaining.second.size () - generated);
				}
				lock.lock ();
			}
			else
...
void vban::request_aggregator::stop ()
{
	{
...
std::size_t vban::request_aggregator::size ()
{
	vban::unique_lock<vban::mutex> lock (mutex);
	return requests.size ();
}

bool vban::request_aggregator::empty ()
{
	return size () == 0;
}

void vban::request_aggregator::reply_action (std::shared_ptr<vban::vote> const & vote_a, std::shared_ptr<vban::transport::channel> const & channel_a) const
{
	vban::confirm_ack confirm (vote_a);
	channel_a->send (confirm);
}

void vban::request_aggregator::erase_duplicates (std::vector<std::pair<vban::block_hash, vban::root>> & requests_a) const
{
...
std::pair<std::vector<std::shared_ptr<vban::block>>, std::vector<std::shared_ptr<vban::block>>> vban::request_aggregator::aggregate (std::vector<std::pair<vban::block_hash, vban::root>> const & requests_a, std::shared_ptr<vban::transport::channel> & channel_a) const
{
	auto transaction (ledger.store.tx_begin_read ());
	size_t cached_hashes = 0;
	std::vector<std::shared_ptr<vban::block>> to_generate;
	std::vector<std::shared_ptr<vban::block>> to_generate_final;
	std::vector<std::shared_ptr<vban::vote>> cached_votes;
	for (auto const & [hash, root] : requests_a)
	{
		...
		else
		{
			...
			{
				stats.inc (vban::stat::type::requests, vban::stat::detail::requests_unknown, stat::dir::in);
			}
		}
	}
	// Unique votes
	...
	stats.add (vban::stat::type::requests, vban::stat::detail::requests_cached_hashes, stat::dir::in, cached_hashes);
	stats.add (vban::stat::type::requests, vban::stat::detail::requests_cached_votes, stat::dir::in, cached_votes.size ());
	return std::make_pair (to_generate, to_generate_final);
}

std::unique_ptr<vban::container_info_component> vban::collect_container_info (vban::request_aggregator & aggregator, std::string const & name)
{
