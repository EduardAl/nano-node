...
TEST (cli, key_create)
{
	...
	vban::raw_key private_key;
	private_key.decode_hex (private_key_str);

	auto public_key = vban::pub_key (private_key);
...
{
	std::vector<vban::config_key_value_pair> key_value_pairs;
	auto config_overrides = vban::config_overrides (key_value_pairs);
	ASSERT_TRUE (config_overrides.empty ());
	key_value_pairs.push_back ({ "key", "value" });
	config_overrides = vban::config_overrides (key_value_pairs);
