...
#include <vban/boost/asio/local/stream_protocol.hpp>
#include <vban/boost/asio/read.hpp>
...
class session final : public vban::ipc::socket_base, public std::enable_shared_from_this<session<SOCKET_TYPE>>
{
public:
	session (vban::ipc::ipc_server & server_a, boost::asio::io_context & io_ctx_a, vban::ipc::ipc_config_transport & config_transport_a) :
	...
	std::shared_ptr<vban::ipc::subscriber> get_subscriber ()
	{
		class subscriber_impl final : public vban::ipc::subscriber, public std::enable_shared_from_this<subscriber_impl>
		{
		public:
			...
			virtual void async_send_message (uint8_t const * data_a, size_t length_a, std::function<void (vban::error const &)> broadcast_completion_handler_a) override
			{
			...
			vban::ipc::payload_encoding get_active_encoding () const override
			{
				vban::ipc::payload_encoding encoding{ vban::ipc::payload_encoding::flatbuffers };
				if (auto session_l = session_m.lock ())
				{
		...
		static vban::mutex subscriber_mutex;
		vban::unique_lock<vban::mutex> lock (subscriber_mutex);
	 ...
	 * @note This function explicitely doesn't use vban::shared_const_buffer, as buffers usually originate from Flatbuffers
	...
	void write_queued_messages ()
	{
		...
		vban::unsafe_async_write (socket, msg.buffer,
	...
	void handle_json_query (bool allow_unsafe)
	{
		...
		// This is called when vban::rpc_handler#process_request is done. We convert to
		...
		node.stats.inc (vban::stat::type::ipc, vban::stat::detail::invocations);
		auto body (std::string (reinterpret_cast<char *> (buffer.data ()), buffer.size ()));

		// Note that if the rpc action is async, the shared_ptr<json_handler> lifetime will be extended by the action handler
		auto handler (std::make_shared<vban::json_handler> (node, server.node_rpc_config, body, response_handler_l, [&server = server] () {
			server.stop ();
	...
	void read_next_request ()
	{
		...
		async_read_exactly (buffer.data (), buffer.size (), std::chrono::seconds::max (), [this_l] () {
			auto encoding (this_l->buffer[vban::ipc::preamble_offset::encoding]);
			this_l->active_encoding = static_cast<vban::ipc::payload_encoding> (encoding);
			if (this_l->buffer[vban::ipc::preamble_offset::lead] != 'N' || this_l->buffer[vban::ipc::preamble_offset::reserved_1] != 0 || this_l->buffer[vban::ipc::preamble_offset::reserved_2] != 0)
			{
				if (this_l->node.config.logging.log_ipc ())
				{
			...
			else if (encoding == static_cast<uint8_t> (vban::ipc::payload_encoding::json_v1) || encoding == static_cast<uint8_t> (vban::ipc::payload_encoding::json_v1_unsafe))
			{
				auto allow_unsafe (encoding == static_cast<uint8_t> (vban::ipc::payload_encoding::json_v1_unsafe));
				// Length of payload
			...
			else if (encoding == static_cast<uint8_t> (vban::ipc::payload_encoding::flatbuffers) || encoding == static_cast<uint8_t> (vban::ipc::payload_encoding::flatbuffers_json))
			{
				// Length of payload
				this_l->async_read_exactly (&this_l->buffer_size, sizeof (this_l->buffer_size), [this_l, encoding] () {
					...
					this_l->async_read_exactly (this_l->buffer.data (), this_l->buffer_size, [this_l, encoding] () {
						...
						{
							this_l->flatbuffers_handler = std::make_shared<vban::ipc::flatbuffers_handler> (this_l->node, this_l->server, this_l->get_subscriber (), this_l->node.config.ipc_config);
						}

						if (encoding == static_cast<uint8_t> (vban::ipc::payload_encoding::flatbuffers_json))
						{
...
private:
	...
	vban::locked<std::string> service_name;
	 ...
	 */
	std::atomic<vban::ipc::payload_encoding> active_encoding;

	/** Timer for measuring the duration of ipc calls */
	vban::timer<std::chrono::microseconds> session_timer;
	...
	vban::ipc::ipc_config_transport & config_transport;

	/** Handler for Flatbuffers requests. This is created lazily on the first request. */
	std::shared_ptr<vban::ipc::flatbuffers_handler> flatbuffers_handler;

	/** Session subscriber */
	std::shared_ptr<vban::ipc::subscriber> subscriber;
};

/** Domain and TCP socket transport */
template <typename ACCEPTOR_TYPE, typename SOCKET_TYPE, typename ENDPOINT_TYPE>
class socket_transport : public vban::ipc::transport
{
public:
	socket_transport (vban::ipc::ipc_server & server_a, ENDPOINT_TYPE endpoint_a, vban::ipc::ipc_config_transport & config_transport_a, int concurrency_a) :
		server (server_a), config_transport (config_transport_a)
	{
		...
		if (concurrency_a > 0)
		{
			runner = std::make_unique<vban::thread_runner> (*io_ctx, static_cast<unsigned> (concurrency_a));
		}
	}
 ...
 */
void await_hup_signal (std::shared_ptr<boost::asio::signal_set> const & signals, vban::ipc::ipc_server & server_a)
{
...
vban::ipc::ipc_server::ipc_server (vban::node & node_a, vban::node_rpc_config const & node_rpc_config_a) :
	node (node_a),
	node_rpc_config (node_rpc_config_a),
	broker (std::make_shared<vban::ipc::broker> (node_a))
{
	try
	{
		vban::error access_config_error (reload_access_config ());
...
std::shared_ptr<vban::ipc::broker> vban::ipc::ipc_server::get_broker ()
{
	return broker;
}

vban::ipc::access & vban::ipc::ipc_server::get_access ()
{
	return access;
}

vban::error vban::ipc::ipc_server::reload_access_config ()
{
	vban::error access_config_error (vban::ipc::read_access_config_toml (node.application_path, access));
	if (access_config_error)
	{
