#include <vban/boost/asio/post.hpp>
...
namespace
{
thread_local vban::thread_role::name current_thread_role = vban::thread_role::name::unknown;
}

vban::thread_role::name vban::thread_role::get ()
{
	return current_thread_role;
}

std::string vban::thread_role::get_string (vban::thread_role::name role)
{
	std::string thread_role_name_string;

	switch (role)
	{
		case vban::thread_role::name::unknown:
			thread_role_name_string = "<unknown>";
			break;
		case vban::thread_role::name::io:
			thread_role_name_string = "I/O";
			break;
		case vban::thread_role::name::work:
			thread_role_name_string = "Work pool";
			break;
		case vban::thread_role::name::packet_processing:
			thread_role_name_string = "Pkt processing";
			break;
		case vban::thread_role::name::vote_processing:
			thread_role_name_string = "Vote processing";
			break;
		case vban::thread_role::name::block_processing:
			thread_role_name_string = "Blck processing";
			break;
		case vban::thread_role::name::request_loop:
			thread_role_name_string = "Request loop";
			break;
		case vban::thread_role::name::wallet_actions:
			thread_role_name_string = "Wallet actions";
			break;
		case vban::thread_role::name::bootstrap_initiator:
			thread_role_name_string = "Bootstrap init";
			break;
		case vban::thread_role::name::bootstrap_connections:
			thread_role_name_string = "Bootstrap conn";
			break;
		case vban::thread_role::name::voting:
			thread_role_name_string = "Voting";
			break;
		case vban::thread_role::name::signature_checking:
			thread_role_name_string = "Signature check";
			break;
		case vban::thread_role::name::rpc_request_processor:
			thread_role_name_string = "RPC processor";
			break;
		case vban::thread_role::name::rpc_process_container:
			thread_role_name_string = "RPC process";
			break;
		case vban::thread_role::name::confirmation_height_processing:
			thread_role_name_string = "Conf height";
			break;
		case vban::thread_role::name::worker:
			thread_role_name_string = "Worker";
			break;
		case vban::thread_role::name::request_aggregator:
			thread_role_name_string = "Req aggregator";
			break;
		case vban::thread_role::name::state_block_signature_verification:
			thread_role_name_string = "State block sig";
			break;
		case vban::thread_role::name::epoch_upgrader:
			thread_role_name_string = "Epoch upgrader";
			break;
		case vban::thread_role::name::db_parallel_traversal:
			thread_role_name_string = "DB par traversl";
			break;
		case vban::thread_role::name::election_scheduler:
...
std::string vban::thread_role::get_string ()
{
	return get_string (current_thread_role);
}

void vban::thread_role::set (vban::thread_role::name role)
{
	auto thread_role_name_string (get_string (role));

	vban::thread_role::set_os_name (thread_role_name_string);

	current_thread_role = role;
}

void vban::thread_attributes::set (boost::thread::attributes & attrs)
{
	auto attrs_l (&attrs);
	attrs_l->set_stack_size (8000000); //8MB
}

vban::thread_runner::thread_runner (boost::asio::io_context & io_ctx_a, unsigned service_threads_a) :
	io_guard (boost::asio::make_work_guard (io_ctx_a))
{
	boost::thread::attributes attrs;
	vban::thread_attributes::set (attrs);
	auto count = (is_sanitizer_build && vban::network_constants{}.is_dev_network ()) ? 1 : service_threads_a; // This is a workaround to a bad interaction between TSAN, multiple coroutines, and multiple threads servicing io_context. Only use 1 thread if sanitizers are attached
		...
		threads.emplace_back (attrs, [&io_ctx_a] () {
			...
			try
			{
				...
				while (true)
				{
					...
					if (count == 1 && timer.since_start ().count () >= VBAN_ASIO_HANDLER_TRACKING)
					{
...
vban::thread_runner::~thread_runner ()
{
	join ();
}

void vban::thread_runner::join ()
{
	io_guard.reset ();
	for (auto & i : threads)
	{
...
void vban::thread_runner::stop_event_processing ()
{
	io_guard.get_executor ().context ().stop ();
}

vban::thread_pool::thread_pool (unsigned num_threads, vban::thread_role::name thread_name) :
...
void vban::thread_pool::push_task (std::function<void ()> task)
{
	++num_tasks;
...
void vban::thread_pool::add_timed_task (std::chrono::steady_clock::time_point const & expiry_time, std::function<void ()> task)
{
	vban::lock_guard<vban::mutex> guard (mutex);
...
unsigned vban::thread_pool::get_num_threads () const
{
	return num_threads;
}

uint64_t vban::thread_pool::num_queued_tasks () const
{
	return num_tasks;
}

// Set the names of all the threads in the thread pool for easier identification
void vban::thread_pool::set_thread_names (unsigned num_threads, vban::thread_role::name thread_name)
{
	...
	for (auto i = 0u; i < num_threads; ++i)
	{
		boost::asio::post (*thread_pool_m, [&promise = promises[i], thread_name] () {
			vban::thread_role::set (thread_name);
			promise.set_value ();
		});
	}
...
std::unique_ptr<vban::container_info_component> vban::collect_container_info (thread_pool & thread_pool, std::string const & name)
{
