...
vban::websocket::confirmation_options::confirmation_options (vban::wallets & wallets_a) :
	wallets (wallets_a)
{
}

vban::websocket::confirmation_options::confirmation_options (boost::property_tree::ptree const & options_a, vban::wallets & wallets_a, vban::logger_mt & logger_a) :
	wallets (wallets_a),
	logger (logger_a)
{
...
bool vban::websocket::confirmation_options::should_filter (vban::websocket::message const & message_a) const
{
	...
	if (destination_opt_l)
	{
		...
		if (all_local_accounts)
		{
			...
			vban::account source_l (0), destination_l (0);
...
bool vban::websocket::confirmation_options::update (boost::property_tree::ptree const & options_a)
{
...
void vban::websocket::confirmation_options::check_filter_empty () const
{
...
vban::websocket::vote_options::vote_options (boost::property_tree::ptree const & options_a, vban::logger_mt & logger_a)
{
...
bool vban::websocket::vote_options::should_filter (vban::websocket::message const & message_a) const
{
...
vban::websocket::session::session (vban::websocket::listener & listener_a, socket_type socket_a) :
...
vban::websocket::session::~session ()
{
	{
		vban::unique_lock<vban::mutex> lk (subscriptions_mutex);
...
void vban::websocket::session::handshake ()
{
...
void vban::websocket::session::close ()
{
...
void vban::websocket::session::write (vban::websocket::message message_a)
{
	vban::unique_lock<vban::mutex> lk (subscriptions_mutex);
	auto subscription (subscriptions.find (message_a.topic));
	if (message_a.topic == vban::websocket::topic::ack || (subscription != subscriptions.end () && !subscription->second->should_filter (message_a)))
	{
		lk.unlock ();
		auto this_l (shared_from_this ());
...
void vban::websocket::session::write_queued_messages ()
{
	auto msg (send_queue.front ().to_string ());
	auto this_l (shared_from_this ());

	ws.async_write (vban::shared_const_buffer (msg),
...
void vban::websocket::session::read ()
{
	auto this_l (shared_from_this ());
...
namespace
{
vban::websocket::topic to_topic (std::string const & topic_a)
{
	vban::websocket::topic topic = vban::websocket::topic::invalid;
	if (topic_a == "confirmation")
	{
		topic = vban::websocket::topic::confirmation;
	}
	else if (topic_a == "stopped_election")
	{
		topic = vban::websocket::topic::stopped_election;
	}
	else if (topic_a == "vote")
	{
		topic = vban::websocket::topic::vote;
	}
	else if (topic_a == "ack")
	{
		topic = vban::websocket::topic::ack;
	}
	else if (topic_a == "work")
	{
		topic = vban::websocket::topic::work;
	}
	else if (topic_a == "bootstrap")
	{
		topic = vban::websocket::topic::bootstrap;
	}
	else if (topic_a == "telemetry")
	{
		topic = vban::websocket::topic::telemetry;
	}
	else if (topic_a == "new_unconfirmed_block")
	{
		topic = vban::websocket::topic::new_unconfirmed_block;
	}

	return topic;
}

std::string from_topic (vban::websocket::topic topic_a)
{
	std::string topic = "invalid";
	if (topic_a == vban::websocket::topic::confirmation)
	{
		topic = "confirmation";
	}
	else if (topic_a == vban::websocket::topic::stopped_election)
	{
		topic = "stopped_election";
	}
	else if (topic_a == vban::websocket::topic::vote)
	{
		topic = "vote";
	}
	else if (topic_a == vban::websocket::topic::ack)
	{
		topic = "ack";
	}
	else if (topic_a == vban::websocket::topic::work)
	{
		topic = "work";
	}
	else if (topic_a == vban::websocket::topic::bootstrap)
	{
		topic = "bootstrap";
	}
	else if (topic_a == vban::websocket::topic::telemetry)
	{
		topic = "telemetry";
	}
	else if (topic_a == vban::websocket::topic::new_unconfirmed_block)
	{
...
void vban::websocket::session::send_ack (std::string action_a, std::string id_a)
{
	vban::websocket::message msg (vban::websocket::topic::ack);
	boost::property_tree::ptree & message_l = msg.contents;
	message_l.add ("ack", action_a);
	message_l.add ("time", std::to_string (vban::milliseconds_since_epoch ()));
	if (!id_a.empty ())
	{
...
void vban::websocket::session::handle_message (boost::property_tree::ptree const & message_a)
{
	...
	if (action == "subscribe" && topic_l != vban::websocket::topic::invalid)
	{
		auto options_text_l (message_a.get_child_optional ("options"));
		vban::lock_guard<vban::mutex> lk (subscriptions_mutex);
		std::unique_ptr<vban::websocket::options> options_l{ nullptr };
		if (options_text_l && topic_l == vban::websocket::topic::confirmation)
		{
			options_l = std::make_unique<vban::websocket::confirmation_options> (options_text_l.get (), ws_listener.get_wallets (), ws_listener.get_logger ());
		}
		else if (options_text_l && topic_l == vban::websocket::topic::vote)
		{
			options_l = std::make_unique<vban::websocket::vote_options> (options_text_l.get (), ws_listener.get_logger ());
		}
		else
		{
			options_l = std::make_unique<vban::websocket::options> ();
		}
	...
	else if (action == "unsubscribe" && topic_l != vban::websocket::topic::invalid)
	{
		vban::lock_guard<vban::mutex> lk (subscriptions_mutex);
...
void vban::websocket::listener::stop ()
{
	stopped = true;
	acceptor.close ();

	vban::lock_guard<vban::mutex> lk (sessions_mutex);
...
vban::websocket::listener::listener (vban::logger_mt & logger_a, vban::wallets & wallets_a, boost::asio::io_context & io_ctx_a, boost::asio::ip::tcp::endpoint endpoint_a) :
	logger (logger_a),
	wallets (wallets_a),
	acceptor (io_ctx_a),
	socket (io_ctx_a)
...
void vban::websocket::listener::run ()
{
	if (acceptor.is_open ())
	{
		accept ();
...
void vban::websocket::listener::accept ()
{
	auto this_l (shared_from_this ());
...
void vban::websocket::listener::on_accept (boost::system::error_code ec)
{
	if (ec)
	{
	...
	else
	{
		...
		auto session (std::make_shared<vban::websocket::session> (*this, std::move (socket)));
		sessions_mutex.lock ();
...
void vban::websocket::listener::broadcast_confirmation (std::shared_ptr<vban::block> const & block_a, vban::account const & account_a, vban::amount const & amount_a, std::string const & subtype, vban::election_status const & election_status_a, std::vector<vban::vote_with_weight_info> const & election_votes_a)
{
	vban::websocket::message_builder builder;

	vban::lock_guard<vban::mutex> lk (sessions_mutex);
	boost::optional<vban::websocket::message> msg_with_block;
	boost::optional<vban::websocket::message> msg_without_block;
	for (auto & weak_session : sessions)
	{
		auto session_ptr (weak_session.lock ());
		if (session_ptr)
		{
			auto subscription (session_ptr->subscriptions.find (vban::websocket::topic::confirmation));
			if (subscription != session_ptr->subscriptions.end ())
			{
				vban::websocket::confirmation_options default_options (wallets);
				auto conf_options (dynamic_cast<vban::websocket::confirmation_options *> (subscription->second.get ()));
...
void vban::websocket::listener::broadcast (vban::websocket::message message_a)
{
	vban::lock_guard<vban::mutex> lk (sessions_mutex);
	for (auto & weak_session : sessions)
	{
...
void vban::websocket::listener::increase_subscriber_count (vban::websocket::topic const & topic_a)
{
	topic_subscriber_count[static_cast<std::size_t> (topic_a)] += 1;
}

void vban::websocket::listener::decrease_subscriber_count (vban::websocket::topic const & topic_a)
{
...
vban::websocket::message vban::websocket::message_builder::stopped_election (vban::block_hash const & hash_a)
{
	vban::websocket::message message_l (vban::websocket::topic::stopped_election);
...
vban::websocket::message vban::websocket::message_builder::block_confirmed (std::shared_ptr<vban::block> const & block_a, vban::account const & account_a, vban::amount const & amount_a, std::string subtype, bool include_block_a, vban::election_status const & election_status_a, std::vector<vban::vote_with_weight_info> const & election_votes_a, vban::websocket::confirmation_options const & options_a)
{
	vban::websocket::message message_l (vban::websocket::topic::confirmation);
	set_common_fields (message_l);
...
vban::websocket::message vban::websocket::message_builder::vote_received (std::shared_ptr<vban::vote> const & vote_a, vban::vote_code code_a)
{
	vban::websocket::message message_l (vban::websocket::topic::vote);
	set_common_fields (message_l);
...
vban::websocket::message vban::websocket::message_builder::work_generation (vban::work_version const version_a, vban::block_hash const & root_a, uint64_t work_a, uint64_t difficulty_a, uint64_t publish_threshold_a, std::chrono::milliseconds const & duration_a, std::string const & peer_a, std::vector<std::string> const & bad_peers_a, bool completed_a, bool cancelled_a)
{
	vban::websocket::message message_l (vban::websocket::topic::work);
	set_common_fields (message_l);
	...
	request_l.put ("version", vban::to_string (version_a));
	request_l.put ("hash", root_a.to_string ());
	request_l.put ("difficulty", vban::to_string_hex (difficulty_a));
	auto request_multiplier_l (vban::difficulty::to_multiplier (difficulty_a, publish_threshold_a));
	request_l.put ("multiplier", vban::to_string (request_multiplier_l));
	...
	if (completed_a)
	{
		...
		result_l.put ("work", vban::to_string_hex (work_a));
		auto result_difficulty_l (vban::work_difficulty (version_a, root_a, work_a));
		result_l.put ("difficulty", vban::to_string_hex (result_difficulty_l));
		auto result_multiplier_l (vban::difficulty::to_multiplier (result_difficulty_l, publish_threshold_a));
		result_l.put ("multiplier", vban::to_string (result_multiplier_l));
...
vban::websocket::message vban::websocket::message_builder::work_cancelled (vban::work_version const version_a, vban::block_hash const & root_a, uint64_t const difficulty_a, uint64_t const publish_threshold_a, std::chrono::milliseconds const & duration_a, std::vector<std::string> const & bad_peers_a)
{
	return work_generation (version_a, root_a, 0, difficulty_a, publish_threshold_a, duration_a, "", bad_peers_a, false, true);
}

vban::websocket::message vban::websocket::message_builder::work_failed (vban::work_version const version_a, vban::block_hash const & root_a, uint64_t const difficulty_a, uint64_t const publish_threshold_a, std::chrono::milliseconds const & duration_a, std::vector<std::string> const & bad_peers_a)
{
	return work_generation (version_a, root_a, 0, difficulty_a, publish_threshold_a, duration_a, "", bad_peers_a, false, false);
}

vban::websocket::message vban::websocket::message_builder::bootstrap_started (std::string const & id_a, std::string const & mode_a)
{
	vban::websocket::message message_l (vban::websocket::topic::bootstrap);
	set_common_fields (message_l);
...
vban::websocket::message vban::websocket::message_builder::bootstrap_exited (std::string const & id_a, std::string const & mode_a, std::chrono::steady_clock::time_point const start_time_a, uint64_t const total_blocks_a)
{
	vban::websocket::message message_l (vban::websocket::topic::bootstrap);
	set_common_fields (message_l);
...
vban::websocket::message vban::websocket::message_builder::telemetry_received (vban::telemetry_data const & telemetry_data_a, vban::endpoint const & endpoint_a)
{
	vban::websocket::message message_l (vban::websocket::topic::telemetry);
	set_common_fields (message_l);

	// Telemetry information
	vban::jsonconfig telemetry_l;
...
vban::websocket::message vban::websocket::message_builder::new_block_arrived (vban::block const & block_a)
{
	vban::websocket::message message_l (vban::websocket::topic::new_unconfirmed_block);
	set_common_fields (message_l);

	boost::property_tree::ptree block_l;
	block_a.serialize_json (block_l);
	auto subtype (vban::state_subtype (block_a.sideband ().details));
...
void vban::websocket::message_builder::set_common_fields (vban::websocket::message & message_a)
{
	// Common message information
	message_a.contents.add ("topic", from_topic (message_a.topic));
	message_a.contents.add ("time", std::to_string (vban::milliseconds_since_epoch ()));
}

std::string vban::websocket::message::to_string () const
{
