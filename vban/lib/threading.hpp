...
#include <vban/boost/asio/deadline_timer.hpp>
#include <vban/boost/asio/executor_work_guard.hpp>
#include <vban/boost/asio/io_context.hpp>
#include <vban/boost/asio/steady_timer.hpp>
#include <vban/boost/asio/thread_pool.hpp>
...
namespace thread_role
{
	 ...
	 */
	vban::thread_role::name get ();
	void set (vban::thread_role::name);

	/*
	 * Get the thread name as a string from enum
	 */
	std::string get_string (vban::thread_role::name);
...
{
public:
	explicit thread_pool (unsigned, vban::thread_role::name);
	~thread_pool ();
...
private:
	...
	void set_thread_names (unsigned num_threads, vban::thread_role::name thread_name);
};

std::unique_ptr<vban::container_info_component> collect_container_info (thread_pool & thread_pool, std::string const & name);
}
