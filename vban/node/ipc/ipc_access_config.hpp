...
namespace ipc
{
	...
	{
	public:
		std::unordered_set<vban::ipc::access_permission, enum_hash> permissions;
	...
	public:
		...
		std::string id;
		std::vector<vban::ipc::access_role> roles;
		void clear () override;
	};
	...
	{
	public:
		bool has_access (std::string const & credentials_a, vban::ipc::access_permission permission_a) const;
		bool has_access_to_all (std::string const & credentials_a, std::initializer_list<vban::ipc::access_permission> permissions_a) const;
		bool has_access_to_oneof (std::string const & credentials_a, std::initializer_list<vban::ipc::access_permission> permissions_a) const;
		vban::error deserialize_toml (vban::tomlconfig &);

	private:
		/** Process allow and deny entries for the given subject */
		void set_effective_permissions (vban::ipc::access_subject & subject_a, std::shared_ptr<cpptoml::table> const & config_subject_a);

		/** Clear current users, roles and default permissions */
		void clear ();

		std::unordered_map<std::string, vban::ipc::access_user> users;
		std::unordered_map<std::string, vban::ipc::access_role> roles;
