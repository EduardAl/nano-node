...
size_t constexpr vban::active_transactions::max_active_elections_frontier_insertion;

constexpr std::chrono::minutes vban::active_transactions::expired_optimistic_election_info_cutoff;

vban::active_transactions::active_transactions (vban::node & node_a, vban::confirmation_height_processor & confirmation_height_processor_a) :
	...
	})
{
	...
	confirmation_height_processor.add_cemented_observer ([this] (std::shared_ptr<vban::block> const & callback_block_a) {
		this->block_cemented_callback (callback_block_a);
	});

	// Register a callback which will get called if a block is already cemented
	confirmation_height_processor.add_block_already_cemented_observer ([this] (vban::block_hash const & hash_a) {
...
vban::active_transactions::~active_transactions ()
{
	stop ();
}

bool vban::active_transactions::insert_election_from_frontiers_confirmation (std::shared_ptr<vban::block> const & block_a, vban::account const & account_a, vban::uint256_t previous_balance_a, vban::election_behavior election_behavior_a)
{
	bool inserted{ false };
	vban::unique_lock<vban::mutex> lock (mutex);
	if (roots.get<tag_root> ().find (block_a->qualified_root ()) == roots.get<tag_root> ().end ())
	{
		std::function<void (std::shared_ptr<vban::block> const &)> election_confirmation_cb;
		if (election_behavior_a == vban::election_behavior::optimistic)
		{
			election_confirmation_cb = [this] (std::shared_ptr<vban::block> const & block_a) {
...
vban::frontiers_confirmation_info vban::active_transactions::get_frontiers_confirmation_info ()
{
	...
	bool agressive_mode (half_princpal_representative || node.config.frontiers_confirmation == vban::frontiers_confirmation_mode::always);
	...
	return vban::frontiers_confirmation_info{ max_elections, agressive_mode };
}

void vban::active_transactions::set_next_frontier_check (bool agressive_mode_a)
{
...
void vban::active_transactions::confirm_prioritized_frontiers (vban::transaction const & transaction_a, uint64_t max_elections_a, uint64_t & elections_count_a)
{
				...
				if (!error)
				{
					if (!this->confirmation_height_processor.is_processing_block (info.head))
					{
						...
						if (info.block_count > confirmation_height_info.height)
						{
							...
							auto inserted_election = this->insert_election_from_frontiers_confirmation (block, cementable_account.account, previous_balance, vban::election_behavior::optimistic);
...
void vban::active_transactions::block_cemented_callback (std::shared_ptr<vban::block> const & block_a)
{
	auto transaction = node.store.tx_begin_read ();

	boost::optional<vban::election_status_type> election_status_type;
	...
	else
	{
		...
		election_status_type = vban::election_status_type::active_confirmed_quorum;
	}

	if (election_status_type.is_initialized ())
	{
		if (election_status_type == vban::election_status_type::inactive_confirmation_height)
		{
			vban::account account (0);
			vban::uint256_t amount (0);
			bool is_state_send (false);
			vban::account pending_account (0);
			node.process_confirmed_data (transaction, block_a, block_a->hash (), account, amount, is_state_send, pending_account);
			node.observers.blocks.notify (vban::election_status{ block_a, 0, 0, std::chrono::duration_cast<std::chrono::milliseconds> (std::chrono::system_clock::now ().time_since_epoch ()), std::chrono::duration_values<std::chrono::milliseconds>::zero (), 0, 1, 0, vban::election_status_type::inactive_confirmation_height }, {}, account, amount, is_state_send);
		}
		else
		{
			auto hash (block_a->hash ());
			vban::unique_lock<vban::mutex> election_winners_lk (election_winner_details_mutex);
			...
			if (existing != election_winner_details.end ())
			{
				...
				{
					vban::unique_lock<vban::mutex> election_lk (election->mutex);
		...
		bool const was_active{ *election_status_type == vban::election_status_type::active_confirmed_quorum || *election_status_type == vban::election_status_type::active_confirmation_height };

		// Activations are only done if there is not a large amount of active elections, ensuring frontier confirmation takes place
		auto const low_active_elections = [this] { return this->size () < vban::active_transactions::max_active_elections_frontier_insertion / 2; };
...
void vban::active_transactions::add_election_winner_details (vban::block_hash const & hash_a, std::shared_ptr<vban::election> const & election_a)
{
	vban::lock_guard<vban::mutex> guard (election_winner_details_mutex);
	election_winner_details.emplace (hash_a, election_a);
}

void vban::active_transactions::remove_election_winner_details (vban::block_hash const & hash_a)
{
	vban::lock_guard<vban::mutex> guard (election_winner_details_mutex);
	election_winner_details.erase (hash_a);
}

void vban::active_transactions::block_already_cemented_callback (vban::block_hash const & hash_a)
{
...
int64_t vban::active_transactions::vacancy () const
{
...
void vban::active_transactions::request_confirm (vban::unique_lock<vban::mutex> & lock_a)
{
	...
	vban::confirmation_solicitor solicitor (node.network, node.config);
	solicitor.prepare (node.rep_crawler.principal_representatives (std::numeric_limits<size_t>::max ()));
	vban::vote_generator_session generator_session (generator);
	vban::vote_generator_session final_generator_session (generator);

	auto const election_ttl_cutoff_l (std::chrono::steady_clock::now () - election_time_to_live);
	size_t unconfirmed_count_l (0);
	vban::timer<std::chrono::milliseconds> elapsed (vban::timer_state::started);
	...
	for (auto const & election_l : elections_l)
	{
		...
		if (election_l->transition_time (solicitor))
		{
			...
			if (!confirmed_l)
			{
				node.stats.inc (vban::stat::type::election, vban::stat::detail::election_drop_expired);
			}
...
void vban::active_transactions::cleanup_election (vban::unique_lock<vban::mutex> & lock_a, vban::election const & election)
{
	if (!election.confirmed ())
	{
		node.stats.inc (vban::stat::type::election, vban::stat::detail::election_drop_all);
	}
...
std::vector<std::shared_ptr<vban::election>> vban::active_transactions::list_active (size_t max_a)
{
	vban::lock_guard<vban::mutex> guard (mutex);
	return list_active_impl (max_a);
}

std::vector<std::shared_ptr<vban::election>> vban::active_transactions::list_active_impl (size_t max_a) const
{
	std::vector<std::shared_ptr<vban::election>> result_l;
...
void vban::active_transactions::add_expired_optimistic_election (vban::election const & election_a)
{
	vban::lock_guard<vban::mutex> guard (mutex);
...
unsigned vban::active_transactions::max_optimistic ()
{
	return node.ledger.cache.cemented_count < node.ledger.bootstrap_weight_max_blocks ? std::numeric_limits<unsigned>::max () : 50u;
}

void vban::active_transactions::frontiers_confirmation (vban::unique_lock<vban::mutex> & lock_a)
{
 ...
 * confirmed.
 */
void vban::active_transactions::confirm_expired_frontiers_pessimistically (vban::transaction const & transaction_a, uint64_t max_elections_a, uint64_t & elections_count_a)
{
	auto i{ node.store.accounts_begin (transaction_a, next_frontier_account) };
	auto n{ node.store.accounts_end () };
	vban::timer<std::chrono::milliseconds> timer (vban::timer_state::started);
	vban::confirmation_height_info confirmation_height_info;

	// Loop through any expired optimistic elections which have not been started yet. This tag stores already started ones first
	std::vector<vban::account> elections_started_for_account;
	for (auto i = expired_optimistic_election_infos.get<tag_election_started> ().lower_bound (false); i != expired_optimistic_election_infos.get<tag_election_started> ().end ();)
	{
		...
		if (!node.store.account_get (transaction_a, account, account_info))
		{
			...
			{
				should_delete = false;
				std::shared_ptr<vban::block> previous_block;
				...
				{
					vban::uint256_t previous_balance{ 0 };
					if (previous_block && previous_block->balance ().is_zero ())
					{
						previous_balance = previous_block->sideband ().balance.number ();
					}

					auto inserted_election = insert_election_from_frontiers_confirmation (block, account, previous_balance, vban::election_behavior::normal);
					if (inserted_election)
					{
						++elections_count_a;
					}
...
bool vban::active_transactions::should_do_frontiers_confirmation () const
{
	auto pending_confirmation_height_size (confirmation_height_processor.awaiting_processing_size ());
	auto disabled_confirmation_mode = (node.config.frontiers_confirmation == vban::frontiers_confirmation_mode::disabled);
...
void vban::active_transactions::request_loop ()
{
	vban::unique_lock<vban::mutex> lock (mutex);
	started = true;
	lock.unlock ();
...
bool vban::active_transactions::prioritize_account_for_confirmation (vban::active_transactions::prioritize_num_uncemented & cementable_frontiers_a, size_t & cementable_frontiers_size_a, vban::account const & account_a, vban::account_info const & info_a, uint64_t confirmation_height_a)
{
	...
	if (info_a.block_count > confirmation_height_a && !confirmation_height_processor.is_processing_block (info_a.head))
	{
		...
		if (it != cementable_frontiers_a.get<tag_account> ().end ())
		{
			if (it->blocks_uncemented != num_uncemented)
			{
				...
				cementable_frontiers_a.get<tag_account> ().modify (it, [num_uncemented] (vban::cementable_account & info) {
...
void vban::active_transactions::prioritize_frontiers_for_confirmation (vban::transaction const & transaction_a, std::chrono::milliseconds ledger_account_traversal_max_time_a, std::chrono::milliseconds wallet_account_traversal_max_time_a)
{
	...
	if (confirmation_height_processor.awaiting_processing_size () < confirmed_frontiers_max_pending_size)
	{
		...
		vban::timer<std::chrono::milliseconds> wallet_account_timer (vban::timer_state::started);
		...
		if (!skip_wallets)
		{
			...
			{
				vban::lock_guard<vban::mutex> lock (node.wallets.mutex);
...
void vban::active_transactions::stop ()
{
	vban::unique_lock<vban::mutex> lock (mutex);
	if (!started)
	{
...
vban::election_insertion_result vban::active_transactions::insert_impl (vban::unique_lock<vban::mutex> & lock_a, std::shared_ptr<vban::block> const & block_a, boost::optional<vban::uint256_t> const & previous_balance_a, vban::election_behavior election_behavior_a, std::function<void (std::shared_ptr<vban::block> const &)> const & confirmation_action_a)
{
	debug_assert (lock_a.owns_lock ());
	debug_assert (block_a->has_sideband ());
	vban::election_insertion_result result;
	if (!stopped)
	{
		...
		if (existing == roots.get<tag_root> ().end ())
		{
			if (recently_confirmed.get<tag_root> ().find (root) == recently_confirmed.get<tag_root> ().end ())
			{
				...
				vban::uint256_t previous_balance (previous_balance_a.value_or (0));
				...
				result.election = vban::make_shared<vban::election> (
				node, block_a, confirmation_action_a, [&node = node] (auto const & rep_a) {
					// Representative is defined as online if replying to live votes or rep_crawler queries
					node.online_reps.observe (rep_a);
				},
				election_behavior_a);
				roots.get<tag_root> ().emplace (vban::active_transactions::conflict_info{ root, result.election, epoch, previous_balance });
				blocks.emplace (hash, result.election);
				auto const cache = find_inactive_votes_cache_impl (hash);
				lock_a.unlock ();
				result.election->insert_inactive_votes_cache (cache);
				node.stats.inc (vban::stat::type::election, vban::stat::detail::election_start);
				vacancy_update ();
			}
		}
		else
...
vban::vote_code vban::active_transactions::vote (std::shared_ptr<vban::vote> const & vote_a)
{
	vban::vote_code result{ vban::vote_code::indeterminate };
	// If all hashes were recently confirmed then it is a replay
	unsigned recently_confirmed_counter (0);
	std::vector<std::pair<std::shared_ptr<vban::election>, vban::block_hash>> process;
	{
		vban::unique_lock<vban::mutex> lock (mutex);
		for (auto vote_block : vote_a->blocks)
		{
			...
			if (vote_block.which ())
			{
				auto const & block_hash (boost::get<vban::block_hash> (vote_block));
			...
			{
				auto block (boost::get<std::shared_ptr<vban::block>> (vote_block));
	...
	if (!process.empty ())
	{
		...
		result = replay ? vban::vote_code::replay : vban::vote_code::vote;
	}
	else if (recently_confirmed_counter == vote_a->blocks.size ())
	{
		result = vban::vote_code::replay;
	}
	return result;
}

bool vban::active_transactions::active (vban::qualified_root const & root_a)
{
	vban::lock_guard<vban::mutex> lock (mutex);
	return roots.get<tag_root> ().find (root_a) != roots.get<tag_root> ().end ();
}

bool vban::active_transactions::active (vban::block const & block_a)
{
	vban::lock_guard<vban::mutex> guard (mutex);
	return roots.get<tag_root> ().find (block_a.qualified_root ()) != roots.get<tag_root> ().end () && blocks.find (block_a.hash ()) != blocks.end ();
}

std::shared_ptr<vban::election> vban::active_transactions::election (vban::qualified_root const & root_a) const
{
	std::shared_ptr<vban::election> result;
	vban::lock_guard<vban::mutex> lock (mutex);
...
std::shared_ptr<vban::block> vban::active_transactions::winner (vban::block_hash const & hash_a) const
{
...
void vban::active_transactions::restart (vban::transaction const & transaction_a, std::shared_ptr<vban::block> const & block_a)
{
	auto hash (block_a->hash ());
	...
	if (ledger_block != nullptr && ledger_block->block_work () != block_a->block_work () && !node.block_confirmed_or_being_confirmed (transaction_a, hash))
	{
		if (block_a->difficulty () > ledger_block->difficulty ())
		{
			...
			{
				node.stats.inc (vban::stat::type::election, vban::stat::detail::election_restart);
				auto previous_balance = node.ledger.balance (transaction_a, ledger_block->previous ());
				auto block_has_account = ledger_block->type () == vban::block_type::state || ledger_block->type () == vban::block_type::open;
...
std::deque<vban::election_status> vban::active_transactions::list_recently_cemented ()
{
	vban::lock_guard<vban::mutex> lock (mutex);
	return recently_cemented;
}

void vban::active_transactions::add_recently_cemented (vban::election_status const & status_a)
{
	vban::lock_guard<vban::mutex> guard (mutex);
...
void vban::active_transactions::add_recently_confirmed (vban::qualified_root const & root_a, vban::block_hash const & hash_a)
{
	vban::lock_guard<vban::mutex> guard (mutex);
...
void vban::active_transactions::erase_recently_confirmed (vban::block_hash const & hash_a)
{
	vban::lock_guard<vban::mutex> guard (mutex);
	recently_confirmed.get<tag_hash> ().erase (hash_a);
}

void vban::active_transactions::erase (vban::block const & block_a)
{
	erase (block_a.qualified_root ());
}

void vban::active_transactions::erase (vban::qualified_root const & root_a)
{
	vban::unique_lock<vban::mutex> lock (mutex);
...
void vban::active_transactions::erase_hash (vban::block_hash const & hash_a)
{
	vban::unique_lock<vban::mutex> lock (mutex);
...
void vban::active_transactions::erase_oldest ()
{
	vban::unique_lock<vban::mutex> lock (mutex);
	if (!roots.empty ())
	{
		node.stats.inc (vban::stat::type::election, vban::stat::detail::election_drop_overflow);
		auto item = roots.get<tag_random_access> ().front ();
		cleanup_election (lock, *item.election);
	}
}

bool vban::active_transactions::empty ()
{
	vban::lock_guard<vban::mutex> lock (mutex);
	return roots.empty ();
}

size_t vban::active_transactions::size ()
{
	vban::lock_guard<vban::mutex> lock (mutex);
	return roots.size ();
}

bool vban::active_transactions::publish (std::shared_ptr<vban::block> const & block_a)
{
	vban::unique_lock<vban::mutex> lock (mutex);
	...
	if (existing != roots.get<tag_root> ().end ())
	{
		...
		if (!result)
		{
			...
			node.stats.inc (vban::stat::type::election, vban::stat::detail::election_block_conflict);
		}
	}
	return result;
}
...
boost::optional<vban::election_status_type> vban::active_transactions::confirm_block (vban::transaction const & transaction_a, std::shared_ptr<vban::block> const & block_a)
{
	auto hash (block_a->hash ());
	vban::unique_lock<vban::mutex> lock (mutex);
	auto existing (blocks.find (hash));
	boost::optional<vban::election_status_type> status_type;
	if (existing != blocks.end ())
	{
		lock.unlock ();
		vban::unique_lock<vban::mutex> election_lock (existing->second->mutex);
		if (existing->second->status.winner && existing->second->status.winner->hash () == hash)
		{
			if (!existing->second->confirmed ())
			{
				existing->second->confirm_once (election_lock, vban::election_status_type::active_confirmation_height);
				status_type = vban::election_status_type::active_confirmation_height;
			}
			else
			{
#ifndef NDEBUG
...
#endif
				status_type = vban::election_status_type::active_confirmed_quorum;
			}
		}
		else
		{
			status_type = boost::optional<vban::election_status_type>{};
		}
	}
	else
	{
		status_type = vban::election_status_type::inactive_confirmation_height;
	}

	return status_type;
}

size_t vban::active_transactions::priority_cementable_frontiers_size ()
{
	vban::lock_guard<vban::mutex> guard (mutex);
	return priority_cementable_frontiers.size ();
}

size_t vban::active_transactions::priority_wallet_cementable_frontiers_size ()
{
	vban::lock_guard<vban::mutex> guard (mutex);
	return priority_wallet_cementable_frontiers.size ();
}

size_t vban::active_transactions::inactive_votes_cache_size ()
{
	vban::lock_guard<vban::mutex> guard (mutex);
	return inactive_votes_cache.size ();
}

void vban::active_transactions::add_inactive_votes_cache (vban::unique_lock<vban::mutex> & lock_a, vban::block_hash const & hash_a, vban::account const & representative_a, uint64_t const timestamp_a)
{
	...
	if (node.ledger.weight (representative_a) > node.minimum_principal_weight ())
	{
		...
		if (existing != inactive_by_hash.end ())
		{
			...
			{
				auto is_new (false);
				inactive_by_hash.modify (existing, [representative_a, timestamp_a, &is_new] (vban::inactive_cache_information & info) {
				...
				if (is_new)
				{
					...
					if (status != old_status)
					{
						...
						if (existing != inactive_by_hash.end ())
						{
							inactive_by_hash.modify (existing, [status] (vban::inactive_cache_information & info) {
								info.status = status;
							});
						}
					}
		...
		else
		{
			...
			inactive_by_arrival.emplace (vban::inactive_cache_information{ std::chrono::steady_clock::now (), hash_a, representative_a, timestamp_a, default_status });
...
void vban::active_transactions::trigger_inactive_votes_cache_election (std::shared_ptr<vban::block> const & block_a)
{
	vban::unique_lock<vban::mutex> lock (mutex);
...
vban::inactive_cache_information vban::active_transactions::find_inactive_votes_cache (vban::block_hash const & hash_a)
{
	vban::lock_guard<vban::mutex> guard (mutex);
	return find_inactive_votes_cache_impl (hash_a);
}

vban::inactive_cache_information vban::active_transactions::find_inactive_votes_cache_impl (vban::block_hash const & hash_a)
{
	auto & inactive_by_hash (inactive_votes_cache.get<tag_hash> ());
	auto existing (inactive_by_hash.find (hash_a));
	if (existing != inactive_by_hash.end ())
	...
	{
		return vban::inactive_cache_information{};
	}
}

void vban::active_transactions::erase_inactive_votes_cache (vban::block_hash const & hash_a)
{
	inactive_votes_cache.get<tag_hash> ().erase (hash_a);
}

vban::inactive_cache_status vban::active_transactions::inactive_votes_bootstrap_check (vban::unique_lock<vban::mutex> & lock_a, vban::account const & voter_a, vban::block_hash const & hash_a, vban::inactive_cache_status const & previously_a)
{
	debug_assert (lock_a.owns_lock ());
	lock_a.unlock ();
...
vban::inactive_cache_status vban::active_transactions::inactive_votes_bootstrap_check (vban::unique_lock<vban::mutex> & lock_a, std::vector<std::pair<vban::account, uint64_t>> const & voters_a, vban::block_hash const & hash_a, vban::inactive_cache_status const & previously_a)
{
...
vban::inactive_cache_status vban::active_transactions::inactive_votes_bootstrap_check_impl (vban::unique_lock<vban::mutex> & lock_a, vban::uint256_t const & tally_a, size_t voters_size_a, vban::block_hash const & hash_a, vban::inactive_cache_status const & previously_a)
{
	debug_assert (!lock_a.owns_lock ());
	vban::inactive_cache_status status (previously_a);
...
bool vban::purge_singleton_inactive_votes_cache_pool_memory ()
{
	return boost::singleton_pool<boost::fast_pool_allocator_tag, sizeof (vban::active_transactions::ordered_cache::node_type)>::purge_memory ();
}

size_t vban::active_transactions::election_winner_details_size ()
{
	vban::lock_guard<vban::mutex> guard (election_winner_details_mutex);
	return election_winner_details.size ();
}

vban::cementable_account::cementable_account (vban::account const & account_a, size_t blocks_uncemented_a) :
	account (account_a), blocks_uncemented (blocks_uncemented_a)
{
}

vban::expired_optimistic_election_info::expired_optimistic_election_info (std::chrono::steady_clock::time_point expired_time_a, vban::account account_a) :
	expired_time (expired_time_a),
	account (account_a)
{
}

bool vban::frontiers_confirmation_info::can_start_elections () const
{
	return max_elections > 0;
}

std::unique_ptr<vban::container_info_component> vban::collect_container_info (active_transactions & active_transactions, std::string const & name)
{
	size_t roots_count;
	size_t blocks_count;
	...
	{
		vban::lock_guard<vban::mutex> guard (active_transactions.mutex);
	...
	composite->add_component (std::make_unique<container_info_leaf> (container_info{ "priority_wallet_cementable_frontiers", active_transactions.priority_wallet_cementable_frontiers_size (), sizeof (vban::cementable_account) }));
	composite->add_component (std::make_unique<container_info_leaf> (container_info{ "priority_cementable_frontiers", active_transactions.priority_cementable_frontiers_size (), sizeof (vban::cementable_account) }));
	...
	composite->add_component (std::make_unique<container_info_leaf> (container_info{ "inactive_votes_cache", active_transactions.inactive_votes_cache_size (), sizeof (vban::gap_information) }));
