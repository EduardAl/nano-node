...
TEST (network, self_discard)
{
	...
	vban::system system (1, vban::transport::transport_type::tcp, node_flags);
	vban::message_buffer data;
	data.endpoint = system.nodes[0]->network.endpoint ();
	ASSERT_EQ (0, system.nodes[0]->stats.count (vban::stat::type::error, vban::stat::detail::bad_sender));
	system.nodes[0]->network.udp_channels.receive_action (&data);
	ASSERT_EQ (1, system.nodes[0]->stats.count (vban::stat::type::error, vban::stat::detail::bad_sender));
}
...
TEST (network, send_node_id_handshake)
{
	...
	auto initial (node0->stats.count (vban::stat::type::message, vban::stat::detail::node_id_handshake, vban::stat::dir::in));
	auto initial_node1 (node1->stats.count (vban::stat::type::message, vban::stat::detail::node_id_handshake, vban::stat::dir::in));
	auto channel (std::make_shared<vban::transport::channel_udp> (node0->network.udp_channels, node1->network.endpoint (), node1->network_params.protocol.protocol_version));
	node0->network.send_keepalive (channel);
	ASSERT_EQ (0, node0->network.size ());
	ASSERT_EQ (0, node1->network.size ());
	ASSERT_TIMELY (10s, node1->stats.count (vban::stat::type::message, vban::stat::detail::node_id_handshake, vban::stat::dir::in) != initial_node1);
	ASSERT_TIMELY (10s, node0->network.size () == 0 || node1->network.size () == 1);
	ASSERT_TIMELY (10s, node0->stats.count (vban::stat::type::message, vban::stat::detail::node_id_handshake, vban::stat::dir::in) == initial + 2);
...
TEST (network, send_node_id_handshake_tcp)
{
	...
	auto initial (node0->stats.count (vban::stat::type::message, vban::stat::detail::node_id_handshake, vban::stat::dir::in));
	auto initial_node1 (node1->stats.count (vban::stat::type::message, vban::stat::detail::node_id_handshake, vban::stat::dir::in));
	auto initial_keepalive (node0->stats.count (vban::stat::type::message, vban::stat::detail::keepalive, vban::stat::dir::in));
	std::weak_ptr<vban::node> node_w (node0);
	node0->network.tcp_channels.start_tcp (node1->network.endpoint (), vban::keepalive_tcp_callback (*node1));
	ASSERT_EQ (0, node0->network.size ());
	ASSERT_EQ (0, node1->network.size ());
	ASSERT_TIMELY (10s, node0->stats.count (vban::stat::type::message, vban::stat::detail::node_id_handshake, vban::stat::dir::in) >= initial + 2);
	ASSERT_TIMELY (5s, node1->stats.count (vban::stat::type::message, vban::stat::detail::node_id_handshake, vban::stat::dir::in) >= initial_node1 + 2);
	ASSERT_TIMELY (5s, node0->stats.count (vban::stat::type::message, vban::stat::detail::keepalive, vban::stat::dir::in) >= initial_keepalive + 2);
	ASSERT_TIMELY (5s, node1->stats.count (vban::stat::type::message, vban::stat::detail::keepalive, vban::stat::dir::in) >= initial_keepalive + 2);
...
TEST (network, last_contacted)
{
	...
	auto node0 = system.nodes[0];
	ASSERT_EQ (0, node0->network.size ());
	vban::node_config node1_config (vban::get_available_port (), system.logging);
	node1_config.tcp_incoming_connections_max = 0; // Prevent ephemeral node1->node0 channel repacement with incoming connection
	auto node1 (std::make_shared<vban::node> (system.io_ctx, vban::unique_path (), node1_config, system.work));
	node1->start ();
	system.nodes.push_back (node1);
	auto channel1 = vban::establish_tcp (system, *node1, node0->network.endpoint ());
	...
	auto keepalive_count = node0->stats.count (vban::stat::type::message, vban::stat::detail::keepalive, vban::stat::dir::in);
	node1->network.send_keepalive (channel1);
	ASSERT_TIMELY (3s, node0->stats.count (vban::stat::type::message, vban::stat::detail::keepalive, vban::stat::dir::in) > keepalive_count);
...
TEST (network, multi_keepalive)
{
	...
	ASSERT_EQ (0, node1->network.size ());
	ASSERT_EQ (0, node0->network.size ());
	node1->network.tcp_channels.start_tcp (node0->network.endpoint (), vban::keepalive_tcp_callback (*node1));
	ASSERT_TIMELY (10s, node0->network.size () == 1 && node0->stats.count (vban::stat::type::message, vban::stat::detail::keepalive) >= 1);
	...
	node2->network.tcp_channels.start_tcp (node0->network.endpoint (), vban::keepalive_tcp_callback (*node2));
	ASSERT_TIMELY (10s, node1->network.size () == 2 && node0->network.size () == 2 && node2->network.size () == 2 && node0->stats.count (vban::stat::type::message, vban::stat::detail::keepalive) >= 2);
}
...
TEST (network, send_discarded_publish)
{
	...
	vban::genesis genesis;
	{
		...
		ASSERT_EQ (genesis.hash (), node1.ledger.latest (transaction, vban::dev_genesis_key.pub));
		ASSERT_EQ (genesis.hash (), node2.latest (vban::dev_genesis_key.pub));
	}
	ASSERT_TIMELY (10s, node2.stats.count (vban::stat::type::message, vban::stat::detail::publish, vban::stat::dir::in) != 0);
	auto transaction (node1.store.tx_begin_read ());
	ASSERT_EQ (genesis.hash (), node1.ledger.latest (transaction, vban::dev_genesis_key.pub));
	ASSERT_EQ (genesis.hash (), node2.latest (vban::dev_genesis_key.pub));
}
...
TEST (network, send_invalid_publish)
{
	...
	auto & node2 (*system.nodes[1]);
	vban::genesis genesis;
	auto block (std::make_shared<vban::send_block> (1, 1, 20, vban::dev_genesis_key.prv, vban::dev_genesis_key.pub, *system.work.generate (vban::root (1))));
	{
		auto transaction (node1.store.tx_begin_read ());
		node1.network.flood_block (block);
		ASSERT_EQ (genesis.hash (), node1.ledger.latest (transaction, vban::dev_genesis_key.pub));
...
TEST (network, send_valid_confirm_ack)
{
	...
	for (auto & type : types)
	{
		...
		vban::send_block block2 (latest1, key2.pub, 50, vban::dev_genesis_key.prv, vban::dev_genesis_key.pub, *system.work.generate (latest1));
		vban::block_hash latest2 (node2.latest (vban::dev_genesis_key.pub));
		node1.process_active (std::make_shared<vban::send_block> (block2));
		// Keep polling until latest block changes
		ASSERT_TIMELY (10s, node2.latest (vban::dev_genesis_key.pub) != latest2);
		// Make sure the balance has decreased after processing the block.
		ASSERT_EQ (50, node2.balance (vban::dev_genesis_key.pub));
	}
}
...
TEST (network, send_valid_publish)
{
	...
	for (auto & type : types)
	{
		...
		vban::block_hash latest2 (node2.latest (vban::dev_genesis_key.pub));
		node2.process_active (std::make_shared<vban::send_block> (block2));
		ASSERT_TIMELY (10s, node1.stats.count (vban::stat::type::message, vban::stat::detail::publish, vban::stat::dir::in) != 0);
...
TEST (network, send_insufficient_work_udp)
{
	...
	auto block (std::make_shared<vban::send_block> (0, 1, 20, vban::dev_genesis_key.prv, vban::dev_genesis_key.pub, 0));
	vban::publish publish (block);
	vban::transport::channel_udp channel (node1.network.udp_channels, node2.network.endpoint (), node1.network_params.protocol.protocol_version);
	channel.send (publish, [] (boost::system::error_code const & ec, size_t size) {});
	ASSERT_EQ (0, node1.stats.count (vban::stat::type::error, vban::stat::detail::insufficient_work));
	ASSERT_TIMELY (10s, node2.stats.count (vban::stat::type::error, vban::stat::detail::insufficient_work) != 0);
	ASSERT_EQ (1, node2.stats.count (vban::stat::type::error, vban::stat::detail::insufficient_work));
}
...
TEST (network, send_insufficient_work)
{
	...
	auto & node2 = *system.nodes[1];
	// Block zero work
	auto block1 (std::make_shared<vban::send_block> (0, 1, 20, vban::dev_genesis_key.prv, vban::dev_genesis_key.pub, 0));
	vban::publish publish1 (block1);
	auto tcp_channel (node1.network.tcp_channels.find_channel (vban::transport::map_endpoint_to_tcp (node2.network.endpoint ())));
	tcp_channel->send (publish1, [] (boost::system::error_code const & ec, size_t size) {});
	ASSERT_EQ (0, node1.stats.count (vban::stat::type::error, vban::stat::detail::insufficient_work));
	ASSERT_TIMELY (10s, node2.stats.count (vban::stat::type::error, vban::stat::detail::insufficient_work) != 0);
	ASSERT_EQ (1, node2.stats.count (vban::stat::type::error, vban::stat::detail::insufficient_work));
	// Legacy block work between epoch_2_recieve & epoch_1
	auto block2 (std::make_shared<vban::send_block> (block1->hash (), 1, 20, vban::dev_genesis_key.prv, vban::dev_genesis_key.pub, system.work_generate_limited (block1->hash (), node1.network_params.network.publish_thresholds.epoch_2_receive, node1.network_params.network.publish_thresholds.epoch_1 - 1)));
	vban::publish publish2 (block2);
	tcp_channel->send (publish2, [] (boost::system::error_code const & ec, size_t size) {});
	ASSERT_TIMELY (10s, node2.stats.count (vban::stat::type::error, vban::stat::detail::insufficient_work) != 1);
	ASSERT_EQ (2, node2.stats.count (vban::stat::type::error, vban::stat::detail::insufficient_work));
	// Legacy block work epoch_1
	auto block3 (std::make_shared<vban::send_block> (block2->hash (), 1, 20, vban::dev_genesis_key.prv, vban::dev_genesis_key.pub, *system.work.generate (block2->hash (), node1.network_params.network.publish_thresholds.epoch_2)));
	vban::publish publish3 (block3);
	tcp_channel->send (publish3, [] (boost::system::error_code const & ec, size_t size) {});
	ASSERT_EQ (0, node2.stats.count (vban::stat::type::message, vban::stat::detail::publish, vban::stat::dir::in));
	ASSERT_TIMELY (10s, node2.stats.count (vban::stat::type::message, vban::stat::detail::publish, vban::stat::dir::in) != 0);
	ASSERT_EQ (1, node2.stats.count (vban::stat::type::message, vban::stat::detail::publish, vban::stat::dir::in));
	// State block work epoch_2_recieve
	auto block4 (std::make_shared<vban::state_block> (vban::dev_genesis_key.pub, block1->hash (), vban::dev_genesis_key.pub, 20, 1, vban::dev_genesis_key.prv, vban::dev_genesis_key.pub, system.work_generate_limited (block1->hash (), node1.network_params.network.publish_thresholds.epoch_2_receive, node1.network_params.network.publish_thresholds.epoch_1 - 1)));
	vban::publish publish4 (block4);
...
TEST (receivable_processor, confirm_insufficient_pos)
{
	...
	auto & node1 (*system.nodes[0]);
	vban::genesis genesis;
	auto block1 (std::make_shared<vban::send_block> (genesis.hash (), 0, 0, vban::dev_genesis_key.prv, vban::dev_genesis_key.pub, 0));
	node1.work_generate_blocking (*block1);
	ASSERT_EQ (vban::process_result::progress, node1.process (*block1).code);
	node1.scheduler.activate (vban::dev_genesis_key.pub, node1.store.tx_begin_read ());
	vban::keypair key1;
	auto vote (std::make_shared<vban::vote> (key1.pub, key1.prv, 0, block1));
...
TEST (receivable_processor, confirm_sufficient_pos)
{
	...
	ASSERT_EQ (vban::process_result::progress, node1.process (*block1).code);
	node1.scheduler.activate (vban::dev_genesis_key.pub, node1.store.tx_begin_read ());
	auto vote (std::make_shared<vban::vote> (vban::dev_genesis_key.pub, vban::dev_genesis_key.prv, 0, block1));
	vban::confirm_ack con1 (vote);
	node1.network.process_message (con1, node1.network.udp_channels.create (node1.network.endpoint ()));
}
...
TEST (receivable_processor, send_with_receive)
{
	...
	for (auto & type : types)
	{
		...
		vban::block_hash latest1 (node1.latest (vban::dev_genesis_key.pub));
		system.wallet (1)->insert_adhoc (key2.prv);
		auto block1 (std::make_shared<vban::send_block> (latest1, key2.pub, amount - node1.config.receive_minimum.number (), vban::dev_genesis_key.prv, vban::dev_genesis_key.pub, *system.work.generate (latest1)));
		ASSERT_EQ (amount, node1.balance (vban::dev_genesis_key.pub));
		ASSERT_EQ (0, node1.balance (key2.pub));
		ASSERT_EQ (amount, node2.balance (vban::dev_genesis_key.pub));
		ASSERT_EQ (0, node2.balance (key2.pub));
		node1.process_active (block1);
		node1.block_processor.flush ();
		node2.process_active (block1);
		node2.block_processor.flush ();
		ASSERT_EQ (amount - node1.config.receive_minimum.number (), node1.balance (vban::dev_genesis_key.pub));
		ASSERT_EQ (0, node1.balance (key2.pub));
		ASSERT_EQ (amount - node1.config.receive_minimum.number (), node2.balance (vban::dev_genesis_key.pub));
		ASSERT_EQ (0, node2.balance (key2.pub));
...
TEST (network, receive_weight_change)
{
	...
	ASSERT_TIMELY (10s, std::all_of (system.nodes.begin (), system.nodes.end (), [&] (std::shared_ptr<vban::node> const & node_a) { return node_a->weight (key2.pub) == system.nodes[0]->config.receive_minimum.number (); }));
}
...
TEST (parse_endpoint, valid)
{
	...
	ASSERT_FALSE (vban::parse_endpoint (string, endpoint));
...
TEST (parse_endpoint, invalid_port)
{
	...
	vban::endpoint endpoint;
	ASSERT_TRUE (vban::parse_endpoint (string, endpoint));
}
...
TEST (network, ipv6)
{
	...
	vban::endpoint endpoint1 (address, 16384);
	std::vector<uint8_t> bytes1;
	{
		vban::vectorstream stream (bytes1);
		vban::write (stream, address.to_bytes ());
	}
	...
	vban::bufferstream stream (bytes1.data (), bytes1.size ());
	auto error (vban::try_read (stream, bytes2));
	ASSERT_FALSE (error);
	vban::endpoint endpoint2 (boost::asio::ip::address_v6 (bytes2), 16384);
	ASSERT_EQ (endpoint1, endpoint2);
}

TEST (network, ipv6_from_ipv4)
{
	vban::endpoint endpoint1 (boost::asio::ip::address_v4::loopback (), 16000);
	ASSERT_TRUE (endpoint1.address ().is_v4 ());
	vban::endpoint endpoint2 (boost::asio::ip::address_v6::v4_mapped (endpoint1.address ().to_v4 ()), 16000);
...
{
	boost::asio::io_context io_ctx;
	auto port1 = vban::get_available_port ();
	auto port2 = vban::get_available_port ();
	vban::endpoint endpoint1 (boost::asio::ip::address_v6::any (), port1);
	vban::endpoint endpoint2 (boost::asio::ip::address_v4::any (), port2);
	std::array<uint8_t, 16> bytes1;
	auto finish1 (false);
	vban::endpoint endpoint3;
	...
	vban::endpoint endpoint5 (boost::asio::ip::address_v4::loopback (), port1);
	vban::endpoint endpoint6 (boost::asio::ip::address_v6::v4_mapped (boost::asio::ip::address_v4::loopback ()), port2);
...
TEST (network, reserved_address)
{
	vban::system system (1);
	// 0 port test
	ASSERT_TRUE (vban::transport::reserved_address (vban::endpoint (boost::asio::ip::make_address_v6 ("2001::"), 0)));
	// Valid address test
	ASSERT_FALSE (vban::transport::reserved_address (vban::endpoint (boost::asio::ip::make_address_v6 ("2001::"), 1)));
	vban::endpoint loopback (boost::asio::ip::make_address_v6 ("::1"), 1);
	ASSERT_FALSE (vban::transport::reserved_address (loopback));
	vban::endpoint private_network_peer (boost::asio::ip::make_address_v6 ("::ffff:10.0.0.0"), 1);
	ASSERT_TRUE (vban::transport::reserved_address (private_network_peer, false));
	ASSERT_FALSE (vban::transport::reserved_address (private_network_peer, true));
}
...
TEST (network, ipv6_bind_subnetwork)
{
	...
	auto address1_subnet (vban::transport::ipv4_address_or_ipv6_subnet (address1));
	...
	auto address2_subnet (vban::transport::ipv4_address_or_ipv6_subnet (address2));
...
TEST (network, network_range_ipv6)
{
	...
	auto address3_subnet (vban::transport::map_address_to_subnetwork (address3));
...
TEST (network, network_range_ipv4)
{
	...
	auto address2_subnet (vban::transport::map_address_to_subnetwork (address2));
...
{
	vban::stat stats;
	vban::message_buffer_manager buffer (stats, 512, 1);
...
{
	vban::stat stats;
	vban::message_buffer_manager buffer (stats, 512, 2);
	auto buffer1 (buffer.allocate ());
	ASSERT_NE (nullptr, buffer1);
...
{
	vban::stat stats;
	vban::message_buffer_manager buffer (stats, 512, 16);
...
TEST (message_buffer_manager, stats)
{
	...
	buffer.enqueue (buffer1);
	buffer.allocate ();
	ASSERT_EQ (1, stats.count (vban::stat::type::udp, vban::stat::detail::overflow));
}

TEST (tcp_listener, tcp_node_id_handshake)
{
	vban::system system (1);
	auto socket (std::make_shared<vban::socket> (*system.nodes[0]));
	auto bootstrap_endpoint (system.nodes[0]->bootstrap.endpoint ());
	auto cookie (system.nodes[0]->network.syn_cookies.assign (vban::transport::map_tcp_to_endpoint (bootstrap_endpoint)));
	vban::node_id_handshake node_id_handshake (cookie, boost::none);
	...
	boost::optional<std::pair<vban::account, vban::signature>> response_zero (std::make_pair (vban::account (0), vban::signature (0)));
	vban::node_id_handshake node_id_handshake_response (boost::none, response_zero);
...
TEST (tcp_listener, tcp_listener_timeout_empty)
{
	vban::system system (1);
	auto node0 (system.nodes[0]);
	auto socket (std::make_shared<vban::socket> (*node0));
	...
	{
		{
			vban::lock_guard<vban::mutex> guard (node0->bootstrap.mutex);
...
TEST (tcp_listener, tcp_listener_timeout_node_id_handshake)
{
	...
	auto node0 (system.nodes[0]);
	auto socket (std::make_shared<vban::socket> (*node0));
	auto cookie (node0->network.syn_cookies.assign (vban::transport::map_tcp_to_endpoint (node0->bootstrap.endpoint ())));
	vban::node_id_handshake node_id_handshake (cookie, boost::none);
	auto input (node_id_handshake.to_shared_const_buffer ());
	...
	ASSERT_TIMELY (5s, node0->stats.count (vban::stat::type::message, vban::stat::detail::node_id_handshake) != 0);
	{
		vban::lock_guard<vban::mutex> guard (node0->bootstrap.mutex);
...
TEST (network, replace_port)
{
	...
	vban::node_config node0_config (vban::get_available_port (), system.logging);
	...
	node1->start ();
	system.nodes.push_back (node1);
	auto wrong_endpoint = vban::endpoint (node1->network.endpoint ().address (), vban::get_available_port ());
	...
	node0->network.udp_channels.modify (channel0, [&node1] (std::shared_ptr<vban::transport::channel> const & channel_a) {
	...
	auto channel1 (std::make_shared<vban::transport::channel_udp> (node0->network.udp_channels, node1->network.endpoint (), node1->network_params.protocol.protocol_version));
...
TEST (network, peer_max_tcp_attempts)
{
	...
	{
		auto node2 (std::make_shared<vban::node> (system.io_ctx, vban::get_available_port (), vban::unique_path (), system.logging, system.work, node_flags));
		node2->start ();
		system.nodes.push_back (node2);
		// Start TCP attempt
	...
	ASSERT_TRUE (node->network.tcp_channels.reachout (vban::endpoint (node->network.endpoint ().address (), vban::get_available_port ())));
	ASSERT_EQ (1, node->stats.count (vban::stat::type::tcp, vban::stat::detail::tcp_max_per_ip, vban::stat::dir::out));
}
...
namespace transport
{
	TEST (network, peer_max_tcp_attempts_subnetwork)
	{
		...
		for (auto i (0); i < node->network_params.node.max_peers_per_subnetwork; ++i)
		{
			...
			vban::endpoint endpoint (address, vban::get_available_port ());
			ASSERT_FALSE (node->network.tcp_channels.reachout (endpoint));
		}
		ASSERT_EQ (0, node->network.size ());
		ASSERT_EQ (0, node->stats.count (vban::stat::type::tcp, vban::stat::detail::tcp_max_per_ip, vban::stat::dir::out));
		ASSERT_TRUE (node->network.tcp_channels.reachout (vban::endpoint (boost::asio::ip::make_address_v6 ("::ffff:127.0.0.1"), vban::get_available_port ())));
		ASSERT_EQ (1, node->stats.count (vban::stat::type::tcp, vban::stat::detail::tcp_max_per_ip, vban::stat::dir::out));
	}
}
}
...
TEST (network, duplicate_detection)
{
	...
	auto & node0 (*system.add_node (node_flags));
	auto & node1 (*system.add_node (node_flags));
	auto udp_channel (std::make_shared<vban::transport::channel_udp> (node0.network.udp_channels, node1.network.endpoint (), node1.network_params.protocol.protocol_version));
	vban::genesis genesis;
	vban::publish publish (genesis.open);

	// Publish duplicate detection through UDP
	ASSERT_EQ (0, node1.stats.count (vban::stat::type::filter, vban::stat::detail::duplicate_publish));
	udp_channel->send (publish);
	udp_channel->send (publish);
	ASSERT_TIMELY (2s, node1.stats.count (vban::stat::type::filter, vban::stat::detail::duplicate_publish) == 1);

	// Publish duplicate detection through TCP
	auto tcp_channel (node0.network.tcp_channels.find_channel (vban::transport::map_endpoint_to_tcp (node1.network.endpoint ())));
	ASSERT_EQ (1, node1.stats.count (vban::stat::type::filter, vban::stat::detail::duplicate_publish));
	tcp_channel->send (publish);
	ASSERT_TIMELY (2s, node1.stats.count (vban::stat::type::filter, vban::stat::detail::duplicate_publish) == 2);
}
...
TEST (network, duplicate_revert_publish)
{
	...
	auto other_node (std::make_shared<vban::node> (system.io_ctx, vban::get_available_port (), vban::unique_path (), system.logging, system.work));
	other_node->start ();
	system.nodes.push_back (other_node);
	auto channel = vban::establish_tcp (system, *other_node, node.network.endpoint ());
...
TEST (network, bandwidth_limiter)
{
	vban::system system;
	vban::genesis genesis;
	vban::publish message (genesis.open);
	...
	ASSERT_TIMELY (1s, 0 == node.stats.count (vban::stat::type::drop, vban::stat::detail::publish, vban::stat::dir::out));

	// Send droppable message; drop stats should increase by one now
	channel1->send (message);
	ASSERT_TIMELY (1s, 1 == node.stats.count (vban::stat::type::drop, vban::stat::detail::publish, vban::stat::dir::out));

	// Send non-droppable message, i.e. drop stats should not increase
	channel2->send (message, nullptr, vban::buffer_drop_policy::no_limiter_drop);
	ASSERT_TIMELY (1s, 1 == node.stats.count (vban::stat::type::drop, vban::stat::detail::publish, vban::stat::dir::out));
	...
	channel1->send (message);
	channel2->send (message);
	ASSERT_TIMELY (1s, 3 == node.stats.count (vban::stat::type::drop, vban::stat::detail::publish, vban::stat::dir::out));
...
TEST (peer_exclusion, validate)
{
	...
	{
		vban::tcp_endpoint endpoint (boost::asio::ip::address_v6::v4_mapped (boost::asio::ip::address_v4 (i)), 0);
	...
	auto & peers_by_endpoint (excluded_peers.peers.get<vban::peer_exclusion::tag_endpoint> ());
	vban::tcp_endpoint oldest (boost::asio::ip::address_v6::v4_mapped (boost::asio::ip::address_v4 (0x0)), 0);
	...
	vban::tcp_endpoint first (boost::asio::ip::address_v6::v4_mapped (boost::asio::ip::address_v4 (0x1)), 0);
	ASSERT_NE (peers_by_endpoint.end (), peers_by_endpoint.find (first.address ()));
	vban::tcp_endpoint second (boost::asio::ip::address_v6::v4_mapped (boost::asio::ip::address_v4 (0x2)), 0);
	...
	auto component (vban::collect_container_info (excluded_peers, ""));
	auto composite (dynamic_cast<vban::container_info_composite *> (component.get ()));
	ASSERT_NE (nullptr, component);
	auto & children (composite->get_children ());
	ASSERT_EQ (1, children.size ());
	auto child_leaf (dynamic_cast<vban::container_info_leaf *> (children.front ().get ()));
...
TEST (network, tcp_no_connect_excluded_peers)
{
	...
	auto endpoint1_tcp (vban::transport::map_endpoint_to_tcp (endpoint1));
	while (!node0->network.excluded_peers.check (endpoint1_tcp))
	{
		node0->network.excluded_peers.add (endpoint1_tcp, 1);
	}
	ASSERT_EQ (0, node0->stats.count (vban::stat::type::tcp, vban::stat::detail::tcp_excluded));
	node1->network.merge_peer (node0->network.endpoint ());
	ASSERT_TIMELY (5s, node0->stats.count (vban::stat::type::tcp, vban::stat::detail::tcp_excluded) >= 1);
...
{
	vban::tcp_message_manager manager (1);
	vban::tcp_message_item item;
	item.node_id = vban::account (100);
	...
	vban::tcp_message_manager manager2 (2);
...
TEST (network, cleanup_purge)
{
	...
	std::weak_ptr<vban::node> node_w = node1.shared ();
	node1.network.tcp_channels.start_tcp (node2->network.endpoint (), [node_w] (std::shared_ptr<vban::transport::channel> const & channel_a) {
...
TEST (network, loopback_channel)
{
	...
	auto & node1 = *system.nodes[0];
	auto & node2 = *system.nodes[1];
	vban::transport::channel_loopback channel1 (node1);
	ASSERT_EQ (channel1.get_type (), vban::transport::transport_type::loopback);
	ASSERT_EQ (channel1.get_endpoint (), node1.network.endpoint ());
	ASSERT_EQ (channel1.get_tcp_endpoint (), vban::transport::map_endpoint_to_tcp (node1.network.endpoint ()));
	...
	vban::transport::channel_loopback channel2 (node2);
