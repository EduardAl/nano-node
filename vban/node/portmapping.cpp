...
std::string vban::mapping_protocol::to_string ()
{
	std::stringstream ss;
...
std::string vban::port_mapping::get_config_port (std::string const & node_port_a)
{
	return node.config.external_port != 0 ? std::to_string (node.config.external_port) : node_port_a;
}

std::string vban::port_mapping::to_string ()
{
	std::stringstream ss;
...
void vban::port_mapping::refresh_devices ()
{
...
vban::endpoint vban::port_mapping::external_address ()
{
	vban::endpoint result_l (boost::asio::ip::address_v6{}, 0);
	vban::lock_guard<vban::mutex> guard_l (mutex);
	for (auto & protocol : protocols | boost::adaptors::filtered ([] (auto const & p) { return p.enabled; }))
	{
		if (protocol.external_port != 0)
		{
			result_l = vban::endpoint (protocol.external_address, protocol.external_port);
		}
	}
	return result_l;
}

void vban::port_mapping::refresh_mapping ()
{
	...
	if (on)
	{
		...
		for (auto & protocol : protocols | boost::adaptors::filtered ([] (auto const & p) { return p.enabled; }))
		{
			auto upnp_description = std::string ("Vban Node (") + network_params.network.get_current_network_as_string () + ")";
...
bool vban::port_mapping::check_lost_or_old_mapping ()
{
...
void vban::port_mapping::check_mapping_loop ()
{
...
std::string vban::upnp_state::to_string ()
{
	std::stringstream ss;
...
vban::upnp_state & vban::upnp_state::operator= (vban::upnp_state && other_a)
{
	if (this == &other_a)
	{
		return *this;
