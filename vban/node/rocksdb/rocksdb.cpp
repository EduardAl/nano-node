...
vban::rocksdb_store::rocksdb_store (vban::logger_mt & logger_a, boost::filesystem::path const & path_a, vban::rocksdb_config const & rocksdb_config_a, bool open_read_only_a) :
	logger{ logger_a },
	rocksdb_config{ rocksdb_config_a },
	max_block_write_batch_num_m{ vban::narrow_cast<unsigned> (blocks_memtable_size_bytes () / (2 * (sizeof (vban::block_type) + vban::state_block::size + vban::block_sideband::size (vban::block_type::state)))) },
...
std::unordered_map<const char *, vban::tables> vban::rocksdb_store::create_cf_name_table_map () const
{
	std::unordered_map<const char *, vban::tables> map{ { rocksdb::kDefaultColumnFamilyName.c_str (), tables::default_unused },
...
void vban::rocksdb_store::open (bool & error_a, boost::filesystem::path const & path_a, bool open_read_only_a)
{
...
void vban::rocksdb_store::generate_tombstone_map ()
{
	tombstone_map.emplace (std::piecewise_construct, std::forward_as_tuple (vban::tables::unchecked), std::forward_as_tuple (0, 50000));
	tombstone_map.emplace (std::piecewise_construct, std::forward_as_tuple (vban::tables::blocks), std::forward_as_tuple (0, 25000));
	tombstone_map.emplace (std::piecewise_construct, std::forward_as_tuple (vban::tables::accounts), std::forward_as_tuple (0, 25000));
	tombstone_map.emplace (std::piecewise_construct, std::forward_as_tuple (vban::tables::pending), std::forward_as_tuple (0, 25000));
}

rocksdb::ColumnFamilyOptions vban::rocksdb_store::get_common_cf_options (std::shared_ptr<rocksdb::TableFactory> const & table_factory_a, unsigned long long memtable_size_bytes_a) const
{
...
rocksdb::ColumnFamilyOptions vban::rocksdb_store::get_cf_options (std::string const & cf_name_a) const
{
	rocksdb::ColumnFamilyOptions cf_options;
	...
	if (cf_name_a == "unchecked")
	{
		...
		cf_options.prefix_extractor.reset (rocksdb::NewFixedPrefixTransform (sizeof (vban::root)));
...
std::vector<rocksdb::ColumnFamilyDescriptor> vban::rocksdb_store::create_column_families ()
{
...
vban::write_transaction vban::rocksdb_store::tx_begin_write (std::vector<vban::tables> const & tables_requiring_locks_a, std::vector<vban::tables> const & tables_no_locks_a)
{
	std::unique_ptr<vban::write_rocksdb_txn> txn;
	...
	if (tables_requiring_locks_a.empty () && tables_no_locks_a.empty ())
	{
		...
		txn = std::make_unique<vban::write_rocksdb_txn> (optimistic_db, all_tables (), tables_no_locks_a, write_lock_mutexes);
	}
	else
	{
		txn = std::make_unique<vban::write_rocksdb_txn> (optimistic_db, tables_requiring_locks_a, tables_no_locks_a, write_lock_mutexes);
	}
	...
	return vban::write_transaction{ std::move (txn) };
}

vban::read_transaction vban::rocksdb_store::tx_begin_read () const
{
	return vban::read_transaction{ std::make_unique<vban::read_rocksdb_txn> (db.get ()) };
}

std::string vban::rocksdb_store::vendor_get () const
{
	return boost::str (boost::format ("RocksDB %1%.%2%.%3%") % ROCKSDB_MAJOR % ROCKSDB_MINOR % ROCKSDB_PATCH);
}

rocksdb::ColumnFamilyHandle * vban::rocksdb_store::table_to_column_family (tables table_a) const
{
...
bool vban::rocksdb_store::exists (vban::transaction const & transaction_a, tables table_a, vban::rocksdb_val const & key_a) const
{
...
int vban::rocksdb_store::del (vban::write_transaction const & transaction_a, tables table_a, vban::rocksdb_val const & key_a)
{
...
void vban::rocksdb_store::flush_tombstones_check (tables table_a)
{
...
void vban::rocksdb_store::flush_table (vban::tables table_a)
{
	db->Flush (rocksdb::FlushOptions{}, table_to_column_family (table_a));
}

void vban::rocksdb_store::version_put (vban::write_transaction const & transaction_a, int version_a)
{
	...
	auto status (put (transaction_a, tables::meta, version_key, vban::rocksdb_val (version_value)));
	release_assert (success (status));
}

rocksdb::Transaction * vban::rocksdb_store::tx (vban::transaction const & transaction_a) const
{
	debug_assert (!is_read (transaction_a));
	return static_cast<rocksdb::Transaction *> (transaction_a.get_handle ());
}

int vban::rocksdb_store::get (vban::transaction const & transaction_a, tables table_a, vban::rocksdb_val const & key_a, vban::rocksdb_val & value_a) const
{
	rocksdb::ReadOptions options;
	rocksdb::PinnableSlice slice;
...
int vban::rocksdb_store::put (vban::write_transaction const & transaction_a, tables table_a, vban::rocksdb_val const & key_a, vban::rocksdb_val const & value_a)
{
	debug_assert (transaction_a.contains (table_a));
...
bool vban::rocksdb_store::not_found (int status) const
{
	return (status_code_not_found () == status);
}

bool vban::rocksdb_store::success (int status) const
{
	return (static_cast<int> (rocksdb::Status::Code::kOk) == status);
}

int vban::rocksdb_store::status_code_not_found () const
{
	return static_cast<int> (rocksdb::Status::Code::kNotFound);
}

uint64_t vban::rocksdb_store::count (vban::transaction const & transaction_a, tables table_a) const
{
	uint64_t sum = 0;
...
int vban::rocksdb_store::drop (vban::write_transaction const & transaction_a, tables table_a)
{
	debug_assert (transaction_a.contains (table_a));
	...
	if (success (status))
	{
		...
		if (table_a == tables::peers)
		{
			...
			for (auto i = peers_begin (transaction_a), n = peers_end (); i != n; ++i)
			{
				status = del (transaction_a, tables::peers, vban::rocksdb_val (i->first));
				release_assert (success (status));
			}
			return status;
		}
...
int vban::rocksdb_store::clear (rocksdb::ColumnFamilyHandle * column_family)
{
...
std::vector<vban::unchecked_info> vban::rocksdb_store::unchecked_get (vban::transaction const & transaction_a, vban::block_hash const & hash_a)
{
	auto cf = table_to_column_family (tables::unchecked);

	std::unique_ptr<rocksdb::Iterator> iter;
	vban::qualified_root upper (hash_a, vban::block_hash (std::numeric_limits<vban::uint256_t>::max ()));
	vban::rocksdb_val upper_bound (sizeof (upper), (void *)&upper);
	if (is_read (transaction_a))
	{
	...
	auto prefix = vban::rocksdb_val (hash_a);
	for (iter->Seek (prefix); iter->Valid () && iter->key ().starts_with (prefix); iter->Next ())
	{
		auto unchecked_info = static_cast<vban::unchecked_info> (vban::rocksdb_val (iter->value ()));
		result.push_back (unchecked_info);
	}
	return result;
}

void vban::rocksdb_store::construct_column_family_mutexes ()
{
...
rocksdb::Options vban::rocksdb_store::get_db_options ()
{
...
rocksdb::BlockBasedTableOptions vban::rocksdb_store::get_active_table_options (size_t lru_size) const
{
...
rocksdb::BlockBasedTableOptions vban::rocksdb_store::get_small_table_options () const
{
	rocksdb::BlockBasedTableOptions table_options;
	// Improve point lookup performance be using the data block hash index (uses about 5% more space).
	table_options.data_block_index_type = rocksdb::BlockBasedTableOptions::DataBlockIndexType::kDataBlockBinaryAndHash;
...
rocksdb::ColumnFamilyOptions vban::rocksdb_store::get_small_cf_options (std::shared_ptr<rocksdb::TableFactory> const & table_factory_a) const
{
...
rocksdb::ColumnFamilyOptions vban::rocksdb_store::get_active_cf_options (std::shared_ptr<rocksdb::TableFactory> const & table_factory_a, unsigned long long memtable_size_bytes_a) const
{
...
void vban::rocksdb_store::on_flush (rocksdb::FlushJobInfo const & flush_job_info_a)
{
...
std::vector<vban::tables> vban::rocksdb_store::all_tables () const
{
	return std::vector<vban::tables>{ tables::accounts, tables::blocks, tables::confirmation_height, tables::final_votes, tables::frontiers, tables::meta, tables::online_weight, tables::peers, tables::pending, tables::pruned, tables::unchecked, tables::vote };
}

bool vban::rocksdb_store::copy_db (boost::filesystem::path const & destination_path)
{
	...
	if (status.ok ())
	{
		vban::rocksdb_store rocksdb_store (logger, destination_path.string (), rocksdb_config, false);
		return !rocksdb_store.init_error ();
	}
	return false;
}

void vban::rocksdb_store::rebuild_db (vban::write_transaction const & transaction_a)
{
	// Not available for RocksDB
}

bool vban::rocksdb_store::init_error () const
{
	return error;
}

void vban::rocksdb_store::serialize_memory_stats (boost::property_tree::ptree & json)
{
	uint64_t val;
...
unsigned long long vban::rocksdb_store::blocks_memtable_size_bytes () const
{
	return base_memtable_size_bytes ();
}

unsigned long long vban::rocksdb_store::base_memtable_size_bytes () const
{
...
unsigned vban::rocksdb_store::max_block_write_batch_num () const
{
	return max_block_write_batch_num_m;
}

std::string vban::rocksdb_store::error_string (int status) const
{
	return std::to_string (status);
}

vban::rocksdb_store::tombstone_info::tombstone_info (uint64_t num_since_last_flush_a, uint64_t const max_a) :
...
template class vban::block_store_partial<rocksdb::Slice, vban::rocksdb_store>;
