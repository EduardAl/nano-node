...
public:
	...
	vban::error deserialize_json (vban::jsonconfig & json);
...
private:
	mutable vban::mutex datapoint_mutex;
...
private:
	mutable vban::mutex histogram_mutex;
	std::vector<bin> bins;
};
...
public:
	...
	vban::observer_set<boost::circular_buffer<stat_datapoint> &> sample_observers;

	/** Observers for count. Called on each update. */
	vban::observer_set<uint64_t, uint64_t> count_observers;
};
...
public:
	...
	virtual void write_entry (tm & tm, std::string const & type, std::string const & detail, std::string const & dir, uint64_t value, vban::stat_histogram * histogram)
	{
	}
...
public:
	...
	vban::stat_histogram * get_histogram (stat::type type, stat::detail detail, stat::dir dir);
...
private:
	...
	std::shared_ptr<vban::stat_entry> get_entry (uint32_t key);

	/** Get entry for key, creating a new entry if necessary */
	std::shared_ptr<vban::stat_entry> get_entry (uint32_t key, size_t sample_interval, size_t max_samples);

	/** Unlocked implementation of get_entry() */
	std::shared_ptr<vban::stat_entry> get_entry_impl (uint32_t key, size_t sample_interval, size_t max_samples);
	...
	vban::stat_config config;

	/** Stat entries are sorted by key to simplify processing of log output */
	std::map<uint32_t, std::shared_ptr<vban::stat_entry>> entries;
