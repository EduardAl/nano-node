...
vban::tomlconfig::tomlconfig () :
	tree (cpptoml::make_table ())
{
	error = std::make_shared<vban::error> ();
}

vban::tomlconfig::tomlconfig (std::shared_ptr<cpptoml::table> const & tree_a, std::shared_ptr<vban::error> const & error_a) :
	vban::configbase (error_a), tree (tree_a)
{
	if (!error)
	{
		error = std::make_shared<vban::error> ();
...
void vban::tomlconfig::doc (std::string const & key, std::string const & doc)
{
	tree->document (key, doc);
}

vban::error & vban::tomlconfig::read (boost::filesystem::path const & path_a)
{
...
vban::error & vban::tomlconfig::read (std::istream & stream_overrides, boost::filesystem::path const & path_a)
{
	std::fstream stream;
...
vban::error & vban::tomlconfig::read (std::istream & stream_a)
{
	std::stringstream stream_override_empty;
	stream_override_empty << std::endl;
...
vban::error & vban::tomlconfig::read (std::istream & stream_first_a, std::istream & stream_second_a)
{
	try
	{
...
void vban::tomlconfig::write (boost::filesystem::path const & path_a)
{
	std::fstream stream;
	open_or_create (stream, path_a.string ());
	write (stream);
...
void vban::tomlconfig::write (std::ostream & stream_a) const
{
...
void vban::tomlconfig::open_or_create (std::fstream & stream_a, std::string const & path_a)
{
	if (!boost::filesystem::exists (path_a))
	{
		...
		vban::set_secure_perm_file (path_a);
	}
...
std::shared_ptr<cpptoml::table> vban::tomlconfig::get_tree ()
{
	return tree;
}

/** Returns true if the toml table is empty */
bool vban::tomlconfig::empty () const
{
	return tree->empty ();
}

boost::optional<vban::tomlconfig> vban::tomlconfig::get_optional_child (std::string const & key_a)
{
...
vban::tomlconfig vban::tomlconfig::get_required_child (std::string const & key_a)
{
	if (!tree->contains (key_a))
	{
		*error = vban::error_config::missing_value;
...
vban::tomlconfig & vban::tomlconfig::put_child (std::string const & key_a, vban::tomlconfig & conf_a)
{
	tree->insert (key_a, conf_a.get_tree ());
	return *this;
}

vban::tomlconfig & vban::tomlconfig::replace_child (std::string const & key_a, vban::tomlconfig & conf_a)
{
	tree->erase (key_a);
...
bool vban::tomlconfig::has_key (std::string const & key_a)
{
	return tree->contains (key_a);
}

/** Erase the property of given key */
vban::tomlconfig & vban::tomlconfig::erase (std::string const & key_a)
{
	tree->erase (key_a);
	return *this;
}

std::shared_ptr<cpptoml::array> vban::tomlconfig::create_array (std::string const & key, boost::optional<const char *> documentation_a)
{
	if (!has_key (key))
	{
...
/**
	 ...
	 */
void vban::tomlconfig::erase_default_values (tomlconfig & defaults_a)
{
...
std::string vban::tomlconfig::to_string ()
{
	std::stringstream ss;
...
std::string vban::tomlconfig::to_string_commented_entries ()
{
...
vban::tomlconfig & vban::tomlconfig::get_config (bool optional, std::string const & key, uint8_t & target, uint8_t default_value)
{
	try
	{
		if (tree->contains_qualified (key))
		{
			...
			{
				conditionally_set_error<uint8_t> (vban::error_config::invalid_value, optional, key);
			}
			else
			{
		...
		{
			conditionally_set_error<uint8_t> (vban::error_config::missing_value, optional, key);
		}
		else
		{
...
vban::tomlconfig & vban::tomlconfig::get_config (bool optional, std::string const & key, bool & target, bool default_value)
{
	auto bool_conv = [this, &target, &key, optional] (std::string val) {
		...
		{
			conditionally_set_error<bool> (vban::error_config::invalid_value, optional, key);
		}
	};
	try
	{
		...
		{
			conditionally_set_error<bool> (vban::error_config::missing_value, optional, key);
		}
		else
		{
			target = default_value;
...
void vban::tomlconfig::erase_defaults (std::shared_ptr<cpptoml::table> const & base, std::shared_ptr<cpptoml::table> const & other, std::shared_ptr<cpptoml::table> const & update_target)
{
...
vban::tomlconfig & vban::tomlconfig::get_config (bool optional, std::string key, boost::asio::ip::address_v6 & target, boost::asio::ip::address_v6 const & default_value)
{
	try
	{
		if (tree->contains_qualified (key))
		{
			...
			if (bec)
			{
				conditionally_set_error<boost::asio::ip::address_v6> (vban::error_config::invalid_value, optional, key);
			}
		}
		else if (!optional)
		{
			conditionally_set_error<boost::asio::ip::address_v6> (vban::error_config::missing_value, optional, key);
		}
		else
		{
			target = default_value;
