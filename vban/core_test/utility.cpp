...
TEST (rate, basic)
{
	vban::rate::token_bucket bucket (10, 10);
...
TEST (rate, network)
{
	...
	vban::rate::token_bucket bucket (10, 5);
...
TEST (rate, reset)
{
	vban::rate::token_bucket bucket (0, 0);
...
TEST (thread, thread_pool)
{
	...
	vban::thread_pool workers (1u, vban::thread_role::name::unknown);
...
TEST (thread_pool_alarm, one)
{
	...
	vban::unique_lock<vban::mutex> unique (mutex);
	condition.wait (unique, [&] () { return !!done; });
}

TEST (thread_pool_alarm, many)
{
	vban::thread_pool workers (50u, vban::thread_role::name::unknown);
...
TEST (filesystem, remove_all_files)
{
	...
	vban::remove_all_files_in_dir (path);
...
TEST (filesystem, move_all_files)
{
	...
	vban::move_all_files_to_dir (dummy_directory, path);
...
{
	vban::relaxed_atomic_integral<uint32_t> atomic{ 0 };
	ASSERT_EQ (0, atomic++);
	ASSERT_EQ (1, atomic);
	ASSERT_EQ (2, ++atomic);
	ASSERT_EQ (2, atomic);
