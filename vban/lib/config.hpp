...
*/
const char * const VBAN_VERSION_STRING = xstr (TAG_VERSION_STRING);
const char * const VBAN_MAJOR_VERSION_STRING = xstr (MAJOR_VERSION_STRING);
const char * const VBAN_MINOR_VERSION_STRING = xstr (MINOR_VERSION_STRING);
const char * const VBAN_PATCH_VERSION_STRING = xstr (PATCH_VERSION_STRING);
const char * const VBAN_PRE_RELEASE_VERSION_STRING = xstr (PRE_RELEASE_VERSION_STRING);
...
struct work_thresholds
{
	...
	work_thresholds operator= (vban::work_thresholds const & other_a)
	{
		return other_a;
	}
};
...
public:
	...
	network_constants (vban_networks network_a) :
	...
	static const vban::work_thresholds publish_full;
	static const vban::work_thresholds publish_beta;
	static const vban::work_thresholds publish_dev;
	static const vban::work_thresholds publish_test;
	...
	vban_networks current_network{ vban::network_constants::active_network };
	vban::work_thresholds publish_thresholds;
	 ...
	 */
	static void set_active_network (vban_networks network_a)
	{
	...
	static bool set_active_network (std::string network_a)
	{
		...
		if (network_a == "live")
		{
			active_network = vban::vban_networks::vban_live_network;
		}
		else if (network_a == "beta")
		{
			active_network = vban::vban_networks::vban_beta_network;
		}
		else if (network_a == "dev")
		{
			active_network = vban::vban_networks::vban_dev_network;
		}
		else if (network_a == "test")
		{
			active_network = vban::vban_networks::vban_test_network;
		}
		else
		{
			error = true;
	...
	{
		return current_network == vban_networks::vban_live_network;
	}
	bool is_beta_network () const
	{
		return current_network == vban_networks::vban_beta_network;
	}
	bool is_dev_network () const
	{
		return current_network == vban_networks::vban_dev_network;
	}
	bool is_test_network () const
	{
		return current_network == vban_networks::vban_test_network;
	}

	/** Initial value is ACTIVE_NETWORK compile flag, but can be overridden by a CLI flag */
	static vban::vban_networks active_network;
};
