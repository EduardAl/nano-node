...
public:
	...
	representative (vban::account account_a, vban::amount weight_a, std::shared_ptr<vban::transport::channel> const & channel_a) :
		account (account_a), weight (weight_a), channel (channel_a)
	{
		debug_assert (channel != nullptr);
	}
	std::reference_wrapper<vban::transport::channel const> channel_ref () const
	{
		return *channel;
	};
	bool operator== (vban::representative const & other_a) const
	{
		return account == other_a.account;
	}
	vban::account account{ 0 };
	vban::amount weight{ 0 };
...
class rep_crawler final
{
	...
	mi::indexed_by<
		mi::hashed_unique<mi::member<representative, vban::account, &representative::account>>,
		mi::random_access<>,
		...
		mi::ordered_non_unique<mi::tag<tag_weight>,
			mi::member<representative, vban::amount, &representative::weight>, std::greater<vban::amount>>,
		mi::hashed_non_unique<mi::tag<tag_channel_ref>,
			mi::const_mem_fun<representative, std::reference_wrapper<vban::transport::channel const>, &representative::channel_ref>>>>;
	// clang-format on

public:
	rep_crawler (vban::node & node_a);

	/** Start crawling */
	void start ();

	/** Remove block hash from list of active rep queries */
	void remove (vban::block_hash const &);

	/** Remove block hash from with delay depending on vote processor size */
	void throttled_remove (vban::block_hash const &, uint64_t const);

	/** Attempt to determine if the peer manages one or more representative accounts */
	void query (std::vector<std::shared_ptr<vban::transport::channel>> const & channels_a);

	/** Attempt to determine if the peer manages one or more representative accounts */
	void query (std::shared_ptr<vban::transport::channel> const & channel_a);

	/** Query if a peer manages a principle representative */
	bool is_pr (vban::transport::channel const &) const;
	 ...
	 */
	bool response (std::shared_ptr<vban::transport::channel> const &, std::shared_ptr<vban::vote> const &);

	/** Get total available weight from representatives */
	vban::uint256_t total_weight () const;
	...
	std::vector<representative> representatives (size_t count_a = std::numeric_limits<size_t>::max (), vban::uint256_t const weight_a = 0, boost::optional<decltype (vban::protocol_constants::protocol_version)> const & opt_version_min_a = boost::none);
	...
	std::vector<representative> principal_representatives (size_t count_a = std::numeric_limits<size_t>::max (), boost::optional<decltype (vban::protocol_constants::protocol_version)> const & opt_version_min_a = boost::none);

	/** Request a list of the top \p count_a known representative endpoints. */
	std::vector<std::shared_ptr<vban::transport::channel>> representative_endpoints (size_t count_a);
...
private:
	...
	vban::mutex active_mutex;

	/** We have solicted votes for these random blocks */
	std::unordered_set<vban::block_hash> active;
	...
	std::vector<std::shared_ptr<vban::transport::channel>> get_crawl_targets (vban::uint256_t total_weight_a);

	/** When a rep request is made, this is called to update the last-request timestamp. */
	void on_rep_request (std::shared_ptr<vban::transport::channel> const & channel_a);
	...
	mutable vban::mutex probable_reps_mutex;
	...
	std::deque<std::pair<std::shared_ptr<vban::transport::channel>, std::shared_ptr<vban::vote>>> responses;
};
}
