...
REGISTER_ERROR_CODES (vban, error_common);
REGISTER_ERROR_CODES (vban, error_blocks);
REGISTER_ERROR_CODES (vban, error_rpc);
REGISTER_ERROR_CODES (vban, error_process);
REGISTER_ERROR_CODES (vban, error_config);
...
public:
	error () = default;
	error (vban::error const & error_a) = default;
	error (vban::error && error_a) = default;
	...
	error & operator= (vban::error const & err_a);
	error & operator= (vban::error && err_a);
	...
	error & then (std::function<vban::error &()> next);
	...
	error & set (std::string message_a, std::error_code code_a = vban::error_common::generic);
...
/**
 * A type that manages a vban::error.
 * The default return type is vban::error&, though shared_ptr<vban::error> is a good option in cases
 * where shared error state is desirable.
 */
template <typename RET_TYPE = vban::error &>
class error_aware
{
	static_assert (std::is_same<RET_TYPE, vban::error &>::value || std::is_same<RET_TYPE, std::shared_ptr<vban::error>>::value, "Must be vban::error& or shared_ptr<vban::error>");
