...
vban::ipc::access_permission from_string (std::string permission)
{
	if (permission == "unrestricted")
		return vban::ipc::access_permission::unrestricted;
	if (permission == "api_account_weight")
		return vban::ipc::access_permission::api_account_weight;
	if (permission == "api_service_register")
		return vban::ipc::access_permission::api_service_register;
	if (permission == "api_service_stop")
		return vban::ipc::access_permission::api_service_stop;
	if (permission == "api_topic_service_stop")
		return vban::ipc::access_permission::api_topic_service_stop;
	if (permission == "api_topic_confirmation")
		return vban::ipc::access_permission::api_topic_confirmation;
	if (permission == "account_query")
		return vban::ipc::access_permission::account_query;
	if (permission == "epoch_upgrade")
		return vban::ipc::access_permission::epoch_upgrade;
	if (permission == "service")
		return vban::ipc::access_permission::service;
	if (permission == "wallet")
		return vban::ipc::access_permission::wallet;
	if (permission == "wallet_read")
		return vban::ipc::access_permission::wallet_read;
	if (permission == "wallet_write")
		return vban::ipc::access_permission::wallet_write;
	if (permission == "wallet_seed_change")
		return vban::ipc::access_permission::wallet_seed_change;

	return vban::ipc::access_permission::invalid;
}
}

void vban::ipc::access::set_effective_permissions (vban::ipc::access_subject & subject_a, std::shared_ptr<cpptoml::table> const & config_subject_a)
{
	...
	for (auto const & permission : allow_strings_l)
	{
		if (!permission.empty ())
		{
			...
			if (permission_enum != vban::ipc::access_permission::invalid)
			{
...
void vban::ipc::access::clear ()
{
	...
	default_user.permissions.insert (vban::ipc::access_permission::api_account_weight);
}

vban::error vban::ipc::access::deserialize_toml (vban::tomlconfig & toml)
{
...
bool vban::ipc::access::has_access (std::string const & credentials_a, vban::ipc::access_permission permssion_a) const
{
	vban::unique_lock<vban::mutex> lock (mutex);
	...
	if (user != users.end ())
	{
		...
		if (!permitted)
		{
			permitted = user->second.permissions.find (vban::ipc::access_permission::unrestricted) != user->second.permissions.end ();
		}
	}
	return permitted;
}

bool vban::ipc::access::has_access_to_all (std::string const & credentials_a, std::initializer_list<vban::ipc::access_permission> permissions_a) const
{
	vban::unique_lock<vban::mutex> lock (mutex);
	bool permitted = false;
	auto user = users.find (credentials_a);
...
bool vban::ipc::access::has_access_to_oneof (std::string const & credentials_a, std::initializer_list<vban::ipc::access_permission> permissions_a) const
{
	vban::unique_lock<vban::mutex> lock (mutex);
	bool permitted = false;
	auto user = users.find (credentials_a);
...
void vban::ipc::access_subject::clear ()
{
	permissions.clear ();
}

void vban::ipc::access_user::clear ()
{
...
namespace ipc
{
	vban::error read_access_config_toml (boost::filesystem::path const & data_path_a, vban::ipc::access & config_a)
	{
		vban::error error;
		auto toml_config_path = vban::get_access_toml_config_path (data_path_a);
