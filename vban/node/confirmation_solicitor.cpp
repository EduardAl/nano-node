...
vban::confirmation_solicitor::confirmation_solicitor (vban::network & network_a, vban::node_config const & config_a) :
...
void vban::confirmation_solicitor::prepare (std::vector<vban::representative> const & representatives_a)
{
...
bool vban::confirmation_solicitor::broadcast (vban::election const & election_a)
{
	debug_assert (prepared);
	bool error (true);
	if (rebroadcasted++ < max_block_broadcasts)
	{
		...
		vban::publish winner (election_a.status.winner);
		unsigned count = 0;
		...
		network.flood_message (winner, vban::buffer_drop_policy::limiter, 0.5f);
		error = false;
	}
	return error;
}

bool vban::confirmation_solicitor::add (vban::election const & election_a)
{
	debug_assert (prepared);
	bool error (true);
	unsigned count = 0;
	auto const max_channel_requests (config.confirm_req_batches_max * vban::network::confirm_req_hashes_max);
	auto const & hash (election_a.status.winner->hash ());
...
void vban::confirmation_solicitor::flush ()
{
	debug_assert (prepared);
	for (auto const & request_queue : requests)
	{
		...
		for (auto const & root_hash : request_queue.second)
		{
			...
			if (roots_hashes_l.size () == vban::network::confirm_req_hashes_max)
			{
