...
void vban::confirmation_height_unbounded::process (std::shared_ptr<vban::block> original_block)
{
	if (pending_empty ())
	{
		clear_process_vars ();
	...
	std::vector<vban::block_hash> orig_block_callback_data;
	...
	do
	{
		...
		if (first_iter)
		{
			...
			vban::lock_guard<vban::mutex> guard (block_cache_mutex);
		...
		std::vector<vban::block_hash> block_callback_datas_required;
...
void vban::confirmation_height_unbounded::collect_unconfirmed_receive_and_sources_for_account (uint64_t block_height_a, uint64_t confirmation_height_a, std::shared_ptr<vban::block> const & block_a, vban::block_hash const & hash_a, vban::account const & account_a, vban::read_transaction const & transaction_a, std::vector<receive_source_pair> & receive_source_pairs_a, std::vector<vban::block_hash> & block_callback_data_a, std::vector<vban::block_hash> & orig_block_callback_data_a, std::shared_ptr<vban::block> original_block)
{
	...
	while ((num_to_confirm > 0) && !hash.is_zero () && !stopped)
	{
		...
		if (block)
		{
			...
			if (!source.is_zero () && !ledger.is_epoch_link (source) && ledger.store.block_exists (transaction_a, source))
			{
				...
				receive_source_pairs_a.emplace_back (std::make_shared<conf_height_details> (account_a, hash, block_height, 1, std::vector<vban::block_hash>{ hash }), source);
			}
...
void vban::confirmation_height_unbounded::prepare_iterated_blocks_for_cementing (preparation_data & preparation_data_a)
{
 ...
 */
void vban::confirmation_height_unbounded::cement_blocks (vban::write_guard & scoped_write_guard_a)
{
	vban::timer<std::chrono::milliseconds> cemented_batch_timer;
	...
	auto error = false;
	{
		...
		while (!pending_writes.empty ())
		{
			...
			if (pending.height > confirmation_height)
			{
				...
				ledger.stats.add (vban::stat::type::confirmation_height, vban::stat::detail::blocks_confirmed, vban::stat::dir::in, pending.height - confirmation_height);
				ledger.stats.add (vban::stat::type::confirmation_height, vban::stat::detail::blocks_confirmed_unbounded, vban::stat::dir::in, pending.height - confirmation_height);
...
std::shared_ptr<vban::block> vban::confirmation_height_unbounded::get_block_and_sideband (vban::block_hash const & hash_a, vban::transaction const & transaction_a)
{
	vban::lock_guard<vban::mutex> guard (block_cache_mutex);
...
bool vban::confirmation_height_unbounded::pending_empty () const
{
	return pending_writes.empty ();
}

void vban::confirmation_height_unbounded::clear_process_vars ()
{
...
bool vban::confirmation_height_unbounded::has_iterated_over_block (vban::block_hash const & hash_a) const
{
	vban::lock_guard<vban::mutex> guard (block_cache_mutex);
	return block_cache.count (hash_a) == 1;
}

uint64_t vban::confirmation_height_unbounded::block_cache_size () const
{
	vban::lock_guard<vban::mutex> guard (block_cache_mutex);
	return block_cache.size ();
}

vban::confirmation_height_unbounded::conf_height_details::conf_height_details (vban::account const & account_a, vban::block_hash const & hash_a, uint64_t height_a, uint64_t num_blocks_confirmed_a, std::vector<vban::block_hash> const & block_callback_data_a) :
	account (account_a),
	hash (hash_a),
	height (height_a),
...
vban::confirmation_height_unbounded::receive_source_pair::receive_source_pair (std::shared_ptr<conf_height_details> const & receive_details_a, const block_hash & source_a) :
	receive_details (receive_details_a),
	source_hash (source_a)
{
}

vban::confirmation_height_unbounded::confirmed_iterated_pair::confirmed_iterated_pair (uint64_t confirmed_height_a, uint64_t iterated_height_a) :
	confirmed_height (confirmed_height_a),
	iterated_height (iterated_height_a)
{
}

std::unique_ptr<vban::container_info_component> vban::collect_container_info (confirmation_height_unbounded & confirmation_height_unbounded, std::string const & name_a)
{
