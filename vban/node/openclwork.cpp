...
bool vban::opencl_loaded{ true };
#else
bool vban::opencl_loaded{ false };
#endif
...
enum IV_Derived {
    vban_xor_iv0 = 0x6a09e667f2bdc900UL,  // iv1 ^ 0x1010000 ^ outlen
    vban_xor_iv4 = 0x510e527fade682f9UL,  // iv4 ^ inbytes
    vban_xor_iv6 = 0xe07c265404be4294UL,  // iv6 ^ ~0
};
...
{
    ulong2 vv[8] = {
        {vban_xor_iv0, iv1}, {iv2, iv3},          {iv4, iv5},
        {iv6, iv7},          {iv0, iv1},          {iv2, iv3},
        {vban_xor_iv4, iv5}, {vban_xor_iv6, iv7},
    };
    ...
    return vban_xor_iv0 ^ vv[0].s0 ^ vv[4].s0;
}
#undef G32
#undef G2v
#undef G2v_split
...
__kernel void vban_work(__constant ulong *attempt,
...
vban::opencl_environment::opencl_environment (bool & error_a)
{
	if (vban::opencl_loaded)
	{
...
void vban::opencl_environment::dump (std::ostream & stream)
{
	if (vban::opencl_loaded)
	{
		auto index (0);
...
vban::opencl_work::opencl_work (bool & error_a, vban::opencl_config const & config_a, vban::opencl_environment & environment_a, vban::logger_mt & logger_a) :
	config (config_a),
	context (0),
	attempt_buffer (0),
	result_buffer (0),
	...
	logger (logger_a)
{
	...
	if (!error_a)
	{
		...
		if (!error_a)
		{
			vban::random_pool::generate_block (reinterpret_cast<uint8_t *> (rand.s.data ()), rand.s.size () * sizeof (decltype (rand.s)::value_type));
			...
			if (!error_a)
			{
				...
				if (!error_a)
				{
					...
					if (!error_a)
					{
						...
						{
							cl_int item_error (0);
							size_t item_size (sizeof (vban::uint256_union));
								...
								if (!error_a)
								{
									...
									if (!error_a)
									{
										...
										if (!error_a)
										{
											...
											kernel = clCreateKernel (program, "vban_work", &kernel_error);
...
boost::optional<uint64_t> vban::opencl_work::generate_work (vban::work_version const version_a, vban::root const & root_a, uint64_t const difficulty_a)
{
	std::atomic<int> ticket_l{ 0 };
	return generate_work (version_a, root_a, difficulty_a, ticket_l);
}

boost::optional<uint64_t> vban::opencl_work::generate_work (vban::work_version const version_a, vban::root const & root_a, uint64_t const difficulty_a, std::atomic<int> & ticket_a)
{
	...
	while (vban::work_difficulty (version_a, root_a, result) < difficulty_a && !error && ticket_a == ticket_l)
	{
		result = rand.next ();
		...
		{
			cl_int write_error2 = clEnqueueWriteBuffer (queue, item_buffer, false, 0, sizeof (vban::root), root_a.bytes.data (), 0, nullptr, nullptr);
...
std::unique_ptr<vban::opencl_work> vban::opencl_work::create (bool create_a, vban::opencl_config const & config_a, vban::logger_mt & logger_a)
{
	std::unique_ptr<vban::opencl_work> result;
	if (create_a)
	{
		auto error (false);
		...
		if (!error)
		{
			result.reset (new vban::opencl_work (error, config_a, environment, logger_a));
			if (error)
			{
				result.reset ();
			}
