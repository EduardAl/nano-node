...
get_env_threshold_or_default ("VBAN_TEST_EPOCH_1", 0xffffffc000000000),
get_env_threshold_or_default ("VBAN_TEST_EPOCH_2", 0xfffffff800000000), // 8x higher than epoch_1
get_env_threshold_or_default ("VBAN_TEST_EPOCH_2_RECV", 0xfffffe0000000000) // 8x lower than epoch_1
);
...
{
	return boost::numeric_cast<uint8_t> (boost::lexical_cast<int> (VBAN_MAJOR_VERSION_STRING));
}
uint8_t get_minor_node_version ()
{
	return boost::numeric_cast<uint8_t> (boost::lexical_cast<int> (VBAN_MINOR_VERSION_STRING));
}
uint8_t get_patch_node_version ()
{
	return boost::numeric_cast<uint8_t> (boost::lexical_cast<int> (VBAN_PATCH_VERSION_STRING));
}
uint8_t get_pre_release_node_version ()
{
	return boost::numeric_cast<uint8_t> (boost::lexical_cast<int> (VBAN_PRE_RELEASE_VERSION_STRING));
}
...
{
	auto test_env = vban::get_env_or_default ("VBAN_TEST_NODE_PORT", "17075");
	return boost::lexical_cast<uint16_t> (test_env);
}
uint16_t test_rpc_port ()
{
	auto test_env = vban::get_env_or_default ("VBAN_TEST_RPC_PORT", "17076");
	return boost::lexical_cast<uint16_t> (test_env);
}
uint16_t test_ipc_port ()
{
	auto test_env = vban::get_env_or_default ("VBAN_TEST_IPC_PORT", "17077");
	return boost::lexical_cast<uint16_t> (test_env);
}
uint16_t test_websocket_port ()
{
	auto test_env = vban::get_env_or_default ("VBAN_TEST_WEBSOCKET_PORT", "17078");
	return boost::lexical_cast<uint16_t> (test_env);
}

std::array<uint8_t, 2> test_magic_number ()
{
	auto test_env = get_env_or_default ("VBAN_TEST_MAGIC_NUMBER", "RX");
