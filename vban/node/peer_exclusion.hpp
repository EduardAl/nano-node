...
class peer_exclusion final
{
	...
	public:
		...
		decltype (std::declval<vban::tcp_endpoint> ().address ()) address;
		uint64_t score;
	};
...
public:
	...
	uint64_t add (vban::tcp_endpoint const &, size_t const);
	bool check (vban::tcp_endpoint const &);
	void remove (vban::tcp_endpoint const &);
