...
namespace
{
void reset_confirmation_heights (vban::write_transaction const & transaction, vban::block_store & store);
...
std::string vban::error_cli_messages::message (int ev) const
{
	switch (static_cast<vban::error_cli> (ev))
	{
		case vban::error_cli::generic:
			return "Unknown error";
		case vban::error_cli::parse_error:
			return "Coud not parse command line";
		case vban::error_cli::invalid_arguments:
			return "Invalid arguments";
		case vban::error_cli::unknown_command:
			return "Unknown command";
		case vban::error_cli::database_write_error:
			return "Database write error";
		case vban::error_cli::reading_config:
			return "Config file read error";
		case vban::error_cli::disable_all_network:
			return "Flags --disable_tcp_realtime and --disable_udp cannot be used together";
		case vban::error_cli::ambiguous_udp_options:
			return "Flags --disable_udp and --enable_udp cannot be used together";
		case vban::error_cli::ambiguous_pruning_voting_options:
...
void vban::add_node_options (boost::program_options::options_description & description_a)
{
	// clang-format off
...
void vban::add_node_flag_options (boost::program_options::options_description & description_a)
{
	// clang-format off
	description_a.add_options()
...
std::error_code vban::update_flags (vban::node_flags & flags_a, boost::program_options::variables_map const & vm)
{
	std::error_code ec;
	...
	if (config != vm.end ())
	{
		flags_a.config_overrides = vban::config_overrides (config->second.as<std::vector<vban::config_key_value_pair>> ());
	}
	auto rpcconfig (vm.find ("rpcconfig"));
	if (rpcconfig != vm.end ())
	{
		flags_a.rpc_config_overrides = vban::config_overrides (rpcconfig->second.as<std::vector<vban::config_key_value_pair>> ());
	}
	return ec;
}

std::error_code vban::flags_config_conflicts (vban::node_flags const & flags_a, vban::node_config const & config_a)
{
	std::error_code ec;
	if (flags_a.enable_pruning && config_a.enable_voting)
	{
		ec = vban::error_cli::ambiguous_pruning_voting_options;
	}
	return ec;
}
...
void database_write_lock_error (std::error_code & ec)
{
	...
	ec = vban::error_cli::database_write_error;
}
...
std::error_code vban::handle_node_options (boost::program_options::variables_map const & vm)
{
	std::error_code ec;
	boost::filesystem::path data_path = vm.count ("data_path") ? boost::filesystem::path (vm["data_path"].as<std::string> ()) : vban::working_path ();
			...
			if (!wallet_id.decode_hex (vm["wallet"].as<std::string> ()))
			{
				...
				if (wallet != nullptr)
				{
					...
					else
					{
						...
						ec = vban::error_cli::invalid_arguments;
					}
				}
				else
				{
		...
		try
		{
			...
			if (!ec)
			{
				...
				if (success)
				{
					...
					if (using_rocksdb)
					{
						vban::remove_all_files_in_dir (backup_path);
						vban::move_all_files_to_dir (source_path, backup_path);
						vban::move_all_files_to_dir (vacuum_path, source_path);
	...
	{
		auto data_path = vm.count ("data_path") ? boost::filesystem::path (vm["data_path"].as<std::string> ()) : vban::working_path ();
		auto node_flags = vban::inactive_node_flag_defaults ();
	...
	else if (vm.count ("generate_config"))
	{
		...
		if (valid_type)
		{
			std::cout << "# This is an example configuration file for Vban. Visit https://docs.vban.org/running-a-node/configuration/ for more information.\n#\n"
					  ...
					  << "# Additional information for notable configuration options is available in https://docs.vban.org/running-a-node/configuration/#notable-configuration-options\n";
	...
	else if (vm.count ("diagnostics"))
	{
		...
		vban::raw_key key;
		key.clear ();
		vban::send_block send (0, 0, 0, key, 0, 0);
		std::cout << "Testing key derivation function" << std::endl;
		vban::raw_key junk1;
		junk1.clear ();
		vban::uint256_union junk2 (0);
		vban::kdf kdf;
		kdf.phs (junk1, "", junk2);
		std::cout << "Testing time retrieval latency... " << std::flush;
		vban::timer<std::chrono::nanoseconds> timer (vban::timer_state::started);
	...
	else if (vm.count ("wallet_create"))
	{
		...
		{
			auto inactive_node = vban::default_inactive_node (data_path, vm);
			auto wallet_key = vban::random_wallet_id ();
						...
						else
						{
							...
							else
							{
								bool error (true);
								{
									...
									auto transaction (node->wallets.tx_begin_write ());
									vban::wallet wallet (error, transaction, node->wallets, wallet_id.to_string (), contents.str ());
								}
								if (error)
								{
									std::cerr << "Unable to import wallet\n";
	...
	else if (vm.count ("wallet_list"))
	{
		...
		for (auto i (node->wallets.items.begin ()), n (node->wallets.items.end ()); i != n; ++i)
		{
			...
			{
				std::cout << vban::account (j->first).to_account () << '\n';
			}
		}
	}
	...
	{
		ec = vban::error_cli::unknown_command;
	}

	return ec;
}

std::unique_ptr<vban::inactive_node> vban::default_inactive_node (boost::filesystem::path const & path_a, boost::program_options::variables_map const & vm_a)
{
	auto node_flags = vban::inactive_node_flag_defaults ();
	vban::update_flags (node_flags, vm_a);
	return std::make_unique<vban::inactive_node> (path_a, node_flags);
}
...
bool is_using_rocksdb (boost::filesystem::path const & data_path, boost::program_options::variables_map const & vm, std::error_code & ec)
{
	...
	{
		config_overrides = vban::config_overrides (config_arg->second.as<std::vector<vban::config_key_value_pair>> ());
	}

	// config override...
	auto error = vban::read_node_config_toml (data_path, config, config_overrides);
	if (!error)
	{
	...
	{
		ec = vban::error_cli::reading_config;
	}
