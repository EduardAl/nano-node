...
inline vban::build_flags operator| (vban::build_flags a, vban::build_flags b)
{
	return static_cast<vban::build_flags> (static_cast<uint8_t> (a) | static_cast<uint8_t> (b));
}
inline uint8_t operator| (uint8_t a, vban::build_flags b)
{
...
public:
	...
	abstract_builder & sign (vban::raw_key const & private_key, vban::public_key const & public_key);
...
protected:
	...
	uint8_t base_fields = static_cast<uint8_t> (vban::build_flags::work_present | vban::build_flags::signature_present);
};

/** Builder for state blocks */
class state_block_builder : public abstract_builder<vban::state_block, state_block_builder>
{
public:
	/** Creates a state block builder by calling make_block() */
	state_block_builder ();
	/** Initialize from an existing block */
	state_block_builder & from (vban::state_block const & block);
	...
	/** Set link */
	state_block_builder & link (vban::link const & link);
	/** Set link from hex representation */
	state_block_builder & link_hex (std::string const & link_hex);
	/** Set link from an xrb_ or vban_ address */
	state_block_builder & link_address (std::string const & link_address);
	/** Provides validation for build() */
	void validate ();

private:
	uint8_t required_fields = base_fields | static_cast<uint8_t> (vban::build_flags::account_present | vban::build_flags::balance_present | vban::build_flags::link_present | vban::build_flags::previous_present | vban::build_flags::representative_present);
};

/** Builder for open blocks */
class open_block_builder : public abstract_builder<vban::open_block, open_block_builder>
{
public:
	...
	open_block_builder & source (vban::block_hash source);
...
private:
	uint8_t required_fields = base_fields | static_cast<uint8_t> (vban::build_flags::account_present | vban::build_flags::representative_present | vban::build_flags::link_present);
};

/** Builder for change blocks */
class change_block_builder : public abstract_builder<vban::change_block, change_block_builder>
{
public:
	...
	/** Set representative */
	change_block_builder & representative (vban::account account);
	/** Set representative from hex representation of public key */
	...
	/** Set previous block hash */
	change_block_builder & previous (vban::block_hash previous);
	/** Set previous block hash from hex representation */
...
private:
	uint8_t required_fields = base_fields | static_cast<uint8_t> (vban::build_flags::previous_present | vban::build_flags::representative_present);
};

/** Builder for send blocks */
class send_block_builder : public abstract_builder<vban::send_block, send_block_builder>
{
public:
...
class receive_block_builder : public abstract_builder<vban::receive_block, receive_block_builder>
{
public:
	...
	/** Set previous block hash */
	receive_block_builder & previous (vban::block_hash previous);
	/** Set previous block hash from hex representation */
	receive_block_builder & previous_hex (std::string previous_hex);
	/** Set source block hash */
	receive_block_builder & source (vban::block_hash source);
	/** Set source block hash from hex representation */
...
public:
	...
	vban::state_block_builder & state ()
	{
	...
	vban::change_block_builder & change ()
	{
	...
	vban::receive_block_builder & receive ()
	{
...
private:
	...
	vban::open_block_builder open_builder;
	vban::change_block_builder change_builder;
	vban::send_block_builder send_builder;
	vban::receive_block_builder receive_builder;
};
}
