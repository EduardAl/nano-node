...
TEST (telemetry, consolidate_data)
{
	...
	data.uptime = 6;
	data.genesis_block = vban::block_hash (3);
	...
	data1.uptime = 10;
	data1.genesis_block = vban::block_hash (4);
	...
	data2.uptime = 3;
	data2.genesis_block = vban::block_hash (4);
	...
	std::vector<vban::telemetry_data> all_data{ data, data1, data2 };

	auto consolidated_telemetry_data = vban::consolidate_telemetry_data (all_data);
	...
	ASSERT_EQ (consolidated_telemetry_data.genesis_block, vban::block_hash (4));
	...
	all_data[2].genesis_block = vban::block_hash (3);
	...
	auto consolidated_telemetry_data1 = vban::consolidate_telemetry_data (all_data);
	...
	ASSERT_EQ (consolidated_telemetry_data1.genesis_block, vban::block_hash (3));
...
TEST (telemetry, consolidate_data_remove_outliers)
{
	...
	std::vector<vban::telemetry_data> all_data (20, data);

	// Insert some outliers
	vban::telemetry_data lower_bound_outlier_data;
	...
	lower_bound_outlier_data.genesis_block = vban::block_hash (2);
	...
	vban::telemetry_data upper_bound_outlier_data;
	...
	upper_bound_outlier_data.genesis_block = vban::block_hash (99);
...
TEST (telemetry, basic)
{
	...
	auto channel = node_client->network.find_channel (node_server->network.endpoint ());
	{
		...
		node_client->telemetry->get_metrics_single_peer_async (channel, [&done, &server_endpoint, &telemetry_data] (vban::telemetry_data_response const & response_a) {
	...
	vban::compare_default_telemetry_response_data (telemetry_data, node_server->network_params, node_server->config.bandwidth_limit, node_server->default_difficulty (vban::work_version::work_1), node_server->node_id);
	...
	std::this_thread::sleep_for (vban::telemetry_cache_cutoffs::dev);
...
TEST (telemetry, receive_from_non_listening_channel)
{
	...
	vban::telemetry_ack message (vban::telemetry_data{});
...
TEST (telemetry, over_udp)
{
	...
	std::atomic<bool> done{ false };
	auto channel = node_client->network.find_channel (node_server->network.endpoint ());
	node_client->telemetry->get_metrics_single_peer_async (channel, [&done, &node_server] (vban::telemetry_data_response const & response_a) {
		ASSERT_FALSE (response_a.error);
		vban::compare_default_telemetry_response_data (response_a.telemetry_data, node_server->network_params, node_server->config.bandwidth_limit, node_server->default_difficulty (vban::work_version::work_1), node_server->node_id);
		done = true;
	});
...
TEST (telemetry, invalid_channel)
{
	...
	std::atomic<bool> done{ false };
	node_client->telemetry->get_metrics_single_peer_async (nullptr, [&done] (vban::telemetry_data_response const & response_a) {
...
TEST (telemetry, blocking_request)
{
	...
	vban::compare_default_telemetry_response_data (telemetry_data_response.telemetry_data, node_server->network_params, node_server->config.bandwidth_limit, node_server->default_difficulty (vban::work_version::work_1), node_server->node_id);
...
TEST (telemetry, disconnects)
{
	...
	std::atomic<bool> done{ false };
	node_client->telemetry->get_metrics_single_peer_async (channel, [&done] (vban::telemetry_data_response const & response_a) {
		ASSERT_TRUE (response_a.error);
		done = true;
	});
...
TEST (telemetry, dos_tcp)
{
	...
	vban::telemetry_req message;
	auto channel = node_client->network.tcp_channels.find_channel (vban::transport::map_endpoint_to_tcp (node_server->network.endpoint ()));
	channel->send (message, [] (boost::system::error_code const & ec, size_t size_a) {
		ASSERT_FALSE (ec);
	});

	ASSERT_TIMELY (10s, 1 == node_server->stats.count (vban::stat::type::message, vban::stat::detail::telemetry_req, vban::stat::dir::in));
	...
	ASSERT_TIMELY (10s, (vban::telemetry_cache_cutoffs::dev + orig) <= std::chrono::steady_clock::now ());
	...
	while (node_server->stats.count (vban::stat::type::message, vban::stat::detail::telemetry_req, vban::stat::dir::in) == 1)
	{
...
TEST (telemetry, dos_udp)
{
	...
	ASSERT_TIMELY (20s, 1 == node_server->stats.count (vban::stat::type::message, vban::stat::detail::telemetry_req, vban::stat::dir::in));
	...
	ASSERT_TIMELY (20s, (vban::telemetry_cache_cutoffs::dev + orig) <= std::chrono::steady_clock::now ());
...
TEST (telemetry, disable_metrics)
{
	...
	node_server->telemetry->get_metrics_single_peer_async (channel1, [&done, node_client] (vban::telemetry_data_response const & response_a) {
		ASSERT_FALSE (response_a.error);
		vban::compare_default_telemetry_response_data (response_a.telemetry_data, node_client->network_params, node_client->config.bandwidth_limit, node_client->default_difficulty (vban::work_version::work_1), node_client->node_id);
		done = true;
	});
...
TEST (telemetry, max_possible_size)
{
	...
	vban::telemetry_data data;
	data.unknown_data.resize (vban::message_header::telemetry_size_mask.to_ulong () - vban::telemetry_data::latest_size);

	vban::telemetry_ack message (data);
	wait_peer_connections (system);
	...
	ASSERT_TIMELY (10s, 1 == node_server->stats.count (vban::stat::type::message, vban::stat::detail::telemetry_ack, vban::stat::dir::in));
}
...
TEST (telemetry, remove_peer_different_genesis)
{
	...
	node1->network_params.ledger.genesis_hash = vban::block_hash ("0");
	node1->start ();
	system.nodes.push_back (node1);
	node0->network.merge_peer (node1->network.endpoint ());
	node1->network.merge_peer (node0->network.endpoint ());
	ASSERT_TIMELY (10s, node0->stats.count (vban::stat::type::telemetry, vban::stat::detail::different_genesis_hash) != 0 && node1->stats.count (vban::stat::type::telemetry, vban::stat::detail::different_genesis_hash) != 0);

	ASSERT_TIMELY (1s, 0 == node0->network.size ());
	ASSERT_TIMELY (1s, 0 == node1->network.size ());
	ASSERT_EQ (node0->stats.count (vban::stat::type::message, vban::stat::detail::node_id_handshake, vban::stat::dir::out), 1);
	ASSERT_EQ (node1->stats.count (vban::stat::type::message, vban::stat::detail::node_id_handshake, vban::stat::dir::out), 1);

	vban::lock_guard<vban::mutex> guard (node0->network.excluded_peers.mutex);
	ASSERT_EQ (1, node0->network.excluded_peers.peers.get<vban::peer_exclusion::tag_endpoint> ().count (node1->network.endpoint ().address ()));
	ASSERT_EQ (1, node1->network.excluded_peers.peers.get<vban::peer_exclusion::tag_endpoint> ().count (node0->network.endpoint ().address ()));
}
...
TEST (telemetry, remove_peer_different_genesis_udp)
{
	...
	node_flags.disable_tcp_realtime = true;
	node_flags.disable_ongoing_telemetry_requests = true;
	vban::system system (1, vban::transport::transport_type::udp, node_flags);
	auto node0 (system.nodes[0]);
	ASSERT_EQ (0, node0->network.size ());
	...
	node1->start ();
	system.nodes.push_back (node1);
	auto channel0 (std::make_shared<vban::transport::channel_udp> (node1->network.udp_channels, node0->network.endpoint (), node0->network_params.protocol.protocol_version));
	...
	node0->telemetry->get_metrics_single_peer_async (channel1, [&done0] (vban::telemetry_data_response const & response_a) {
		done0 = true;
	});

	node1->telemetry->get_metrics_single_peer_async (channel0, [&done1] (vban::telemetry_data_response const & response_a) {
		done1 = true;
	});
...
TEST (telemetry, remove_peer_invalid_signature)
{
	...
	node->telemetry->recent_or_initial_request_telemetry_data.emplace (channel->get_endpoint (), vban::telemetry_data (), std::chrono::steady_clock::now (), true);
	...
	auto telemetry_ack = vban::telemetry_ack (telemetry_data);
	node->network.process_message (telemetry_ack, channel);

	ASSERT_TIMELY (10s, node->stats.count (vban::stat::type::telemetry, vban::stat::detail::invalid_signature) > 0);
	ASSERT_NO_ERROR (system.poll_until_true (3s, [&node, address = channel->get_endpoint ().address ()] () -> bool {
		vban::lock_guard<vban::mutex> guard (node->network.excluded_peers.mutex);
		return node->network.excluded_peers.peers.get<vban::peer_exclusion::tag_endpoint> ().count (address);
	}));
}
}
...
TEST (telemetry, maker_pruning)
{
	...
	ASSERT_EQ (vban::telemetry_maker::nf_pruned_node, static_cast<vban::telemetry_maker> (telemetry_data.maker));
}
