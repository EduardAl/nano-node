...
class configbase : public vban::error_aware<>
{
public:
	configbase () = default;
	configbase (std::shared_ptr<vban::error> const & error_a) :
		error (error_a)
	{
	}

	/** Returns the current error */
	vban::error & get_error () override
	{
		return *error;
	}
...
protected:
	...
	/** We're a vban::error_aware type. Child nodes share the error state. */
	std::shared_ptr<vban::error> error;
