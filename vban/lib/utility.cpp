...
std::size_t vban::get_file_descriptor_limit ()
{
...
void vban::set_file_descriptor_limit (std::size_t limit)
{
#ifndef _WIN32
	rlimit fd_limit{};
...
vban::container_info_composite::container_info_composite (std::string const & name) :
	name (name)
{
}

bool vban::container_info_composite::is_composite () const
{
	return true;
}

void vban::container_info_composite::add_component (std::unique_ptr<container_info_component> child)
{
	children.push_back (std::move (child));
}

const std::vector<std::unique_ptr<vban::container_info_component>> & vban::container_info_composite::get_children () const
{
	return children;
}

const std::string & vban::container_info_composite::get_name () const
{
	return name;
}

vban::container_info_leaf::container_info_leaf (const container_info & info) :
	info (info)
{
}

bool vban::container_info_leaf::is_composite () const
{
	return false;
}

const vban::container_info & vban::container_info_leaf::get_info () const
{
	return info;
}

void vban::dump_crash_stacktrace ()
{
	boost::stacktrace::safe_dump_to ("vban_node_backtrace.dump");
}

std::string vban::generate_stacktrace ()
{
...
void vban::remove_all_files_in_dir (boost::filesystem::path const & dir)
{
...
void vban::move_all_files_to_dir (boost::filesystem::path const & from, boost::filesystem::path const & to)
{
...
void assert_internal (const char * check_expr, const char * func, const char * file, unsigned int line, bool is_release_assert, std::string_view error_msg)
{
	...
	auto backtrace_str = vban::generate_stacktrace ();
...
#ifdef _WIN32
	{
		...
		std::string filename = is_release_assert ? "vban_node_backtrace_release_assert.txt" : "vban_node_backtrace_assert.txt";
		...
		vban::set_secure_perm_file (filepath);
		file << backtrace_str;
	}
#endif
