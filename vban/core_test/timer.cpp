...
TEST (timer, states)
{
	vban::timer<std::chrono::milliseconds> t1;
	ASSERT_EQ (t1.current_state (), vban::timer_state::stopped);
	t1.start ();
	ASSERT_EQ (t1.current_state (), vban::timer_state::started);
	t1.restart ();
	ASSERT_EQ (t1.current_state (), vban::timer_state::started);
	t1.pause ();
	ASSERT_EQ (t1.current_state (), vban::timer_state::stopped);
	...
	vban::timer<std::chrono::milliseconds> t2 (vban::timer_state::started);
	ASSERT_EQ (t2.current_state (), vban::timer_state::started);
	t2.stop ();
	ASSERT_EQ (t2.current_state (), vban::timer_state::stopped);
}

TEST (timer, measure_and_compare)
{
	using namespace std::chrono_literals;
	vban::timer<std::chrono::milliseconds> t1 (vban::timer_state::started);
	ASSERT_EQ (t1.current_state (), vban::timer_state::started);
