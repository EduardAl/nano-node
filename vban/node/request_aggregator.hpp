...
class request_aggregator final
{
	...
	{
		channel_pool () = delete;
		explicit channel_pool (std::shared_ptr<vban::transport::channel> const & channel_a) :
			channel (channel_a),
			endpoint (vban::transport::map_endpoint_to_v6 (channel_a->get_endpoint ()))
		{
		}
		std::vector<std::pair<vban::block_hash, vban::root>> hashes_roots;
...
public:
	request_aggregator (vban::network_constants const &, vban::node_config const & config, vban::stat & stats_a, vban::vote_generator &, vban::vote_generator &, vban::local_vote_history &, vban::ledger &, vban::wallets &, vban::active_transactions &);

	/** Add a new request by \p channel_a for hashes \p hashes_roots_a */
	void add (std::shared_ptr<vban::transport::channel> const & channel_a, std::vector<std::pair<vban::block_hash, vban::root>> const & hashes_roots_a);
	void stop ();
...
private:
	...
	void erase_duplicates (std::vector<std::pair<vban::block_hash, vban::root>> &) const;
	...
	std::pair<std::vector<std::shared_ptr<vban::block>>, std::vector<std::shared_ptr<vban::block>>> aggregate (std::vector<std::pair<vban::block_hash, vban::root>> const & requests_a, std::shared_ptr<vban::transport::channel> & channel_a) const;
	void reply_action (std::shared_ptr<vban::vote> const & vote_a, std::shared_ptr<vban::transport::channel> const & channel_a) const;

	vban::stat & stats;
	vban::local_vote_history & local_votes;
	vban::ledger & ledger;
	...
	mi::indexed_by<
		mi::hashed_unique<mi::tag<tag_endpoint>,
			mi::member<channel_pool, vban::endpoint, &channel_pool::endpoint>>,
	...
	vban::mutex mutex{ mutex_identifier (mutexes::request_aggregator) };
	std::thread thread;
