...
if [[ "$OSTYPE" == "linux-gnu" ]]; then
    ...
    DISTRO_CFG="-DVBAN_DISTRO_NAME=${distro}"
    ...
    CPACK_TYPE="TBZ2"
    DISTRO_CFG="-DVBAN_DISTRO_NAME='freebsd'"
else
    CPACK_TYPE="TBZ2"
fi

if [[ ${SIMD} -eq 1 ]]; then
    SIMD_CFG="-DVBAN_SIMD_OPTIMIZATIONS=ON"
...
if [[ "${BETA}" -eq 1 ]]; then
    NETWORK_CFG="-DACTIVE_NETWORK=vban_beta_network"
    CONFIGURATION="RelWithDebInfo"
else
    NETWORK_CFG="-DACTIVE_NETWORK=vban_live_network"
