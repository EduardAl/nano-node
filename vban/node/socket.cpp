...
vban::socket::socket (vban::node & node_a, boost::optional<std::chrono::seconds> io_timeout_a) :
...
void vban::socket::async_connect (vban::tcp_endpoint const & endpoint_a, std::function<void (boost::system::error_code const &)> callback_a)
{
	checkup ();
...
void vban::socket::async_read (std::shared_ptr<std::vector<uint8_t>> const & buffer_a, size_t size_a, std::function<void (boost::system::error_code const &, size_t)> callback_a)
{
		...
		if (!closed)
		{
			...
			boost::asio::post (strand, boost::asio::bind_executor (strand, [buffer_a, callback_a, size_a, this_l] () {
				...
				[this_l, buffer_a, callback_a] (boost::system::error_code const & ec, size_t size_a) {
					this_l->node.stats.add (vban::stat::type::traffic_tcp, vban::stat::dir::in, size_a);
					this_l->stop_timer ();
	...
	{
		debug_assert (false && "vban::socket::async_read called with incorrect buffer size");
...
void vban::socket::async_write (vban::shared_const_buffer const & buffer_a, std::function<void (boost::system::error_code const &, size_t)> const & callback_a)
{
	if (!closed)
	{
		++queue_size;
		boost::asio::post (strand, boost::asio::bind_executor (strand, [buffer_a, callback_a, this_l = shared_from_this ()] () {
			...
			{
				this_l->start_timer ();
				vban::async_write (this_l->tcp_socket, buffer_a,
				boost::asio::bind_executor (this_l->strand,
				[buffer_a, callback_a, this_l] (boost::system::error_code ec, std::size_t size_a) {
					--this_l->queue_size;
					this_l->node.stats.add (vban::stat::type::traffic_tcp, vban::stat::dir::out, size_a);
					this_l->stop_timer ();
					if (callback_a)
					{
						callback_a (ec, size_a);
...
void vban::socket::start_timer ()
{
	start_timer (io_timeout.get ());
}

void vban::socket::start_timer (std::chrono::seconds deadline_a)
{
	next_deadline = deadline_a.count ();
}

void vban::socket::stop_timer ()
{
	last_completion_time = vban::seconds_since_epoch ();
}

void vban::socket::checkup ()
{
	std::weak_ptr<vban::socket> this_w (shared_from_this ());
	node.workers.add_timed_task (std::chrono::steady_clock::now () + std::chrono::seconds (node.network_params.network.is_dev_network () ? 1 : 2), [this_w] () {
		...
		{
			uint64_t now (vban::seconds_since_epoch ());
...
bool vban::socket::has_timed_out () const
{
	return timed_out;
}

void vban::socket::set_timeout (std::chrono::seconds io_timeout_a)
{
	auto this_l (shared_from_this ());
...
void vban::socket::close_internal ()
{
	if (!closed.exchange (true))
	{
		...
		if (ec)
		{
			...
			node.stats.inc (vban::stat::type::bootstrap, vban::stat::detail::error_socket_close);
		}
	}
}

vban::tcp_endpoint vban::socket::remote_endpoint () const
{
	return remote;
}

vban::server_socket::server_socket (vban::node & node_a, boost::asio::ip::tcp::endpoint local_a, size_t max_connections_a) :
...
void vban::server_socket::start (boost::system::error_code & ec_a)
{
...
void vban::server_socket::close ()
{
	auto this_l (std::static_pointer_cast<vban::server_socket> (shared_from_this ()));
...
void vban::server_socket::on_connection (std::function<bool (std::shared_ptr<vban::socket> const &, boost::system::error_code const &)> callback_a)
{
	auto this_l (std::static_pointer_cast<vban::server_socket> (shared_from_this ()));
	...
	boost::asio::post (strand, boost::asio::bind_executor (strand, [this_l, callback_a] () {
		...
		auto new_connection = std::make_shared<vban::socket> (this_l->node, boost::none);
		...
		[this_l, new_connection, callback_a] (boost::system::error_code const & ec_a) {
			...
			if (this_l->connections.size () >= this_l->max_inbound_connections)
			{
				...
				this_l->node.stats.inc (vban::stat::type::tcp, vban::stat::detail::tcp_accept_failure, vban::stat::dir::in);
			...
			if (!ec_a)
			{
				...
				this_l->node.stats.inc (vban::stat::type::tcp, vban::stat::detail::tcp_accept_success, vban::stat::dir::in);
...
void vban::server_socket::on_connection_requeue_delayed (std::function<bool (std::shared_ptr<vban::socket> const &, boost::system::error_code const &)> callback_a)
{
	auto this_l (std::static_pointer_cast<vban::server_socket> (shared_from_this ()));
...
bool vban::server_socket::is_temporary_error (boost::system::error_code const ec_a)
{
	switch (ec_a.value ())
	{
...
void vban::server_socket::evict_dead_connections ()
{
