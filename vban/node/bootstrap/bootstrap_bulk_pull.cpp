...
vban::pull_info::pull_info (vban::hash_or_account const & account_or_head_a, vban::block_hash const & head_a, vban::block_hash const & end_a, uint64_t bootstrap_id_a, count_t count_a, unsigned retry_limit_a) :
...
vban::bulk_pull_client::bulk_pull_client (std::shared_ptr<vban::bootstrap_client> const & connection_a, std::shared_ptr<vban::bootstrap_attempt> const & attempt_a, vban::pull_info const & pull_a) :
...
vban::bulk_pull_client::~bulk_pull_client ()
{
	...
	{
		pull.head = expected;
		if (attempt->mode != vban::bootstrap_mode::legacy)
		{
...
void vban::bulk_pull_client::request ()
{
	...
	req, [this_l] (boost::system::error_code const & ec, size_t size_a) {
		...
		else
		{
			...
			this_l->connection->node->stats.inc (vban::stat::type::bootstrap, vban::stat::detail::bulk_pull_request_failure, vban::stat::dir::in);
		}
	},
	vban::buffer_drop_policy::no_limiter_drop);
}

void vban::bulk_pull_client::throttled_receive_block ()
{
...
void vban::bulk_pull_client::receive_block ()
{
	auto this_l (shared_from_this ());
	connection->socket->async_read (connection->receive_buffer, 1, [this_l] (boost::system::error_code const & ec, size_t size_a) {
		...
		else
		{
			...
			this_l->connection->node->stats.inc (vban::stat::type::bootstrap, vban::stat::detail::bulk_pull_receive_block_failure, vban::stat::dir::in);
			this_l->network_error = true;
		}
	});
}

void vban::bulk_pull_client::received_type ()
{
	auto this_l (shared_from_this ());
	vban::block_type type (static_cast<vban::block_type> (connection->receive_buffer->data ()[0]));
	...
	switch (type)
	{
		...
		{
			socket_l->async_read (connection->receive_buffer, vban::send_block::size, [this_l, type] (boost::system::error_code const & ec, size_t size_a) {
		...
		{
			socket_l->async_read (connection->receive_buffer, vban::receive_block::size, [this_l, type] (boost::system::error_code const & ec, size_t size_a) {
				this_l->received_block (ec, size_a, type);
			});
			break;
		}
		...
		{
			socket_l->async_read (connection->receive_buffer, vban::open_block::size, [this_l, type] (boost::system::error_code const & ec, size_t size_a) {
				this_l->received_block (ec, size_a, type);
			});
			break;
		}
		...
		{
			socket_l->async_read (connection->receive_buffer, vban::change_block::size, [this_l, type] (boost::system::error_code const & ec, size_t size_a) {
				this_l->received_block (ec, size_a, type);
			});
			break;
		}
		...
		{
			socket_l->async_read (connection->receive_buffer, vban::state_block::size, [this_l, type] (boost::system::error_code const & ec, size_t size_a) {
				this_l->received_block (ec, size_a, type);
			});
			break;
		}
...
void vban::bulk_pull_client::received_block (boost::system::error_code const & ec, size_t size_a, vban::block_type type_a)
{
	if (!ec)
	{
		vban::bufferstream stream (connection->receive_buffer->data (), size_a);
		...
		if (block != nullptr && !vban::work_validate_entry (*block))
		{
			...
			// Unconfirmed head is used only for lazy destinations if legacy bootstrap is not available, see vban::bootstrap_attempt::lazy_destinations_increment (...)
			...
			if (!stop_pull && !connection->hard_stop.load ())
			{
				...
				to prevent spam */
				if (attempt->mode != vban::bootstrap_mode::legacy || unexpected_count < 16384)
				{
		...
		else if (block == nullptr)
		{
			...
			connection->node->stats.inc (vban::stat::type::bootstrap, vban::stat::detail::bulk_pull_deserialize_receive_block, vban::stat::dir::in);
		}
		else // Work invalid
		{
			if (connection->node->config.logging.bulk_pull_logging ())
	...
	else
	{
		...
		connection->node->stats.inc (vban::stat::type::bootstrap, vban::stat::detail::bulk_pull_receive_block_failure, vban::stat::dir::in);
		network_error = true;
	}
}

vban::bulk_pull_account_client::bulk_pull_account_client (std::shared_ptr<vban::bootstrap_client> const & connection_a, std::shared_ptr<vban::bootstrap_attempt> const & attempt_a, vban::account const & account_a) :
	connection (connection_a),
	attempt (attempt_a),
	account (account_a),
	pull_blocks (0)
...
vban::bulk_pull_account_client::~bulk_pull_account_client ()
{
	attempt->pull_finished ();
}

void vban::bulk_pull_account_client::request ()
{
	vban::bulk_pull_account req;
	req.account = account;
	req.minimum_amount = connection->node->config.receive_minimum;
	req.flags = vban::bulk_pull_account_flags::pending_hash_and_amount;
	if (connection->node->config.logging.bulk_pull_logging ())
	{
	...
	req, [this_l] (boost::system::error_code const & ec, size_t size_a) {
		...
		else
		{
			...
			this_l->connection->node->stats.inc (vban::stat::type::bootstrap, vban::stat::detail::bulk_pull_error_starting_request, vban::stat::dir::in);
		}
	},
	vban::buffer_drop_policy::no_limiter_drop);
}

void vban::bulk_pull_account_client::receive_pending ()
{
	auto this_l (shared_from_this ());
	size_t size_l (sizeof (vban::uint256_union) + sizeof (vban::uint128_union));
	connection->socket->async_read (connection->receive_buffer, size_l, [this_l, size_l] (boost::system::error_code const & ec, size_t size_a) {
		...
		if (size_a == size_l)
		{
			if (!ec)
			{
				vban::block_hash pending;
				vban::bufferstream frontier_stream (this_l->connection->receive_buffer->data (), sizeof (vban::uint256_union));
				auto error1 (vban::try_read (frontier_stream, pending));
				(void)error1;
				debug_assert (!error1);
				vban::amount balance;
				vban::bufferstream balance_stream (this_l->connection->receive_buffer->data () + sizeof (vban::uint256_union), sizeof (vban::uint128_union));
				auto error2 (vban::try_read (balance_stream, balance));
				(void)error2;
				debug_assert (!error2);
 ...
 */
void vban::bulk_pull_server::set_current_end ()
{
	include_start = false;
...
void vban::bulk_pull_server::send_next ()
{
	auto block (get_next ());
	if (block != nullptr)
	{
		std::vector<uint8_t> send_buffer;
		{
			...
			vban::serialize_block (stream, *block);
		}
		auto this_l (shared_from_this ());
		if (connection->node->config.logging.bulk_pull_logging ())
		{
...
std::shared_ptr<vban::block> vban::bulk_pull_server::get_next ()
{
...
void vban::bulk_pull_server::sent_action (boost::system::error_code const & ec, size_t size_a)
{
	if (!ec)
	{
		send_next ();
...
void vban::bulk_pull_server::send_finished ()
{
	vban::shared_const_buffer send_buffer (static_cast<uint8_t> (vban::block_type::not_a_block));
	auto this_l (shared_from_this ());
	if (connection->node->config.logging.bulk_pull_logging ())
	{
...
void vban::bulk_pull_server::no_block_sent (boost::system::error_code const & ec, size_t size_a)
{
	if (!ec)
	{
...
vban::bulk_pull_server::bulk_pull_server (std::shared_ptr<vban::bootstrap_server> const & connection_a, std::unique_ptr<vban::bulk_pull> request_a) :
	connection (connection_a),
 ...
 */
void vban::bulk_pull_account_server::set_params ()
{
	debug_assert (request != nullptr);
	...
	if (request->flags == vban::bulk_pull_account_flags::pending_address_only)
	{
		pending_address_only = true;
	}
	else if (request->flags == vban::bulk_pull_account_flags::pending_hash_amount_and_address)
	{
		/**
	...
	else if (request->flags == vban::bulk_pull_account_flags::pending_hash_and_amount)
	{
...
void vban::bulk_pull_account_server::send_frontier ()
{
	/*
	...
	if (!invalid_request)
	{
		...
		vban::uint128_union account_frontier_balance (account_frontier_balance_int);

		// Write the frontier block hash and balance into a buffer
		std::vector<uint8_t> send_buffer;
		{
			vban::vectorstream output_stream (send_buffer);
...
void vban::bulk_pull_account_server::send_next_block ()
{
	/*
...
std::pair<std::unique_ptr<vban::pending_key>, std::unique_ptr<vban::pending_info>> vban::bulk_pull_account_server::get_next ()
{
	std::pair<std::unique_ptr<vban::pending_key>, std::unique_ptr<vban::pending_info>> result;
	...
	while (true)
	{
		...
		if (stream == vban::store_iterator<vban::pending_key, vban::pending_info> (nullptr))
		{
			break;
		}

		vban::pending_key key (stream->first);
		vban::pending_info info (stream->second);
		...
		result.first = std::make_unique<vban::pending_key> (key);
		result.second = std::make_unique<vban::pending_info> (info);
...
void vban::bulk_pull_account_server::sent_action (boost::system::error_code const & ec, size_t size_a)
{
	if (!ec)
	{
		send_next_block ();
...
void vban::bulk_pull_account_server::send_finished ()
{
	/*
	...
	{
		vban::vectorstream output_stream (send_buffer);
		vban::uint256_union account_zero (0);
		vban::uint128_union balance_zero (0);
...
void vban::bulk_pull_account_server::complete (boost::system::error_code const & ec, size_t size_a)
{
	if (!ec)
	{
		if (pending_address_only)
...
vban::bulk_pull_account_server::bulk_pull_account_server (std::shared_ptr<vban::bootstrap_server> const & connection_a, std::unique_ptr<vban::bulk_pull_account> request_a) :
	connection (connection_a),
	request (std::move (request_a)),
	current_key (0, 0)
{
