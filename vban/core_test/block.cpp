...
{
	vban::keypair key1;
	vban::send_block block (0, 1, 13, key1.prv, key1.pub, 2);
	auto hash (block.hash ());
	ASSERT_FALSE (vban::validate_message (key1.pub, hash, block.signature));
	block.signature.bytes[32] ^= 0x1;
	ASSERT_TRUE (vban::validate_message (key1.pub, hash, block.signature));
}

TEST (block, send_serialize)
{
	vban::send_block block1 (0, 1, 2, vban::keypair ().prv, 4, 5);
	...
	ASSERT_NE (0, size);
	vban::bufferstream stream2 (data, size);
	bool error (false);
	vban::send_block block2 (error, stream2);
	ASSERT_FALSE (error);
...
TEST (block, send_serialize_json)
{
	...
	bool error (false);
	vban::send_block block2 (error, tree1);
	ASSERT_FALSE (error);
	ASSERT_EQ (block1, block2);
}
...
{
	vban::receive_block block1 (0, 1, vban::keypair ().prv, 3, 4);
	vban::keypair key1;
	std::vector<uint8_t> bytes;
	{
		vban::vectorstream stream1 (bytes);
	...
	bool error (false);
	vban::receive_block block2 (error, stream2);
	ASSERT_FALSE (error);
	ASSERT_EQ (block1, block2);
}
...
TEST (block, receive_serialize_json)
{
	...
	boost::property_tree::read_json (istream, tree1);
	bool error (false);
	vban::receive_block block2 (error, tree1);
	ASSERT_FALSE (error);
	ASSERT_EQ (block1, block2);
}
...
TEST (block, open_serialize_json)
{
	...
	boost::property_tree::read_json (istream, tree1);
	bool error (false);
	vban::open_block block2 (error, tree1);
	ASSERT_FALSE (error);
	ASSERT_EQ (block1, block2);
}
...
{
	vban::change_block block1 (0, 1, vban::keypair ().prv, 3, 4);
	std::string string1;
	block1.serialize_json (string1);
	ASSERT_NE (0, string1.size ());
	boost::property_tree::ptree tree1;
	...
	boost::property_tree::read_json (istream, tree1);
	bool error (false);
	vban::change_block block2 (error, tree1);
	ASSERT_FALSE (error);
	ASSERT_EQ (block1, block2);
}
...
{
	vban::uint512_union input (vban::uint512_t (0));
	std::string text;
	input.encode_hex (text);
	vban::uint512_union output;
...
{
	vban::uint512_union input (vban::uint512_t (1));
	std::string text;
	input.encode_hex (text);
	vban::uint512_union output;
	auto error (output.decode_hex (text));
...
{
	vban::uint512_union input (vban::uint512_t (1000));
	std::string text;
	input.encode_hex (text);
	text[5] = '!';
	vban::uint512_union output;
...
{
	vban::uint512_union input (std::numeric_limits<vban::uint512_t>::max ());
	std::string text;
	input.encode_hex (text);
	vban::uint512_union output;
	auto error (output.decode_hex (text));
	ASSERT_FALSE (error);
	ASSERT_EQ (input, output);
	ASSERT_EQ (vban::uint512_t ("0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff"), output.number ());
}
...
TEST (send_block, deserialize)
{
	...
	ASSERT_EQ (vban::send_block::size, bytes.size ());
	vban::bufferstream stream2 (bytes.data (), bytes.size ());
	bool error (false);
	vban::send_block block2 (error, stream2);
	ASSERT_FALSE (error);
...
TEST (receive_block, deserialize)
{
	...
	ASSERT_EQ (vban::receive_block::size, bytes.size ());
	vban::bufferstream stream2 (bytes.data (), bytes.size ());
	bool error (false);
	vban::receive_block block2 (error, stream2);
	ASSERT_FALSE (error);
...
TEST (open_block, deserialize)
{
	...
	ASSERT_EQ (vban::open_block::size, bytes.size ());
	vban::bufferstream stream (bytes.data (), bytes.size ());
	bool error (false);
	vban::open_block block2 (error, stream);
	ASSERT_FALSE (error);
	ASSERT_EQ (block1, block2);
}
...
{
	vban::change_block block1 (1, 2, vban::keypair ().prv, 4, 5);
	ASSERT_EQ (block1.hash (), block1.hash ());
	std::vector<uint8_t> bytes;
	{
		vban::vectorstream stream1 (bytes);
	...
	ASSERT_EQ (vban::change_block::size, bytes.size ());
	auto data (bytes.data ());
	auto size (bytes.size ());
	ASSERT_NE (nullptr, data);
	ASSERT_NE (0, size);
	vban::bufferstream stream2 (data, size);
	bool error (false);
	vban::change_block block2 (error, stream2);
	ASSERT_FALSE (error);
	ASSERT_EQ (block1, block2);
}
...
TEST (frontier_req, serialization)
{
	...
	ASSERT_FALSE (error);
	vban::frontier_req request2 (error, stream, header);
	ASSERT_FALSE (error);
...
TEST (block, publish_req_serialization)
{
	...
	ASSERT_FALSE (error);
	vban::publish req2 (error, stream2, header);
	ASSERT_FALSE (error);
	ASSERT_EQ (req, req2);
...
{
	vban::send_block block (0, 1, 2, vban::keypair ().prv, 4, 5);
	ASSERT_EQ (block.difficulty (), vban::work_difficulty (block.work_version (), block.root (), block.block_work ()));
}
...
TEST (state_block, serialization)
{
	...
	ASSERT_EQ (vban::block_hash (1), block1->previous ());
	ASSERT_EQ (key2.pub, block1->hashables.representative);
	ASSERT_EQ (vban::amount (2), block1->hashables.balance);
	ASSERT_EQ (vban::uint256_union (4), block1->hashables.link);
	std::vector<uint8_t> bytes;
	{
		vban::vectorstream stream (bytes);
		block1->serialize (stream);
	...
	ASSERT_EQ (vban::state_block::size, bytes.size ());
	bool error1 (false);
	vban::bufferstream stream (bytes.data (), bytes.size ());
	vban::state_block block2 (error1, stream);
	ASSERT_FALSE (error1);
	ASSERT_EQ (*block1, block2);
	...
	bool error2 (false);
	vban::state_block block3 (error2, tree);
	ASSERT_FALSE (error2);
...
{
	ASSERT_EQ (vban::work_version::work_1, vban::send_block ().work_version ());
	ASSERT_EQ (vban::work_version::work_1, vban::receive_block ().work_version ());
	ASSERT_EQ (vban::work_version::work_1, vban::change_block ().work_version ());
	ASSERT_EQ (vban::work_version::work_1, vban::open_block ().work_version ());
	ASSERT_EQ (vban::work_version::work_1, vban::state_block ().work_version ());
}
...
TEST (block_uniquer, single)
{
	...
	ASSERT_EQ (*block1, *block2);
	std::weak_ptr<vban::state_block> block3 (block2);
...
TEST (block_builder, zeroed_state_block)
{
	vban::block_builder builder;
	vban::keypair key;
	vban::state_block_builder state_builder;
	...
	ASSERT_FALSE (vban::validate_message (key.pub, zero_block_build->hash (), zero_block_build->signature));
}
...
TEST (block_builder, state_missing_rep)
{
	...
	auto block = builder
				 ...
				 .work (0)
				 .build (ec);
	ASSERT_EQ (ec, vban::error_common::missing_representative);
}
...
TEST (block_builder, state_equality)
{
	...
	vban::state_block block1 (key1.pub, 1, key2.pub, 2, 4, key1.prv, key1.pub, 5);
...
TEST (block_builder, state_errors)
{
	...
	ASSERT_EQ (ec, vban::error_common::bad_account_number);
...
TEST (block_builder, open_equality)
{
	...
	// With constructor
	vban::keypair key1, key2;
	vban::open_block block1 (1, key1.pub, key2.pub, key1.prv, key1.pub, 5);
...
TEST (block_builder, change_equality)
{
	...
	// With constructor
	vban::keypair key1, key2;
	vban::change_block block1 (1, key1.pub, key1.prv, key1.pub, 5);
...
TEST (block_builder, send)
{
	...
	ASSERT_TRUE (block->source ().is_zero ());
	ASSERT_EQ (block->destination ().to_account (), "vban_1gys8r4crpxhp94n4uho5cshaho81na6454qni5gu9n53gksoyy1wcd4udyb");
	ASSERT_TRUE (block->link ().is_zero ());
}
...
TEST (block_builder, send_equality)
{
	...
	// With constructor
	vban::keypair key1, key2;
	vban::send_block block1 (1, key1.pub, 2, key1.prv, key1.pub, 5);
...
TEST (block_builder, receive_equality)
{
	...
	// With constructor
	vban::keypair key1;
	vban::receive_block block1 (1, 2, key1.prv, key1.pub, 5);
