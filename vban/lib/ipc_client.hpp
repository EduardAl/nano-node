...
namespace ipc
{
	...
	public:
		...
		vban::error connect (std::string const & path);

		/** Connect to a tcp socket synchronously */
		vban::error connect (std::string const & host, uint16_t port);

		/** Connect to a tcp socket asynchronously */
		void async_connect (std::string const & host, uint16_t port, std::function<void (vban::error)> callback);

		/** Write buffer asynchronously */
		void async_write (vban::shared_const_buffer const & buffer_a, std::function<void (vban::error, size_t)> callback_a);

		/** Read \p size_a bytes asynchronously */
		void async_read (std::shared_ptr<std::vector<uint8_t>> const & buffer_a, size_t size_a, std::function<void (vban::error, size_t)> callback_a);
		 ...
		 */
		void async_read_message (std::shared_ptr<std::vector<uint8_t>> const & buffer_a, std::chrono::seconds timeout_a, std::function<void (vban::error, size_t)> callback_a);
	...
	std::string request (vban::ipc::payload_encoding encoding_a, vban::ipc::ipc_client & ipc_client, std::string const & rpc_action_a);

	/**
	 * Returns a buffer with an IPC preamble for the given \p encoding_a
	 */
	std::vector<uint8_t> get_preamble (vban::ipc::payload_encoding encoding_a);

	/**
	 * Returns a buffer with an IPC preamble, followed by 32-bit BE lenght, followed by payload
	 */
	vban::shared_const_buffer prepare_flatbuffers_request (std::shared_ptr<flatbuffers::FlatBufferBuilder> const & flatbuffer_a);

	template <typename T>
	vban::shared_const_buffer shared_buffer_from (T & object_a, std::string const & correlation_id_a = {}, std::string const & credentials_a = {})
	{
		auto buffer_l (vban::ipc::flatbuffer_producer::make_buffer (object_a, correlation_id_a, credentials_a));
		return vban::ipc::prepare_flatbuffers_request (buffer_l);
	}
	 ...
	 */
	vban::shared_const_buffer prepare_request (vban::ipc::payload_encoding encoding_a, std::string const & payload_a);
}
}
