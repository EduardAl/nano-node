...
namespace
{
void construct_json (vban::container_info_component * component, boost::property_tree::ptree & parent);
using ipc_json_handler_no_arg_func_map = std::unordered_map<std::string, std::function<void (vban::json_handler *)>>;
...
bool block_confirmed (vban::node & node, vban::transaction & transaction, vban::block_hash const & hash, bool include_active, bool include_only_confirmed);
const char * epoch_as_string (vban::epoch);
}

vban::json_handler::json_handler (vban::node & node_a, vban::node_rpc_config const & node_rpc_config_a, std::string const & body_a, std::function<void (std::string const &)> const & response_a, std::function<void ()> stop_callback_a) :
	body (body_a),
	node (node_a),
	response (response_a),
...
std::function<void ()> vban::json_handler::create_worker_task (std::function<void (std::shared_ptr<vban::json_handler> const &)> const & action_a)
{
...
void vban::json_handler::process_request (bool unsafe_a)
{
	try
	{
		...
		else
		{
			...
			else if (action == "kvban_from_raw" || action == "krai_from_raw")
			{
				mvban_from_raw (vban::kxrb_ratio);
			}
			else if (action == "kvban_to_raw" || action == "krai_to_raw")
			{
				mvban_to_raw (vban::kxrb_ratio);
			}
			else if (action == "vban_from_raw" || action == "rai_from_raw")
			{
				mvban_from_raw (vban::xrb_ratio);
			}
			else if (action == "vban_to_raw" || action == "rai_to_raw")
			{
				mvban_to_raw (vban::xrb_ratio);
			}
			else if (action == "mvban_from_raw" || action == "mrai_from_raw")
			{
				mvban_from_raw ();
			}
			else if (action == "mvban_to_raw" || action == "mrai_to_raw")
			{
				mvban_to_raw ();
			}
...
std::shared_ptr<vban::wallet> vban::json_handler::wallet_impl ()
{
	if (!ec)
	{
		...
		if (!wallet.decode_hex (wallet_text))
		{
			...
			{
				ec = vban::error_common::wallet_not_found;
			}
		}
		else
		{
			ec = vban::error_common::bad_wallet_number;
		}
	}
	return nullptr;
}

bool vban::json_handler::wallet_locked_impl (vban::transaction const & transaction_a, std::shared_ptr<vban::wallet> const & wallet_a)
{
	bool result (false);
	if (!ec)
	{
		...
		{
			ec = vban::error_common::wallet_locked;
			result = true;
		}
	}
	return result;
...
bool vban::json_handler::wallet_account_impl (vban::transaction const & transaction_a, std::shared_ptr<vban::wallet> const & wallet_a, vban::account const & account_a)
{
	bool result (false);
	if (!ec)
	{
		...
		{
			ec = vban::error_common::account_not_found_wallet;
		}
	}
	return result;
}

vban::account vban::json_handler::account_impl (std::string account_text, std::error_code ec_a)
{
	...
	if (!ec)
	{
		...
		{
			// vban- and xrb- prefixes are deprecated
...
vban::account_info vban::json_handler::account_info_impl (vban::transaction const & transaction_a, vban::account const & account_a)
{
	...
	if (!ec)
	{
		...
		{
			ec = vban::error_common::account_not_found;
...
vban::amount vban::json_handler::amount_impl ()
{
	vban::amount result (0);
	if (!ec)
	{
		std::string amount_text (request.get<std::string> ("amount"));
		if (result.decode_dec (amount_text))
		{
			ec = vban::error_common::invalid_amount;
		}
	}
	return result;
}

std::shared_ptr<vban::block> vban::json_handler::block_impl (bool signature_work_required)
{
	const bool json_block_l = request.get<bool> ("json_block", false);
	std::shared_ptr<vban::block> result{ nullptr };
	if (!ec)
	{
		...
		else
		{
			...
			{
				ec = vban::error_blocks::invalid_block;
			}
		}
		if (!ec)
		{
			...
			result = vban::deserialize_block_json (block_l);
			if (result == nullptr)
			{
				ec = vban::error_blocks::invalid_block;
			}
...
vban::block_hash vban::json_handler::hash_impl (std::string search_text)
{
	vban::block_hash result (0);
	if (!ec)
	{
		std::string hash_text (request.get<std::string> (search_text));
		if (result.decode_hex (hash_text))
		{
			ec = vban::error_blocks::invalid_block_hash;
		}
	}
	return result;
}

vban::amount vban::json_handler::threshold_optional_impl ()
{
	vban::amount result (0);
	...
	if (!ec && threshold_text.is_initialized ())
	{
		...
		{
			ec = vban::error_common::bad_threshold;
		}
	}
	return result;
}

uint64_t vban::json_handler::work_optional_impl ()
{
	uint64_t result (0);
	boost::optional<std::string> work_text (request.get_optional<std::string> ("work"));
	if (!ec && work_text.is_initialized ())
	{
		if (vban::from_string_hex (work_text.get (), result))
		{
			ec = vban::error_common::bad_work_format;
		}
	}
	return result;
}

uint64_t vban::json_handler::difficulty_optional_impl (vban::work_version const version_a)
{
	...
	{
		if (vban::from_string_hex (difficulty_text.get (), difficulty))
		{
			ec = vban::error_rpc::bad_difficulty_format;
		}
	}
	return difficulty;
}

uint64_t vban::json_handler::difficulty_ledger (vban::block const & block_a)
{
	vban::block_details details (vban::epoch::epoch_0, false, false, false);
	bool details_found (false);
	auto transaction (node.store.tx_begin_read ());
	// Previous block find
	std::shared_ptr<vban::block> block_previous (nullptr);
	...
	if (!link.is_zero () && !details.is_send)
	{
		...
		if (block_link != nullptr && node.store.pending_exists (transaction, vban::pending_key (block_a.account (), link.as_block_hash ())))
		{
	...
	return details_found ? vban::work_threshold (block_a.work_version (), details) : node.default_difficulty (block_a.work_version ());
}

double vban::json_handler::multiplier_optional_impl (vban::work_version const version_a, uint64_t & difficulty)
{
	double multiplier (1.);
	...
	if (!ec && multiplier_text.is_initialized ())
	{
		...
		{
			difficulty = vban::difficulty::from_multiplier (multiplier, node.default_difficulty (version_a));
		}
		else
		{
			ec = vban::error_rpc::bad_multiplier_format;
		}
	}
	return multiplier;
}

vban::work_version vban::json_handler::work_version_optional_impl (vban::work_version const default_a)
{
	vban::work_version result = default_a;
	boost::optional<std::string> version_text (request.get_optional<std::string> ("version"));
	if (!ec && version_text.is_initialized ())
	{
		if (*version_text == vban::to_string (vban::work_version::work_1))
		{
			result = vban::work_version::work_1;
		}
		else
		{
			ec = vban::error_rpc::bad_work_version;
		}
	}
	return result;
}
...
uint64_t vban::json_handler::count_impl ()
{
	uint64_t result (0);
	if (!ec)
	{
		...
		{
			ec = vban::error_common::invalid_count;
		}
	}
	return result;
}

uint64_t vban::json_handler::count_optional_impl (uint64_t result)
{
...
uint64_t vban::json_handler::offset_optional_impl (uint64_t result)
{
	...
	if (!ec && offset_text.is_initialized ())
	{
		...
		{
			ec = vban::error_rpc::invalid_offset;
		}
	}
	return result;
}

void vban::json_handler::account_balance ()
{
...
void vban::json_handler::account_block_count ()
{
	auto account (account_impl ());
	if (!ec)
	{
...
void vban::json_handler::account_create ()
{
	node.workers.push_task (create_worker_task ([] (std::shared_ptr<vban::json_handler> const & rpc_l) {
		auto wallet (rpc_l->wallet_impl ());
		if (!rpc_l->ec)
		{
			const bool generate_work = rpc_l->request.get<bool> ("work", true);
			vban::account new_key;
			...
			if (index_text.is_initialized ())
			{
				...
				{
					rpc_l->ec = vban::error_common::invalid_index;
				}
				else
				{
			...
			if (!rpc_l->ec)
			{
				...
				{
					rpc_l->ec = vban::error_common::wallet_locked;
				}
			}
		}
...
void vban::json_handler::account_get ()
{
	...
	{
		ec = vban::error_common::bad_public_key;
	}
	response_errors ();
}

void vban::json_handler::account_info ()
{
	auto account (account_impl ());
	if (!ec)
	{
		...
		if (!ec)
		{
			...
			vban::amount balance_l (info.balance);
			std::string balance;
			balance_l.encode_dec (balance);

			response_l.put ("balance", balance);

			vban::amount confirmed_balance_l;
			...
			std::shared_ptr<vban::block> confirmed_frontier_block;
			...
			if (representative)
			{
				...
				if (include_confirmed)
				{
					vban::account confirmed_representative{ 0 };
...
void vban::json_handler::account_key ()
{
	auto account (account_impl ());
	if (!ec)
	{
...
void vban::json_handler::account_list ()
{
	...
	if (!ec)
	{
		...
		for (auto i (wallet->store.begin (transaction)), j (wallet->store.end ()); i != j; ++i)
		{
			...
			entry.put ("", vban::account (i->first).to_account ());
...
void vban::json_handler::account_move ()
{
	node.workers.push_task (create_worker_task ([] (std::shared_ptr<vban::json_handler> const & rpc_l) {
		auto wallet (rpc_l->wallet_impl ());
		if (!rpc_l->ec)
		{
			...
			if (!source.decode_hex (source_text))
			{
				...
				if (existing != rpc_l->node.wallets.items.end ())
				{
					...
					std::vector<vban::public_key> accounts;
				...
				{
					rpc_l->ec = vban::error_rpc::source_not_found;
				}
			}
			else
			{
				rpc_l->ec = vban::error_rpc::bad_source;
			}
		}
		rpc_l->response_errors ();
	}));
...
void vban::json_handler::account_remove ()
{
	node.workers.push_task (create_worker_task ([] (std::shared_ptr<vban::json_handler> const & rpc_l) {
		auto wallet (rpc_l->wallet_impl ());
...
void vban::json_handler::account_representative ()
{
	auto account (account_impl ());
	if (!ec)
	{
...
void vban::json_handler::account_representative_set ()
{
	node.workers.push_task (create_worker_task ([work_generation_enabled = node.work_generation_enabled ()] (std::shared_ptr<vban::json_handler> const & rpc_l) {
		auto wallet (rpc_l->wallet_impl ());
		auto account (rpc_l->account_impl ());
		std::string representative_text (rpc_l->request.get<std::string> ("representative"));
		auto representative (rpc_l->account_impl (representative_text, vban::error_rpc::bad_representative_number));
		if (!rpc_l->ec)
		{
			...
			if (!rpc_l->ec && work)
			{
				...
				if (!rpc_l->ec)
				{
					...
					if (!rpc_l->ec)
					{
						vban::block_details details (info.epoch (), false, false, false);
						if (vban::work_difficulty (vban::work_version::work_1, info.head, work) < vban::work_threshold (vban::work_version::work_1, details))
						{
							rpc_l->ec = vban::error_common::invalid_work;
						}
					}
				}
			}
			else if (!rpc_l->ec) // work == 0
			{
				...
				{
					rpc_l->ec = vban::error_common::disabled_work_generation;
				}
			}
			if (!rpc_l->ec)
			{
				...
				wallet->change_async (
				account, representative, [response_a, response_data] (std::shared_ptr<vban::block> const & block) {
					if (block != nullptr)
					{
...
void vban::json_handler::account_weight ()
{
	auto account (account_impl ());
	if (!ec)
	{
...
void vban::json_handler::accounts_balances ()
{
...
void vban::json_handler::accounts_create ()
{
	node.workers.push_task (create_worker_task ([] (std::shared_ptr<vban::json_handler> const & rpc_l) {
		auto wallet (rpc_l->wallet_impl ());
		...
		if (!rpc_l->ec)
		{
			...
			{
				vban::account new_key (wallet->deterministic_insert (generate_work));
				if (!new_key.is_zero ())
				{
					boost::property_tree::ptree entry;
...
void vban::json_handler::accounts_frontiers ()
{
...
void vban::json_handler::accounts_pending ()
{
	...
	for (auto & accounts : request.get_child ("accounts"))
	{
		...
		if (!ec)
		{
			...
			for (auto i (node.store.pending_begin (transaction, vban::pending_key (account, 0))), n (node.store.pending_end ()); i != n && vban::pending_key (i->first).account == account && peers_l.size () < count; ++i)
			{
				vban::pending_key const & key (i->first);
			...
			if (sorting && !simple)
			{
				if (source)
				{
					peers_l.sort ([] (const auto & child1, const auto & child2) -> bool {
						return child1.second.template get<vban::uint256_t> ("amount") > child2.second.template get<vban::uint256_t> ("amount");
					});
				}
				else
				{
					peers_l.sort ([] (const auto & child1, const auto & child2) -> bool {
						return child1.second.template get<vban::uint256_t> ("") > child2.second.template get<vban::uint256_t> ("");
					});
				}
			}
...
void vban::json_handler::active_difficulty ()
{
	auto include_trend (request.get<bool> ("include_trend", false));
	auto const multiplier_active = 1.0;
	auto const default_difficulty (node.default_difficulty (vban::work_version::work_1));
	auto const default_receive_difficulty (node.default_receive_difficulty (vban::work_version::work_1));
	auto const receive_current_denormalized (vban::denormalized_multiplier (multiplier_active, node.network_params.network.publish_thresholds.epoch_2_receive));
	response_l.put ("deprecated", "1");
	response_l.put ("network_minimum", vban::to_string_hex (default_difficulty));
	response_l.put ("network_receive_minimum", vban::to_string_hex (default_receive_difficulty));
	response_l.put ("network_current", vban::to_string_hex (vban::difficulty::from_multiplier (multiplier_active, default_difficulty)));
	response_l.put ("network_receive_current", vban::to_string_hex (vban::difficulty::from_multiplier (receive_current_denormalized, default_receive_difficulty)));
...
void vban::json_handler::available_supply ()
{
	auto genesis_balance (node.balance (node.network_params.ledger.genesis_account)); // Cold storage genesis
	auto landing_balance (node.balance (vban::account ("059F68AAB29DE0D3A27443625C7EA9CDDB6517A8B76FE37727EF6A4D76832AD5"))); // Active unavailable account
	auto faucet_balance (node.balance (vban::account ("8E319CE6F3025E5B2DF66DA7AB1467FE48F1679C13DD43BFDB29FA2E9FC40D3B"))); // Faucet account
	auto burned_balance ((node.balance_pending (vban::account (0), false)).second); // Burning 0 account
...
void vban::json_handler::block_info ()
{
	auto hash (hash_impl ());
	if (!ec)
	{
		...
		if (block != nullptr)
		{
			vban::account account (block->account ().is_zero () ? block->sideband ().account : block->account ());
			...
			if (block->type () == vban::block_type::state)
			{
				auto subtype (vban::state_subtype (block->sideband ().details));
				response_l.put ("subtype", subtype);
			}
		}
		else
		{
			ec = vban::error_blocks::not_found;
		}
	}
	response_errors ();
}

void vban::json_handler::block_confirm ()
{
	auto hash (hash_impl ());
	if (!ec)
	{
		...
		if (block_l != nullptr)
		{
			...
			else
			{
				...
				vban::election_status status{ block_l, 0, 0, std::chrono::duration_cast<std::chrono::milliseconds> (std::chrono::system_clock::now ().time_since_epoch ()), std::chrono::duration_values<std::chrono::milliseconds>::zero (), 0, 1, 0, vban::election_status_type::active_confirmation_height };
				...
				if (!error_or_pruned)
				{
					if (auto state = dynamic_cast<vban::state_block *> (block_l.get ()))
					{
...
void vban::json_handler::blocks ()
{
	...
	for (boost::property_tree::ptree::value_type & hashes : request.get_child ("hashes"))
	{
		if (!ec)
		{
			...
			{
				ec = vban::error_blocks::bad_hash_number;
			}
		}
	}
...
void vban::json_handler::blocks_info ()
{
	const bool pending = request.get<bool> ("pending", false);
	const bool source = request.get<bool> ("source", false);
	const bool json_block_l = request.get<bool> ("json_block", false);
			...
			if (!hash.decode_hex (hash_text))
			{
				...
				if (block != nullptr)
				{
					...
					if (pending)
					{
						...
						{
							exists = node.store.pending_exists (transaction, vban::pending_key (destination, hash));
						}
						entry.put ("pending", exists ? "1" : "0");
					}
					if (source)
					{
						vban::block_hash source_hash (node.ledger.block_source (transaction, *block));
...
void vban::json_handler::block_account ()
{
	auto hash (hash_impl ());
	if (!ec)
	{
...
void vban::json_handler::block_count ()
{
...
void vban::json_handler::block_create ()
{
	std::string type (request.get<std::string> ("type"));
	vban::wallet_id wallet (0);
	// Default to work_1 if not specified
	auto work_version (work_version_optional_impl (vban::work_version::work_1));
	...
	{
		representative = account_impl (representative_text.get (), vban::error_rpc::bad_representative_number);
	}
	vban::account destination (0);
	...
	{
		destination = account_impl (destination_text.get (), vban::error_rpc::bad_destination);
	}
	vban::block_hash source (0);
	...
	{
		ec = vban::error_common::disabled_work_generation;
	}
	...
	if (!ec && key_text.is_initialized ())
	{
		...
		{
			ec = vban::error_common::bad_private_key;
		}
	}
	...
	if (!ec && previous_text.is_initialized ())
	{
		...
		{
			ec = vban::error_rpc::bad_previous;
		}
	}
	...
	if (!ec && balance_text.is_initialized ())
	{
		...
		{
			ec = vban::error_rpc::invalid_balance;
		}
	}
	vban::link link (0);
	...
	if (!ec)
	{
		...
		auto block_response_put_l = [rpc_l, this] (vban::block const & block_a) {
			boost::property_tree::ptree response_l;
			response_l.put ("hash", block_a.hash ().to_string ());
			response_l.put ("difficulty", vban::to_string_hex (block_a.difficulty ()));
			bool json_block_l = request.get<bool> ("json_block", false);
			if (json_block_l)
			{
				boost::property_tree::ptree block_node_l;
		...
		auto get_callback_l = [rpc_l, block_response_put_l] (std::shared_ptr<vban::block> const & block_a) {
			...
			return [block_a, rpc_l, block_response_put_l] (boost::optional<uint64_t> const & work_a) {
				if (block_a != nullptr)
				{
					...
					{
						rpc_l->ec = vban::error_common::failure_work_generation;
					}
				}
				else
				{
					rpc_l->ec = vban::error_common::generic;
				}
				if (rpc_l->ec)
				{
					rpc_l->response_errors ();
		...
		if (prv != 0)
		{
			vban::account pub (vban::pub_key (prv));
			...
			else if (previous_text.is_initialized () && balance_text.is_initialized () && type == "send")
			{
				...
				{
					ec = vban::error_rpc::block_create_balance_mismatch;
				}
			}
			...
			if (!ec && account_text.is_initialized ())
			{
				if (account != pub)
				{
					ec = vban::error_rpc::block_create_public_key_mismatch;
				}
			}
			vban::block_builder builder_l;
			std::shared_ptr<vban::block> block_l{ nullptr };
			vban::root root_l;
			...
			if (type == "state")
			{
				...
				{
					ec = vban::error_rpc::block_create_requirements_state;
				}
			}
			else if (type == "open")
			{
				...
				{
					ec = vban::error_rpc::block_create_requirements_open;
				}
			}
			else if (type == "receive")
			{
				...
				{
					ec = vban::error_rpc::block_create_requirements_receive;
				}
			}
			else if (type == "change")
			{
				...
				{
					ec = vban::error_rpc::block_create_requirements_change;
				}
			}
			else if (type == "send")
			{
				if (destination != 0 && previous != 0 && balance != 0 && amount != 0)
				{
					...
					{
						ec = vban::error_common::insufficient_balance;
					}
				}
				else
				{
					ec = vban::error_rpc::block_create_requirements_send;
				}
			}
			else
			{
				ec = vban::error_blocks::invalid_type;
			}
			if (!ec && (!ec_build || ec_build == vban::error_common::missing_work))
			{
				if (work == 0)
				{
					...
					node.work_generate (work_version, root_l, difficulty_l, get_callback_l (block_l), vban::account (pub));
				}
				else
				{
		...
		{
			ec = vban::error_rpc::block_create_key_required;
		}
	}
	// Because of callback
	if (ec)
...
void vban::json_handler::block_hash ()
{
...
void vban::json_handler::bootstrap ()
{
	...
	{
		uint16_t port;
		if (!vban::parse_port (port_text, port))
		{
			if (!node.flags.disable_legacy_bootstrap)
			{
				std::string bootstrap_id (request.get<std::string> ("id", ""));
				node.bootstrap_initiator.bootstrap (vban::endpoint (address, port), true, bootstrap_id);
				response_l.put ("success", "");
			}
			else
			{
				ec = vban::error_rpc::disabled_bootstrap_legacy;
			}
		}
		else
		{
			ec = vban::error_common::invalid_port;
		}
	}
	else
	{
		ec = vban::error_common::invalid_ip_address;
	}
	response_errors ();
}

void vban::json_handler::bootstrap_any ()
{
...
void vban::json_handler::bootstrap_lazy ()
{
	auto hash (hash_impl ());
	const bool force = request.get<bool> ("force", false);
	if (!ec)
	{
		...
		{
			ec = vban::error_rpc::disabled_bootstrap_lazy;
		}
	}
	response_errors ();
}
 ...
 */
void vban::json_handler::bootstrap_status ()
{
	...
	{
		vban::lock_guard<vban::mutex> connections_lock (node.bootstrap_initiator.connections->mutex);
	...
	{
		vban::lock_guard<vban::mutex> attempts_lock (node.bootstrap_initiator.attempts.bootstrap_attempts_mutex);
...
void vban::json_handler::chain (bool successors)
{
...
void vban::json_handler::confirmation_active ()
{
...
void vban::json_handler::confirmation_height_currently_processing ()
{
	...
	{
		ec = vban::error_rpc::confirmation_height_not_processing;
	}
	response_errors ();
}

void vban::json_handler::confirmation_history ()
{
	boost::property_tree::ptree elections;
...
void vban::json_handler::confirmation_info ()
{
	...
	if (!root.decode_hex (root_text))
	{
		...
		if (election != nullptr && !election->confirmed ())
		{
			...
			for (auto const & [tally, block] : info.tally)
			{
				...
				if (representatives)
				{
					std::multimap<vban::uint256_t, vban::account, std::greater<vban::uint256_t>> representatives;
		...
		{
			ec = vban::error_rpc::confirmation_not_found;
		}
	}
	else
	{
		ec = vban::error_rpc::invalid_root;
	}
	response_errors ();
}

void vban::json_handler::confirmation_quorum ()
{
...
void vban::json_handler::database_txn_tracker ()
{
	...
	{
		ec = vban::error_common::tracking_not_enabled;
	}

	response_errors ();
}

void vban::json_handler::delegators ()
{
	auto account (account_impl ());
	if (!ec)
	{
		...
		for (auto i (node.store.accounts_begin (transaction)), n (node.store.accounts_end ()); i != n; ++i)
		{
			...
			{
				std::string balance;
				vban::uint128_union (info.balance).encode_dec (balance);
...
void vban::json_handler::delegators_count ()
{
	auto account (account_impl ());
	if (!ec)
	{
...
void vban::json_handler::deterministic_key ()
{
	...
	if (!seed.decode_hex (seed_text))
	{
		try
		{
			...
			vban::raw_key prv = vban::deterministic_key (seed, index);
			vban::public_key pub (vban::pub_key (prv));
		...
		{
			ec = vban::error_common::invalid_index;
		}
	}
	else
	{
		ec = vban::error_common::bad_seed;
	}
	response_errors ();
}
 ...
 * @warning This is an internal/diagnostic RPC, do not rely on its interface being stable
 */
void vban::json_handler::epoch_upgrade ()
{
	vban::epoch epoch (vban::epoch::invalid);
	uint8_t epoch_int (request.get<uint8_t> ("epoch"));
	switch (epoch_int)
	{
		case 1:
			epoch = vban::epoch::epoch_1;
			break;
		case 2:
			epoch = vban::epoch::epoch_2;
			break;
		default:
			break;
	}
	if (epoch != vban::epoch::invalid)
	{
		...
		if (!ec && threads_text.is_initialized ())
		{
			...
			{
				ec = vban::error_rpc::invalid_threads_count;
			}
		}
		std::string key_text (request.get<std::string> ("key"));
		vban::raw_key prv;
		...
		{
			if (vban::pub_key (prv) == node.ledger.epoch_signer (node.ledger.epoch_link (epoch)))
			{
			...
			{
				ec = vban::error_rpc::invalid_epoch_signer;
			}
		}
		else
		{
	...
	{
		ec = vban::error_rpc::invalid_epoch;
	}
	response_errors ();
}

void vban::json_handler::frontiers ()
{
...
void vban::json_handler::account_count ()
{
...
namespace
{
class history_visitor : public vban::block_visitor
{
public:
	history_visitor (vban::json_handler & handler_a, bool raw_a, vban::transaction & transaction_a, boost::property_tree::ptree & tree_a, vban::block_hash const & hash_a, std::vector<vban::public_key> const & accounts_filter_a) :
		handler (handler_a),
		raw (raw_a),
	...
	void send_block (vban::send_block const & block_a)
	{
	...
	void receive_block (vban::receive_block const & block_a)
	{
	...
	void open_block (vban::open_block const & block_a)
	{
		if (raw)
		{
			tree.put ("type", "open");
	...
	void change_block (vban::change_block const & block_a)
	{
	...
	void state_block (vban::state_block const & block_a)
	{
		if (raw)
		{
			tree.put ("type", "state");
	...
	bool should_ignore_account (vban::public_key const & account)
	{
		bool ignore (false);
	...
	vban::json_handler & handler;
	bool raw;
	vban::transaction & transaction;
	boost::property_tree::ptree & tree;
	vban::block_hash const & hash;
	vban::network_params network_params;
	std::vector<vban::public_key> const & accounts_filter;
};
}

void vban::json_handler::account_history ()
{
	std::vector<vban::public_key> accounts_to_filter;
		...
		while (block != nullptr && count > 0)
		{
			...
			else
			{
				...
				if (!entry.empty ())
				{
					...
					if (output_raw)
					{
						entry.put ("work", vban::to_string_hex (block->block_work ()));
...
void vban::json_handler::keepalive ()
{
	if (!ec)
	{
		std::string address_text (request.get<std::string> ("address"));
...
void vban::json_handler::key_create ()
{
	vban::keypair pair;
...
void vban::json_handler::key_expand ()
{
	std::string key_text (request.get<std::string> ("key"));
	vban::raw_key prv;
	if (!prv.decode_hex (key_text))
...
void vban::json_handler::ledger ()
{
	auto count (count_optional_impl ());
	auto threshold (threshold_optional_impl ());
	if (!ec)
	{
		vban::account start (0);
		boost::optional<std::string> account_text (request.get_optional<std::string> ("account"));
		if (account_text.is_initialized ())
		{
		...
		if (modified_since_text.is_initialized ())
		{
			...
			{
				ec = vban::error_rpc::invalid_timestamp;
			}
		}
		const bool sorting = request.get<bool> ("sorting", false);
		const bool representative = request.get<bool> ("representative", false);
		...
		{
			std::vector<std::pair<vban::uint128_union, vban::account>> ledger_l;
			for (auto i (node.store.accounts_begin (transaction, start)), n (node.store.accounts_end ()); i != n; ++i)
			{
				vban::account_info const & info (i->second);
				vban::uint128_union balance (info.balance);
			...
			for (auto i (ledger_l.begin ()), n (ledger_l.end ()); i != n && accounts.size () < count; ++i)
			{
				...
				{
					vban::account const & account (i->second);
					boost::property_tree::ptree response_a;
					if (pending)
					{
						auto account_pending (node.ledger.account_pending (transaction, account));
...
void vban::json_handler::mvban_from_raw (vban::uint256_t ratio)
{
...
void vban::json_handler::mvban_to_raw (vban::uint256_t ratio)
{
	auto amount (amount_impl ());
	if (!ec)
	{
		...
		{
			ec = vban::error_common::invalid_amount_big;
		}
	}
	response_errors ();
}
 ...
 * @warning This is an internal/diagnostic RPC, do not rely on its interface being stable
 */
void vban::json_handler::node_id ()
{
	if (!ec)
	{
 ...
 * @warning This is an internal/diagnostic RPC, do not rely on its interface being stable
 */
void vban::json_handler::node_id_delete ()
{
	response_l.put ("deprecated", "1");
	response_errors ();
}

void vban::json_handler::password_change ()
{
	node.workers.push_task (create_worker_task ([] (std::shared_ptr<vban::json_handler> const & rpc_l) {
		auto wallet (rpc_l->wallet_impl ());
		if (!rpc_l->ec)
...
void vban::json_handler::password_enter ()
{
	node.workers.push_task (create_worker_task ([] (std::shared_ptr<vban::json_handler> const & rpc_l) {
		auto wallet (rpc_l->wallet_impl ());
		if (!rpc_l->ec)
...
void vban::json_handler::password_valid (bool wallet_locked)
{
	auto wallet (wallet_impl ());
	if (!ec)
	{
...
void vban::json_handler::peers ()
{
	...
	for (auto i (peers_list.begin ()), n (peers_list.end ()); i != n; ++i)
	{
		...
		if (peer_details)
		{
			...
			pending_tree.put ("type", channel->get_type () == vban::transport::transport_type::tcp ? "tcp" : "udp");
...
void vban::json_handler::pending ()
{
	auto account (account_impl ());
	auto count (count_optional_impl ());
	auto threshold (threshold_optional_impl ());
	...
	if (!ec)
	{
		...
		std::vector<std::pair<std::string, vban::uint256_t>> hash_amount_pairs;
		for (auto i (node.store.pending_begin (transaction, vban::pending_key (account, 0))), n (node.store.pending_end ()); i != n && vban::pending_key (i->first).account == account && (should_sort || peers_l.size () < count); ++i)
		{
			vban::pending_key const & key (i->first);
			if (block_confirmed (node, transaction, key.hash, include_active, include_only_confirmed))
			{
		...
		if (should_sort)
		{
			if (source || min_version)
			{
				...
				std::partial_sort (hash_ptree_pairs.begin (), mid, hash_ptree_pairs.end (), [] (const auto & lhs, const auto & rhs) {
					return lhs.second.template get<vban::uint256_t> ("amount") > rhs.second.template get<vban::uint256_t> ("amount");
				});
...
void vban::json_handler::pending_exists ()
{
	auto hash (hash_impl ());
	const bool include_active = request.get<bool> ("include_active", false);
	const bool include_only_confirmed = request.get<bool> ("include_only_confirmed", true);
...
void vban::json_handler::process ()
{
	node.workers.push_task (create_worker_task ([] (std::shared_ptr<vban::json_handler> const & rpc_l) {
		...
		if (!rpc_l->ec && block->type () == vban::block_type::state)
		{
			std::string subtype_text (rpc_l->request.get<std::string> ("subtype", ""));
			if (!subtype_text.empty ())
			{
				std::shared_ptr<vban::state_block> block_state (std::static_pointer_cast<vban::state_block> (block));
				...
				{
					rpc_l->ec = vban::error_process::gap_previous;
				}
				else
				{
					...
					if (subtype_text == "send")
					{
						...
						{
							rpc_l->ec = vban::error_rpc::invalid_subtype_balance;
						}
					...
					else if (subtype_text == "open")
					{
						...
						{
							rpc_l->ec = vban::error_rpc::invalid_subtype_previous;
						}
					}
					...
					else if (subtype_text == "epoch")
					{
						...
						{
							rpc_l->ec = vban::error_rpc::invalid_subtype_epoch_link;
						}
					}
					else
					{
						rpc_l->ec = vban::error_rpc::invalid_subtype;
					}
				}
			}
		}
		if (!rpc_l->ec)
		{
			if (!vban::work_validate_entry (*block))
			{
				if (!is_async)
				{
					...
					switch (result.code)
					{
						...
						case vban::process_result::fork:
						{
							...
							{
								rpc_l->ec = vban::error_process::fork;
							}
							break;
						}
						case vban::process_result::insufficient_work:
						{
							rpc_l->ec = vban::error_process::insufficient_work;
							break;
						}
						default:
						{
							rpc_l->ec = vban::error_process::other;
							break;
						}
					}
				}
				else
				{
					...
					{
						rpc_l->ec = vban::error_common::is_not_state_block;
					}
				}
			}
			else
			{
				rpc_l->ec = vban::error_blocks::work_low;
			}
		}
		rpc_l->response_errors ();
	}));
...
void vban::json_handler::pruned_exists ()
{
	auto hash (hash_impl ());
	if (!ec)
	{
		...
		{
			ec = vban::error_rpc::pruning_disabled;
		}
	}
	response_errors ();
}

void vban::json_handler::receive ()
{
	auto wallet (wallet_impl ());
	auto account (account_impl ());
	auto hash (hash_impl ("block"));
	if (!ec)
	{
		...
		if (!ec)
		{
			...
			if (node.ledger.block_or_pruned_exists (block_transaction, hash))
			{
				...
				if (!node.store.pending_get (block_transaction, vban::pending_key (account, hash), pending_info))
				{
					auto work (work_optional_impl ());
					if (!ec && work)
					{
						vban::account_info info;
						vban::root head;
						vban::epoch epoch = pending_info.epoch;
						...
						vban::block_details details (epoch, false, true, false);
						if (vban::work_difficulty (vban::work_version::work_1, head, work) < vban::work_threshold (vban::work_version::work_1, details))
						{
							ec = vban::error_common::invalid_work;
						}
					}
					else if (!ec) // && work == 0
					{
					...
					if (!ec)
					{
						...
						vban::account representative (wallet->store.representative (wallet_transaction));
						bool generate_work (work == 0); // Disable work generation if "work" option is provided
						auto response_a (response);
						wallet->receive_async (
						hash, representative, node.network_params.ledger.genesis_amount, account, [response_a] (std::shared_ptr<vban::block> const & block_a) {
							if (block_a != nullptr)
							{
								boost::property_tree::ptree response_l;
				...
				{
					ec = vban::error_process::unreceivable;
				}
			}
			else
			{
...
void vban::json_handler::receive_minimum ()
{
	if (!ec)
	{
...
void vban::json_handler::receive_minimum_set ()
{
	auto amount (amount_impl ());
	if (!ec)
	{
...
void vban::json_handler::representatives ()
{
	auto count (count_optional_impl ());
	if (!ec)
	{
		...
		if (!sorting) // Simple
		{
			std::map<vban::account, vban::uint256_t> ordered (rep_amounts.begin (), rep_amounts.end ());
		...
		{
			std::vector<std::pair<vban::uint256_t, std::string>> representation;
...
void vban::json_handler::representatives_online ()
{
...
void vban::json_handler::republish ()
{
	...
	if (!ec && sources_text.is_initialized ())
	{
		...
		{
			ec = vban::error_rpc::invalid_sources;
		}
	}
	...
	if (!ec && destinations_text.is_initialized ())
	{
		...
		{
			ec = vban::error_rpc::invalid_destinations;
		}
	}
	auto hash (hash_impl ());
	if (!ec)
	{
		...
		if (block != nullptr)
		{
			std::deque<std::shared_ptr<vban::block>> republish_bundle;
			for (auto i (0); !hash.is_zero () && i < count; ++i)
			{
				block = node.store.block_get (transaction, hash);
				if (sources != 0) // Republish source chain
				{
					vban::block_hash source (node.ledger.block_source (transaction, *block));
				...
				if (destinations != 0) // Republish destination chain
				{
					...
					if (!destination.is_zero ())
					{
						if (!node.store.pending_exists (transaction, vban::pending_key (destination, hash)))
						{
							vban::block_hash previous (node.ledger.latest (transaction, destination));
...
void vban::json_handler::search_pending ()
{
	auto wallet (wallet_impl ());
	if (!ec)
	{
...
void vban::json_handler::search_pending_all ()
{
	if (!ec)
	{
...
void vban::json_handler::send ()
{
	...
	auto source (account_impl (source_text, vban::error_rpc::bad_source));
	std::string destination_text (request.get<std::string> ("destination"));
	auto destination (account_impl (destination_text, vban::error_rpc::bad_destination));
	if (!ec)
	{
		auto work (work_optional_impl ());
		...
		if (!ec)
		{
			...
			if (!ec && work)
			{
				vban::block_details details (info.epoch (), true, false, false);
				if (vban::work_difficulty (vban::work_version::work_1, info.head, work) < vban::work_threshold (vban::work_version::work_1, details))
				{
					ec = vban::error_common::invalid_work;
				}
		...
		if (!ec)
		{
			...
			wallet->send_async (
			source, destination, amount.number (), [balance, amount, response_a, response_data] (std::shared_ptr<vban::block> const & block_a) {
				if (block_a != nullptr)
				{
				...
				else
				{
					...
					{
						std::error_code ec (vban::error_common::insufficient_balance);
...
void vban::json_handler::sign ()
{
	...
	{
		ec = vban::error_rpc::sign_hash_disabled;
	}
	if (!ec)
	{
		vban::raw_key prv;
		...
		{
			vban::public_key pub (vban::pub_key (prv));
			vban::signature signature (vban::sign_message (prv, pub, hash));
...
void vban::json_handler::stats ()
{
	...
	{
		ec = vban::error_rpc::invalid_missing_type;
	}
	if (!ec && use_sink)
	{
...
void vban::json_handler::stats_clear ()
{
	node.stats.clear ();
	response_l.put ("success", "");
	std::stringstream ostream;
...
void vban::json_handler::telemetry ()
{
	auto rpc_l (shared_from_this ());
	...
	if (address_text.is_initialized () || port_text.is_initialized ())
	{
		...
		if (address_text.is_initialized () && port_text.is_initialized ())
		{
			...
			if (!vban::parse_port (*port_text, port))
			{
				...
				if (!vban::parse_address (*address_text, address))
				{
					vban::endpoint endpoint (address, port);
					if (address.is_loopback () && port == rpc_l->node.network.endpoint ().port ())
					{
						// Requesting telemetry metrics locally
						auto telemetry_data = vban::local_telemetry_data (rpc_l->node.ledger, rpc_l->node.network, rpc_l->node.config.bandwidth_limit, rpc_l->node.network_params, rpc_l->node.startup_time, rpc_l->node.default_difficulty (vban::work_version::work_1), rpc_l->node.node_id);

						vban::jsonconfig config_l;
					...
					{
						channel = node.network.find_channel (vban::transport::map_endpoint_to_v6 (endpoint));
						if (!channel)
						{
							ec = vban::error_rpc::peer_not_found;
						}
					}
				}
				else
		...
		{
			ec = vban::error_rpc::requires_port_and_address;
		}
			...
			if (node.telemetry)
			{
				node.telemetry->get_metrics_single_peer_async (channel, [rpc_l] (auto const & telemetry_response_a) {
					if (!telemetry_response_a.error)
					{
						...
						{
							rpc_l->ec = vban::error_rpc::generic;
						}
					}
					else
					{
	...
	else
	{
		...
		if (node.telemetry)
		{
			...
			{
				vban::jsonconfig config_l;
				std::vector<vban::telemetry_data> telemetry_datas;
				...
				auto average_telemetry_metrics = vban::consolidate_telemetry_data (telemetry_datas);
...
void vban::json_handler::unchecked ()
{
	const bool json_block_l = request.get<bool> ("json_block", false);
	auto count (count_optional_impl ());
	if (!ec)
	{
		...
		{
			vban::unchecked_info const & info (i->second);
			if (json_block_l)
			{
				boost::property_tree::ptree block_node_l;
...
void vban::json_handler::unchecked_clear ()
{
	node.workers.push_task (create_worker_task ([] (std::shared_ptr<vban::json_handler> const & rpc_l) {
...
void vban::json_handler::unchecked_get ()
{
	const bool json_block_l = request.get<bool> ("json_block", false);
	auto hash (hash_impl ());
	if (!ec)
	{
		...
		{
			vban::unchecked_key const & key (i->first);
			if (key.hash == hash)
			{
				vban::unchecked_info const & info (i->second);
				response_l.put ("modified_timestamp", std::to_string (info.modified));
...
void vban::json_handler::unchecked_keys ()
{
	const bool json_block_l = request.get<bool> ("json_block", false);
	auto count (count_optional_impl ());
	vban::block_hash key (0);
	...
	if (!ec)
	{
		boost::property_tree::ptree unchecked;
		auto transaction (node.store.tx_begin_read ());
		for (auto i (node.store.unchecked_begin (transaction, vban::unchecked_key (key, 0))), n (node.store.unchecked_end ()); i != n && unchecked.size () < count; ++i)
		{
			boost::property_tree::ptree entry;
			vban::unchecked_info const & info (i->second);
...
void vban::json_handler::unopened ()
{
	auto count (count_optional_impl ());
	auto threshold (threshold_optional_impl ());
	vban::account start (1); // exclude burn account by default
	boost::optional<std::string> account_text (request.get_optional<std::string> ("account"));
	if (account_text.is_initialized ())
	{
		start = account_impl (account_text.get ());
	...
	{
		auto transaction (node.store.tx_begin_read ());
		auto iterator (node.store.pending_begin (transaction, vban::pending_key (start, 0)));
		auto end (node.store.pending_end ());
		vban::account current_account (start);
		vban::uint256_t current_account_sum{ 0 };
		boost::property_tree::ptree accounts;
		while (iterator != end && accounts.size () < count)
		{
			vban::pending_key key (iterator->first);
			vban::account account (key.account);
			vban::pending_info info (iterator->second);
			if (node.store.account_exists (transaction, account))
			{
				if (account.number () == std::numeric_limits<vban::uint256_t>::max ())
				{
					break;
				}
				// Skip existing accounts
				iterator = node.store.pending_begin (transaction, vban::pending_key (account.number () + 1, 0));
			}
			else
			{
...
void vban::json_handler::validate_account_number ()
{
	auto account (account_impl ());
	(void)account;
...
void vban::json_handler::wallet_add ()
{
	node.workers.push_task (create_worker_task ([] (std::shared_ptr<vban::json_handler> const & rpc_l) {
		auto wallet (rpc_l->wallet_impl ());
		if (!rpc_l->ec)
		{
			...
			{
				rpc_l->ec = vban::error_common::bad_private_key;
			}
		}
		rpc_l->response_errors ();
	}));
...
void vban::json_handler::wallet_add_watch ()
{
	node.workers.push_task (create_worker_task ([] (std::shared_ptr<vban::json_handler> const & rpc_l) {
		auto wallet (rpc_l->wallet_impl ());
		if (!rpc_l->ec)
			...
			if (wallet->store.valid_password (transaction))
			{
				for (auto & accounts : rpc_l->request.get_child ("accounts"))
				{
					...
					if (!rpc_l->ec)
					{
						...
						{
							rpc_l->ec = vban::error_common::bad_public_key;
						}
					}
				}
				if (!rpc_l->ec)
...
void vban::json_handler::wallet_info ()
{
	auto wallet (wallet_impl ());
	if (!ec)
	{
		vban::uint256_t balance (0);
		vban::uint256_t pending (0);
		uint64_t count (0);
		...
		for (auto i (wallet->store.begin (transaction)), n (wallet->store.end ()); i != n; ++i)
		{
			...
			vban::key_type key_type (wallet->store.key_type (i->second));
			if (key_type == vban::key_type::deterministic)
			{
				deterministic_count++;
			}
			else if (key_type == vban::key_type::adhoc)
			{
				adhoc_count++;
			}
			count++;
...
void vban::json_handler::wallet_balances ()
{
	auto wallet (wallet_impl ());
	auto threshold (threshold_optional_impl ());
	if (!ec)
	{
		...
		{
			vban::account const & account (i->first);
			vban::uint256_t balance = node.ledger.account_balance (block_transaction, account);
			if (balance >= threshold.number ())
			{
				boost::property_tree::ptree entry;
				vban::uint256_t pending = node.ledger.account_pending (block_transaction, account);
				entry.put ("balance", balance.convert_to<std::string> ());
...
void vban::json_handler::wallet_change_seed ()
{
	node.workers.push_task (create_worker_task ([] (std::shared_ptr<vban::json_handler> const & rpc_l) {
		auto wallet (rpc_l->wallet_impl ());
		if (!rpc_l->ec)
		{
			...
			if (!seed.decode_hex (seed_text))
			{
				...
				if (wallet->store.valid_password (transaction))
				{
					vban::public_key account (wallet->change_seed (transaction, seed, count));
					rpc_l->response_l.put ("success", "");
			...
			{
				rpc_l->ec = vban::error_common::bad_seed;
			}
		}
		rpc_l->response_errors ();
	}));
...
void vban::json_handler::wallet_contains ()
{
	auto account (account_impl ());
	auto wallet (wallet_impl ());
	if (!ec)
...
void vban::json_handler::wallet_create ()
{
	node.workers.push_task (create_worker_task ([] (std::shared_ptr<vban::json_handler> const & rpc_l) {
		vban::raw_key seed;
		...
		if (!rpc_l->ec)
		{
			...
			{
				rpc_l->ec = vban::error_common::wallet_lmdb_max_dbs;
			}
			if (!rpc_l->ec && seed_text.is_initialized ())
			{
				auto transaction (rpc_l->node.wallets.tx_begin_write ());
				vban::public_key account (wallet->change_seed (transaction, seed));
				rpc_l->response_l.put ("last_restored_account", account.to_account ());
				auto index (wallet->store.deterministic_index_get (transaction));
				debug_assert (index > 0);
				rpc_l->response_l.put ("restored_count", std::to_string (index));
...
void vban::json_handler::wallet_destroy ()
{
	node.workers.push_task (create_worker_task ([] (std::shared_ptr<vban::json_handler> const & rpc_l) {
		...
		if (!wallet.decode_hex (wallet_text))
		{
			...
			{
				rpc_l->ec = vban::error_common::wallet_not_found;
			}
		}
		else
		{
			rpc_l->ec = vban::error_common::bad_wallet_number;
		}
		rpc_l->response_errors ();
	}));
}

void vban::json_handler::wallet_export ()
{
	auto wallet (wallet_impl ());
	if (!ec)
	{
...
void vban::json_handler::wallet_frontiers ()
{
	auto wallet (wallet_impl ());
	if (!ec)
	{
...
void vban::json_handler::wallet_history ()
{
		...
		for (auto i (wallet->store.begin (transaction)), n (wallet->store.end ()); i != n; ++i)
		{
			...
			if (!node.store.account_get (block_transaction, account, info))
			{
				...
				while (timestamp >= modified_since && !hash.is_zero ())
				{
					...
					{
						boost::property_tree::ptree entry;
						std::vector<vban::public_key> no_filter;
...
void vban::json_handler::wallet_key_valid ()
{
	auto wallet (wallet_impl ());
	if (!ec)
	{
...
void vban::json_handler::wallet_ledger ()
{
	const bool representative = request.get<bool> ("representative", false);
	const bool weight = request.get<bool> ("weight", false);
	const bool pending = request.get<bool> ("pending", false);
...
void vban::json_handler::wallet_lock ()
{
	auto wallet (wallet_impl ());
	if (!ec)
	{
...
void vban::json_handler::wallet_pending ()
{
	auto wallet (wallet_impl ());
	auto count (count_optional_impl ());
	auto threshold (threshold_optional_impl ());
	...
	if (!ec)
	{
		...
		for (auto i (wallet->store.begin (transaction)), n (wallet->store.end ()); i != n; ++i)
		{
			vban::account const & account (i->first);
			boost::property_tree::ptree peers_l;
			for (auto ii (node.store.pending_begin (block_transaction, vban::pending_key (account, 0))), nn (node.store.pending_end ()); ii != nn && vban::pending_key (ii->first).account == account && peers_l.size () < count; ++ii)
			{
				vban::pending_key key (ii->first);
				if (block_confirmed (node, block_transaction, key.hash, include_active, include_only_confirmed))
				{
					...
					{
						vban::pending_info info (ii->second);
						if (info.amount.number () >= threshold.number ())
						{
							if (source || min_version)
							{
...
void vban::json_handler::wallet_representative ()
{
	auto wallet (wallet_impl ());
	if (!ec)
	{
...
void vban::json_handler::wallet_representative_set ()
{
	node.workers.push_task (create_worker_task ([] (std::shared_ptr<vban::json_handler> const & rpc_l) {
		auto wallet (rpc_l->wallet_impl ());
		std::string representative_text (rpc_l->request.get<std::string> ("representative"));
		...
		if (!rpc_l->ec)
		{
			...
			if (!rpc_l->ec && update_existing_accounts)
			{
				...
				{
					wallet->change_async (
					account, representative, [] (std::shared_ptr<vban::block> const &) {}, 0, false);
				}
			}
		}
		rpc_l->response_errors ();
...
void vban::json_handler::wallet_republish ()
{
	auto wallet (wallet_impl ());
	auto count (count_impl ());
	if (!ec)
...
void vban::json_handler::wallet_seed ()
{
	auto wallet (wallet_impl ());
	if (!ec)
	{
...
void vban::json_handler::wallet_work_get ()
{
	auto wallet (wallet_impl ());
	if (!ec)
	{
		...
		for (auto i (wallet->store.begin (transaction)), n (wallet->store.end ()); i != n; ++i)
		{
			...
			(void)error_work;
			works.put (account.to_account (), vban::to_string_hex (work));
		}
...
void vban::json_handler::work_generate ()
{
	boost::optional<vban::account> account;
	...
	if (!ec)
	{
		...
		if (!ec && (difficulty > node.max_work_generate_difficulty (work_version) || difficulty < vban::work_threshold_entry (work_version, vban::block_type::state)))
		{
			ec = vban::error_rpc::difficulty_limit;
		}
		...
		if (!ec && request.count ("block"))
		{
			...
			if (block != nullptr)
			{
				if (hash != block->root ())
				{
					ec = vban::error_rpc::block_root_mismatch;
				}
				...
				{
					ec = vban::error_rpc::block_work_version_mismatch;
				}
				// Difficulty calculation
				...
				{
					ec = vban::error_rpc::block_work_enough;
				}
			}
		}
		if (!ec && response_l.empty ())
		{
			...
			auto callback = [rpc_l, hash, work_version, this] (boost::optional<uint64_t> const & work_a) {
				if (work_a)
				{
					...
					response_l.put ("work", vban::to_string_hex (work));
					std::stringstream ostream;
					auto result_difficulty (vban::work_difficulty (work_version, hash, work));
					response_l.put ("difficulty", vban::to_string_hex (result_difficulty));
					auto result_multiplier = vban::difficulty::to_multiplier (result_difficulty, node.default_difficulty (work_version));
					response_l.put ("multiplier", vban::to_string (result_multiplier));
					boost::property_tree::write_json (ostream, response_l);
					rpc_l->response (ostream.str ());
				}
				else
			...
			if (!use_peers)
			{
				if (node.local_work_generation_enabled ())
				{
					...
					if (error)
					{
						ec = vban::error_common::failure_work_generation;
					}
				}
				else
				{
					ec = vban::error_common::disabled_local_work_generation;
				}
			}
			else
			{
...
void vban::json_handler::work_cancel ()
{
	auto hash (hash_impl ());
	if (!ec)
	{
...
void vban::json_handler::work_get ()
{
	auto wallet (wallet_impl ());
	auto account (account_impl ());
	if (!ec)
...
void vban::json_handler::work_set ()
{
	node.workers.push_task (create_worker_task ([] (std::shared_ptr<vban::json_handler> const & rpc_l) {
		auto wallet (rpc_l->wallet_impl ());
		auto account (rpc_l->account_impl ());
...
void vban::json_handler::work_validate ()
{
	auto hash (hash_impl ());
	auto work (work_optional_impl ());
	// Default to work_1 if not specified
	...
	if (!ec)
	{
		...
		response_l.put ("valid_receive", (result_difficulty >= vban::work_threshold (work_version, vban::block_details (vban::epoch::epoch_2, false, true, false))) ? "1" : "0");
		response_l.put ("difficulty", vban::to_string_hex (result_difficulty));
		auto result_multiplier = vban::difficulty::to_multiplier (result_difficulty, node.default_difficulty (work_version));
		response_l.put ("multiplier", vban::to_string (result_multiplier));
	}
...
void vban::json_handler::work_peer_add ()
{
	std::string address_text = request.get<std::string> ("address");
	std::string port_text = request.get<std::string> ("port");
	uint16_t port;
...
void vban::json_handler::work_peers ()
{
...
void vban::json_handler::work_peers_clear ()
{
...
void vban::inprocess_rpc_handler::process_request (std::string const &, std::string const & body_a, std::function<void (std::string const &)> response_a)
{
	// Note that if the rpc action is async, the shared_ptr<json_handler> lifetime will be extended by the action handler
	auto handler (std::make_shared<vban::json_handler> (node, node_rpc_config, body_a, response_a, [this] () {
		this->stop_callback ();
		this->stop ();
	}));
...
void vban::inprocess_rpc_handler::process_request_v2 (rpc_handler_request_params const & params_a, std::string const & body_a, std::function<void (std::shared_ptr<std::string> const &)> response_a)
{
	std::string body_l = params_a.json_envelope (body_a);
	auto handler (std::make_shared<vban::ipc::flatbuffers_handler> (node, ipc_server, nullptr, node.config.ipc_config));
...
void construct_json (vban::container_info_component * component, boost::property_tree::ptree & parent)
{
	...
	{
		auto & leaf_info = static_cast<vban::container_info_leaf *> (component)->get_info ();
	...
	auto composite = static_cast<vban::container_info_composite *> (component);
...
ipc_json_handler_no_arg_func_map create_ipc_json_handler_no_arg_func_map ()
{
	...
	no_arg_funcs.emplace ("account_balance", &vban::json_handler::account_balance);
	no_arg_funcs.emplace ("account_block_count", &vban::json_handler::account_block_count);
	no_arg_funcs.emplace ("account_count", &vban::json_handler::account_count);
	no_arg_funcs.emplace ("account_create", &vban::json_handler::account_create);
	no_arg_funcs.emplace ("account_get", &vban::json_handler::account_get);
	no_arg_funcs.emplace ("account_history", &vban::json_handler::account_history);
	no_arg_funcs.emplace ("account_info", &vban::json_handler::account_info);
	no_arg_funcs.emplace ("account_key", &vban::json_handler::account_key);
	no_arg_funcs.emplace ("account_list", &vban::json_handler::account_list);
	no_arg_funcs.emplace ("account_move", &vban::json_handler::account_move);
	no_arg_funcs.emplace ("account_remove", &vban::json_handler::account_remove);
	no_arg_funcs.emplace ("account_representative", &vban::json_handler::account_representative);
	no_arg_funcs.emplace ("account_representative_set", &vban::json_handler::account_representative_set);
	no_arg_funcs.emplace ("account_weight", &vban::json_handler::account_weight);
	no_arg_funcs.emplace ("accounts_balances", &vban::json_handler::accounts_balances);
	no_arg_funcs.emplace ("accounts_create", &vban::json_handler::accounts_create);
	no_arg_funcs.emplace ("accounts_frontiers", &vban::json_handler::accounts_frontiers);
	no_arg_funcs.emplace ("accounts_pending", &vban::json_handler::accounts_pending);
	no_arg_funcs.emplace ("active_difficulty", &vban::json_handler::active_difficulty);
	no_arg_funcs.emplace ("available_supply", &vban::json_handler::available_supply);
	no_arg_funcs.emplace ("block_info", &vban::json_handler::block_info);
	no_arg_funcs.emplace ("block", &vban::json_handler::block_info);
	no_arg_funcs.emplace ("block_confirm", &vban::json_handler::block_confirm);
	no_arg_funcs.emplace ("blocks", &vban::json_handler::blocks);
	no_arg_funcs.emplace ("blocks_info", &vban::json_handler::blocks_info);
	no_arg_funcs.emplace ("block_account", &vban::json_handler::block_account);
	no_arg_funcs.emplace ("block_count", &vban::json_handler::block_count);
	no_arg_funcs.emplace ("block_create", &vban::json_handler::block_create);
	no_arg_funcs.emplace ("block_hash", &vban::json_handler::block_hash);
	no_arg_funcs.emplace ("bootstrap", &vban::json_handler::bootstrap);
	no_arg_funcs.emplace ("bootstrap_any", &vban::json_handler::bootstrap_any);
	no_arg_funcs.emplace ("bootstrap_lazy", &vban::json_handler::bootstrap_lazy);
	no_arg_funcs.emplace ("bootstrap_status", &vban::json_handler::bootstrap_status);
	no_arg_funcs.emplace ("confirmation_active", &vban::json_handler::confirmation_active);
	no_arg_funcs.emplace ("confirmation_height_currently_processing", &vban::json_handler::confirmation_height_currently_processing);
	no_arg_funcs.emplace ("confirmation_history", &vban::json_handler::confirmation_history);
	no_arg_funcs.emplace ("confirmation_info", &vban::json_handler::confirmation_info);
	no_arg_funcs.emplace ("confirmation_quorum", &vban::json_handler::confirmation_quorum);
	no_arg_funcs.emplace ("database_txn_tracker", &vban::json_handler::database_txn_tracker);
	no_arg_funcs.emplace ("delegators", &vban::json_handler::delegators);
	no_arg_funcs.emplace ("delegators_count", &vban::json_handler::delegators_count);
	no_arg_funcs.emplace ("deterministic_key", &vban::json_handler::deterministic_key);
	no_arg_funcs.emplace ("epoch_upgrade", &vban::json_handler::epoch_upgrade);
	no_arg_funcs.emplace ("frontiers", &vban::json_handler::frontiers);
	no_arg_funcs.emplace ("frontier_count", &vban::json_handler::account_count);
	no_arg_funcs.emplace ("keepalive", &vban::json_handler::keepalive);
	no_arg_funcs.emplace ("key_create", &vban::json_handler::key_create);
	no_arg_funcs.emplace ("key_expand", &vban::json_handler::key_expand);
	no_arg_funcs.emplace ("ledger", &vban::json_handler::ledger);
	no_arg_funcs.emplace ("node_id", &vban::json_handler::node_id);
	no_arg_funcs.emplace ("node_id_delete", &vban::json_handler::node_id_delete);
	no_arg_funcs.emplace ("password_change", &vban::json_handler::password_change);
	no_arg_funcs.emplace ("password_enter", &vban::json_handler::password_enter);
	no_arg_funcs.emplace ("wallet_unlock", &vban::json_handler::password_enter);
	no_arg_funcs.emplace ("peers", &vban::json_handler::peers);
	no_arg_funcs.emplace ("pending", &vban::json_handler::pending);
	no_arg_funcs.emplace ("pending_exists", &vban::json_handler::pending_exists);
	no_arg_funcs.emplace ("process", &vban::json_handler::process);
	no_arg_funcs.emplace ("pruned_exists", &vban::json_handler::pruned_exists);
	no_arg_funcs.emplace ("receive", &vban::json_handler::receive);
	no_arg_funcs.emplace ("receive_minimum", &vban::json_handler::receive_minimum);
	no_arg_funcs.emplace ("receive_minimum_set", &vban::json_handler::receive_minimum_set);
	no_arg_funcs.emplace ("representatives", &vban::json_handler::representatives);
	no_arg_funcs.emplace ("representatives_online", &vban::json_handler::representatives_online);
	no_arg_funcs.emplace ("republish", &vban::json_handler::republish);
	no_arg_funcs.emplace ("search_pending", &vban::json_handler::search_pending);
	no_arg_funcs.emplace ("search_pending_all", &vban::json_handler::search_pending_all);
	no_arg_funcs.emplace ("send", &vban::json_handler::send);
	no_arg_funcs.emplace ("sign", &vban::json_handler::sign);
	no_arg_funcs.emplace ("stats", &vban::json_handler::stats);
	no_arg_funcs.emplace ("stats_clear", &vban::json_handler::stats_clear);
	no_arg_funcs.emplace ("stop", &vban::json_handler::stop);
	no_arg_funcs.emplace ("telemetry", &vban::json_handler::telemetry);
	no_arg_funcs.emplace ("unchecked", &vban::json_handler::unchecked);
	no_arg_funcs.emplace ("unchecked_clear", &vban::json_handler::unchecked_clear);
	no_arg_funcs.emplace ("unchecked_get", &vban::json_handler::unchecked_get);
	no_arg_funcs.emplace ("unchecked_keys", &vban::json_handler::unchecked_keys);
	no_arg_funcs.emplace ("unopened", &vban::json_handler::unopened);
	no_arg_funcs.emplace ("uptime", &vban::json_handler::uptime);
	no_arg_funcs.emplace ("validate_account_number", &vban::json_handler::validate_account_number);
	no_arg_funcs.emplace ("version", &vban::json_handler::version);
	no_arg_funcs.emplace ("wallet_add", &vban::json_handler::wallet_add);
	no_arg_funcs.emplace ("wallet_add_watch", &vban::json_handler::wallet_add_watch);
	no_arg_funcs.emplace ("wallet_balances", &vban::json_handler::wallet_balances);
	no_arg_funcs.emplace ("wallet_change_seed", &vban::json_handler::wallet_change_seed);
	no_arg_funcs.emplace ("wallet_contains", &vban::json_handler::wallet_contains);
	no_arg_funcs.emplace ("wallet_create", &vban::json_handler::wallet_create);
	no_arg_funcs.emplace ("wallet_destroy", &vban::json_handler::wallet_destroy);
	no_arg_funcs.emplace ("wallet_export", &vban::json_handler::wallet_export);
	no_arg_funcs.emplace ("wallet_frontiers", &vban::json_handler::wallet_frontiers);
	no_arg_funcs.emplace ("wallet_history", &vban::json_handler::wallet_history);
	no_arg_funcs.emplace ("wallet_info", &vban::json_handler::wallet_info);
	no_arg_funcs.emplace ("wallet_balance_total", &vban::json_handler::wallet_info);
	no_arg_funcs.emplace ("wallet_key_valid", &vban::json_handler::wallet_key_valid);
	no_arg_funcs.emplace ("wallet_ledger", &vban::json_handler::wallet_ledger);
	no_arg_funcs.emplace ("wallet_lock", &vban::json_handler::wallet_lock);
	no_arg_funcs.emplace ("wallet_pending", &vban::json_handler::wallet_pending);
	no_arg_funcs.emplace ("wallet_representative", &vban::json_handler::wallet_representative);
	no_arg_funcs.emplace ("wallet_representative_set", &vban::json_handler::wallet_representative_set);
	no_arg_funcs.emplace ("wallet_republish", &vban::json_handler::wallet_republish);
	no_arg_funcs.emplace ("wallet_work_get", &vban::json_handler::wallet_work_get);
	no_arg_funcs.emplace ("work_generate", &vban::json_handler::work_generate);
	no_arg_funcs.emplace ("work_cancel", &vban::json_handler::work_cancel);
	no_arg_funcs.emplace ("work_get", &vban::json_handler::work_get);
	no_arg_funcs.emplace ("work_set", &vban::json_handler::work_set);
	no_arg_funcs.emplace ("work_validate", &vban::json_handler::work_validate);
	no_arg_funcs.emplace ("work_peer_add", &vban::json_handler::work_peer_add);
	no_arg_funcs.emplace ("work_peers", &vban::json_handler::work_peers);
	no_arg_funcs.emplace ("work_peers_clear", &vban::json_handler::work_peers_clear);
	return no_arg_funcs;
}
...
const char * epoch_as_string (vban::epoch epoch)
{
	switch (epoch)
	{
