...
vban::signal_manager::signal_manager () :
...
vban::signal_manager::signal_descriptor::signal_descriptor (std::shared_ptr<boost::asio::signal_set> sigset_a, signal_manager & sigman_a, std::function<void (int)> handler_func_a, bool repeat_a) :
	sigset (sigset_a), sigman (sigman_a), handler_func (handler_func_a), repeat (repeat_a)
{
}

void vban::signal_manager::register_signal_handler (int signum, std::function<void (int)> handler, bool repeat)
{
	...
	sigset->async_wait ([descriptor] (const boost::system::error_code & error, int signum) {
		vban::signal_manager::base_handler (descriptor, error, signum);
	});

	log (boost::str (boost::format ("Registered signal handler for signal %d") % signum));
}

std::function<void (int)> vban::signal_manager::get_debug_files_handler (void)
{
	return [] (int) {
		vban::dump_crash_stacktrace ();
		vban::create_load_memory_address_files ();
	};
}

void vban::signal_manager::base_handler (vban::signal_manager::signal_descriptor descriptor, const boost::system::error_code & error, int signum)
{
	if (!error)
	{
